        ...
        const hp::FECollection<dim> &fe_coarse)
{
  ...
  DataOut<dim> data_out_coarse, data_out_fine;

  data_out_coarse.add_data_vector(dof_handler_coarse,
                                  vec_coarse,
                                  "solution_coarse");
  data_out_fine.add_data_vector(dof_handler_fine, vec_fine, "solution_fine");

  data_out_coarse.build_patches();
  data_out_fine.build_patches();



#if 1
  static unsigned int counter = 0;
  data_out_coarse.write_vtu_with_pvtu_record(
    "./", "result_coarse", counter, tria_coarse.get_communicator(), 3, 1);
  data_out_fine.write_vtu_with_pvtu_record(
    "./", "result_fine", counter, tria_fine.get_communicator(), 3, 1);

  counter++;
#else
  deallog << std::endl;
  data_out_coarse.write_vtk(deallog.get_file_stream());
#endif
}
