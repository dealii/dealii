...
// Solve the system using a PSBLAS solver
psb_c_dprec *psblas_preconditioner = PSCToolkit::Solvers::CreateBasePreconditioner(*cctxt, "DIAG");
if (psblas_preconditioner == nullptr) {
  deallog << "Error creating PSBLAS preconditioner." << std::endl;
}
// Build the preconditioner
info = PSCToolkit::Solvers::BasePrecBuild(psblas_preconditioner, psblas_sparse_matrix, descriptor);
if (info != 0) {
  deallog << "Error building PSBLAS preconditioner: " << info << std::endl;
}
// Create the PSBLAS solver options
psb_c_SolverOptions *solver_options = PSCToolkit::Solvers::CreateSolverOptions();
if (solver_options == nullptr) {
  deallog << "Error creating PSBLAS solver options." << std::endl;
}
// Print the solver options
PSCToolkit::Solvers::PrintSolverOptions(solver_options, cctxt, output);

// Initialize a vector to hold the solution
psb_c_dvector *psblas_solution_vector = PSCToolkit::PSBVector::CreateVector(descriptor);
if (psblas_solution_vector == nullptr) {
  deallog << "Error creating PSBLAS solution vector." << std::endl;
}
// Set the vector to zero
psb_c_dvect_set_scal(psblas_solution_vector, 0.0);
// Assemble the solution vector
info = PSCToolkit::PSBVector::AssembleVector(psblas_solution_vector, descriptor);
if (info != 0) {
  deallog << "Error assembling PSBLAS solution vector: " << info << std::endl;
}

// Solve the system
info = PSCToolkit::Solvers::BaseKrylov("CG",psblas_sparse_matrix, psblas_preconditioner, psblas_rhs_vector, psblas_solution_vector, descriptor, solver_options);
if (info != 0) {
  deallog << "Error solving PSBLAS system: " << info << std::endl;
} else {
  deallog << "PSBLAS system solved successfully." << std::endl;
}

// Extract the iter and error from the solver options structure
int iter = solver_options->iter;
double error = solver_options->err;
output << "Process " << iam << " of  " << nproc
       << " solved the system in " << iter << " iterations with error: " << error << std::endl;
...
if (iam == 0)
{
  ...
  info = PSCToolkit::PSBVector::FreeVector(psblas_solution_vector, descriptor);
  if (info != 0) {
    deallog << "Error freeing PSBLAS solution vector: " << info << std::endl;
  }
  // Free the PSBLAS preconditioner
  info = PSCToolkit::Solvers::FreeBasePreconditioner(psblas_preconditioner);
  if (info != 0) {
    deallog << "Error freeing PSBLAS preconditioner: " << info << std::endl;
  }
  // Free the PSBLAS solver options
  PSCToolkit::Solvers::FreeSolverOptions(solver_options);
