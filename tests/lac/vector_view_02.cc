...
checkReinit1(const size_type N, const bool fast = false)
{
  ...
  deallog << "Creating Vector<number> of size N+10 and filling with values 1 to N+10" << std::endl;

  Vector<number> V(N + 10);
  for (unsigned int i = 0; i < V.size(); i++)
    V(i) = i + 1;

  deallog << "Creating VectorView<number> of size N+10 pointing to Vector<number>" << std::endl;
  ...
  deallog << "Reinit VectorView<number> to size N from N+10 with fast=" << fast << std::endl;
  VV.reinit(N, fast);
