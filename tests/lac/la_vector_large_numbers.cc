...
check_large_numbers()
{
  ...
  DEAL_II_AssertThrow(std::abs(v.l2_norm() - correct) < 1e-6 * correct,
                      ExcInternalError());
  ...
  DEAL_II_AssertThrow(std::abs(v.l2_norm() - correct2) < 1e-6 * correct2,
                      ExcInternalError());
  ...
  DEAL_II_AssertThrow(std::abs(w.l2_norm() - correct3) < 1e-13 * correct3,
                      ExcInternalError());
  ...
  DEAL_II_AssertThrow(std::abs(w.l2_norm() - correct4) <= 1e-13 * correct4,
                      ExcInternalError());
