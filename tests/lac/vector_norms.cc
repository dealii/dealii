  ...
  for (unsigned int size = 1; size < 200000; size += skip)
    {
      ...
      AssertThrow(std::abs(lp_norma - value * std::pow(static_cast<number>(size),
                    ...
                    std::max(acc, number(1e-15)) *
                      std::pow(static_cast<number>(size), static_cast<number>(1. / 3.)) * value,
                  ExcInternalError());
    }
}
  ...
  for (unsigned int size = 1; size < 100000; size += skip)
    {
      ...
      const number lp_norma = vec.lp_norm(3.);
      AssertThrow(
        std::abs(lp_norma - std::abs(value) *
                              std::pow(static_cast<number>(size), static_cast<number>(1. / 3.))) <
          acc * std::pow(static_cast<number>(size), static_cast<number>(1. / 3.)) * std::abs(value),
        ExcInternalError());
    }
}
