...
// Test affine variants of LinearOperator
...
int main()
{
  ...
  Vector<double> w;
  op_c.reinit_domain_vector(w, false);

  op_c.vmult_add(w, u);
  deallog << "Au-v: " << w << std::endl;
  op_c.vmult(w, u);
  deallog << "Au-v: " << w << std::endl;
  ...
  op_c.vmult_add(w, u);
  deallog << "2*(0.5*(v+Au)-v+v-v): " << w << std::endl;
  op_c.vmult(w, u);
  deallog << "2*(0.5*(v+Au)-v+v-v): " << w << std::endl;
}
