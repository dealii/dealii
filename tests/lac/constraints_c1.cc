          ...
          for (unsigned int fvertex = 0; fvertex < GeometryInfo<dim>::vertices_per_face; ++fvertex)
            {
              ...
              unsigned int neighbor_vertex =
                GeometryInfo<dim>::face_to_cell_vertices(neighbor_face, fvertex);
              ...
              const unsigned int                                      constrained =
                fe.face_to_cell_index((fvertex == 0) ? 2 : fe.dofs_per_face - 1, face);
              ...
              for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
                {
                  if (i == constrained)
                    ...
                    rhs.push_back(
                      std::make_pair(cell_indices[i], -vertex_constraints[vertex][d][i]));
                  if (vertex_constraints[neighbor_vertex][d][i] != 0.)
                    rhs.push_back(std::make_pair(neighbor_indices[i],
                                                 vertex_constraints[neighbor_vertex][d][i]));
                }
              deallog << " v" << vertex << " f" << face << " o" << other_face << " d" << d << " l"
                      << constrained << " g"
