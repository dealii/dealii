...
int main (int argc, char **argv)
{
  ...
  const types::global_dof_index begin_dof = (current_process*(current_process + 1))/2;
  const types::global_dof_index end_dof = begin_dof + n_local_dofs;

  const types::global_dof_index n_global_dofs = ((n_processes - 1)*(n_processes))/2
                                                + n_processes;
  ...
  N_Vector sundials_vector = N_VNew_Parallel(MPI_COMM_WORLD,
                                             n_local_dofs,
                                             n_dofs);
