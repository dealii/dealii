...
Step4<dim>::assemble_preconditioner()
{
  ...
  typename DoFHandler<dim>::active_cell_iterator cell = dof_handler_precondition.begin_active(),
  ...
  for (; cell != endc; ++cell)
    {
      ...
      cell->get_dof_indices(local_dof_indices);
      constraints.distribute_local_to_global(cell_matrix, local_dof_indices, preconditioner_matrix);
    }
