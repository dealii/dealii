...
// like the test without _prebuilt_graph, but use the colored graph of
// cells precomputed by DoFHandler
...
template <int dim>
void
LaplaceProblem<dim>::local_assemble (const typename DoFHandler<dim>::active_cell_iterator &cell,
                                     Assembly::Scratch::Data<dim>  &scratch,
                                     Assembly::Copy::Data          &data)
{
...
void LaplaceProblem<dim>::assemble_reference ()
{
  ...
  const std::vector<std::vector<typename DoFHandler<dim>::active_cell_iterator> > &graph
    = dof_handler.get_colored_locally_owned_active_cells();
  
  for (unsigned int color=0; color<graph.size(); ++color)
    for (typename std::vector<typename DoFHandler<dim>::active_cell_iterator>::const_iterator p = graph[color].begin();
         p != graph[color].end(); ++p)
      {
...
void LaplaceProblem<dim>::assemble_test ()
{
  ...
  WorkStream::
    run (dof_handler.get_colored_locally_owned_active_cells(),
