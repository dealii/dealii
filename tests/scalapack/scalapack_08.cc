...
test(const unsigned int size, const unsigned int block_size, const NumberType tol)
{
  ...
  std::vector<NumberType> eigenvalues =
    scalapack_A_ev.eigenpairs_symmetric_by_index(std::make_pair(0, size - 1), true);
  ...
  pcout << "   with respect to the given tolerance the singular and eigenvalues coincide"
        << std::endl;
