...
test(const unsigned int block_size_i, const unsigned int block_size_j)
{
  ...
  pcout << "2D grid matrix: dim=" << scalapack_matrix.m() << "x" << scalapack_matrix.n()
        ...
        << ";  grid=" << grid->get_process_grid_rows() << "x" << grid->get_process_grid_columns()
        << std::endl
        << std::endl;
  ...
  ScaLAPACKMatrix<NumberType>           scalapack_matrix_dest(
    sub_size + offset_B.first, sub_size + offset_B.second, grid, block_size_j, block_size_i);
