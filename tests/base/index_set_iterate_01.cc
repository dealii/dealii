...
test(IndexSet &index_set)
{
  ...
  for (IndexSet::IntervalIterator it = index_set.begin_intervals(); it != endit;
       ++it, ++c)
    {
      ...
      DEAL_II_Assert(it == it2, ExcInternalError());
      DEAL_II_Assert(it - it2 == 0, ExcInternalError());
      DEAL_II_Assert(endit != it, ExcInternalError());
      DEAL_II_Assert(it != endit, ExcInternalError());

      IndexSet::IntervalIterator it3 = it2++;
      DEAL_II_Assert(it == it3, ExcInternalError());
      DEAL_II_Assert(it2 - it3 == 1, ExcInternalError());
      DEAL_II_Assert(it3 < it2, ExcInternalError());
      DEAL_II_Assert(++it3 == it2, ExcInternalError());

      DEAL_II_Assert(it->is_valid(), ExcInternalError());

      DEAL_II_Assert(it < endit, ExcInternalError());
      DEAL_II_Assert(!(it < it), ExcInternalError());

      DEAL_II_Assert((it - index_set.begin_intervals()) == (int)c,
                     ExcInternalError());
      DEAL_II_Assert((index_set.begin_intervals() - it) == -(int)c,
                     ExcInternalError());
  ...
  // ElementIterator checks
  {
    ...
    for (; it != index_set.end(); ++it, ++c)
      {
        DEAL_II_Assert(it < index_set.end(), ExcInternalError());
        ...
        DEAL_II_Assert((it - index_set.begin()) == c, ExcInternalError());
        DEAL_II_Assert((index_set.begin() - it) == -(int)c, ExcInternalError());
      }
  }
  ...
  {
    IndexSet::IntervalIterator it = index_set.begin_intervals();
    DEAL_II_Assert(it->is_valid() || index_set.n_elements() == 0,
                   ExcInternalError());
