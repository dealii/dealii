...
// check jacobi_polynomial_roots in long double precision by comparison to
    ...
    for (unsigned int degree = 1; degree < 40; degree += 3)
      {
        ...
        std::vector<long double> roots = jacobi_polynomial_roots<long double>(degree, alpha, beta);
        ...
        for (unsigned int i = 0; i < degree; ++i)
          deallog << std::setw(22) << std::setprecision(19) << roots[i]
                  << " (fval = " << std::setw(9) << std::setprecision(3)
                  << jacobi_polynomial_value(degree, alpha, beta, roots[i]) << " ) "
                  << std::setw(22) << std::setprecision(19) << 0.5 + 0.5 * roots_reference[i]
                  << " (fval = " << std::setw(9) << std::setprecision(3)
                  << jacobi_polynomial_value(degree, alpha, beta, 0.5 + 0.5 * roots_reference[i])
                  << " )" << std::endl;
        deallog << std::endl;
      }
}
