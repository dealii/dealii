  ...
  if (true)
    {
      ...
      for (unsigned int i = 0; i < I; ++i)
        for (unsigned int j = 0; j < J; ++j)
          AssertThrow(Td[i][j].end() - Td[i][j].begin() == static_cast<signed int>(K),
                      ExcInternalError());
    };
              ...
              for (unsigned int m = 0; m < M; ++m)
                {
                  ...
                  AssertThrow(*(Td[i][j][k][l].begin() + m) == Td(i, j, k, l, m),
                              ExcInternalError());
                  AssertThrow(*(Ti[i][j][k][l].begin() + m) == Ti(i, j, k, l, m),
                              ExcInternalError());
                  AssertThrow(*(Ti[i][j][k][l].begin() + m) == Td(i, j, k, l, m),
                              ExcInternalError());
                ...
                for (unsigned int n = 0; n < N; ++n)
                  {
                    ...
                    AssertThrow(*(Td[i][j][k][l][m].begin() + n) == Td(i, j, k, l, m, n),
                                ExcInternalError());
                    AssertThrow(*(Ti[i][j][k][l][m].begin() + n) == Ti(i, j, k, l, m, n),
                                ExcInternalError());
                    AssertThrow(*(Ti[i][j][k][l][m].begin() + n) == Td(i, j, k, l, m, n),
                                ExcInternalError());
        ...
        for (unsigned int j = 0; j < J; ++j)
          for (unsigned int k = 0; k < K; ++k)
            for (unsigned int l = 0; l < L; ++l)
              for (unsigned int m = 0; m < M; ++m)
                AssertThrow(
                  Td[i][j][k][l][m].end() - Td[i][j][k][l][m].begin() == static_cast<signed int>(N),
