...
void test()
{
  ...
  QGauss<dim> rhs(4);
  CellDataStorage<typename Triangulation<dim,dim>::cell_iterator,MyDataBase> data_storage;
  parallel::distributed::ContinuousQuadratureDataTransfer<dim,MyDataBase> data_transfer(FE_Q<dim>(2),QGauss<dim>(3),rhs);
  {
    DoFHandler<dim> dof_handler(tr);
    FE_Q<dim> dummy_fe(1);
