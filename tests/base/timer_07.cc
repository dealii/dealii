...
DEAL_II_DeclException0(Timer07Exception);
  ...
  while (next_number != output.end())
    {
      ...
      const std::string::iterator start_pipe =
        std::find(std::string::reverse_iterator(next_number),
                  ...
                  '|')
          .base();
      DEAL_II_Assert(start_pipe != output.end(), ExcInternalError());
      const std::string::iterator end_pipe =
        std::find(next_number, output.end(), '|');
      DEAL_II_Assert(end_pipe != output.end(), ExcInternalError());
      DEAL_II_Assert(end_pipe - start_pipe > 1, ExcInternalError());
