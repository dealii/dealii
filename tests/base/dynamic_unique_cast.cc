...
test()
{
  ...
  DEAL_II_Assert(d.get() == nullptr, ExcInternalError());
  DEAL_II_Assert(b.get() != nullptr, ExcInternalError());
  DEAL_II_Assert(dynamic_cast<D *>(b.get()) != nullptr, ExcInternalError());
  ...
  // Ownership now rests with b, but it's still a D. Verify this:
  DEAL_II_Assert(b.get() == nullptr, ExcInternalError());
  DEAL_II_Assert(dd.get() != nullptr, ExcInternalError());
