      ...
      if (quadrature.is_tensor_product())
        {
          ...
          DEAL_II_AssertThrow(q_basis.size() == 1, ExcInternalError());
          const auto &q_points  = quadrature.get_points();
          const auto &q_weights = quadrature.get_weights();
          DEAL_II_AssertThrow(q_basis[0].size() == q_points.size(),
                              ExcInternalError());
          for (unsigned int q = 0; q < quadrature.size(); ++q)
            {
              ...
              DEAL_II_AssertThrow(
                ...
                ExcInternalError());
              DEAL_II_AssertThrow(
                std::abs(q_weights[q] - q_basis[0].get_weights()[q]) < 1.e-10,
                ExcInternalError());
            }
          deallog << " OK" << std::endl;
        }
      ...
      if (quadrature.is_tensor_product())
        {
          ...
          const auto &q_basis = quadrature.get_tensor_basis();
          DEAL_II_AssertThrow(q_basis.size() == 2, ExcInternalError());
          const auto &q_points  = quadrature.get_points();
          const auto &q_weights = quadrature.get_weights();
          DEAL_II_AssertThrow(q_basis[0].size() * q_basis[1].size() ==
                                q_points.size(),
                              ExcInternalError());
          unsigned int q = 0;
          for (unsigned int q2 = 0; q2 < q_basis[0].size(); ++q2)
            for (unsigned int q1 = 0; q1 < q_basis[1].size(); ++q1)
              {
                DEAL_II_AssertThrow(std::abs(q_points[q][0] -
                                             q_basis[0].get_points()[q1][0]) <
                                      1.e-10,
                                    ExcInternalError());
                DEAL_II_AssertThrow(std::abs(q_points[q][1] -
                                             q_basis[1].get_points()[q2][0]) <
                                      1.e-10,
                                    ExcInternalError());
                DEAL_II_AssertThrow(std::abs((q_weights[q] -
      ...
      if (quadrature.is_tensor_product())
        {
          ...
          const auto &q_basis = quadrature.get_tensor_basis();
          DEAL_II_AssertThrow(q_basis.size() == 3, ExcInternalError());
          const auto &q_points  = quadrature.get_points();
          const auto &q_weights = quadrature.get_weights();
          DEAL_II_AssertThrow(q_basis[0].size() * q_basis[1].size() *
                                  q_basis[2].size() ==
              ...
              for (unsigned int q1 = 0; q1 < q_basis[0].size(); ++q1)
                {
                  ...
                  DEAL_II_AssertThrow(std::abs(q_points[q][1] -
                                        ...
                                        1.e-10,
                                      ExcInternalError());
                  DEAL_II_AssertThrow(std::abs(q_points[q][2] -
                                               q_basis[2].get_points()[q3][0]) <
                                        1.e-10,
                                      ExcInternalError());
                  DEAL_II_AssertThrow(std::abs(q_weights[q] -
                                               q_basis[0].get_weights()[q1] *
                                                 q_basis[1].get_weights()[q2] *
                                                 q_basis[2].get_weights()[q3]) <
                                        1.e-10,
                                      ExcInternalError());
                  ++q;
                }
