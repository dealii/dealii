...
{
  for (typename Triangulation<dim, spacedim>::cell_iterator cell = tria.begin(); cell != tria.end();
       ++cell)
    {
        ...
        if (cell->at_boundary(face))
          {
            ...
            weights[1] = 0.9;
            Point<spacedim> p =
              cell->get_manifold().get_new_point(make_array_view(points), make_array_view(weights));
            Point<spacedim> pref = cell->face(face)->get_manifold().get_new_point(
              make_array_view(points), make_array_view(weights));
            deallog << "Distance between cell manifold and face manifold: " << (pref - p)
                    << std::endl;
            weights[0] = 0.55;
            weights[1] = 0.45;
            p =
