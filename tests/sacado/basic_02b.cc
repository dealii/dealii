...
main()
{
  ...
  // dependent variable
  Sacado::Rad::ADvar<Sacado::Fad::DFad<double>> f_rfad = ::f(x_ad, y_ad, z_ad); // Cannot be const
  Sacado::Rad::ADvar<Sacado::Fad::DFad<double>> h_rfad =
    ::h(x_ad, y_ad, z_ad); // Cannot be const <----- Before g_rad
  Sacado::Rad::ADvar<Sacado::Fad::DFad<double>> g_rfad = ::g(x_ad, y_ad, z_ad); // Cannot be const
  ...
  const Sacado::Fad::DFad<double> dg_dx_fad =
    x_ad.adj() - d_dx_rad_acc; // dg/dx ; Note: Accumulation of partial derivatives
  const Sacado::Fad::DFad<double> dg_dy_fad =
    y_ad.adj() - d_dy_rad_acc;                 // dg/dy ; Note: Accumulation of partial derivatives
  ...
  const Sacado::Fad::DFad<double> dh_dx_fad =
    x_ad.adj() - d_dx_rad_acc; // dh/dx ; Note: Accumulation of partial derivatives
  const Sacado::Fad::DFad<double> dh_dy_fad =
    y_ad.adj() - d_dy_rad_acc;                 // dh/dy ; Note: Accumulation of partial derivatives
  ...
  Assert(std::fabs(d2g_dx_dx - d2g_dx_dx_ad) < tol && std::fabs(d2g_dy_dy - d2g_dy_dy_ad) < tol &&
  ...
  Assert(std::fabs(d2h_dx_dx - d2h_dx_dx_ad) < tol && std::fabs(d2h_dy_dy - d2h_dy_dy_ad) < tol &&
