...
void
dg(const double &x, const double &y, const double &z,
   double &dg_dx, double &dg_dy)
{
  dg_dx = z*std::cos(x*z)*std::cos(y/z);
  dg_dy = -(1.0/z)*std::sin(x*z)*std::sin(y/z);
}
void
dh(const double &x, const double &y, const double &z,
...
void
d2g(const double &x, const double &y, const double &z,
    double &d2g_dx_dx, double &d2g_dy_dy,
    double &d2g_dy_dx)
{
...
void
d2h(const double &x, const double &y, const double &z,
...
int main()
{
  ...
  Sacado::Rad::ADvar< Sacado::Fad::DFad<double> > h_rfad = ::h(x_ad, y_ad, z_ad); // Cannot be const <----- Before g_rad
