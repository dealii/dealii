...
main()
{
  ...
  FEFaceValues<2>    c1_values(
    c1_mapping, fe, quadrature, update_quadrature_points | update_normal_vectors);

  // to compare with, also print the
  // normal vectors as generated by a
  // cubic mapping
  const MappingQ<2> q3_mapping(3);
  FEFaceValues<2>   q3_values(
    q3_mapping, fe, quadrature, update_quadrature_points | update_normal_vectors);
      ...
      if (cell->face(f)->at_boundary())
        {
          ...
          Assert(c1_values.get_all_normal_vectors().size() == 2, ExcInternalError());
          ...
          for (unsigned int i = 0; i < 2; ++i)
            {
              ...
              deallog << "C1 normal vector " << i << ": " << c1_values.normal_vector(i)
                      << std::endl;
              deallog << "Q3 normal vector " << i << ": " << q3_values.normal_vector(i)
                      << std::endl;
            };
          ...
          for (unsigned int i = 0; i < 2; ++i)
            {
              AssertThrow(std::fabs(c1_values.normal_vector(i) * c1_values.normal_vector(i) - 1) <
                            1e-14,
                          ExcInternalError());
              Point<2> radius = c1_values.quadrature_point(i);
              radius /= std::sqrt(radius.square());
