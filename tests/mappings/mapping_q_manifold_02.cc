...
PullBack<dim>::value(const Point<dim>& p, const unsigned int component) const
{
  ...
  // y component
    ...
    {
      if(p[0] <= x_max / 2.0)
        result = (p[1] - f_x(p[0]) * (1 + y_FoR / y_max))
                 / (1.0 - f_x(p[0]) / y_max);
      else if(p[0] > x_max / 2.0)
