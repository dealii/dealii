...
//
// In this test case, there is are two quadrature objects, but they
// are identical, so again it does not matter.
...
test()
{
  ...
  const hp::QCollection<dim - 1> q_collection(quadrature, quadrature);
