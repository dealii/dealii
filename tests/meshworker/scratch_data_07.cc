...
// Compute Laplacian using mesh_loop, sacado, ScratchData, with
// second derivatives, using forward and backward derivatives.
...
test()
{
  ...
  auto cell_worker = [&rhs_function, &solution, &scalar](const Iterator &cell,
    ...
    s.extract_local_dof_values("solution", solution, energy);
    ...
    for (unsigned int q = 0; q < p.size(); ++q)
      {
        energy +=
          (.5 * grad_u[q] * grad_u[q] - rhs_function.value(p[q]) * u[q]) *
          JxW[q];
      }
    ...
    for (unsigned int i = 0; i < fev.dofs_per_cell; ++i)
      {
        ...
        const auto res_i = local_dof_values[i].adj();
        c.vectors[0][i]  = -res_i.val();

        // And the matrix
        for (unsigned int j = 0; j < fev.dofs_per_cell; ++j)
          c.matrices[0](i, j) = res_i.dx(j);
      }
  };
