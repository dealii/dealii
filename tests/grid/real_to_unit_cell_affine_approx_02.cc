...
make_linear_part()
{
  ...
  for (unsigned int d = 0; d < spacedim; ++d)
    for (unsigned int e = 0; e < dim; ++e)
      A[d][e] = (d == e ? 1.0 + 0.2 * d : 0.0) + 0.05 * (d + 1) * (e + 2);
     ...
     const bool                     compare_with_mapping_p1)
{
  ...
  std::vector<CellData<dim>> cells(1,
                                   CellData<dim>(reference_cell.n_vertices()));
  for (unsigned int v = 0; v < reference_cell.n_vertices(); ++v)
...
main()
{
  ...
  test<2, 2>(ReferenceCells::Triangle, triangle_points, "Triangle", true);
  ...
  test<2, 3>(ReferenceCells::Triangle,
             ...
             true);
  test<3, 3>(ReferenceCells::Wedge, wedge_points, "Wedge", false);
  test<3, 3>(ReferenceCells::Pyramid, pyramid_points, "Pyramid", false);
}
