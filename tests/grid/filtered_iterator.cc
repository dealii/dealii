  ...
  if (true)
    {
      ...
      FilteredIterator<active_cell_iterator> begin(predicate), end(predicate, tria.end());
      begin.set_to_next_positive(tria.begin_active());

      Assert(std::distance(begin, end) == static_cast<signed int>(tria.n_active_cells(3)),
             ExcInternalError());
      logfile << "Check 1: "
              << (std::distance(begin, end) == static_cast<signed int>(tria.n_active_cells(3)) ?
                    "OK" :
                    "Failed")
              << std::endl;
    };
  ...
  if (true)
    {
      ...
      FilteredIterator<active_cell_iterator> begin(predicate, tria.begin_active(3)),
