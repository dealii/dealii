...
bool
pred_locally_owned(
  const typename parallel::distributed::Triangulation<spacedim>::cell_iterator &cell)
{
...
test_hypercube(unsigned int ref, unsigned int max_bbox)
{
  ...
  typedef
    typename parallel::distributed::Triangulation<spacedim>::active_cell_iterator cell_iterator;
  ...
  typename parallel::distributed::Triangulation<dim, spacedim>::active_cell_iterator cell =
    tria.begin_active();
  typename parallel::distributed::Triangulation<dim, spacedim>::active_cell_iterator endc =
    tria.last_active();
