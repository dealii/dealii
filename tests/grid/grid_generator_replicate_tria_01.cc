...
void test()
{
  ...
  for (typename Triangulation<1>::active_cell_iterator
       cell=res1.begin_active(); cell!=res1.end(); ++cell)
    deallog << "(" << cell->center()[0] << ")" << std::endl;

  Triangulation<2,2> tr2;
  GridGenerator::hyper_cube_with_cylindrical_hole(tr2, 0.3,0.5);
  const std::array<Point<2>, 2> diags2 = {{Point<2>(-0.5,-0.5), Point<2>(0.5,0.5)}};
  ...
  for (typename Triangulation<2>::active_cell_iterator
       cell=res2.begin_active(); cell!=res2.end(); ++cell)
    deallog << "(" << cell->center()[0] << ","
            << cell->center()[1] << ")" << std::endl;

  Triangulation<3,3> tr3;
  std::array<Point<3>, 2> diags3 = {{Point<3>(-0.7,-0.5,-1.5), Point<3>(0.5,0.5,2.)}};
  std::array<unsigned int, 3> reps3 = {{2,2,2}};
  std::vector<unsigned int> rp(reps3.begin(), reps3.end());
  GridGenerator::subdivided_hyper_rectangle(tr3, rp, diags3.front(), diags3.back());
  ...
  for (typename Triangulation<3>::active_cell_iterator
       cell=res3.begin_active(); cell!=res3.end(); ++cell)
