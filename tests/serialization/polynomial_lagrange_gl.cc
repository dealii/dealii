...
test()
{
  ...
  Polynomials::Polynomial<double> p1 =
    Polynomials::generate_complete_Lagrange_basis(quad1.get_points())[support_point1];

  unsigned int                    n2             = 4;
  unsigned int                    support_point2 = 2;
  QGaussLobatto<1>                quad2(n2 + 1);
  Polynomials::Polynomial<double> p2 =
    Polynomials::generate_complete_Lagrange_basis(quad2.get_points())[support_point2];
