...
// Check and illustrate the serialization process for
// ParticleHandler. This specific test is like the _01 test, but in
// addition serializes the data of the particles individually, using
// the Particles::ParticleAccessor::serialize() function that had been
// declared but not implemented at the time this test was written.
...
test()
{
  ...
  std::ostringstream oss;
  {
    ...
    // now also write the particles by hand into the stream. this is
    // unnecessary, since it has been done already in the previous
    // line, but it tests ParticleAccessor::serialize
    for (auto &cell : tr.active_cell_iterators())
      {
        const boost::iterator_range<
          typename Particles::ParticleHandler<dim, spacedim>::particle_iterator>
