...
// In the case where there are 5 or more cells adjacent to one edge, special
// care about the orientation of that edge is required.
//
// Here, we set up a simple Maxwell problem, for which the solution is known,
// and compute the solution via FEM on a mesh where 5 or more cells are adjacent
// to one edge. If the orientation of that edge, which is adjacent to 5 or more
// cells, is not handled correctly, this leads to an error in the FEM solution.
// Therefore, this test compares the FEM solution with the known solution. If
// the edge orientation is not handled correctly, this test will fail.
