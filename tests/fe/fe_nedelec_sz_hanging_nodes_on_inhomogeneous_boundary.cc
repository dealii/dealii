...
namespace ConvergenceTest
{
  ...
  {
  public:
    MaxwellProblem(unsigned int refinements, unsigned int poly_degree);
  ...
  MaxwellProblem<dim>::run()
  {
    ...
    // compute the error
    double L2_error = output_error();

    // check that we are close to the analytical solution
    bool passed = (std::fabs(L2_error) < 1e-2);
    ...
    if (passed)
    ...
    else
      {
        ...
        deallog << "L2 Error = " << L2_error << "(threshold = 1e-2)"
                << std::endl;
      }
  }
} // namespace ConvergenceTest
...
main()
{
  ...
  MaxwellProblem<2> maxwell_2d(refinements, poly_degree);
  maxwell_2d.run();

  MaxwellProblem<3> maxwell_3d(refinements, poly_degree);
  maxwell_3d.run();
