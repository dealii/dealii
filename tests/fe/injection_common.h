...
{
  FullMatrix<double> injection_1(fine_fe.dofs_per_cell, coarse_fe.dofs_per_cell);
  FullMatrix<double> injection_2(fine_fe.dofs_per_cell, coarse_fe.dofs_per_cell);

  for (unsigned int child_1 = 0; child_1 < GeometryInfo<dim>::max_children_per_cell; ++child_1)
    for (unsigned int child_2 = 0; child_2 < GeometryInfo<dim>::max_children_per_cell; ++child_2)
      {
        injection_1 = 0;
        injection_2 = 0;
        ...
        // to finer cells
        {
          FullMatrix<double> tmp1(fine_fe.dofs_per_cell, coarse_fe.dofs_per_cell);
          FullMatrix<double> tmp2(fine_fe.dofs_per_cell, coarse_fe.dofs_per_cell);
        ...
        // then to finer fe
        {
          FullMatrix<double> tmp1(coarse_fe.dofs_per_cell, coarse_fe.dofs_per_cell);
          FullMatrix<double> tmp2(fine_fe.dofs_per_cell, coarse_fe.dofs_per_cell);
        ...
        AssertThrow(injection_2.frobenius_norm() <= 1e-12 * injection_1.frobenius_norm(),
                    ExcInternalError());
      }
}
