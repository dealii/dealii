  ...
  for (; cell != endc; ++cell)
    {
      ...
      std::vector<std::vector<Tensor<1, 3>>> grads_here(n_q_points,
                                                        std::vector<Tensor<1, 3>>(n_components));
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              w0 +=
                (solution(local_dof_indices[i]) * fe_values.shape_value_component(i, q_point, 2));
            }
