  ...
  for (unsigned cycle = 0; cycle < n_cycles; ++cycle)
    {
          ...
          for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
            {
              ...
              if (dim == 3)
                {
                  total_curl_curl[0] +=
                    JxW_values[q_point] * (hessians[q_point][1][0][1] + hessians[q_point][2][0][2] -
                                           hessians[q_point][0][1][1] - hessians[q_point][0][2][2]);
                  total_curl_curl[1] +=
                    JxW_values[q_point] * (hessians[q_point][2][1][2] + hessians[q_point][0][0][1] -
                                           hessians[q_point][1][2][2] - hessians[q_point][1][0][0]);
                  total_curl_curl[2] +=
                    JxW_values[q_point] * (hessians[q_point][0][0][2] + hessians[q_point][1][1][2] -
              ...
              for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
                {
                  ...
                  if (dim == 2)
                    n_x_v[0] =
                      (-normal[1] * face_values[q_point][0] + normal[0] * face_values[q_point][1]);
                  else if (dim == 3)
                    cross_product(
                      *reinterpret_cast<Tensor<1, dim> *>(&n_x_v), normal, face_values[q_point]);
                  ...
                  if (dim == 3)
                    {
                      ...
                      cross_product(n_x_curl_u,
                                    normal,
                                    *reinterpret_cast<Tensor<1, dim> *>(&face_curls[q_point]));
                      if (cell->at_boundary(face))
      ...
      deallog << dof_handler.n_dofs() << "\t\t" << diff.l1_norm() << "\t" << total_curl.norm()
              ...
              << boundary_curl_curl_traces.norm() << "\t" << total_div << "\t" << boundary_flux
              << std::endl;
