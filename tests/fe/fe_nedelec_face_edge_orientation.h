...
// line_orientation parameter which is used in deal.II on two-dimensional meshes.
//
...

{
  ...
  // rotated by 2*[pi/2]). Odd values of combined_face_orientation produce aligned 
  // faces (the middle square is not rotated).
                          ...
                          unsigned int N)
{
  ...
  for (unsigned int j = 0; j < N; j++)
   for (unsigned int i = 0; i < N; i++)
   {
     ...
     sampling_points_right.at(indx)[0] = 2.0 - sampling_points_left.at(indx)[0];
                        ...
                        std::vector<Point<2>>  & sampling_points_right)
{
  ...
  {
    ofs_left << 
      sampling_points_left.at(i)[0] << " , " << 
      sampling_points_left.at(i)[1] << std::endl; 

    ofs_right << 
      sampling_points_right.at(i)[0] << " , " << 
...
run(unsigned char combined_face_orientation)
{
 ...
 // save_sampling_points<dim>(sampling_points_left, sampling_points_right);
  ...
  for (unsigned int i = 0; i < Q; i++)
  {
    ...
    field_function.vector_value_list(sampling_points_left,  shape_function_left);
    field_function.vector_value_list(sampling_points_right, shape_function_right);
    ...
    if (result.error)
      deallog << "error: " 
        << result.cum_norm_left << "   " 
        << result.cum_norm_right << "   " 
        << result.cum_norm_diff << std::endl;
