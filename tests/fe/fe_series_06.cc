...
void test_2d ()
{
  ...
  double dofs[] = {0.0000000000000000e+00, 0.0000000000000000e+00, 0.0000000000000000e+00, 2.3801522930483391e-04, 0.0000000000000000e+00, 1.1949018981806140e-04, 0.0000000000000000e+00, 1.1949019042912971e-04, 5.9982796422221083e-05};
  ...
  for (unsigned int i = 0; i < fourier_coefficients.size(0); i++)
    for (unsigned int j = 0; j < fourier_coefficients.size(1); j++)
      if ((i*i+j*j < N*N) && ( i*i+j*j>0))
        deallog << (i*i+j*j)
                << " : " << fourier_coefficients(i,j) << std::endl;
