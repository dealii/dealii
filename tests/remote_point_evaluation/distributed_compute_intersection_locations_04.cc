...
// (3D-3D). We request the intersection for hexahedral and simplex elements
...
main(int argc, char **argv)
{
  ...
  deallog << "Cell intersections 3D-3D with marked vertices" << std::endl;
  do_test<3, 3>(true);
  deallog << "Cell intersections 3D-3D without marked vertices" << std::endl;
  do_test<3, 3>(false);
}
