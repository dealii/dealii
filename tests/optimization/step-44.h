...
// This is a copy of step-44 (git rev 3f7e617), with modifications made
// to test the implementation of the line-search algorithm.
...
namespace Step44
{
  ...
  namespace Parameters
  {
    ...
    NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry("Use line search",
                          "false",
                          Patterns::Bool(),
                          "Use line-search minimization algorithm");
        prm.declare_entry(
          "Use line search approximate gradient",
          "false",
          Patterns::Bool(),
    ...
    NonlinearSolver::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        use_line_search   = prm.get_bool("Use line search");
        use_line_search_ptrb =
          prm.get_bool("Use line search approximate gradient");
      }
      prm.leave_subsection();
    }
    struct Time
  ...
  private:
    ...
    void
    assemble_system_tangent(BlockSparseMatrix<double> &tangent_matrix,
    ...
    void
    assemble_system_rhs(BlockVector<double> &system_rhs,
                        const bool           print = true);
    void
    ...
    for (; newton_iteration < parameters.max_iterations_NR; ++newton_iteration)
      {
        ...
        tangent_matrix = 0.0;
        system_rhs     = 0.0;
        assemble_system_rhs(system_rhs);
        ...
        assemble_system_tangent(this->tangent_matrix);
        ...
        residual_0 *= -1.0; // Residual = -RHS
        constraints.set_zero(residual_0);
        const double tang_mtrx_norm_old =
        ...
        auto ls_minimization_function =
          [this, &solution_delta, &newton_update, &tang_mtrx_norm_old](
            ...
            // Compute residual for proposed step
            this->update_qph_incremental(solution_delta_trial, false);
            this->assemble_system_rhs(this->system_rhs, false);
            this->assemble_system_tangent(this->tangent_matrix, false);
            constraints.condense(this->tangent_matrix, this->system_rhs);
            ...
            if (ss != 0.0)
              DEAL_II_Assert(tang_mtrx_norm_new != tang_mtrx_norm_old,
                             ExcInternalError());

            // Negelect the constrained entries in the consideration
            this->constraints.set_zero(residual_trial);

            // Wriggers p159 eq 5.11
            const double f = 0.5 * (residual_trial * residual_trial); // Value
            // Wriggers p159 eq 5.14 is wrong...
            // const double g = -(residual_0*residual_trial); // Gradient
            // This should be g = G(V + alpha*delta)*[ K(V + alpha*delta)*delta
            // ]
            BlockVector<double> tmp(newton_update);
        ...
        // Minimization function (perturbation method, to confirm result
        // of exact minimization function's gradient -- this verifies
        // that Wriggers 5.14 is incorrect)
        auto ls_minimization_function_ptrb =
          [&ls_minimization_function](const double ss) {
            const double ptrb    = 0.005 * 1e-2;
            const auto   f_m     = ls_minimization_function(ss);
            const auto   f_minus = ls_minimization_function(ss - ptrb);
            const auto   f_plus  = ls_minimization_function(ss + ptrb);
            return std::make_pair(f_m.first,
                                  (f_plus.first - f_minus.first) / (2. * ptrb));
          };

        // Invoke line search
        const bool use_ptrb           = parameters.use_line_search_ptrb;
        auto       perform_linesearch = [&ls_minimization_function,
          ...
          DEAL_II_Assert(res_0.second < 0.0,
                         ExcMessage(
                           "Gradient should be negative. Current value: " +
        ...
        const double linesearch_step_size =
          (parameters.use_line_search ? perform_linesearch() : 1.0);
        ...
        std::cout << " | " << std::fixed << std::setprecision(3) << std::setw(7)
                  ...
                  << lin_solver_output.second << "  " << linesearch_step_size
  ...
  {
    static const unsigned int l_width = 166;
    for (unsigned int i = 0; i < l_width; ++i)
      std::cout << "_";
    std::cout << std::endl;
    std::cout << "                 SOLVER STEP                  "
              << " |  LIN_IT   LIN_RES   LS_STP_SZ   RES_NORM    "
  ...
  template <int dim>
  void
  Solid<dim>::assemble_system_tangent(BlockSparseMatrix<double> &tangent_matrix,
                                      const bool                 print)
  {
    ...
    const UpdateFlags uf_cell(update_values | update_gradients |
                              update_JxW_values);
    PerTaskData_K     per_task_data(dofs_per_cell, tangent_matrix);
  ...
  {
    BlockSparseMatrix<double> &tangent_matrix = *data.tangent_matrix;
  ...
  template <int dim>
  void
  Solid<dim>::assemble_system_rhs(BlockVector<double> &system_rhs,
                                  const bool           print)
  {
    ...
    const UpdateFlags uf_face(update_values | update_normal_vectors |
                              update_JxW_values);
    PerTaskData_RHS   per_task_data(dofs_per_cell, system_rhs);
  ...
  {
    BlockVector<double> &system_rhs = *data.system_rhs;
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
