...
// Test invariants: Coupled transverse isotropic
...
run()
{
  ...
  const auto test_invariant = [&C,
    ...
    std::cout << "\nCmpd Ii: " << TestInvariants::Ii(i, C, C_inv, H, G)
              << "\nExpt Ii: " << values.Psi << std::endl;
    std::cout << "\nCmpd dIi_dC: " << TestInvariants::dIi_dC(i, C, C_inv, H, G)
              << "\nExpt dIi_dC: " << values.dPsi_dC << std::endl;
    std::cout << "\nCmpd dIi_dH: " << TestInvariants::dIi_dH(i, C, C_inv, H, G)
              << "\nExpt dIi_dH: " << values.dPsi_dH << std::endl;
    std::cout << "\nCmpd d2Ii_dC_dC: "
              << TestInvariants::d2Ii_dC_dC(i, C, C_inv, H, G)
              << "\nExpt d2Ii_dC_dC: " << values.d2Psi_dC_dC << std::endl;
    std::cout << "\nCmpd d2Ii_dH_dH: "
              << TestInvariants::d2Ii_dH_dH(i, C, C_inv, H, G)
              << "\nExpt d2Ii_dH_dH: " << values.d2Psi_dH_dH << std::endl;
    std::cout << "\nCmpd d2Ii_dC_dH: "
              << TestInvariants::d2Ii_dC_dH(i, C, C_inv, H, G)
              << "\nExpt d2Ii_dC_dH: " << values.d2Psi_dC_dH << std::endl;
    std::cout << "\nCmpd d2Ii_dH_dC: "
              << TestInvariants::d2Ii_dH_dC(i, C, C_inv, H, G)
              << "\nExpt d2Ii_dH_dC: " << values.d2Psi_dH_dC << std::endl;

    const double tol = (dim == 2 ? 1e-12 : 1e-9);
    Assert(std::abs(TestInvariants::Ii(i, C, C_inv, H, G) - values.Psi) < tol,
           ExcMessage("No match in value."));
    Assert((TestInvariants::dIi_dC(i, C, C_inv, H, G) - values.dPsi_dC).norm() <
             tol,
           ExcMessage("No match in first derivative."));
    Assert((TestInvariants::dIi_dH(i, C, C_inv, H, G) - values.dPsi_dH).norm() <
             tol,
           ExcMessage("No match in first derivative."));
    Assert((TestInvariants::d2Ii_dC_dC(i, C, C_inv, H, G) - values.d2Psi_dC_dC)
               .norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((TestInvariants::d2Ii_dH_dH(i, C, C_inv, H, G) - values.d2Psi_dH_dH)
               .norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((TestInvariants::d2Ii_dC_dH(i, C, C_inv, H, G) - values.d2Psi_dC_dH)
               .norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((TestInvariants::d2Ii_dH_dC(i, C, C_inv, H, G) - values.d2Psi_dH_dC)
               .norm() < tol,
           ExcMessage("No match in second derivative."));
  };
