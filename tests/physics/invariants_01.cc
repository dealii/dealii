...
run()
{
  ...
  const auto test_invariant =
    [&C, &C_inv](const enum InvariantList &    i,
      ...
      Assert(std::abs(TestInvariants::Ii(i, C) - values.Psi) < tol,
             ExcMessage("No match in value."));
      Assert((TestInvariants::dIi_dC(i, C, C_inv) - values.dPsi_dC).norm() <
               tol,
             ExcMessage("No match in first derivative."));
      Assert((TestInvariants::d2Ii_dC_dC(i, C, C_inv) - values.d2Psi_dC_dC)
                 .norm() < tol,
  ...
  // First invariant
  {
    ...
    const psi_function_type<dim> get_psi_ad =
      ...
      const ADNumberType psi_ad = TestInvariants::Ii(I1, C_ad);
      return psi_ad;
    };
  ...
  // Second invariant
  {
    ...
    const psi_function_type<dim> get_psi_ad =
      [](const SymmetricTensor<2, dim, ADNumberType> &C_ad) -> ADNumberType {
      const ADNumberType psi_ad = TestInvariants::Ii(I2, C_ad);
      return psi_ad;
    };
  ...
  // Third invariant
  {
    ...
    const psi_function_type<dim> get_psi_ad =
      [](const SymmetricTensor<2, dim, ADNumberType> &C_ad) -> ADNumberType {
      const ADNumberType psi_ad = TestInvariants::Ii(I3, C_ad);
      return psi_ad;
    };
  ...
  // Pseudo third invariant
  {
    ...
    const psi_function_type<dim> get_psi_ad =
      [](const SymmetricTensor<2, dim, ADNumberType> &C_ad) -> ADNumberType {
      const ADNumberType psi_ad = TestInvariants::Ii(pI3, C_ad);
      return psi_ad;
    };
