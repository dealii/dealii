...
// Test constitutive models: Coupled isotropic
...
run()
{
  ...
  const double coeff = (dim == 2 ? 1e-9 : 1e-16);
  ...
  //   when i != j.
  {
    ...
    for (const auto i : TestInvariants::valid_invariants())
      {
        ...
        double dPsi_dIi = 1.0 * coeff;
        for (const auto kk : TestInvariants::valid_invariants())
          {
            if (kk == i)
              dPsi_dIi *=
                3.0 *
                std::pow(ConstitutiveModel::InvariantsType::Ii(kk, C, C_inv, H),
                         2);
            else
              dPsi_dIi *=
                std::pow(ConstitutiveModel::InvariantsType::Ii(kk, C, C_inv, H),
        ...
        for (const auto j : TestInvariants::valid_invariants())
          {
            double d2Psi_dIi_dIj = 1.0 * coeff;
                ...
                if (i == j)
                  {
                    if (kk == i)
                      d2Psi_dIi_dIj *=
                        6.0 *
                        ConstitutiveModel::InvariantsType::Ii(kk, C, C_inv, H);
                    else
    ...
    std::cout << "\nCmpd dPsi_dC: " << cm.get_dPsi_dC(C, C_inv, H)
              << "\nExpt dPsi_dC: " << values.dPsi_dC << std::endl;
    std::cout << "\nCmpd dPsi_dH: " << cm.get_dPsi_dH(C, C_inv, H)
              << "\nExpt dPsi_dH: " << values.dPsi_dH << std::endl;
    std::cout << "\nCmpd d2Psi_dC_dC: " << cm.get_d2Psi_dC_dC(C, C_inv, H)
              << "\nExpt d2Psi_dC_dC: " << values.d2Psi_dC_dC << std::endl;
    std::cout << "\nCmpd d2Psi_dH_dH: " << cm.get_d2Psi_dH_dH(C, C_inv, H)
              << "\nExpt d2Psi_dH_dH: " << values.d2Psi_dH_dH << std::endl;
    std::cout << "\nCmpd d2Psi_dC_dH: " << cm.get_d2Psi_dC_dH(C, C_inv, H)
              << "\nExpt d2Psi_dC_dH: " << values.d2Psi_dC_dH << std::endl;
    std::cout << "\nCmpd d2Psi_dH_dC: " << cm.get_d2Psi_dH_dC(C, C_inv, H)
              << "\nExpt d2Psi_dH_dC: " << values.d2Psi_dH_dC << std::endl;

    const double tol = (dim == 2 ? 1e-9 : 1e-6);
    Assert((cm.get_dPsi_dC(C, C_inv, H) - values.dPsi_dC).norm() < tol,
           ExcMessage("No match in first derivative."));
    Assert((cm.get_dPsi_dH(C, C_inv, H) - values.dPsi_dH).norm() < tol,
           ExcMessage("No match in first derivative."));
    Assert((cm.get_d2Psi_dC_dC(C, C_inv, H) - values.d2Psi_dC_dC).norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((cm.get_d2Psi_dH_dH(C, C_inv, H) - values.d2Psi_dH_dH).norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((cm.get_d2Psi_dC_dH(C, C_inv, H) - values.d2Psi_dC_dH).norm() < tol,
           ExcMessage("No match in second derivative."));
    Assert((cm.get_d2Psi_dH_dC(C, C_inv, H) - values.d2Psi_dH_dC).norm() < tol,
           ExcMessage("No match in second derivative."));
