...
// Test constitutive models: Transverse Isotropic
...
run()
{
  ...
  const psi_function_type<dim> get_psi_ad =
    ...
    for (const auto i : TestInvariantsAD::valid_invariants())
      psi_ad *= std::pow(TestInvariantsAD::Ii(i, C_ad, C_inv_ad, G), 3);
  ...
  //   when i != j.
  {
    ...
    std::cout << "\nCmpd dPsi_dC: " << cm.get_dPsi_dC(C, C_inv, G)
              << "\nExpt dPsi_dC: " << values.dPsi_dC << std::endl;
    std::cout << "\nCmpd d2Psi_dC_dC: " << cm.get_d2Psi_dC_dC(C, C_inv, G)
              << "\nExpt d2Psi_dC_dC: " << values.d2Psi_dC_dC << std::endl;

    const double tol = (dim == 2 ? 1e-9 : 1e-6);
    Assert((cm.get_dPsi_dC(C, C_inv, G) - values.dPsi_dC).norm() < tol,
