      ...
      for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
        {
          ...
          DEAL_II_Assert((F_t1 - unit_symmetric_tensor<dim>()).norm() < tol,
                         ExcMessage("Incorrect computation of F_t1"));
          const Tensor<2, dim> F = Kinematics::F(Grad_u);
          DEAL_II_Assert((F - (static_cast<Tensor<2, dim>>(
          ...
          DEAL_II_Assert(determinant(F) != 1.0,
                         ExcMessage(
                           "No volume change - cannot test vol/iso split"));
          DEAL_II_Assert(std::abs(determinant(Kinematics::F_iso(F)) - 1.0) <
                           tol,
                         ExcMessage("F_iso is not volume preserving"));
          DEAL_II_Assert(std::abs(determinant(Kinematics::F_vol(F)) -
                                  determinant(F)) < tol,
                         ExcMessage("F_vol has no dilatating action"));

          // Right Cauchy-Green tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::C(F)) -
                          transpose(F) * F)
                             .norm() < tol,
                         ExcMessage("Incorrect computation of C"));

          // Left Cauchy-Green tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::b(F)) -
                          F * transpose(F))
                             .norm() < tol,
                         ExcMessage("Incorrect computation of b"));

          // Small strain tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(
                            Kinematics::epsilon(Grad_u)) -
                          0.5 * (Grad_u + transpose(Grad_u)))
                             .norm() < tol,
                         ExcMessage("Incorrect computation of epsilon"));

          // Green-Lagrange strain tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::E(F)) -
                          0.5 * (Grad_u + transpose(Grad_u) +
                                 transpose(Grad_u) * Grad_u))
                             .norm() < tol,
          ...
          DEAL_II_Assert(
            (F_dot - (1.0 / delta_t) * (Grad_u - Grad_u_t1)).norm() < tol,
          ...
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::l(F, F_dot)) -
                          dot_grad_u)
                             .norm() < tol,
                         ExcMessage("Incorrect computation of l"));

          // Rate of deformation tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::d(F, F_dot)) -
                          0.5 * (dot_grad_u + transpose(dot_grad_u)))
                             .norm() < tol,
                         ExcMessage("Incorrect computation of d"));

          // Rate of spin tensor
          DEAL_II_Assert((static_cast<Tensor<2, dim>>(Kinematics::w(F, F_dot)) -
                          0.5 * (dot_grad_u - transpose(dot_grad_u)))
                             .norm() < tol,
