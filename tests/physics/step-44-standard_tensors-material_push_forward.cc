...
namespace Step44
{
  ...
  namespace Parameters
  {
    ...
    FESystem::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry(
          "Polynomial degree", "2", Patterns::Integer(0), "Displacement system polynomial order");
    ...
    Geometry::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry(
          "Global refinement", "2", Patterns::Integer(0), "Global refinement level");
        prm.declare_entry(
          "Grid scale", "1e-3", Patterns::Double(0.0), "Global grid scaling factor");
    ...
    Materials::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry(
          "Poisson's ratio", "0.4999", Patterns::Double(-1.0, 0.5), "Poisson's ratio");
    ...
    NonlinearSolver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Nonlinear solver");
      {
        ...
        prm.declare_entry(
          "Tolerance force", "1.0e-9", Patterns::Double(0.0), "Force residual tolerance");
      ...
      if (std::abs(det_F - 1.0) > 1e-9)
        {
          ...
          Assert(
            (Physics::Transformations::Contravariant::push_forward(get_S_vol(), F) - get_tau_vol())
                  .norm() /
                get_tau_vol().norm() <
              tol,
            ExcInternalError());
          Assert(
            (Physics::Transformations::Contravariant::push_forward(get_S_iso(), F) - get_tau_iso())
                  .norm() /
                get_tau_iso().norm() <
              tol,
            ExcInternalError());
          ...
          Assert(
            (Physics::Transformations::Contravariant::pull_back(get_tau_vol(), F) - get_S_vol())
                  .norm() /
                get_S_vol().norm() <
              tol,
            ExcInternalError());
          Assert(
            (Physics::Transformations::Contravariant::pull_back(get_tau_iso(), F) - get_S_iso())
                  .norm() /
                get_S_iso().norm() <
              tol,
            ExcInternalError());
      ...
      if (std::abs(det_F - 1.0) > 1e-9)
        {
          ...
          // Verify the push-forward transformation
          Assert(
            (Physics::Transformations::Contravariant::push_forward(get_H_vol(), F) - get_Jc_vol())
                  .norm() /
                get_Jc_vol().norm() <
              tol,
            ExcInternalError());
          Assert(
            (Physics::Transformations::Contravariant::push_forward(get_H_iso(), F) - get_Jc_iso())
                  .norm() /
                get_Jc_iso().norm() <
              tol,
            ExcInternalError());
          ...
          // Verify the pull-back transformation
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_vol(), F) - get_H_vol())
                       .norm() /
                     get_H_vol().norm() <
                   tol,
                 ExcInternalError());
          Assert((Physics::Transformations::Contravariant::pull_back(get_Jc_iso(), F) - get_H_iso())
                       .norm() /
                     get_H_iso().norm() <
                   tol,
                 ExcInternalError());
  ...
  protected:
    ...
    get_H_vol() const
    {
      ...
      return p_tilde * det_F *
             (C_inv_x_C_inv + (2.0 * Physics::Elasticity::StandardTensors<dim>::dC_inv_dC(F)));
    }
    SymmetricTensor<4, dim>
    get_H_iso() const
    {
  ...
  public:
    ...
    {
      material.reset(
        new Material_Compressible_Neo_Hook_Three_Field<dim>(parameters.mu, parameters.nu));
  ...
  private:
    ...
    Errors error_residual, error_residual_0, error_residual_norm, error_update, error_update_0,
      error_update_norm;
    void
  ...
  Solid<dim>::run()
  {
    ...
    system_setup();
    {
      ...
      VectorTools::project(
        dof_handler_ref, constraints, QGauss<dim>(degree + 2), J_mask, solution_n);
    }
    // output_results();
    time.increment();
  ...
  {
    pcout << std::endl
          << "Timestep " << time.get_timestep() << " @ " << time.current() << "s" << std::endl;
    ...
    for (; newton_iteration < parameters.max_iterations_NR; ++newton_iteration)
      {
        ...
        pcout << " | " << std::fixed << std::setprecision(3) << std::setw(7) << std::scientific
              << lin_solver_output.first << "  " << lin_solver_output.second << "  "
              ...
              << error_update_norm.p << "  " << error_update_norm.J << "  " << std::endl;
      }
  ...
  template <int dim>
  void
  Solid<dim>::get_error_update(const BlockVector<double> &newton_update, Errors &error_update)
  {
  ...
  Solid<dim>::assemble_system_tangent()
  {
    ...
    WorkStream::run(dof_handler_ref.begin_active(),
                    ...
                    std::bind(&Solid<dim>::assemble_system_tangent_one_cell,
                              ...
                              std::placeholders::_2,
                              std::placeholders::_3),
                    std::bind(&Solid<dim>::copy_local_to_global_K, this, std::placeholders::_1),
                    scratch_data,
                    per_task_data);
    timer.leave_subsection();
  }
  ...
  Solid<dim>::copy_local_to_global_K(const PerTaskData_K &data)
  {
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      for (unsigned int j = 0; j < dofs_per_cell; ++j)
        tangent_matrix.add(
          data.local_dof_indices[i], data.local_dof_indices[j], data.cell_matrix(i, j));
  }
  template <int dim>
  void
                ...
                if ((i_group == j_group) && (i_group == u_dof))
                  {
                    ...
                    if (component_i == component_j) // geometrical stress contribution
                      data.cell_matrix(i, j) +=
                        grad_Nx[i][component_i] * tau * grad_Nx[j][component_j] * JxW;
                  }
                else if ((i_group == p_dof) && (j_group == u_dof))
                  {
                    data.cell_matrix(i, j) +=
                      N[i] * det_F *
                      (symm_grad_Nx[j] * Physics::Elasticity::StandardTensors<dim>::I) * JxW;
                  }
  ...
  Solid<dim>::assemble_system_rhs()
  {
    ...
    WorkStream::run(dof_handler_ref.begin_active(),
                    ...
                    std::bind(&Solid<dim>::assemble_system_rhs_one_cell,
                              ...
                              std::placeholders::_2,
                              std::placeholders::_3),
                    std::bind(&Solid<dim>::copy_local_to_global_rhs, this, std::placeholders::_1),
                    scratch_data,
                    per_task_data);
    timer.leave_subsection();
  }
  ...
  Solid<dim>::assemble_sc()
  {
    ...
    PerTaskData_SC per_task_data(
      dofs_per_cell, element_indices_u.size(), element_indices_p.size(), element_indices_J.size());
          ...
          if (parameters.type_lin == "CG")
            {
              const int solver_its =
                tangent_matrix.block(u_dof, u_dof).m() * parameters.max_iterations_lin;
        ...
        if (parameters.type_lin == "CG")
          {
            ...
            PreconditionSelector<SparseMatrix<double>, Vector<double>> preconditioner_K_Jp_inv(
              "jacobi");
            ...
            const auto K_uu_con_inv =
              inverse_operator(K_uu_con, solver_K_con_inv, preconditioner_K_con_inv);
  ...
  Solid<dim>::output_results() const
  {
    ...
    data_out.add_data_vector(
      solution_n, solution_name, DataOut<dim>::type_dof_data, data_component_interpretation);
