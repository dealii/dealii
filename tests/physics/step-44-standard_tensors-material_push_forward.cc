      ...
      if(std::abs(det_F - 1.0) > 1e-9)
        {
          ...
          // Verify the pull-back transformation
          Assert(
            (Physics::Transformations::Contravariant::pull_back(get_Jc_vol(), F)
             - get_H_vol())
                  .norm()
                / get_H_vol().norm()
              < tol,
            ExcInternalError());
          Assert(
            (Physics::Transformations::Contravariant::pull_back(get_Jc_iso(), F)
             - get_H_iso())
                  .norm()
                / get_H_iso().norm()
              < tol,
        ...
        if(parameters.type_lin == "CG")
          {
            ...
            const auto K_Jp_inv = inverse_operator(
              K_Jp, solver_K_Jp_inv, preconditioner_K_Jp_inv);
