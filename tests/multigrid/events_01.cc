...
namespace Step50
{
            ...
            for (unsigned int j = 0; j < dofs_per_cell; ++j)
              if (interface_dofs_on_level.is_element(local_dof_indices[i]) // at_refinement_edge(i)
                  ...
                  ((!mg_constrained_dofs.is_boundary_index(lvl, local_dof_indices[i]) &&
                   ...
                   || (mg_constrained_dofs.is_boundary_index(lvl, local_dof_indices[i]) &&
                       local_dof_indices[i] == local_dof_indices[j]) // ( boundary(i) && boundary(j)
                                                                     // && i==j )
                   ))
                {
  ...
  LaplaceProblem<dim>::vcycle(const MGCoarseGridBase<vector_t> &coarse_grid_solver)
  {
    ...
    Multigrid<vector_t> mg(
      mg_dof_handler, mg_matrix, coarse_grid_solver, mg_transfer, mg_smoother, mg_smoother);
    ...
    auto print_coarse_solve = [](const bool start, const unsigned int level) {
      deallog << "coarse_solve " << (start ? "started" : "finished") << " on level " << level
              << std::endl;
    };

    auto print_restriction = [](const bool start, const unsigned int level) {
      deallog << "restriction " << (start ? "started" : "finished") << " on level " << level
              << std::endl;
    };

    auto print_prolongation = [](const bool start, const unsigned int level) {
      deallog << "prolongation " << (start ? "started" : "finished") << " on level " << level
              << std::endl;
    };

    auto print_pre_smoother_step = [](const bool start, const unsigned int level) {
      deallog << "pre_smoother_step " << (start ? "started" : "finished") << " on level " << level
              << std::endl;
    };

    auto print_post_smoother_step = [](const bool start, const unsigned int level) {
      deallog << "post_smoother_step " << (start ? "started" : "finished") << " on level " << level
              << std::endl;
    };
