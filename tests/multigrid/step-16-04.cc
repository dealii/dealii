...
LaplaceProblem<dim>::solve()
{
  ...
  mg::Matrix<LinearAlgebra::distributed::Vector<double>> mg_interface_up(
    mg_interface_matrices);
  mg::Matrix<LinearAlgebra::distributed::Vector<double>> mg_interface_down(
    mg_interface_matrices);
  ...
  DEAL_II_Assert(min_level == mg.get_minlevel(), ExcInternalError());
  DEAL_II_Assert(triangulation.n_global_levels() - 1 == mg.get_maxlevel(),
                 ExcInternalError());
