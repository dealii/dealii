...
#ifndef dealii_portable_multigrid_transfer_tests_h
#define dealii_portable_multigrid_transfer_tests_h
...
void
print_if_non_zero(
  const LinearAlgebra::distributed::Vector<Number, MemorySpace::Host> &vec,
...
test_transfer_operator(
  const Portable::MGTwoLevelTransfer<
    dim,
    LinearAlgebra::distributed::Vector<Number, MemorySpace::Default>> &transfer,
  ...
  const unsigned int mg_level_coarse = numbers::invalid_unsigned_int)
{
  ...
  // test prolongation
  {
    ...
    if (Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD) == 1 && false)
      {
        FullMatrix<Number> prolongation_matrix(dst_host.size(),
                                               src_host.size());
        for (unsigned int i = 0; i < src_host.size(); ++i)
          {
            ...
            for (unsigned int j = 0; j < dst_host.size(); ++j)
              prolongation_matrix[j][i] = dst_host[j];
          }
  ...
  // test restriction
  {
    ...
    if (true)
      {
        deallog << src_host.l2_norm() << std::endl;
      }
    ...
    if (Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD) == 1 && false)
      {
        FullMatrix<Number> restriction_matrix(src_host.size(), dst_host.size());
        for (unsigned int i = 0; i < dst_host.size(); ++i)
          {
            dst_host    = 0.0;
            dst_host[i] = 1.0;
            src_host    = 0.0;
            ...
            for (unsigned int j = 0; j < src_host.size(); ++j)
              restriction_matrix[j][i] = src_host[j];
          }
