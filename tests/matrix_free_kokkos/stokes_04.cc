...
// Solve the step-104 manufactured Stokes problem with Portable::MatrixFree
// and an adaptive distributed mesh.
...
void
solve_cycle(parallel::distributed::Triangulation<dim> &tria,
            const unsigned int                         cycle)
{
  ...
  parallel::distributed::GridRefinement::refine_and_coarsen_fixed_fraction(
    tria, cellwise_errors_ul2, 0.3, 0.0);
...
test()
{
  ...
  for (unsigned int cycle = 0; cycle < 3; ++cycle)
    solve_cycle<dim, degree_p>(tria, cycle);
}
