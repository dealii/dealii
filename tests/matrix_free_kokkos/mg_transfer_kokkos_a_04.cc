...
/**
 * Like mg_transfer_copy_to_host_a_04.cc / mg_transfer_a_04.cc but for
 * Portable::MGTwoLevelTransfer (serial triangulation, adaptive refinement).
 */
  ...
  if (fe_degree > 0)
    {
      ...
      do_test<dim, Number>(FE_Q<dim>(fe_degree), FE_Q<dim>(fe_degree));
      deallog.pop();
    }
}
...
main(int argc, char **argv)
{
  ...
  // Match mg_transfer_copy_to_host_a_04 CG coverage (degree 1).
  test<2, double>(1);
}
