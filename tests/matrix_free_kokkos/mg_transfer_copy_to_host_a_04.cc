...
/**
 * Like mg_transfer_a_04.cc but for MGTwoLevelTransferCopyToHost.
 */
...
do_test(const FiniteElement<dim> &fe_fine, const FiniteElement<dim> &fe_coarse)
{
  ...
  test_copy_to_host_transfer_operator(transfer,
                                      dof_handler_fine,
                                      dof_handler_coarse);
  ...
  if (fe_degree > 0)
    {
      ...
      do_test<dim, Number, MemorySpace>(FE_DGQ<dim>(fe_degree),
                                        FE_Q<dim>(fe_degree));
      deallog.pop();
    }
...
main()
{
  ...
  for (unsigned int i = 0; i < 2; ++i)
    test<2, double, MemorySpace::Default>(i);
}
