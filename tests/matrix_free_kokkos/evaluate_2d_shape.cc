  ...
  Kokkos::View<double *, MemorySpace::Default::kokkos_space> tmp)
{
  ...
  if (type == 0)
    {
      evaluator.template values<0, dof_to_quad, false, false>(src, tmp);
      team_member.team_barrier();
      evaluator.template values<1, dof_to_quad, add, false>(tmp, dst);
    }
  if (type == 1)
    {
      evaluator.template gradients<0, dof_to_quad, false, false>(src, tmp);
      team_member.team_barrier();
      evaluator.template gradients<1, dof_to_quad, add, false>(tmp, dst);
    }
}
