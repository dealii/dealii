...
/**
 * Like mg_transfer_a_06.cc but for MGTwoLevelTransferCopyToHost.
 */
  ...
  const unsigned int           mg_level_coarse = numbers::invalid_unsigned_int)
{
  ...
  // test interpolate
  {
    VectorTools::interpolate(dof_handler_fine, function, dst_host); // dst = 1.0
  ...
  if ((fe_degree > 0) && (do_simplex_mesh == false))
    {
      deallog.push("HCG<" + str_dim + ">(" + str_fine + ")<->HCG<" + str_dim +
                   ">(" + str_coarse + ")");
      do_test<dim, Number, MemorySpace>(FE_Q<dim>(fe_degree),
                                        FE_Q<dim>(fe_degree),
                                        function);
      deallog.pop();
    }
  ...
  if ((fe_degree > 0) && do_simplex_mesh)
    {
      deallog.push("SCG<" + str_dim + ">(" + str_fine + ")<->SCG<" + str_dim +
                   ">(" + str_coarse + ")");
      do_test<dim, Number, MemorySpace>(FE_SimplexP<dim>(fe_degree),
...
main(int argc, char **argv)
{
  ...
  for (unsigned int i = 0; i <= 4; ++i)
    test<2, double, MemorySpace::Default>(i, fu, false);

  for (unsigned int i = 0; i <= 3; ++i)
    test<2, double, MemorySpace::Default>(i, fu, true);

  for (unsigned int i = 1; i <= 3; ++i)
    test<3, double, MemorySpace::Default>(i, fu3, true);
}
