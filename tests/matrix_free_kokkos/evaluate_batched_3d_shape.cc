...
// in evaluate_3d_shape.cc, where n_cells "elements" share one team and are
...
test(const int n_cells)
{
  ...
  const unsigned int                     N_batch = n_cells * N_3d;
  const unsigned int                     M_batch = n_cells * M_3d;
  ...
  Kokkos::View<double *, MemorySpace::Default::kokkos_space> tmp1_dev(
    Kokkos::view_alloc("tmp1_dev", Kokkos::WithoutInitializing), n_cells * M2N);
  Kokkos::View<double *, MemorySpace::Default::kokkos_space> tmp2_dev(
  ...
  // independently by the same 3d shape matrix.
  for (int e = 0; e < n_cells; ++e)
    for (unsigned int i = 0; i < M_3d; ++i)
      {
        y_host(e * M_3d + i) = 1.;
        y_ref[e * M_3d + i]  = add ? y_host(e * M_3d + i) : 0.;
        for (unsigned int j = 0; j < N_3d; ++j)
          y_ref[e * M_3d + i] += shape_3d(i, j) * x_host(e * N_3d + j);
      }
  ...
  for (int e = 0; e < n_cells; ++e)
    for (unsigned int i = 0; i < N_3d; ++i)
      {
        x_host(e * N_3d + i) = 2.;
        x_ref[e * N_3d + i]  = add ? x_host(e * N_3d + i) : 0.;
        for (unsigned int j = 0; j < M_3d; ++j)
          x_ref[e * N_3d + i] += shape_3d(j, i) * y_host(e * M_3d + j);
      }
