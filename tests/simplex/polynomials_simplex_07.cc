// ------------------------------------------------------------------------
//
// SPDX-License-Identifier: LGPL-2.1-or-later
// Copyright (C) 2020 - 2026 by the deal.II authors
//
// This file is part of the deal.II library.
//
// Part of the source code is dual licensed under Apache-2.0 WITH
// LLVM-exception OR LGPL-2.1-or-later. Detailed license information
// governing the source code and code contributions can be found in
// LICENSE.md and CONTRIBUTING.md at the top level directory of deal.II.
//
// ------------------------------------------------------------------------


// Test PolynomialsSimplex by comparing it to the barycentric implementation
// for 4th derivatives

#include <deal.II/base/polynomials_barycentric.h>
#include <deal.II/base/polynomials_simplex.h>
#include <deal.II/base/quadrature_lib.h>

#include <deal.II/fe/fe_simplex_p.h>

#include "../tests.h"



template <int dim>
void
test(const unsigned int degree)
{
  FE_SimplexP<dim> fe_simplex(degree);
  const auto       support_points = fe_simplex.get_unit_support_points();
  deallog << "N support points: " << support_points.size() << " at degree "
          << degree << std::endl;
  const auto poly = ScalarLagrangePolynomialSimplex(degree, support_points);
  const auto barycentric = BarycentricPolynomials<dim>::get_fe_p_basis(degree);


  std::vector<Point<dim>> points;
  for (const auto &p : support_points)
    points.emplace_back(p);
  // Add some random points
  if constexpr (dim == 2)
    {
      points.emplace_back(Point<dim>(0.36658, 0.58775));
      points.emplace_back(Point<dim>(0.64464, 0.3546));
      points.emplace_back(Point<dim>(0.0, 0.999));
    }
  else if constexpr (dim == 3)
    {
      points.emplace_back(Point<dim>(0.36658, 0.58775, 0.21455));
      points.emplace_back(Point<dim>(0.64464, 0.3546, 0.3246));
      points.emplace_back(Point<dim>(0.0, 0.0, 0.999));
    }

  // Add points from the quadrature
  QGaussSimplex<dim> quad(2);
  for (unsigned int i = 0; i < quad.size(); ++i)
    points.emplace_back(quad.point(i));

  for (unsigned int i = 0; i < points.size(); ++i)
    for (unsigned int j = 0; j < fe_simplex.n_dofs_per_cell(); ++j)
      {
        const auto d1 = poly.compute_4th_derivative(j, points[i]);
        const auto d2 = barycentric.compute_4th_derivative(j, points[i]);

        bool is_same = true;
        for (unsigned int d = 0; d < dim; ++d)
          for (unsigned int e = 0; e < dim; ++e)
            for (unsigned int f = 0; f < dim; ++f)
              for (unsigned int g = 0; g < dim; ++g)
                if (std::abs(d2[d][e][f][g] - d1[d][e][f][g]) > 1e-12)
                  is_same = false;

        if (is_same)
          deallog << "ok ";
        else
          deallog << "Failure in 2nd derivative!!! at point " << i
                  << " at location " << points[i] << " for shape function " << j
                  << " poly gives " << d1 << " while barycentric gives " << d2
                  << std::endl;

        deallog << std::endl;
      }
}


int
main()
{
  initlog();
  {
    deallog.push("1d-1");
    test<1>(1);
    deallog.pop();

    deallog.push("1d-2");
    test<1>(2);
    deallog.pop();

    deallog.push("1d-3");
    test<1>(3);
    deallog.pop();


    deallog.push("2d-1");
    test<2>(1);
    deallog.pop();

    deallog.push("2d-2");
    test<2>(2);
    deallog.pop();

    deallog.push("2d-3");
    test<2>(3);
    deallog.pop();


    deallog.push("3d-1");
    test<3>(1);
    deallog.pop();

    deallog.push("3d-2");
    test<3>(2);
    deallog.pop();

    deallog.push("3d-3");
    test<3>(3);
    deallog.pop();
  }
}
