...
test<2>(const unsigned int n_q)
{
  ...
  // test FESubFaceValues for mixed meshes; run this test twice, once refining
  // the only the quad cells and once only the tri cells to get the subface
  ...
  for (unsigned int step = 0; step < 2; ++step)
    {
      ...
      hp::FESubfaceValues<dim> hp_fe_subface_values(mapping,
                                                    fe,
                                                    quad_ref,
                                                    flags);
