...
// Test a mesh with two wedges for all possible orientations. Similar to
// orientation_04
...
{
  const ReferenceCell ref_cell  = ReferenceCells::Wedge;
  ...
  if (face_n < 2)
    {
      ...
      vertices.push_back(Point<3>(0.0, 0.0, -1.0));
      vertices.push_back(Point<3>(1.0, 0.0, -1.0));
      vertices.push_back(Point<3>(0.0, 1.0, -1.0));
      cell_data.back().vertices.resize(0);

      std::vector<unsigned int>     vertices_index = {{0, 1, 2}};
      ArrayView<const unsigned int> const_view(&vertices_index[0], 3);
      ...
      for (unsigned int i = 0; i < vertices_index.size(); ++i)
        cell_data.back().vertices.push_back(permuted_indices[i] + 6);
  ...
  else
    {
      ...
      for (unsigned int face_index = 2; face_index < 5 && found_config == false;
           ++face_index)
        for (unsigned int o = 0; o < 8 && found_config == false; ++o)
          {
            auto vertices = vertices_;
            ...
            vertices_index.emplace_back(
              ref_cell.face_to_cell_vertices(face_index, 3, o));

            ArrayView<const unsigned int> const_vertices_index_0(
              &vertices_index[0], 3);


            ArrayView<const unsigned int> const_vertices_index_1(
              &vertices_index[3], 3);


            for (unsigned int counter = 0; counter < 6 && found_config == false;
                 ++counter)
              {
                cell_data.back().vertices.resize(0);

                const auto permuted_indices_0 =
                  ReferenceCells::Triangle.permute_by_combined_orientation(
                    const_vertices_index_0, counter);
                const auto permuted_indices_1 =
                  ReferenceCells::Triangle.permute_by_combined_orientation(
                    const_vertices_index_1, counter);

                for (unsigned int i = 0; i < 3; ++i)
                  cell_data.back().vertices.push_back(permuted_indices_0[i]);
                for (unsigned int i = 0; i < 3; ++i)
                  cell_data.back().vertices.push_back(permuted_indices_1[i]);
...
test()
{
  ...
  for (unsigned int f = 0; f < 5; ++f)
    {
      const unsigned int n_orientations = f < 2 ? 6 : 8;
      for (unsigned int r = 0; r < n_orientations; ++r)
        {
          ...
          GridGenerator::reference_cell(dummy, ReferenceCells::Wedge);
          ...
          const unsigned int lines_per_face = face_no < 2 ? 3 : 4;
