...
// Test MappingP1's cell values for mapping quantities by comparing with
// MappingFE.
...
test()
{
  ...
  UpdateFlags flags =
    ...
    update_jacobian_pushed_forward_3rd_derivatives | update_volume_elements;
  if (dim + 1 == spacedim)
    flags |= update_normal_vectors | update_boundary_forms;
  ...
  for (const auto &cell : tria.active_cell_iterators())
    {
      ...
      deallog << "transform_unit_to_real_cell" << std::endl;
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          const Point<spacedim> p =
            mapping.transform_unit_to_real_cell(cell, quadrature.point(qp_n));
          deallog << "  " << p << std::endl;

          Assert((p - fe_values_2.quadrature_point(qp_n)).norm() < 1e-12,
                 ExcInternalError());
        }
      deallog << std::endl;

      deallog << "transform_real_to_unit_cell" << std::endl;
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          const Point<dim> p = mapping.transform_real_to_unit_cell(
            cell, fe_values_2.quadrature_point(qp_n));
          deallog << "  " << p << std::endl;

          Assert((quadrature.point(qp_n) - p).norm() < 1e-12,
                 ExcInternalError());
        }
      deallog << std::endl;
      ...
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          ...
          deallog << "  " << p << " -> " << fe_values.quadrature_point(qp_n)
                  << std::endl;
          Assert(reference_cell.contains_point(p, 1e-12), ExcInternalError());
      ...
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          ...
          for (unsigned int d = 0; d < spacedim; ++d)
            Assert((fe_values.jacobian(qp_n)[d] - fe_values_2.jacobian(qp_n)[d])
                       .norm() < 1e-12,
                   ExcInternalError());
        }
      deallog << std::endl;
      ...
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          deallog << "  all zero = "
                  << (fe_values.jacobian_grad(qp_n).norm() == 0) << std::endl;
      ...
      for (unsigned int qp_n = 0; qp_n < quadrature.size(); ++qp_n)
        {
          deallog << "  all zero = "
                  << (fe_values.jacobian_pushed_forward_grad(qp_n).norm() == 0)
                  << std::endl;
        }
      deallog << std::endl;
