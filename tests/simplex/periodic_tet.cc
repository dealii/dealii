...
check()
{
  ...
  // default orientation. Check that this holds.
  {
    ...
    const unsigned char         default_orientation =
      ReferenceCells::Tetrahedron.default_combined_face_orientation();
    const unsigned char default_flip_orientation =
        ...
        for (const auto &match : pair)
          {
            if (match.orientation != default_orientation &&
                match.orientation != default_flip_orientation)
              {
        ...
        for (int face = 0; face < static_cast<int>(pair.size()); ++face)
          {
            ...
            std::array<const bool, 3> results = {
