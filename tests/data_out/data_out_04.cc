           ...
           const Vector<double> & v_cell)
{
  ...
  // correctly
  DEAL_II_Assert(data_out.get_dataset_names() == reader.get_dataset_names(),
                 ExcInternalError());

  DEAL_II_Assert(data_out.get_patches().size() == reader.get_patches().size(),
                 ExcInternalError());

  for (unsigned int i = 0; i < reader.get_patches().size(); ++i)
    DEAL_II_Assert(data_out.get_patches()[i] == reader.get_patches()[i],
                   ExcInternalError());
