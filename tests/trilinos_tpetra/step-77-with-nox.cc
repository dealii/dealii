  ...
  MinimalSurfaceProblem<dim>::run()
  {
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle < 3;
         ++refinement_cycle)
      {
        ...
        {
          typename LinearAlgebra::TpetraWrappers::NOXSolver<
            Vector<double>>::AdditionalData additional_data;
          additional_data.abs_tol = target_tolerance;

          LinearAlgebra::TpetraWrappers::NOXSolver<Vector<double>>
            nonlinear_solver(additional_data);
