...
// LinearAlgebra::TpetraWrappers::BlockSparseMatrix::set() and a
// FullMatrix<double> with setting the same elements on an entry-by-entry. Use
// the entries as they would result from a vector-valued problem with a Laplace
// operator in 1D in each component
...
test()
{
  ...
  LinearAlgebra::TpetraWrappers::SparseMatrix<double> m_small(block_size,
                                                              block_size,
                                                              3U);
  ...
  // later.
  LinearAlgebra::TpetraWrappers::BlockSparseMatrix m, m2;
  m.reinit(3, 2);
  m2.reinit(3, 2);
