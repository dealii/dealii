...
reinit_vector(const dealii::DoFHandler<dim, spacedim> &mg_dof,
              MGLevelObject<LinearAlgebra::TpetraWrappers::Vector<double>> &v)
{
...
check_simple(const FiniteElement<dim> &fe)
{
  ...
  MGTransferPrebuilt<LinearAlgebra::TpetraWrappers::Vector<double>> transfer;
  transfer.build(mgdof);

  MGLevelObject<LinearAlgebra::TpetraWrappers::Vector<double>> u(0,
                                                                 tr.n_levels() -
                                                                   1);
