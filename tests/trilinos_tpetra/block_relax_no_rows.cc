...
test()
{
  ...
  LinearAlgebra::TpetraWrappers::Vector<double> src(rows, MPI_COMM_WORLD),
    dst(rows, MPI_COMM_WORLD);

  LinearAlgebra::TpetraWrappers::SparseMatrix<double> mat(rows,
                                                          rows,
                                                          MPI_COMM_WORLD);
...
main(int argc, char **argv)
{
  ...
  test<LinearAlgebra::TpetraWrappers::PreconditionBlockJacobi>();
  test<LinearAlgebra::TpetraWrappers::PreconditionBlockSOR>();
  test<LinearAlgebra::TpetraWrappers::PreconditionBlockSSOR>();
}
