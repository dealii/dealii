...
// Check LinearAlgebra::TpetraWrappers::NOXSolver by solving f(x) = x^2 = 0 with
// initial condition x=2.
//
// This test runs the same solver twice: Once via the deal.II interface to NOX
// in the LinearAlgebra::TpetraWrappers::NOXSolver class and once using the
// native, Epetra-based interface to NOX. The output should of course be the
// same.
...
main(int argc, char **argv)
{
   ...
   */
  {
    ...
    LinearAlgebra::TpetraWrappers::Vector<double> solution(is, MPI_COMM_WORLD);
    solution[0] = 2.0;

    LinearAlgebra::TpetraWrappers::SparsityPattern dsp(is, MPI_COMM_WORLD);
    dsp.add(0, 0);
    dsp.compress();
