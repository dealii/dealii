...
// cell-level symbolic-differentiation (via linearisation of the residual).
...
namespace Step44
{
  ...
  public:
    ...
    // get_S(const Tensor<2, dim, NumberType> &F,
    ...
    //                                p_tilde));
    //   // NOTE: The recursive substitution is not really required in this
    //   case, but good to
    //   // use in practise in case a more complex energy function is employed
    //   later const SD::types::substitution_map sub_vals =
    //   SD::resolve_explicit_dependencies(sub_vals_unresolved);
    //
    //   // Step 1b: Perform substitution of symbols
    //   const SymmetricTensor<2,dim,SDNumberType> substitution_S =
    //   SD::substitute(symbolic_S,sub_vals);
    //
    //   // Compute real-valued deal.II tensors that remain sensitive to local
    //   DoF values const SymmetricTensor<2,dim,NumberType> S =
    //   static_cast<SymmetricTensor<2,dim,NumberType> >(substitution_S);
    //
    //   // Push-forward
    //   return S;
    // }
    ...
    //   // Push-forward
    //   return
    //   Physics::Transformations::Contravariant::push_forward(get_S(F,p_tilde),F);
    // }
    // template<typename NumberType>
    // NumberType
    ...
    //   SD::types::substitution_map sub_vals_unresolved;
    //   SD::add_to_substitution_map(sub_vals_unresolved,
    //                                SD::make_symbol_value_map(J_tilde_SD,J_tilde));
    //   // NOTE: The recursive substitution is not really required in this
    //   case, but good to
    //   // use in practise in case a more complex energy function is employed
    //   later const SD::types::substitution_map sub_vals =
    //   SD::resolve_explicit_dependencies(sub_vals_unresolved);
    //
    //   // Step 2b: Perform substitution of symbols
    //   const SDNumberType substitution_dPsi_vol_dJ =
    //   SD::substitute(symbolic_dPsi_vol_dJ,sub_vals);
    //
    //   // Compute real values that remain sensitive to local DoF values
    //   return static_cast<NumberType>(substitution_dPsi_vol_dJ);
    // }
  private:
  ...
  class Solid
  {
    ...
    typedef SD::ResidualLinearization<dim, double> SDHelper;
      ...
      PerTaskData_ASM &                                     data) const
  {
    ...
    Assert(n_dependent_variables == n_independent_variables,
           ExcMessage("Expect system to be square."));
    ...
    static SDHelper_AD sd_helper_ad(n_batches,
                                    ...
                                    opt_method,
                                    opt_flags);
    static SDHelper    sd_helper(n_independent_variables,
    ...
    if (initialised == false)
      {
        ...
        // Symbolic expressions for kinematic terms
        const Tensor<2, dim, SDNumberType> Grad_u =
          sd_helper.solution_gradient(scratch.fe_values_ref, u_fe, "u");
        const Tensor<2, dim, SDNumberType> F =
          Physics::Elasticity::Kinematics::F(Grad_u);
        const SDNumberType det_F = determinant(F);
        const SDNumberType p_tilde =
          sd_helper.solution_value(scratch.fe_values_ref, p_fe, "p_tilde");
        const SDNumberType J_tilde =
          sd_helper.solution_value(scratch.fe_values_ref, J_fe, "J_tilde");

        // Symbolic expressions for kinetic terms
        const SymmetricTensor<2, dim, SDNumberType> S =
          lqph[q_point]->get_symbolic_S();
        const SDNumberType dPsi_vol_dJ =
          lqph[q_point]->get_symbolic_dPsi_vol_dJ();

        // Symbolic expressions for finite element data
        const SDNumberType                              JxW = sd_helper.JxW();
        const std::vector<Tensor<2, dim, SDNumberType>> Grad_Nx_u =
          sd_helper.shape_function_gradients(scratch.fe_values_ref, u_fe, "u");
        const std::vector<SDNumberType> Nx_p =
          sd_helper.shape_function_values(scratch.fe_values_ref,
                                          p_fe,
                                          "p_tilde");
        const std::vector<SDNumberType> Nx_J =
          sd_helper.shape_function_values(scratch.fe_values_ref,
                                          J_fe,
                                          "J_tilde");
        Assert(Grad_Nx_u.size() == dofs_per_cell, ExcInternalError());
        Assert(Nx_p.size() == dofs_per_cell, ExcInternalError());
        Assert(Nx_J.size() == dofs_per_cell, ExcInternalError());

        // Express the cell energy contribution in terms of the field
        // variables that are "local" or natural to the material law itself.
        // Note: If incrementing the contribution to the residual at each
        // degree-of-freedom it is critical to explicitly zero this vector
        // otherwise each entry may be initialised with garbage.
        std::vector<SDNumberType> residual_qp(dofs_per_cell, SDNumberType(0.0));
        for (unsigned int I = 0; I < dofs_per_cell; ++I)
          {
            Assert(numbers::values_are_equal(residual_qp[I], SDNumberType(0.0)), ExcInternalError());
            ...
            if (block_I == u_block) // u-terms
              {
                ...
                const SymmetricTensor<2, dim, SDNumberType> dE_I =
                  symmetrize(transpose(F) * Grad_Nx_u[I]);
                residual_qp[I] += (dE_I * S) * JxW;
              }
            else if (block_I == p_block)
              {
                residual_qp[I] += Nx_p[I] * (det_F - J_tilde) * JxW;
              }
            else if (block_I == J_block)
              {
                residual_qp[I] += Nx_J[I] * (dPsi_vol_dJ - p_tilde) * JxW;
              }
            else
              Assert(block_I <= J_block, ExcInternalError());
          }
        ...
        sd_helper.register_residual_vector(residual_qp, sub_vals_local_to_fe);
