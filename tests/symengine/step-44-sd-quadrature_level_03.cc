...
// A batch optimizer, using the LLVM JIT compiler with all available
// optimizations enabled, is also used.
// Compared to the run-time for step-44-sd-quadrature_level-[01,02].cc this
...
namespace Step44
{
  ...
  class PointHistory
  {
    ...
    using OptimizerType = SD::BatchOptimizer<double>;
    const bool debug;
  ...
  public:
    PointHistory()
      ...
      , p_tilde_SD("p")
      , J_tilde_SD("J")
      , optimizer(SD::OptimizerType::llvm, SD::OptimizationFlags::optimize_all)
                          ...
                          const double          J_tilde)
    {
      ...
      // employed later
      return SD::resolve_explicit_dependencies(sub_vals_unresolved);
    }
    void
    setup_symbol_differentiation()
    {
      ...
      // Step 1: Update stress and material tangent
      {
        ...
        if (debug)
          {
            deallog << "symbolic_psi: " << symbolic_psi << std::endl;
                  ...
                  const double          J_tilde)
    {
      ...
      // Pass substitution map to the optimizer
      ...
      // Step 1: Update stress and material tangent
      {
        ...
        const SymmetricTensor<2, dim> S = optimizer.evaluate(symbolic_S);
        const SymmetricTensor<4, dim> H = optimizer.evaluate(symbolic_H);
      ...
      // Step 2: Update volumetric penalty terms
      {
        // 1b: Perform substitution of symbols
        // Compute real values
        dPsi_vol_dJ   = optimizer.evaluate(symbolic_dPsi_vol_dJ);
        d2Psi_vol_dJ2 = optimizer.evaluate(symbolic_d2Psi_vol_dJ2);
      }
