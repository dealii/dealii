...
// cell-level symbolic-differentiation (via linearization of the residual).
// The optimization/evaluation of symbolic expressions may be split across
// multiple optimizer instances.

// Optimizer mode: LLVM
// Symbolic expression optimizations: Enabled
// Load balancing: Enabled (Max performance)
// Batching: Enabled

#include "../tests.h"
#include "sd_common_tests/step-44-sd_sd_helper-res_lin.h"
  ...
  try
    {
      ...
      const enum SD::LoadBalancing load_balancing =
        SD::LoadBalancing::max_performance;
      const unsigned int n_batches = 4;
