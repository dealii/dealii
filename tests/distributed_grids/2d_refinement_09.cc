...
// Test 2d mesh refinement with smoothing flag dealii::Triangulation<dim>::limit_level_difference_at_vertices.
//
...
int main (int argc, char *argv[])
{
  ...
  TriaTest<2> tria_test(dealii::Triangulation<2>::limit_level_difference_at_vertices);
  tria_test.run();
  if (I_am_host)
    {
      deallog.pop();
