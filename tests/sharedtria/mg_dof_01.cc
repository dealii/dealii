  ...
  for (unsigned int lvl=0; lvl<n_levels; ++lvl)
    {
      ...
      cell = distributed_dof_handler.begin(lvl),
      ...
      for (; cell!=endc; ++cell)
        {
          ...
          typename DoFHandler<dim>::cell_iterator dof_shared_cell(&shared_dof_handler.get_triangulation(),
...
void test()
{
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell=distributed_tria.begin_active(); cell != distributed_tria.end(); ++cell)
    {
      if (dim==1)
      ...
      if (dim==2)
        if (cell->is_locally_owned() && cell->center()[0] < 0.5 && cell->center()[1] < 0.5)
          cell->set_refine_flag();
      if (dim==3)
        if (cell->is_locally_owned() && cell->center()[0] < 0.5 && cell->center()[1] && cell->center()[2] < 0.5)
          cell->set_refine_flag();
    }
