...
void compare_meshes (parallel::shared::Triangulation<dim> &shared_tria, parallel::distributed::Triangulation<dim> &p4est_tria)
{
  ...
  cell1 = shared_tria.begin_active(),
...
void test()
{
  ...
  parallel::distributed::Triangulation<dim> p4est_tria(MPI_COMM_WORLD,Triangulation<dim>::
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell=p4est_tria.begin_active(); cell != p4est_tria.end(); ++cell)
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell=p4est_tria.begin_active(); cell != p4est_tria.end(); ++cell)
    if (cell->is_locally_owned() &&
        cell->at_boundary() && (cell->center()[0] < 0 || cell->center()[1] < 0))
      cell->set_refine_flag();
  p4est_tria.execute_coarsening_and_refinement();
