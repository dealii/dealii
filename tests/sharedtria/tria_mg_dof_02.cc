...
void write_dof_data (DoFHandler<dim> &dof_handler)
{
  ...
  for (unsigned int lvl=0; lvl<n_levels; ++lvl)
    {
      std::vector<IndexSet> dof_index_per_proc = dof_handler.locally_owned_mg_dofs_per_processor(lvl);
      for (unsigned int i=0; i<dof_index_per_proc.size(); ++i)
        dof_index_per_proc[i].print(deallog);


      typename DoFHandler<dim>::cell_iterator
      cell = dof_handler.begin(lvl),
...
void test()
{
  ...
  for (std::set<types::subdomain_id>::iterator it=tria.level_ghost_owners().begin(); it!=tria.level_ghost_owners().end(); ++it)
    deallog << *it << " ";
  deallog << std::endl;
