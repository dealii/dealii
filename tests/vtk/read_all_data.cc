...
test(const std::string &filename)
{
  ...
  deallog << "All data size: " << data.size() << std::endl;
  if (data.size() ==
      (triangulation.n_vertices() * 4 + triangulation.n_active_cells() * 4))
    deallog << "  Size matches expected number of data entries." << std::endl;
}
