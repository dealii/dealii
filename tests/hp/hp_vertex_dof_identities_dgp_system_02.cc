...
// check FESystem(FE_DGP)::hp_vertex_dof_identities, but with a different
  ...
  for (unsigned int i = 1; i < 8 - dim; ++i)
    {
      ...
      fe_collection.push_back(FESystem<dim>(FE_DGP<dim>(i), 2, FE_DGP<dim>(i), 1));
      fe_collection.push_back(FESystem<dim>(FE_DGP<dim>(i), 1, FE_DGP<dim>(i), 2));
    }
