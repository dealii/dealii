...
/**
 * The number of iterations the program shall do. This is given as a
...
*/  
  ...
  create_coarse_grid (Triangulation<3> &coarse_grid)
  {
    ...
    static HyperBallBoundary<3> boundary;
...
namespace LaplaceSolver
{
  ...
  WeightedResidual<dim>::output_solution () const
  {
    ...
    Vector<double> coarse_gradient_indicators (coarser_dof_handler.n_dofs());
    Vector<double> coarse_second_indicators (coarser_dof_handler.n_dofs());
	...
	
    {
      ...
      data_out.add_data_vector (coarse_primal_solution, "primal_solution");
      data_out.add_data_vector (coarse_dual_solution, "dual_solution");
      data_out.add_data_vector (coarse_error_indicators, "errors");
