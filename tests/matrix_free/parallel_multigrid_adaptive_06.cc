...
do_test(const DoFHandler<dim> &dof, const unsigned int nb)
{
  ...
  constraints.reinit(dof.locally_owned_dof(), locally_relevant_dofs);
  ...
  // set constant rhs vector
  {
    ...
    hanging_node_constraints.reinit(dof.locally_owned_dof(),
  ...
  for (unsigned int level = 0;
       ...
       ++level)
    {
      ...
      level_constraints.reinit(dof.locally_owned_dof(), relevant_dofs);
