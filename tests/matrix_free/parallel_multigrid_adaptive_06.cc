...
private:
  MatrixFreeOperators::
    LaplaceOperator<dim, fe_degree, n_q_points_1d, 1, typename BlockVectorType::BlockType>
      laplace;
};
...
do_test(const DoFHandler<dim> &dof, const unsigned int nb)
{
  ...
  BlockLaplace<dim, fe_degree, n_q_points_1d, LinearAlgebra::distributed::BlockVector<number>>
                                           fine_matrix;
