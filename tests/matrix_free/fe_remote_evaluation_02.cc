...
template <int dim>
struct QuadraticFunction : public Function<dim>
{
  QuadraticFunction(unsigned int offset)
  ...
  value(const Point<dim> &p, const unsigned int component) const final
  {
    if (component < offset)
      return 0.0;
    return p[component - offset] * p[component - offset];
  }
                ...
                for (unsigned int q : phi.quadrature_point_indices())
                  {
                    ...
                    for (unsigned int i = 0; i < 2; ++i)
                      for (unsigned int j = 0; j < dim; ++j)
                        AssertThrow(std::abs(error[i][j]) < 1e-13,
                                    ExcMessage("Error too large."));
                  }
              }
          }
