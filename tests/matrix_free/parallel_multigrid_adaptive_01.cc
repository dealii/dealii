...
public:
  ...
  void
  vmult_interface_down(LinearAlgebra::distributed::Vector<double> &      dst,
  ...
  void
  vmult_interface_up(LinearAlgebra::distributed::Vector<double> &      dst,
                     const LinearAlgebra::distributed::Vector<double> &src) const
  {
    Assert(src.partitioners_are_globally_compatible(*data.get_dof_info(0).vector_partitioner),
           ExcInternalError());
...
do_test(const DoFHandler<dim> &dof)
{
  ...
  fine_matrix.initialize(
    mapping, dof, mg_constrained_dofs, dirichlet_boundary, numbers::invalid_unsigned_int);
  ...
  for (unsigned int level = 0; level < dof.get_triangulation().n_global_levels(); ++level)
    {
      mg_matrices[level].initialize(mapping, dof, mg_constrained_dofs, dirichlet_boundary, level);
    }
  ...
  PreconditionMG<dim, LinearAlgebra::distributed::Vector<double>, MGTransferMF<LevelMatrixType>>
