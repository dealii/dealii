...
// comparing to the output of FEValues with the same settings for divergence
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      ...
      for (unsigned int i = 0; i < function_gradients.size(); ++i)
        deallog << mapping.transform_unit_to_real_cell(cell, unit_points[i])
                << ": "
                << " error div "
                << (evaluator.get_divergence(i) - trace(function_gradients[i]))
                << std::endl;
      deallog << std::endl;

      for (unsigned int i = 0; i < unit_points.size(); ++i)
        {
          evaluator.submit_divergence(evaluator.get_divergence(i), i);
        }
