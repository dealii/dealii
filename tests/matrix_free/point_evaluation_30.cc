...
test(const unsigned int degree)
{
  ...
  FEPointEvaluation<1, dim, spacedim, Number> evaluator(
    mapping, fe, update_values | update_gradients);

  Tensor<1, spacedim, Number> exponents;
  exponents[0] = 1.;
  VectorTools::interpolate(mapping,
                           dof_handler,
                           Functions::Monomial<spacedim, Number>(exponents),
                           vector);

  std::vector<Number>                      solution_values(fe.dofs_per_cell);
  std::vector<Number>                      function_values(unit_points.size());
  std::vector<Tensor<1, spacedim, Number>> function_gradients(
    unit_points.size());
