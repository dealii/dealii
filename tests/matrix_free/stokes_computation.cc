...
namespace StokesClass
{
  ...
  namespace StokesSolver
  {
     ...
     */
    template <class StokesMatrixType, class MassMatrixType, class PreconditionerA, class PreconditionerMp>
           ...
           const LinearAlgebra::distributed::BlockVector<double> &src) const
    {
      ...
      {
        SolverControl solver_control(1000, src.block(1).l2_norm() * S_block_tolerance, false, false);
...
// Parameters for Sinker example
  ...
  public:
    ...
    void clear ();
    void evaluate_2_x_viscosity(const Viscosity<dim> &viscosity_function);
    virtual void compute_diagonal ();

  private:
    virtual void apply_add (LinearAlgebra::distributed::BlockVector<number> &dst,
  ...
  public:
    ...
    void clear ();
    void evaluate_1_over_viscosity(const Viscosity<dim> &viscosity_function);
    virtual void compute_diagonal ();
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
      {
        ...
        for (unsigned int q=0; q<pressure.n_q_points; ++q)
          pressure.submit_value(one_over_viscosity(cell,q)*pressure.get_value(q),
                                q);
        pressure.integrate (true, false);
  ...
  private:
    void make_grid (const unsigned int ref = 4);
    void create_sinker (const unsigned int n_sinkers, const double visc_jump);
    void setup_system ();
    void assemble_rhs ();
    void solve ();
  ...
  template <int dim>
  void StokesProblem<dim>::create_sinker(const unsigned int n_sinkers, const double visc_jump)
  {
  ...
  template <int dim>
  void StokesProblem<dim>::make_grid(const unsigned int ref)
  {
  ...
  void StokesProblem<dim>::setup_system ()
  {
    ...
    VectorTools::interpolate_boundary_values (dof_handler_u,
                                              0,
                                              ExactSolution_BoundaryValues_u<dim>(),
                                              constraints_u);
    constraints_u.close ();
    ...
    additional_data_stokes.mapping_update_flags = (update_values | update_gradients |
    ...
    stokes_mf_storage->reinit(stokes_dofs, stokes_constraints,
                              QGauss<1>(degree_u+1), additional_data_stokes);
    ...
    additional_data_mass.tasks_parallel_scheme =
      MatrixFree<dim,double>::AdditionalData::none;
    additional_data_mass.mapping_update_flags = (update_values | update_JxW_values |
                                                 update_quadrature_points);
    std::shared_ptr<MatrixFree<dim,double> >
    mass_mf_storage(new MatrixFree<dim,double>());
    mass_mf_storage->reinit(dof_handler_p, constraints_p,
                            QGauss<1>(degree_u+1), additional_data_mass);
    ...
    for (unsigned int level=0; level<n_levels; ++level)
      {
        ...
        mg_mf_storage_level->reinit(dof_handler_u, level_constraints,
                                    QGauss<1>(degree_u+1), additional_data);
  ...
  template <int dim>
  void StokesProblem<dim>::assemble_rhs ()
  {
    system_rhs = 0.0;
    ...
    std::shared_ptr<MatrixFree<dim,double> >
    matrix_free_homogeneous(new MatrixFree<dim,double>());
    matrix_free_homogeneous->reinit(dofs, constraints_no_dirchlet,
                                    QGauss<1>(degree_u+1), data);
  ...
  void StokesProblem<dim>::solve ()
  {
    ...
    MGCoarseGridIterativeSolver<vector_t, SolverCG<vector_t>,
                                LevelMatrixType, PreconditionIdentity> mg_coarse;
    ...
    PreconditionMG<dim, vector_t, Transfer>
    prec_A(dof_handler_u, mg, mg_transfer);
    ...
    const StokesSolver::BlockSchurPreconditioner<StokesMatrixType, MassMatrixType, A_prec_type, MassPrec>
    ...
    deallog << "Solved-in "
            << (solver_control_cheap.last_step() != numbers::invalid_unsigned_int ?
                ...
                0) << " iterations, final residual: " << solver_control_cheap.last_value() << std::endl;
  }
  ...
  void StokesProblem<dim>::run ()
  {
    ...
    deallog << "n_sinker: " << sinker.n_sinkers
            << "       max/min viscosity ratio: " << sinker.DR_mu << std::endl << std::endl;
    ...
    for (unsigned int cycle=0; cycle<n_cycles; ++cycle)
      {
        ...
        deallog     << "Number of active cells: "
                    ...
                    << " (on " << triangulation.n_global_levels() << " levels)"
                    << std::endl;
