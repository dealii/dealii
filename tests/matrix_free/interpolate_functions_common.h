          ...
          for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
            {
              ...
              cell_errors[0] += std::abs(fe_eval.get_value(q)[j] - function.value(p, 0));
              for (unsigned int d = 0; d < dim; ++d)
                cell_errors[1] +=
                  std::abs(fe_eval.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
              for (unsigned int d = 0; d < dim; ++d)
                for (unsigned int e = 0; e < dim; ++e)
                  cell_errors[2] +=
                    std::abs(fe_eval.get_hessian(q)[d][e][j] - function.hessian(p, 0)[d][e]);
            }
      }
  }
            ...
            for (unsigned int q = 0; q < fe_evalm.n_q_points; ++q)
              {
                ...
                facem_errors[0] += std::abs(fe_evalm.get_value(q)[j] - function.value(p, 0));
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    // std::cout << fe_evalm.get_gradient(q)[d][j] << " ";
                    facem_errors[1] +=
                      std::abs(fe_evalm.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                  }
                double normal_derivative = 0;
                for (unsigned int d = 0; d < dim; ++d)
                  normal_derivative +=
                    function.gradient(p, 0)[d] * fe_evalm.get_normal_vector(q)[d][j];
                facem_errors[3] +=
                  std::abs(fe_evalm.get_normal_derivative(q)[j] - normal_derivative);
                facep_errors[0] += std::abs(fe_evalp.get_value(q)[j] - function.value(p, 0));
                for (unsigned int d = 0; d < dim; ++d)
                  facep_errors[1] +=
                    std::abs(fe_evalp.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                facep_errors[3] +=
                  std::abs(fe_evalp.get_normal_derivative(q)[j] - normal_derivative);
            ...
            for (unsigned int q = 0; q < fe_evalm.n_q_points; ++q)
              {
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  p[d] = fe_evalm.quadrature_point(q)[d][j];
                boundary_errors[0] += std::abs(fe_evalm.get_value(q)[j] - function.value(p, 0));
                for (unsigned int d = 0; d < dim; ++d)
                  boundary_errors[1] +=
                    std::abs(fe_evalm.get_gradient(q)[d][j] - function.gradient(p, 0)[d]);
                double normal_derivative = 0;
