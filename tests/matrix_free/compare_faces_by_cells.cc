    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++)
      {
        ...
        sigmaF = std::abs((phi.get_normal_vector(0) * phi_outer.inverse_jacobian(0))[dim - 1]) *
                 (number)(std::max(fe_degree, 1) * (fe_degree + 1.0)) * 2.0;
        for (unsigned int j = 0; j < phi.dofs_per_cell; ++j)
          phi.begin_dof_values()[j] = VectorizedArray<number>();
        phi.evaluate(true, true);
        ...
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
          {
            ...
            std::array<types::boundary_id, VectorizedArray<number>::n_array_elements> boundary_ids =
...
test()
{
  ...
  res2 -= res1;
  deallog << "Error in diagonal on active cells: " << (double)res2.linfty_norm() << std::endl;
  ...
  for (unsigned int level = 0; level < dof.get_triangulation().n_global_levels(); ++level)
    {
      ...
      fine_matrix.compute_diagonal_by_cell(res2);
      res2 -= res1;
      deallog << "Error in diagonal on level " << level << ":      " << (double)res2.linfty_norm()
              << std::endl;
    }
}
