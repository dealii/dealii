...
// check FEPointEvaluation for scalar FE_Q(1) and MappingQ(1) on a mesh
// consisting of a non-Cartesian cell followed by a Cartesian cell by
...
test(const unsigned int degree)
{
  ...
  GridGenerator::subdivided_hyper_rectangle(tria,
                                            ...
                                            Point<dim>(),
                                            Point<dim>(2, 1));
  // shift one point to make the first cell non-affine
  tria.begin()->vertex(0) += Point<dim>(0.1, 0.04);
