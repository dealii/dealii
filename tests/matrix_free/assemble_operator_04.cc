...
// assemble_operator for block-systems. No hanging nodes and no other
...
test()
{
  ...
  BlockSparseMatrix<double> output;
  ...
  output.reinit(sparsity_pattern);
  ...
  // MatrixFreeTools::assemble_operator(output, system_matrix,
  // sparsity_pattern);
  for (unsigned int r = 0; r < system_matrix.n_block_rows(); r++)
    for (unsigned int c = 0; c < system_matrix.n_block_cols(); c++)
      {
        auto cache =
          MatrixFreeTools::assemble_operator(output.block(r, c),
                                             system_matrix.block(r, c),
                                             sparsity_pattern.block(r, c));
        deallog << cache->num_colors << std::endl;
      }

  // Diff:
  output.add(-1.0, system_matrix);

  deallog << "  Verification fe degree " << fe_degree << ": "
          << output.frobenius_norm() << std::endl
          << std::endl;
}
