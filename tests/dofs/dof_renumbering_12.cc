...
check_renumbering(DoFHandler<dim> &mgdof, bool discontinuous)
{
  ...
  // dof_renumbering.cc test uses the order setup from the commented out code
  // snippet below. The Resultant order is 2, 1, 0.
  ...
  */
  ...
  // In this test we use the component_wise function overload that extracts the
  // order from a vector of FEValuesExtractors. We use the same order as above
  // and we expect identical test output as the dof_renumbering.cc test.
  std::vector<FEValuesExtractors::AnyExtractor> extractors = {
    FEValuesExtractors::Scalar(2),
  ...
  DoFRenumbering::component_wise(dof, extractors);
  print_dofs(dof);
