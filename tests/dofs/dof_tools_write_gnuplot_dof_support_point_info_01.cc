...
test()
{
  ...
  rep[0] = 2;
  GridGenerator::subdivided_hyper_rectangle(triangulation, rep, Point<dim>(), Point<dim>(2.0, 1.0));
  FESystem<dim> fe(FE_Q<dim>(velocity_degree), dim, FE_Q<dim>(velocity_degree - 1), 1);
