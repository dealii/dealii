...
// like particle_handler_04, but for shared triangulations in
// parallel computations. The shared triangulation is partitioned
// using zoltan to maximise compatibility
...
{
  {
    ...
    parallel::shared::Triangulation<dim, spacedim> tria_shared(
      MPI_COMM_WORLD,
      typename Triangulation<dim, spacedim>::MeshSmoothing(
        Triangulation<dim, spacedim>::limit_level_difference_at_vertices),
      true,
      typename parallel::shared::Triangulation<dim, spacedim>::Settings(
        parallel::shared::Triangulation<dim, spacedim>::partition_zorder));

    GridGenerator::hyper_cube(tria_shared);
    tria_shared.refine_global(2);
