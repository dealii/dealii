...
// Test multiple insertions using insert_global_particles.
...
test()
{
  ...
  std::vector<Point<spacedim>>     points1(n_points), points2(n_points);
  std::vector<std::vector<double>> properties(n_points, {1. * my_cpu});

  for (auto &p1 : points1)
    p1 = random_point<spacedim>();

  for (auto &p2 : points2)
    p2 = random_point<spacedim>();

  auto cpu_to_index1 =
    particle_handler.insert_global_particles(points1,
                                             global_bounding_boxes,
                                             properties);

  auto cpu_to_index2 =
    particle_handler.insert_global_particles(points2,
                                             global_bounding_boxes,
                                             properties);
