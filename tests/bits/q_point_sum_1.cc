...
check(const Triangulation<dim> &tria)
{
  ...
  FEFaceValues<dim>    fe_face_values(fe, q_face, update_quadrature_points | update_JxW_values);
  FESubfaceValues<dim> fe_subface_values(fe, q_face, update_quadrature_points | update_JxW_values);
