...
test()
{
  ...
  std::shared_ptr<dealii::Utilities::MPI::Partitioner> partitioner_owned =
    std::make_shared<dealii::Utilities::MPI::Partitioner>(locally_owned_blocks,
                                                          mpi_communicator);
            ...
            for (unsigned int jj = 0; jj < col_size; ++jj)
              {
                const double val =
                  *(it->data() + BlockCSRMatrix<double>::local_index(
                                   ii, jj, row_size, col_size));
                const double expect = (row_indices[ii] + 1) * row_mult_expect +
                                      (col_start + jj + 1) * col_mult_expect +
                                      shift_expect;
