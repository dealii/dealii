...
test()
{
  ...
  get_owned_columns(owned_columns, M_blocks_local, M_blocks, mpi_communicator);
  ...
  C.reinit(dsp_A, Nb, Mb, bcsr_row_part);
  ...
  // now compare to full matrices
  LAPACKFullMatrix<double> full_A(full_N, full_M), full_B(full_M, full_M),
    full_C(full_N, full_M), full_C_check(full_N, full_M);
  ...
  const auto truncate_full = [](const BlockCSRMatrix<double> &mat,
                                LAPACKFullMatrix<double> &    full,
                                const MPI_Comm &              comm) {
    LAPACKFullMatrix<double> copy(full);
    full = 0.;
    for (unsigned int i = 0; i < mat.n_local_row_blocks(); ++i)
      {
        const auto M = mat.get_row_blocks()->block_size(i);
        const auto i_start =
          mat.local_range().first + mat.get_row_blocks()->block_start(i);
        for (auto it = mat.begin_local(i); it != mat.end_local(i); ++it)
          {
            const auto j       = it->column();
            const auto N       = mat.get_col_blocks()->block_size(j);
            const auto j_start = mat.get_col_blocks()->block_start(j);
            for (unsigned int ii = 0; ii < M; ++ii)
              for (unsigned int jj = 0; jj < N; ++jj)
                full(i_start + ii, j_start + jj) =
                  copy(i_start + ii, j_start + jj);
          }
      }
    dealii::Utilities::MPI::sum(full, comm, full);
  };

  truncate_full(C, full_C_check, mpi_communicator);
