...
void test(const unsigned int n_cells = 1, const bool with_constrains = false)
{
  ...
  const std::vector<unsigned int> col_blocks = {{12, 6, 3}};
  ...
  // do the loop via MatrixFree cells:
  {
    ...
    FEEvaluation<dim, fe_degree, n_q_points_1d, 1, Number> phi(*fine_level_data);
    ...
    const std::vector<unsigned int> cell_to_fe_eval =
      dealii::Utilities::invert_permutation(phi.get_internal_dof_numbering());
                ...
                for (unsigned int c = 0; c < n_vec; ++c)
                  {
                    ...
                    read_values(src_row_accessor, c, phi, touched);
                    ...
                    // write data
                    distribute_local_to_global(dst_row_accessor, c, phi);
