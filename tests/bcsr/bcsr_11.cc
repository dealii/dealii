// test write via RowAccessor
// similar to bcsr_10 but set non-zero elements and also test addition.
// use advance(unsigned int) to
// got to some pre-chosen columns: 1,4
// and set non-zero elements: 1,2,5
...
void test()
{
  ...
  // now test:
  const std::vector<unsigned int> my_rows = {{1,2,5}};
  const std::vector<unsigned int> my_cols = {{1,4}};
  ...
  // set non-zero:
  {
    ...
    deallog << std::endl << "Setting: " << std::endl;
    for (auto &c : my_cols)
      {
        row_accessor.advance(c);
        deallog << "Column: " << row_accessor.current_column() << std::endl;
        for (auto ind : my_rows)
          row_accessor.set_local_element(ind, (ind+1)*100 + row_accessor.current_column() + 1);
      }
  }
    ...
    for (auto &c : my_cols)
      {
        ...
        deallog << "Column: " << row_accessor.current_column() << std::endl;
        for (auto ind : my_rows)
          row_accessor.add_local_element(ind, 500);
      }
  }
