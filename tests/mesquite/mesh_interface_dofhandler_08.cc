 ...
 * This test is the same as mesh_interface_dofhandler_03a, except that there's
 * now a non-primitive element within the constructed FESystem.
 *
...
main(int argc, char **argv)
{
  ...
  const unsigned int degree_ned =
    1; // "0" works fine, but there is a bug with "1"
  ...
  const FESystem<dim> fe(
    ...
    FE_Nedelec<dim>(degree_ned),
    n_dummy_fields_2); // Another dummy field (non-primitive)
  //                          FE_RaviartThomas<dim>(degree_ned),
  ...
  const unsigned int n_dof_components_total =
    first_dummy_2_component + dim * n_dummy_fields_2; // Nedelec is vector field
  ...
  DoFRenumbering::block_wise(
    dof_handler); // Compatible with Nedelec and Raviat-Thomas elements
