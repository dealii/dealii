 ...
 * This test is the same as mesh_interface_dofhandler_03, except that it checks
 * the transfer of mesh motion data back to the solution vector via the
 * call to MesquiteMeshInterface::update_mesh_displacement_solution().
 *
...
main(int argc, char **argv)
{
  ...
  const unsigned int  n_dummy_fields = 1;
  const FESystem<dim> fe(FE_Q<dim>(degree),
                         dim, // The displacement field,
                         FE_Q<dim>(1),
                         n_dummy_fields); // A dummy field
  ...
  VectorTools::interpolate_boundary_values(
    ...
    TransformationSinusoid<dim>(
      false, 0.0, n_dof_components, first_u_component),
    constraints_x,
    component_mask_u);
  constraints_x.close();
  VectorTools::project(dof_handler,
                       ...
                       QGauss<dim>(degree + 2),
                       TransformationSinusoid<dim>(
                         true, 0.75, n_dof_components, first_u_component),
                       solution_x);
  ...
  VectorTools::interpolate(dof_handler,
                           GridTools::internal::ReferencePosition<dim>(
                             n_dof_components, first_u_component),
                           solution_X,
                           component_mask_u);
