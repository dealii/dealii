...
main(int argc, char **argv)
{
  ...
  output_mesh(dof_handler, solution_x, first_u_dof, "grid.01");
  ...
  const FEValuesExtractors::Vector displacement(first_u_dof);
  ...
  output_mesh(dof_handler, solution_u, first_u_dof, "grid.02");

  // Perform mesh smoothing
  GridTools::MesquiteMeshInterface<dim> mesquite_interface(
    dof_handler, solution_u, first_u_dof, true /*fix_all_boundary_vertices*/);
