 ...
 * This test is the same as mesh_interface_dofhandler_03, except that now
 * higher order polynomials are chosen for the displacement field and multiple
 * dummy fields are also present.
 *
...
main(int argc, char **argv)
{
  ...
  const unsigned int  n_dummy_fields = 3;
  const FESystem<dim> fe(FE_Q<dim>(1),
                         n_dummy_fields, // A dummy field
                         FE_Q<dim>(degree),
                         dim); // The displacement field
  const unsigned int  first_u_component = n_dummy_fields;
