...
/**
 * A 3d example of smoothing of a distorted grid using the Mesquite interface.
 * This test is the same as mesh_interface_shared_tria_dofhandler_01, except
 * that it uses an hp::DoFHandler with multiple FEs.
 *
...
main(int argc, char **argv)
{
  ...
  const unsigned int degree_u         = 1;
  const unsigned int n_fe_indices     = 2;
  const unsigned int n_dummy_fields_0 = 1;
  const unsigned int n_dummy_fields_1 = 1;
  const unsigned int n_u_fields       = dim;
  const unsigned int n_dummy_fields_2 = 1;

  hp::FECollection<dim> fe;
  hp::QCollection<dim>  q_project;
  for (unsigned int degree_u = 1; degree_u <= n_fe_indices; ++degree_u)
    {
      fe.push_back(FESystem<dim>(FE_Q<dim>(1),
      ...
      q_project.push_back(QGauss<dim>(degree_u + 2));
    }
  ...
  for (auto cell : dof_handler.active_cell_iterators())
    {
      if (cell->is_locally_owned() == false)
        continue;
      cell->set_active_fe_index(random_value<unsigned int>(0, n_fe_indices));
    }
