...
main(int argc, char **argv)
{
  ...
  Utilities::MPI::MPI_InitFinalize mpi_initialization(argc, argv, 1);
  {
    ...
    check_solver_within_range(solver.solve(A, u, f, preconditioner), control.last_step(), 1, 11);

    // run twice because this errored out at some point
    u = 0.;
    deallog << "Solver type: " << typeid(solver).name() << std::endl;
    check_solver_within_range(solver.solve(A, u, f, preconditioner), control.last_step(), 1, 1);
  }
}
