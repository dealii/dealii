...
// check that computation of hp constraints works for FE_Q_Hierarchical elements correctly
...
void test ()
{
  ...
  for (unsigned int i=1; i<4; ++i)
    {
      fe.push_back (FE_Q_Hierarchical<dim>(i));
      degrees.push_back (i);
    }
  ...
  // don't run this test right now because
  // this FE has no (meaningful) support points and
  // therefore can't use
