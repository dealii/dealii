...
private:
  ...
  Vector<double> solution, smooth_solution, system_rhs, tangential_derivative, tangential_velocity,
    error;
...
BEM<spacedim>::assemble_system()
{
  ...
  FEValues<spacedim - 1, spacedim> fe_values_i(
    fe, quadrature_formula, update_JxW_values | update_normal_vectors | update_quadrature_points),
    fe_values_j(
      fe, quadrature_formula, update_JxW_values | update_normal_vectors | update_quadrature_points);
  ...
  typename DoFHandler<spacedim - 1, spacedim>::active_cell_iterator cell_i =
  ...
  for (; cell_i != endc; ++cell_i)
    {
      ...
      double constant_factor =
        -pow(fe_values_i.JxW(0) / fe_values_i.get_quadrature().weight(0), 2) / numbers::PI *
                  ...
                  for (unsigned int q_point_j = 0; q_point_j < n_q_points; q_point_j++)
                    {
                      ...
                      for (unsigned int b = 0; b < dofs_per_cell; ++b)
                        {
                          ...
                          cell_DLP_matrix(a, b) += 1. / numbers::PI * cell_normals_i[q_point_i] *
                      ...
                      cell_rhs(a) += -1. / numbers::PI * velocity * cell_normals_j[q_point_j] *
  ...
  system_matrix.add(1, mass_matrix, -1, DLP_matrix);
}
...
BEM<spacedim>::solve()
{
  ...
  FETools::interpolate(dof_handler, solution, dof_handler_q, smooth_solution);
  ...
  for (; cell != endc; ++cell)
    {
      ...
      for (unsigned int pnt = 0; pnt < q_iterated.size(); ++pnt)
        for (unsigned int fun = 0; fun < fe_q.dofs_per_cell; ++fun)
          gradient[pnt] +=
            smooth_solution(local_dof_indices[fun]) * fe_values_q.shape_grad(fun, pnt);

      for (unsigned int pnt = 0; pnt < q_iterated.size(); ++pnt)
        {
          tangential_derivative(cell->index()) =
            gradient[pnt] * cell_tangentials[pnt] + velocity * cell_tangentials[0];

          error(cell->index()) = tangential_derivative(cell->index()) -
                                 2. * velocity[0] *
...
BEM<spacedim>::output_results()
{
  ...
  dataout.add_data_vector(
    error, "error", DataOut<spacedim - 1, DoFHandler<spacedim - 1, spacedim>>::type_cell_data);
