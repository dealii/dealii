...
template <int dim, int spacedim>
const unsigned int hp::DoFHandler<dim, spacedim>::dimension;
...
template <int dim, int spacedim>
const types::global_dof_index hp::DoFHandler<dim, spacedim>::invalid_dof_index;

template <int dim, int spacedim>
const unsigned int DoFHandler<dim, spacedim>::default_fe_index;

template <int dim, int spacedim>
const unsigned int hp::DoFHandler<dim, spacedim>::default_fe_index;
...
template <int dim, int spacedim>
hp::DoFHandler<dim, spacedim>::DoFHandler()
  : tria(nullptr, typeid(*this).name())
  , faces(nullptr)
{}
...
hp::DoFHandler<dim, spacedim>::~DoFHandler()
{
  ...
  // override in a derived class
  // for clarity be explicit on which function is called
  hp::DoFHandler<dim, spacedim>::clear();
}
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::initialize(
  const Triangulation<dim, spacedim> &   tria,
...
hp::DoFHandler<dim, spacedim>::begin(const unsigned int level) const
{
...
hp::DoFHandler<dim, spacedim>::begin_active(const unsigned int level) const
{
  // level is checked in begin
  cell_iterator i = begin(level);
  if (i.state() != IteratorState::valid)
...
template <int dim, int spacedim>
typename hp::DoFHandler<dim, spacedim>::cell_iterator
hp::DoFHandler<dim, spacedim>::end() const
{
  return cell_iterator(&this->get_triangulation(), -1, -1, this);
}
...
template <int dim, int spacedim>
typename hp::DoFHandler<dim, spacedim>::cell_iterator
hp::DoFHandler<dim, spacedim>::end(const unsigned int level) const
{
  return (level == this->get_triangulation().n_levels() - 1 ? end() :
                                                              begin(level + 1));
}
...
template <int dim, int spacedim>
typename hp::DoFHandler<dim, spacedim>::active_cell_iterator
hp::DoFHandler<dim, spacedim>::end_active(const unsigned int level) const
{
...
template <int dim, int spacedim>
IteratorRange<typename hp::DoFHandler<dim, spacedim>::cell_iterator>
hp::DoFHandler<dim, spacedim>::cell_iterators() const
{
  return IteratorRange<typename hp::DoFHandler<dim, spacedim>::cell_iterator>(
    begin(), end());
}
...
template <int dim, int spacedim>
IteratorRange<typename hp::DoFHandler<dim, spacedim>::active_cell_iterator>
hp::DoFHandler<dim, spacedim>::active_cell_iterators() const
{
  return IteratorRange<
    typename hp::DoFHandler<dim, spacedim>::active_cell_iterator>(
    begin_active(), end());
...
template <int dim, int spacedim>
IteratorRange<typename hp::DoFHandler<dim, spacedim>::cell_iterator>
hp::DoFHandler<dim, spacedim>::cell_iterators_on_level(
  const unsigned int level) const
{
  return IteratorRange<typename hp::DoFHandler<dim, spacedim>::cell_iterator>(
    begin(level), end(level));
...
template <int dim, int spacedim>
IteratorRange<typename hp::DoFHandler<dim, spacedim>::active_cell_iterator>
hp::DoFHandler<dim, spacedim>::active_cell_iterators_on_level(
  const unsigned int level) const
{
  return IteratorRange<
    typename hp::DoFHandler<dim, spacedim>::active_cell_iterator>(
...
template <int dim, int spacedim>
types::global_dof_index
hp::DoFHandler<dim, spacedim>::n_boundary_dofs() const
{
...
template <int dim, int spacedim>
types::global_dof_index
hp::DoFHandler<dim, spacedim>::n_boundary_dofs(
  const std::set<types::boundary_id> &boundary_ids) const
{
  Assert(fe_collection.size() > 0, ExcNoFESelected());
  Assert(boundary_ids.find(numbers::internal_face_boundary_id) ==
...
template <int dim, int spacedim>
std::size_t
hp::DoFHandler<dim, spacedim>::memory_consumption() const
{
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::set_fe(const hp::FECollection<dim, spacedim> &ff)
{
  Assert(
    tria != nullptr,
    ExcMessage(
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::distribute_dofs(
  const hp::FECollection<dim, spacedim> &ff)
{
  ...
  dealii::internal::hp::DoFHandlerImplementation::Implementation::reserve_space(
    *this);
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::clear_space()
{
  levels.clear();
  faces.reset();
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::renumber_dofs(
  const std::vector<types::global_dof_index> &new_numbers)
{
  Assert(levels.size() > 0,
         ExcMessage(
...
template <int dim, int spacedim>
unsigned int
hp::DoFHandler<dim, spacedim>::max_couplings_between_dofs() const
{
  Assert(fe_collection.size() > 0, ExcNoFESelected());
...
template <int dim, int spacedim>
unsigned int
hp::DoFHandler<dim, spacedim>::max_couplings_between_boundary_dofs() const
{
  Assert(fe_collection.size() > 0, ExcNoFESelected());
...
template <int structdim>
types::global_dof_index
hp::DoFHandler<dim, spacedim>::get_dof_index(const unsigned int,
                                             const unsigned int,
                                             const unsigned int,
...
template <int structdim>
void
hp::DoFHandler<dim, spacedim>::set_dof_index(
  const unsigned int,
  const unsigned int,
  const unsigned int,
  const unsigned int,
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::set_active_fe_indices(
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::get_active_fe_indices(
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::setup_policy_and_listeners()
{
  ...
  if (dynamic_cast<const parallel::distributed::Triangulation<dim, spacedim> *>(
        &this->get_triangulation()))
    {
      policy = std_cxx14::make_unique<
        internal::DoFHandlerImplementation::Policy::ParallelDistributed<
          hp::DoFHandler<dim, spacedim>>>(*this);
             ...
             &this->get_triangulation()) != nullptr)
    {
      policy =
        std_cxx14::make_unique<internal::DoFHandlerImplementation::Policy::
                                 ParallelShared<hp::DoFHandler<dim, spacedim>>>(
          *this);
  ...
  else
    {
      policy =
        std_cxx14::make_unique<internal::DoFHandlerImplementation::Policy::
                                 Sequential<hp::DoFHandler<dim, spacedim>>>(
          *this);
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::create_active_fe_table()
{
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::pre_refinement_action()
{
  create_active_fe_table();
}
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::post_refinement_action()
{
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::pre_active_fe_index_transfer()
{
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::pre_distributed_active_fe_index_transfer()
{
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::post_active_fe_index_transfer()
{
  // Finite elements need to be assigned to each cell by calling
  // distribute_dofs() first to make this functionality available.
  if (fe_collection.size() > 0)
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::post_distributed_active_fe_index_transfer()
{
#ifndef DEAL_II_WITH_P4EST
  Assert(false, ExcInternalError());
#else
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::prepare_for_serialization_of_active_fe_indices()
{
#ifndef DEAL_II_WITH_P4EST
  Assert(false,
         ExcMessage(
...
template <int dim, int spacedim>
void
hp::DoFHandler<dim, spacedim>::deserialize_active_fe_indices()
{
#ifndef DEAL_II_WITH_P4EST
  Assert(false,
         ExcMessage(
