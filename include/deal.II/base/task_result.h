...
namespace Threads
{
  ...
  inline TaskResult<T>::~TaskResult()
  {
    ...
    // documented, we consider this an error.
    if (result_is_available == false)
      {
        std::lock_guard<std::mutex> lock(mutex);

        // Check again, now under the mutex. It is possible that the status
        // has changed between the initial 'if' and the time we obtained the
        // lock.
        if (result_is_available == false)
    ...
    // Having established that there is no task still running,
    // let the compiler clear all member variables at the end of
    // this destructor.
  }
    ...
    if (result_is_available == false)
      {
        ...
        if (result_is_available == false)
          Assert(
            ...
            ExcMessage(
              "You cannot assign a new task to a TaskResult object "
              "while it is still waiting for its previous task "
              "to finish. See the documentation of this class' operator=() "
              "for more information."));
      }
    // Having established that there is no previous task still running,
    // clear previous content of this class and set the current task
