...
public:
   ...
   * @ref DataOutBase::CFFlags for options to control the output. Currently
   * only supports 2D meshes (codimension 0) that are either
   * only quads or only triangles, no mixed meshes. Below is an example of how
   * to use this function:
   *
   * @code
   * // Create DataOut and attach mesh and data
   * DataOut<dim> data_out;
   * data_out.attach_dof_handler(dof);
   * data_out.add_vector("solution", solution);
   * data_out.build_patches();
   * // Run the mesh through DataOutFilter to eliminate duplicate vertices
   * DataOutBase::DataOutFilterFlags filter_flags(true);
   * DataOutBase::DataOutFilter data_filter(filter_flags);
   * data_out.write_filtered_data(data_filter);
   * // Set flags
   * DataOutBase::CFFlags cf_flags;
   * cf_flags.attributes["solution"].push_back({"units", "m/s"});
   * data_out.set_flags(cf_flags);
   * // Write the filtered data to NetCDF
   * data_out.write_cf_parallel(data_filter, "solution.nc", MPI_COMM_WORLD);
   * @endcode
   *
   * Currently, writing CF data requires using @ref DataOutBase::DataOutFilter.
   * Setting @ref DataOutBase::DataOutFilterFlags::filter_duplicate_vertices
   * to `true` reduces file size by writing vertices that belong to multiple
   * mesh cells only once. Therefore, output of discontinuous fields requires
   * @ref DataOutBase::DataOutFilterFlags::filter_duplicate_vertices to be
   * `false`. See @ref DataOutBase::DataOutFilter for more information.
   *
   * @param data_filter @ref DataOutBase::DataOutFilter that has been filled
   * with @ref write_filtered_data.
   * @param filename Path of the file to write to.
   * @param comm MPI communicator. This is a collective function, all processes on the
