      ...
      while (particle != particle_handler.end())
        {
          ...
          for (unsigned int particle_index = 0; particle != pic.end();
            ...
            {
              const types::particle_index global_particle_id =
                particle_indices[particle_index];

              if constexpr (n_components == 1)
                {
                  // The single component may be deselected by the mask, in
                  // which case there is nothing to write (and the output
                  // vector has no entry for this particle).
                  if (comps[0])
                    interpolated_field[global_particle_id] =
