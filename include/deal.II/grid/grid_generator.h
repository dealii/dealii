...
namespace GridGenerator
{
   ...
   * and that the dimensions of the channel along the x, y and z dimensions must be an integer multiple of this cube length.
   * Consequently, the length before the sphere
   * ($L_{pre}$), the length after the sphere ($L_{post}$), the height below
   * ($H_{below}$), the height above ($H_{above}$), the width at the front ($H_{front}$)
   * and the width at the back ($H_{back}$) must be integer values.
   * The geometry consists of a channel of size $[-1 - L_{pre} * 2, 1 + L_{post} * 2] \times
   * [-1 - H_{below} * 2, 1 + H_{above} * 2] \times [-1 - W_{front} * 2, 1 + W_{back} * 2] $ 
   ...
   *   <li>a bulk region consisting of Cartesian cells.</li>
   * </ol>
   * Here is an example of a grid (with one additional global refinement)
   * where the arguments were {1,3,1,1,1,1}: 
   *
   ...
   * function.
   *
   * @param lengths_heights_widths  A vector containing the number of cube length, of the domain to the center
   * of the sphere. As the cube length is 2, and as the cube is centered at 0, each unit must be multiplied by 2 and be added 1
   * to obtain the corresponding distance.
   * The vector must contain 6 unsigned integers which consist in the length (in
   * number of cube lengths) before the sphere, after the sphere, below
   * the sphere, above the sphere, in front of the sphere, and behind the sphere.
   *
   * @param use_transfinite_region If `true`, then a tranfinite manifold is used
   ...
   * information on boundary indicators see
   * @ref GlossBoundaryIndicator "this glossary entry".
   * The left boundary (at $x = -1 - L_{pre} * 2$) is assigned an id of $0$, the right
   * boundary (at $x = 1 + L_{post} * 2$) is assigned an id of $1$; the boundary of
   * the sphere is assigned an id of $2$, the bottom wall (at $y=-1 - H_{below} * 2$) is
   * assigned and id of $3$, the top wall (at $y=H_{above}$) is assigned an id of $4$,
   * the front wall (at $y=-1 - W_{front} * 2$) is assigned an id of $5$,
   * and the back wall (at $y=1 + W_{back} * 2$) is assigned an id of $6$.
   */
  template <int dim>
  void
