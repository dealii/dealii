 ...
 * mesh structure into a Triangulation object in a variety of formats (see
 * the list of the various `read_*()` functions in this class for which
 * formats are supported). For most input formats, any numerical data other than
 ...
 *
 *
 * <h3>Reading meshes generated by Cubit</h3>
 *
 * Cubit is a widely used hexahedral mesh generator. deal.II doesn't
 * directly support importing from
 ...
 *   0---1---2
 * @endverbatim
 * may be characterized by the vertex numbers <tt>(0 1 4 3)</tt> and <tt>(1 2
...
public:
  ...
  /**
   * Read grid data from a unstructured VTK file. This reader only uses the
   * "legacy" VTK format that is text-based and does not use the newer,
   * XML-based format for which one typically uses file the ending `.vtu`.
   * The VTK file may contain
   * the following VTK cell types: VTK_HEXAHEDRON (12), VTK_TETRA (10),
   * VTK_QUAD (9), VTK_TRIANGLE (5), and VTK_LINE (3). The documentation
   * for the general legacy vtk file, including Unstructured Grid format
   * can be found here:
   ...
   *
   * The UNV format is generated by the Salome mesh
   * generator, see http://www.salome-platform.org/ . The sections of this
   * format that the current function supports are (only) 2411, 2412 and 2467.
   ...
   *
   * The UCD format can be used for grid input as well as data output via
   * the GridOut and DataOut classes. If there are data vectors in the input
   * file, they are ignored, as we are only interested in the grid in this
   * class. The UCD format requires the vertices to be in following ordering: in
   * 2d
   * @verbatim
   *      3-----2
   *      |     |
   ...
   * be found in the AVS Explorer manual (see http://www.avs.com).
   */
  void
   ...
   *
   * This format is used by the @p BAMG mesh
   ...
   * intended by the mesh generator.
   */
  void
   ...
   *
   * XDA is a rather simple format used by the MGF code. We
   ...
   * http://www.gmsh.info/.
   *
   * This format is used by the @p %Gmsh mesh
   * generator (see http://gmsh.info/). @p %Gmsh has an extensive manual that,
   * among other things, explains how to generate the quadrilateral and
   * hexahedral meshes that historically were favored by the deal.II library.
   * Of course, deal.II today also supports triangular, tetrahedral, and mixed
   * meshes including wedges and pyramids.
   *
   * %Gmsh has several versions of the file format. The reader supports versions
   * 1.0 and 2.0. If you want to use a specific version, you can instruct %Gmsh
   * to output the file in that version by adding a line such as
   * "Mesh.MshFileVersion = 1" to the gmsh input file.
   *
   * Also see
   * @ref simplex "Simplex support".
   */
   ...
   * in deal.II. An exception will be thrown if the attached triangulation is
   * not a parallel::fullydistributed::Triangulation.
   *
   ...
   * options:
   * @code
   * gmsh -part n_partitions -part_ghosts -part_split {mesh_file_name}
   * @endcode
   *
   ...
   * @endcode
   *
   * This command will generate a set of mesh files named like:
   * `mesh_file_1.msh`, `mesh_file_2.msh`, `mesh_file_3.msh`, and
   * `mesh_file_4.msh`.
   *
   * In the command above:
   *   - `mesh_file.vtk` can be replaced with any valid GMSH input file.
   ...
   *   - Multiple processors: {file_prefix}_{rank+1}.{file_suffix} where rank
   *     goes from 0 to number_of_processors-1.
   *
   ...
   *
   * @note This function interprets physical groups in the Gmsh files as either
   * @ref GlossBoundaryIndicator "boundary indicators" (for faces and edges of
   ...
   * if deal.II has been configured without directly linking to a tecplot
   * installation on your system.
   *
   * The tecplot format is used by @p TECPLOT and often
  ...
  /**
   * Read grid data from an UGRID file. UGRID files are used in the
   * aerospace domain to represent unstructured meshes. The format of these
   * kinds of files is described as part of the
   * [documentation of the UG IO package](https://www.simcenter.msstate.edu/),
   * see specifically the description
   * [here](https://www.simcenter.msstate.edu/software/documentation/ug_io/3d_grid_file_type_ugrid.html).
   */
  void
  read_ugrid(std::istream &in);
