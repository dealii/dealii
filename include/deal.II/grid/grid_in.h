...
public:
   ...
   * options:
   * @code
   * gmsh fname -part 4 -part_ghosts -part_split -save {input_file}
   * @endcode
   *
   * This will partition the mesh into 4 parts, include ghost cells, and write
   * each partition to a separate file.
   *
   * For example: If we have a VTK mesh file named `input_file.vtk`, the command
   * becomes:
   *  @code
   *  gmsh input_file.vtk -part 4 -part_ghosts -part_split -save input_file.msh
   *  @endcode
   *
   ...
   *   - `-save` specifies the base name and format for the output files.
   *
