      ...
      switch (this->kind)
        {
          ...
          case ReferenceCells::Pyramid:
            switch (face_ref_type)
              {
                case ReferenceCells::Triangle:
                  return std_cxx20::ranges::iota_view<unsigned int,
                                                      unsigned int>(1U, 5U);
                case ReferenceCells::Quadrilateral:
                  return std_cxx20::ranges::iota_view<unsigned int,
                                                      unsigned int>(0U, 1U);
          ...
          case ReferenceCells::Quadrilateral:
            {
              ...
              return face_to_cell_vertices(face,
                                           vertex,
      ...
      switch (this->kind)
        {
          case ReferenceCells::Quadrilateral:
            return 1.0;
          case ReferenceCells::Triangle:
            const std::array<double, 5> tri_faces{
      ...
      switch (this->kind)
        {
          ...
          case ReferenceCells::Tetrahedron:
            {
              const std::array<double, 5> tet_faces{
                {0.5, 0.5, 0.5, std::sqrt(3.0) / 2.0, X}};
              return tet_faces[face_no];
            }
          case ReferenceCells::Pyramid:
            {
              const std::array<double, 5> pyramid_faces{{4,
                                                         std::sqrt(2.0),
                                                         std::sqrt(2.0),
                                                         std::sqrt(2.0),
                                                         std::sqrt(2.0)}};
