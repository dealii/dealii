      ...
      for(unsigned int k = 0; k < nq; ++k)
        {
          ...
          for(unsigned int i = 0; i < n_dofs; ++i)
            for(unsigned int d1 = 0; d1 < dim; ++d1)
              for(unsigned int d2 = 0; d2 < dim; ++d2)
                {
                  result(i) += dx * .25 * (input[d1][k][d2] + input[d2][k][d1])
                               * (fe.shape_grad_component(i, k, d1)[d2]
                                  + fe.shape_grad_component(i, k, d2)[d1]);
                }
        }
              ...
              for(unsigned int d1 = 0; d1 < dim; ++d1)
                {
                  ...
                  for(unsigned int d2 = 0; d2 < dim; ++d2)
                    {
                      // v . nabla u n
                      M(i, j) -= .5 * dx * fe.shape_grad_component(j, k, d1)[d2]
                                 * n[d2] * v;
                      // v (nabla u)^T n
                      M(i, j) -= .5 * dx * fe.shape_grad_component(j, k, d2)[d1]
                                 * n[d2] * v;
                      // u  nabla v n
                      M(i, j) -= .5 * dx * fe.shape_grad_component(i, k, d1)[d2]
                                 * n[d2] * u;
                      // u (nabla v)^T n
                      M(i, j) -= .5 * dx * fe.shape_grad_component(i, k, d2)[d1]
                                 * n[d2] * u;
                    }
                }
        }
