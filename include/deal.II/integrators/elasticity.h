          ...
          for (unsigned int i = 0; i < n_dofs; ++i)
            for (unsigned int d1 = 0; d1 < dim; ++d1)
              for (unsigned int d2 = 0; d2 < dim; ++d2)
                {
                  result(i) +=
                    dx * .25 * (input[d1][k][d2] + input[d2][k][d1]) *
                    (fe.shape_grad_component(i, k, d1)[d2] + fe.shape_grad_component(i, k, d2)[d1]);
                }
        }
    }
                  ...
                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                    {
                      ...
                      // u  nabla v n
                      result(i) -=
                        .5 * dx * (u - g) * fe.shape_grad_component(i, k, d1)[d2] * n[d2];
                      // u (nabla v)^T n
                      result(i) -=
                        .5 * dx * (u - g) * fe.shape_grad_component(i, k, d2)[d1] * n[d2];
                    }
                }
            }
        }
                  ...
                  for (unsigned int d2 = 0; d2 < dim; ++d2)
                    {
                      // v . nabla u n
                      M11(i, j) -=
                        .25 * dx * nu1 * fe1.shape_grad_component(j, k, d1)[d2] * n[d2] * v1;
                      M12(i, j) -=
                        .25 * dx * nu2 * fe2.shape_grad_component(j, k, d1)[d2] * n[d2] * v1;
                      M21(i, j) +=
                        .25 * dx * nu1 * fe1.shape_grad_component(j, k, d1)[d2] * n[d2] * v2;
                      M22(i, j) +=
                        .25 * dx * nu2 * fe2.shape_grad_component(j, k, d1)[d2] * n[d2] * v2;
                      // v (nabla u)^T n
                      M11(i, j) -=
                        .25 * dx * nu1 * fe1.shape_grad_component(j, k, d2)[d1] * n[d2] * v1;
                      M12(i, j) -=
                        .25 * dx * nu2 * fe2.shape_grad_component(j, k, d2)[d1] * n[d2] * v1;
                      M21(i, j) +=
                        .25 * dx * nu1 * fe1.shape_grad_component(j, k, d2)[d1] * n[d2] * v2;
                      M22(i, j) +=
                        .25 * dx * nu2 * fe2.shape_grad_component(j, k, d2)[d1] * n[d2] * v2;
                      // u  nabla v n
                      M11(i, j) -=
                        .25 * dx * nu1 * fe1.shape_grad_component(i, k, d1)[d2] * n[d2] * u1;
                      M12(i, j) +=
                        .25 * dx * nu1 * fe1.shape_grad_component(i, k, d1)[d2] * n[d2] * u2;
                      M21(i, j) -=
                        .25 * dx * nu2 * fe2.shape_grad_component(i, k, d1)[d2] * n[d2] * u1;
                      M22(i, j) +=
                        .25 * dx * nu2 * fe2.shape_grad_component(i, k, d1)[d2] * n[d2] * u2;
                      // u (nabla v)^T n
                      M11(i, j) -=
                        .25 * dx * nu1 * fe1.shape_grad_component(i, k, d2)[d1] * n[d2] * u1;
                      M12(i, j) +=
                        .25 * dx * nu1 * fe1.shape_grad_component(i, k, d2)[d1] * n[d2] * u2;
                      M21(i, j) -=
                        .25 * dx * nu2 * fe2.shape_grad_component(i, k, d2)[d1] * n[d2] * u1;
                      M22(i, j) +=
                        .25 * dx * nu2 * fe2.shape_grad_component(i, k, d2)[d1] * n[d2] * u2;
                    }
                }
        }
    }
                ...
                for (unsigned int d2 = 0; d2 < dim; ++d2)
                  {
                    // v . nabla u n
                    result1(i) -=
                      .25 * dx * (nu1 * Dinput1[d1][k][d2] + nu2 * Dinput2[d1][k][d2]) * n[d2] * v1;
                    result2(i) +=
                      .25 * dx * (nu1 * Dinput1[d1][k][d2] + nu2 * Dinput2[d1][k][d2]) * n[d2] * v2;
                    // v . (nabla u)^T n
                    result1(i) -=
                      .25 * dx * (nu1 * Dinput1[d2][k][d1] + nu2 * Dinput2[d2][k][d1]) * n[d2] * v1;
                    result2(i) +=
                      .25 * dx * (nu1 * Dinput1[d2][k][d1] + nu2 * Dinput2[d2][k][d1]) * n[d2] * v2;
                    // u  nabla v n
                    result1(i) -=
                      .25 * dx * nu1 * fe1.shape_grad_component(i, k, d1)[d2] * n[d2] * (u1 - u2);
                    result2(i) -=
                      .25 * dx * nu2 * fe2.shape_grad_component(i, k, d1)[d2] * n[d2] * (u1 - u2);
                    // u  (nabla v)^T n
                    result1(i) -=
                      .25 * dx * nu1 * fe1.shape_grad_component(i, k, d2)[d1] * n[d2] * (u1 - u2);
                    result2(i) -=
                      .25 * dx * nu2 * fe2.shape_grad_component(i, k, d2)[d1] * n[d2] * (u1 - u2);
                  }
              }
        }
    }
