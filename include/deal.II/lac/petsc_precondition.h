...
#  endif
   ...
   *
   * Each processor can have one or more blocks, but a block cannot be shared by
   * more than one processor.
   *
   * @note Currently, each block is solved with a direct solver.
   *
   * @ingroup PETScWrappers
   */
  ...
  public:
    ...
    struct AdditionalData
    {
      struct LocalSubdomains
      {
        LocalSubdomains(
          const std::vector<std::vector<PetscInt>> &subdomain_indices = {},
          const std::vector<std::vector<PetscInt>> &local_indices     = {})
        {
          this->subdomain_indices = subdomain_indices;
          this->local_indices     = local_indices;
        }

        /**
         * The (overlapping) subdomains owned by this processor. Each entry of
         * the outer vector describes one subdomain (or "patch", e.g. a
         * vertex-star) through the global indices of the degrees of freedom it
         * contains. A processor may own an arbitrary number of subdomains;
         * the number of subdomains is given by the size of this vector.
         */
        std::vector<std::vector<PetscInt>> subdomain_indices;

        /**
         * The (non-overlapping) local part of each subdomain owned by this
         * processor, in global numbering. If provided, this vector must have
         * the same length as @p subdomain_indices, and each entry must be a
         * subset of the corresponding entry of @p subdomain_indices. If left
         * empty, PETSc determines the local parts automatically.
         */
        std::vector<std::vector<PetscInt>> local_indices;
      };
      ...
      AdditionalData(
        ...
        const ASMType          type             = ASMType::restrict,
        const LocalSubdomains &local_subdomains = LocalSubdomains({}, {}));
      ...
      /**
       * The local subdomains for each processor. This is used to set the
       * subdomains with PCASMSetLocalSubdomains(). If the vectors are empty,
       * then the subdomains are generated by PETSc.
       */
      LocalSubdomains local_subdomains;
    };
