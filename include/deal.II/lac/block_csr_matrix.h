...
struct DoFInfo
{
   ...
   */
  void
  initialize(const std::vector<unsigned int> &          my_rows,
             const std::shared_ptr<const BlockIndices> &row_blocks);
  ...
  /**
   * Stores the rowstart indices of the compressed row storage in the
   * dof_indices and block_indices fields for each cell.
   */
...
private:
   ...
   */
  void
  process_cell_indices(std::vector<TUP> &cell_dof_indices_local);
};
...
namespace BlockCSRMatrixIterators
{
   ...
   * gain access to the pointer representing the beginning of the non-empty
   * block.
   */
  template <typename NumberType, bool Constness>
  ...
  public:
    ...
    using pointer = typename std::
      conditional<Constness, const NumberType *, NumberType *>::type;
  ...
  public:
    ...
    get_current_block_column() const;

    /**
     * Return the block size of the current column.
     */
    types::global_dof_index
    get_col_block_size() const;
     ...
     */
    types::global_dof_index
    reinit(const unsigned int cell, const unsigned int subcell = 0);
     ...
     */
    void
    process_active_rows_vectorized(
      const std::function<
  ...
  public:
     ...
     */
    NumberType
    local_element(const unsigned int local_index) const;

    /**
     * Same as above but for global DoF/row @p index.
     */
    NumberType
    operator()(const types::global_dof_index index) const;
     ...
     */
    void
    advance(const unsigned int col);
     ...
     */
    void
    reinit(const std::vector<unsigned int> &local_rows);
     ...
     */
    void
    reinit_blocks(const std::vector<unsigned int> &local_row_blocks);
  ...
  class RowsAccessor : public RowsAccessorBase<NumberType, Constness>
  {
     ...
     */
    void
    set_local_element(const unsigned int local_index,
  ...
  public:
     ...
     */
    void
    add_local_element(const unsigned int local_index,
                      const NumberType & val) const;
     ...
     */
    void
    add(const types::global_dof_index index, const NumberType &val) const;
  ...
  private:
    ...
    void
    modify_local_element(const unsigned int local_index,
                         const NumberType & val) const;
  };
...
public:
   ...
   * course can't change the sparsity pattern as this is fixed once a block
   * sparse matrix is attached to it.
   */
   ...
   * equal to 'sparsity.m()'
   * @parameter col_blocks defines column blocks of this matrix, its size should
   * be equal to 'sparsity.n()`.
   * @parameter symmetric if `true`, the matrix is considered to be
   * symmetric/hermitian. In this case row blocks and column blocks should
   * match.
   * @parameter row_partitioner specifies 1D partitioner. This can be both row
   * block partitioner and row partitioner.
   */
  void
  reinit(const DynamicSparsityPattern &             sparsity,
         const std::shared_ptr<const BlockIndices> &row_blocks,
   ...
   */
  value_type
  local_el(const unsigned int i, const size_type j) const;
   ...
   */
  void
  mmult(BlockCSRMatrix<NumberType> &      C,
   ...
   */
  void
  Tmmult(BlockCSRMatrix<NumberType> &      C,
         const BlockCSRMatrix<NumberType> &B,
         const bool rebuild_sparsity_pattern = true) const;
   ...
   */
  value_type
  Tr_Tmmult(const BlockCSRMatrix<NumberType> &B) const;
   ...
   */
  const_iterator
  begin_local(const unsigned int r) const;

  /**
   * Same as above but for local row @p r.
   */
  iterator
  begin_local(const unsigned int r);

  /**
   * Same as above but for local row @p r
   */
  const_iterator
  end_local(const unsigned int r) const;

  /**
   * Same as above but for local row @p r
   */
  iterator
  end_local(const unsigned int r);
  ...
  void
  copy_to(DenseMatrix &matrix) const;
  ...
  /**
   * Return row/column major linear index for the element (i,j) within the block
   * (M,N).
   */
  static unsigned int
  local_index(const unsigned int i,
...
private:
   ...
   */
  void
  setup_ghosts(const DynamicSparsityPattern &locally_owned_sparsity);
   ...
   */
  std::vector<
    std::pair<unsigned int, std::vector<std::pair<unsigned int, unsigned int>>>>
    import_indices;
...
namespace internal
{
   ...
   */
  inline unsigned int
  ceil_divisible_by(const unsigned int number, const unsigned int factor)
  {
  ...
  template <typename NumberType>
  inline unsigned int
  padded_size(const unsigned int size)
  {
    Assert(64 % sizeof(NumberType) == 0, ExcInternalError());
    Assert(
      ceil_divisible_by(size, VectorizedArray<NumberType>::n_array_elements) <=
        ceil_divisible_by(size, 64 / sizeof(NumberType)),
      ExcMessage("We assume that SIMD width is less or equal to 64 bytes."));

    return ceil_divisible_by(size,
                             BlockCSRMatrix<NumberType>::col_block_size_factor);
  }
} // namespace internal
...
template <typename NumberType>
inline unsigned int
BlockCSRMatrix<NumberType>::local_index(const unsigned int i,
                                        const unsigned int j,
                                        const unsigned int,
                                        const unsigned int N)
{
...
template <typename NumberType>
inline bool
BlockCSRMatrix<NumberType>::is_symmetric() const
{
  return symmetric;
}
...
template <typename NumberType>
inline std::pair<types::global_dof_index,
                 std::vector<std::pair<unsigned int, unsigned int>>>
BlockCSRMatrix<NumberType>::get_block_data(const unsigned int r) const
{
  ...
  if (r < n_owned_row_blocks)
    return {owned_row_start + row_blocks->block_start(r),
            {{0, row_blocks->block_size(r)}}};
  else
...
template <typename DenseMatrix>
void
BlockCSRMatrix<NumberType>::copy_to(DenseMatrix &matrix) const
{
  ...
  dealii::Utilities::MPI::sum(matrix,
...
template <typename NumberType>
inline void
BlockCSRMatrix<NumberType>::clear()
{
  sp.reinit(0, 0, 0);
  values.clear();
  row_blocks.reset();
...
namespace BlockCSRMatrixIterators
{
  ...
  template <typename NumberType, bool Constness>
  inline typename Accessor<NumberType, Constness>::pointer
  ...
  template <typename NumberType, bool Constness>
  inline void
  Accessor<NumberType, Constness>::advance()
  {
    // first move data
    Assert(this->global_index() + 1 < matrix->data_start.size(),
           ExcMessage("AssertRange"));
    current_data += matrix->data_start[this->global_index() + 1] -
                    matrix->data_start[this->global_index()];
  ...
  template <typename NumberType, bool Constness>
  inline Iterator<NumberType, Constness> &
  Iterator<NumberType, Constness>::operator++()
  {
    accessor.advance();
    return *this;
  }
  ...
  template <typename NumberType, bool Constness>
  inline Iterator<NumberType, Constness>
  Iterator<NumberType, Constness>::operator++(int)
  {
  ...
  template <typename NumberType, bool Constness>
  inline bool
  Iterator<NumberType, Constness>::operator==(const Iterator &other) const
  {
  ...
  template <typename NumberType, bool Constness>
  inline Iterator<NumberType, Constness>
  Iterator<NumberType, Constness>::operator+(const size_type n) const
  {
    Iterator x = *this;
  ...
  template <typename NumberType, bool Constness>
  inline unsigned int
  RowsAccessorBase<NumberType, Constness>::size() const
  {
    return matrix->m();
  }
  ...
  template <typename NumberType, bool Constness>
  inline unsigned int
  RowsAccessorBase<NumberType, Constness>::n_row_blocks() const
  {
  ...
  template <typename NumberType, bool Constness>
  inline NumberType
  RowsAccessorBase<NumberType, Constness>::
  operator()(const types::global_dof_index index) const
  {
...
#  ifdef BINARY_SEARCH
    ...
    const auto d_it =
      std::lower_bound(active_row_blocks.rbegin(),
    ...
    Assert(d_it != active_row_blocks.rend(),
           ExcMessage("The RowsAccessor was not initialized to access row " +
                      std::to_string(local_index) + " at block (" +
...
#  endif
    ...
    // make sure what we found actually is the same as going through the
    // global_to_local route:
    ...
    if (d.active)
      {
        ...
        return *(d.data + BlockCSRMatrix<NumberType>::local_index(
                            local_index - d.block_start,
                            col_within_block,
                            /*does not matter*/ 0,
  ...
  template <typename NumberType>
  inline void
  RowsAccessor<NumberType, false>::set_local_element(
  ...
  template <typename NumberType>
  inline void
  RowsAccessor<NumberType, false>::add_local_element(
    const unsigned int local_index,
    const NumberType & val) const
  {
  ...
  inline void
  RowsAccessor<NumberType, false>::modify_local_element(
    const unsigned int local_index,
    const NumberType & val) const
  {
...
#  ifdef BINARY_SEARCH
    ...
    const auto d_it = std::lower_bound(
      ...
      [](const typename RowsAccessorBase<NumberType, false>::DataType &d,
         const unsigned int &row) -> bool { return d.block_start > row; });
...
#  endif
    ...
    // make sure what we found actually is the same as going through the
    // global_to_local route:
    AssertDimension(d.block_start,
                    this->matrix->row_blocks->block_start(pair.first));
        ...
        if (addition)
          *(d.data +
            BlockCSRMatrix<NumberType>::local_index(
              ...
              /*does not matter*/ 0,
              this->matrix->get_col_blocks()->block_size(this->block_col))) +=
            val;
        else
          *(d.data +
            BlockCSRMatrix<NumberType>::local_index(
  ...
  template <typename NumberType, bool Constness>
  inline typename RowsAccessorBase<NumberType, Constness>::MatrixType::size_type
  ...
  template <typename NumberType, bool Constness>
  inline void
  RowsAccessorBase<NumberType, Constness>::move_iterators()
  {
  ...
  template <typename NumberType, bool Constness>
  inline void
  RowsAccessorBase<NumberType, Constness>::advance(const unsigned int col)
  {
    const std::pair<unsigned int, unsigned int> col_pair =
      matrix->col_blocks->global_to_local(col);
  ...
  template <typename NumberType, bool Constness>
  bool
  RowsAccessorBase<NumberType, Constness>::advance()
  {
...
BlockCSRMatrix<NumberType>::begin_local(const unsigned int r) const
{
  ...
  Assert(r < n_owned_row_blocks || matrix_is_ghosted == true, ExcReadGhost(r));
                          ...
                          static_dofs_per_cell> &                   touched)
{
  ...
  static const VectorizedArray<Number> zero = make_vectorized_array<Number>(0);
...
struct is_serial_vector<
  BlockCSRMatrixIterators::RowsAccessor<Number, Constness>> : std::false_type
{};
 ...
 */
template <typename Number>
struct is_serial_vector<BlockCSRMatrix<Number>> : std::false_type
{};
