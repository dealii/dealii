...
public:
   ...
   */
  void
  reinit(const Table<2, Number> &mass_matrix, const Table<2, Number> &derivative_matrix);
...
template <int dim, typename Number, int size>
inline TensorProductMatrixSymmetricSum<dim, Number, size>::TensorProductMatrixSymmetricSum(
...
template <typename MatrixArray>
inline void
TensorProductMatrixSymmetricSum<dim, Number, size>::reinit_impl(MatrixArray &&mass_matrices_,
                                                                MatrixArray &&derivative_matrices_)
{
  ...
  for (int dir = 0; dir < dim; ++dir)
    {
      Assert(size == -1 ||
               (size > 0 && static_cast<unsigned int>(size) == mass_matrices[dir].n_rows()),
      ...
      internal::TensorProductMatrix ::spectral_assembly<Number>(&(mass_matrices[dir](0, 0)),
...
template <typename MatrixArray>
inline void
TensorProductMatrixSymmetricSum<dim, VectorizedArray<Number>, size>::reinit_impl(
  ...
  for (int dir = 0; dir < dim; ++dir)
    {
      ...
      vectorized_transpose_and_store(
        false, nm, &(mass_matrix[dir](0, 0)), offsets_nm.cbegin(), mass_matrix_flat.data());
      vectorized_transpose_and_store(
        false, nm, &(derivative_matrix[dir](0, 0)), offsets_nm.cbegin(), deriv_matrix_flat.data());
      ...
      for (unsigned int lane = 0; lane < macro_size; ++lane)
        internal::TensorProductMatrix ::spectral_assembly<Number>(mass_cbegin + nm * lane,
      ...
      vectorized_load_and_transpose(
        n_rows, eigenvalues_flat.data(), offsets_n.cbegin(), this->eigenvalues[dir].begin());
      vectorized_load_and_transpose(
        nm, eigenvectors_flat.data(), offsets_nm.cbegin(), &(this->eigenvectors[dir](0, 0)));
    }
}
