      ...
      for (int i = 0; i < col; ++i)
        {
          ...
          const double tmp = h(i);
          h(i)             = c * tmp + s * h(i + 1);
          h(i + 1)         = -s * tmp + c * h(i + 1);
        }

      const double r =
        1. / std::sqrt(h(col) * h(col) + h(col + 1) * h(col + 1));
      rotations[col].second = h(col + 1) * r;
      rotations[col].first  = h(col) * r;
      h(col) =
        rotations[col].first * h(col) + rotations[col].second * h(col + 1);
