 ...
 * SparseMatrix<float>, FullMatrix<double>, and FullMatrix<float>.
 */
...
public:
  ...
  struct ScalingControl
  {
    ...
    struct SKParameters
    {
       ...
       */
      SKParameters(const NormType     norm_type      = NormType::l1,
    ...
    struct l1linfParameters
    {
       ...
       */
      l1linfParameters(const unsigned int start_inf_norm_steps = 1,
    ...
    ScalingControl(
      ...
      const ScalingAlgorithm alg       = ScalingAlgorithm::l1_linf_sympreserve,
   ...
   * selected.
   */
  MatrixScaling(const ScalingControl &control = ScalingControl());
