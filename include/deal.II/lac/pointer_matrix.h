...
{
  DEAL_II_Assert(m != nullptr, ExcNotInitialized());
  m->vmult(dst, src);
}
...
{
  DEAL_II_Assert(m != nullptr, ExcNotInitialized());
  DEAL_II_Assert(dst.size() == 1, ExcDimensionMismatch(dst.size(), 1));
...
{
  DEAL_II_Assert(m != nullptr, ExcNotInitialized());
  DEAL_II_Assert(src.size() == 1, ExcDimensionMismatch(src.size(), 1));
