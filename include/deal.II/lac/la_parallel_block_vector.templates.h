...
namespace LinearAlgebra
{
  ...
  {
    template <typename Number>
    BlockVector<Number>::BlockVector(const size_type n_blocks, const size_type block_size)
    {
    ...
    template <typename Number>
    void
    BlockVector<Number>::reinit(const std::vector<size_type> &n, const bool omit_zeroing_entries)
    {
    ...
    BlockVector<Number>::compress(::dealii::VectorOperation::values operation)
    {
      const unsigned int n_chunks =
        (this->n_blocks() + communication_block_size - 1) / communication_block_size;
    ...
    template <typename Number>
    void
    BlockVector<Number>::reinit(const VectorSpaceVector<Number> &V, const bool omit_zeroing_entries)
    {
    ...
    template <typename Number>
    void
    BlockVector<Number>::equ(const Number a, const VectorSpaceVector<Number> &vv)
    {
      // Downcast. Throws an exception if invalid.
      Assert(dynamic_cast<const BlockVector<Number> *>(&vv) != nullptr,
             ExcVectorTypeNotCompatible());
    ...
    template <typename Number>
    void
    BlockVector<Number>::add(const Number a, const VectorSpaceVector<Number> &vv)
    {
      // Downcast. Throws an exception if invalid.
      Assert(dynamic_cast<const BlockVector<Number> *>(&vv) != nullptr,
             ExcVectorTypeNotCompatible());
    ...
    template <typename Number>
    void
    BlockVector<Number>::sadd(const Number x, const Number a, const VectorSpaceVector<Number> &vv)
    {
      // Downcast. Throws an exception if invalid.
      Assert(dynamic_cast<const BlockVector<Number> *>(&vv) != nullptr,
             ExcVectorTypeNotCompatible());
    ...
    BlockVector<Number>::all_zero() const
    {
      ...
      int local_result = -1;
      for (unsigned int i = 0; i < this->n_blocks(); ++i)
        local_result = std::max(local_result, -static_cast<int>(this->block(i).all_zero_local()));

      if (this->block(0).partitioner->n_mpi_processes() > 1)
        return -Utilities::MPI::max(local_result, this->block(0).partitioner->get_communicator());
      else
        return local_result;
    }
    ...
    template <typename Number>
    Number BlockVector<Number>::operator*(const VectorSpaceVector<Number> &vv) const
    {
      Assert(this->n_blocks() > 0, ExcEmptyObject());
      ...
      if (this->block(0).partitioner->n_mpi_processes() > 1)
        return Utilities::MPI::sum(local_result, this->block(0).partitioner->get_communicator());
      else
        return local_result;
    }
    ...
    BlockVector<Number>::mean_value() const
    {
      ...
      for (unsigned int i = 0; i < this->n_blocks(); ++i)
        local_result +=
          this->block(i).mean_value_local() * (real_type)this->block(i).partitioner->local_size();
    ...
    BlockVector<Number>::lp_norm(const real_type p) const
    {
      ...
      if (this->block(0).partitioner->n_mpi_processes() > 1)
        return std::pow(
          Utilities::MPI::sum(local_result, this->block(0).partitioner->get_communicator()),
