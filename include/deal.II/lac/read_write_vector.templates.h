...
namespace LinearAlgebra
{
  ...
  ReadWriteVector<Number>::resize_val(const size_type new_alloc_size)
  {
    if (new_alloc_size == 0)
    ...
    else
      {
        ...
        if (new_alloc_size >=
            4 * dealii::internal::VectorImplementation::minimum_parallel_grain_size)
          thread_loop_partitioner = std::make_shared<parallel::internal::TBBPartitioner>();
      }
  }
  ...
  template <typename Number>
  void
  ReadWriteVector<Number>::reinit(const size_type size, const bool omit_zeroing_entries)
  {
    ...
    if (communication_pattern.get() == nullptr)
      {
        comm_pattern = std::make_shared<Utilities::MPI::Partitioner>(
          vec.locally_owned_elements(), get_stored_elements(), vec.get_mpi_communicator());
      }
    else
      {
        comm_pattern =
    ...
    if (operation == VectorOperation::insert)
      {
        ...
        AssertThrow(
          err == 0,
          ExcMessage("Epetra Import() failed with error code: " + Utilities::to_string(err)));
    ...
    if (operation == VectorOperation::insert)
      {
        cudaError_t error_code = cudaMemcpy(
          values.get(), cuda_vec.get_values(), n_elements * sizeof(Number), cudaMemcpyDeviceToHost);
    ...
    else
      {
        ...
        cudaError_t         error_code = cudaMemcpy(
          tmp.data(), cuda_vec.get_values(), n_elements * sizeof(Number), cudaMemcpyDeviceToHost);
        AssertCuda(error_code);
