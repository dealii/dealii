...
namespace parallel
{
   ...
   * @endcode
   *
   * If the weighting function only requires information about the finite
   * element (for example about the number of degrees of freedom per cell), the
   * weights can be computed in advance. Such a cache can be set up
   * automatically by using the `enable_fe_cache` flag in the constructor or
   * reinit() function, or by using the make_cached_weighting_callback()
   * function. Note that no other cell-specific information can be used to build
   * the cache in the current implementation.
   *
  ...
  public:
    ...
    static std::function<unsigned int(
      const typename dealii::Triangulation<dim, spacedim>::cell_iterator &cell,
      const CellStatus status)>
    make_cached_weighting_callback(const DoFHandler<dim, spacedim> &dof_handler,
                                   const WeightingFunction &weighting_function);
