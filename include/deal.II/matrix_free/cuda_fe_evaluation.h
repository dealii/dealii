...
namespace CUDAWrappers
{
  ...
  {
    if (internal::global_co_shape_gradients[0] != 0.0)
      {
        ...
        if (evaluate_val == true && evaluate_grad == true)
          eval.value_and_gradient_at_quad_pts(values, gradients);
        else if (evaluate_grad == true)
          eval.gradient_at_quad_pts(values, gradients);
        else if (evaluate_val == true)
          eval.value_at_quad_pts(values);
    ...
    else
      {
        if (evaluate_grad == true)
          {
            internal::FEEvaluationCollocationImp<
    ...
    if (internal::global_co_shape_gradients[0] != 0.0)
      {
        ...
        if (integrate_val == true && integrate_grad == true)
          eval.integrate_value_and_gradient(values, gradients);
        else if (integrate_val == true)
          eval.integrate_value(values);
        else if (integrate_grad == true)
          eval.integrate_gradient<false>(values, gradients);
        __syncthreads();
      }
    else
      {
        ...
        if (integrate_val == true && integrate_grad == true)
          {
            eval.integrate_gradient<true>(values, gradients);
            __syncthreads();
          }
        else if (integrate_grad == true)
          {
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    get_value() const
  {
    return get_value(compute_index<dim, n_q_points_1d>());
  }
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    get_dof_value() const
  {
    return get_dof_value(compute_index<dim, fe_degree + 1>());
  }
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    submit_value(const value_type &val_in)
  {
    submit_value(val_in, compute_index<dim, n_q_points_1d>());
  }
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    submit_dof_value(const value_type &val_in)
  {
    submit_dof_value(val_in, compute_index<dim, fe_degree + 1>());
  }
    ...
    get_gradient() const
  {
    ...
    return get_gradient(compute_index<dim, n_q_points_1d>());
  }
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    submit_gradient(const gradient_type &grad_in)
  {
    submit_gradient(grad_in, compute_index<dim, n_q_points_1d>());
  }
  ...
  FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::
    apply_quad_point_operations(const Functor &func)
  {
    func(this, compute_index<dim, n_q_points_1d>());
