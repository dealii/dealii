...
public:
  ...
  get_curl(const unsigned int point_index);
...
Tensor<1,
       (dim == 2 ? 1 : dim),
       typename FEPointEvaluationBase<n_components_, dim, spacedim, Number>::
         VectorizedArrayType>
  ...
  const unsigned int point_index)
{
  ...
  const Tensor<2, dim, VectorizedArrayType> grad = get_gradient(point_index);
