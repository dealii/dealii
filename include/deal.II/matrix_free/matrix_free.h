...
private:
   ...
   */
  unsigned int first_dof_handler_index;
};
  ...
  const unsigned int cell_batch_index) const
{
  ...
  AssertIndexRange(
    cell_batch_index,
    dof_info[first_dof_handler_index].cell_active_fe_index.size());
  if (dof_info[first_dof_handler_index].cell_active_fe_index.empty())
    return 0;
  else
    return dof_info[first_dof_handler_index]
