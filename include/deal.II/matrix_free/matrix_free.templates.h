...
template <int dim, typename Number>
MatrixFree<dim, Number>::MatrixFree(const MatrixFree<dim, Number> &other) : Subscriptor()
{
  copy_from(other);
}
  ...
  const unsigned int                           vector_component) const
{
  ...
  const std::vector<unsigned int> &fe_indices = dof_info[vector_component].cell_active_fe_index;
  if (fe_indices.empty() == true)
  ...
  else
    {
      ...
      return_range.first =
        std::lower_bound(&fe_indices[0] + range.first, &fe_indices[0] + range.second, fe_index) -
        &fe_indices[0];
      return_range.second = std::lower_bound(&fe_indices[0] + return_range.first,
                                             &fe_indices[0] + range.second,
                                             fe_index + 1) -
                            &fe_indices[0];
      Assert(return_range.first >= range.first && return_range.second <= range.second,
             ExcInternalError());
      return return_range;
    }
}
...
MatrixFree<dim, Number>::get_dof_handler(const unsigned int dof_index) const
{
  ...
  if (dof_handlers.active_dof_handler == DoFHandlers::usual)
    {
      AssertDimension(dof_handlers.dof_handler.size(), dof_handlers.n_dof_handlers);
                                              ...
                                              const unsigned int dof_index) const
{
  ...
  Assert(dof_handlers.active_dof_handler == DoFHandlers::hp, ExcNotImplemented());
  ...
  const typename MatrixFree<dim, Number>::AdditionalData &additional_data)
{
  ...
  if (additional_data.initialize_indices == true)
    {
      ...
      if (Utilities::MPI::job_supports_mpi() == true)
        {
          ...
          task_info.my_pid  = Utilities::MPI::this_mpi_process(task_info.communicator);
          task_info.n_procs = Utilities::MPI::n_mpi_processes(task_info.communicator);
        }
      else
        {
      ...
      if (additional_data.tasks_parallel_scheme != AdditionalData::none &&
        ...
        {
          task_info.scheme = internal::MatrixFreeFunctions::TaskInfo::TasksParallelScheme(
      ...
      for (unsigned int i = 0; i < dof_info.size(); ++i)
        {
          ...
          // mapping_info...
          while (cell_level_index.size() % VectorizedArray<Number>::n_array_elements != 0)
  ...
  if (additional_data.initialize_mapping == true)
    {
      std::vector<unsigned int> dummy;
      mapping_info.initialize(dof_handler[0]->get_triangulation(),
                              cell_level_index,
                              face_info,
                              dummy,
                              mapping,
  ...
  const typename MatrixFree<dim, Number>::AdditionalData &additional_data)
{
  ...
  // gradients and Hessians for quadrature points.
  {
    ...
    for (unsigned int i = 0; i < n_components; ++i)
      n_fe_in_collection = std::max(n_fe_in_collection, dof_handler[i]->get_fe_collection().size());
    ...
    shape_info.reinit(
      TableIndices<4>(n_components, n_quad, n_fe_in_collection, n_quad_in_collection));
    for (unsigned int no = 0, c = 0; no < dof_handler.size(); no++)
...
MatrixFree<dim, Number>::is_supported(const FiniteElement<dim, spacedim> &fe)
{
  ...
  if (dynamic_cast<const FE_Poly<TensorProductPolynomials<dim>, dim, spacedim> *>(fe_ptr) !=
      nullptr)
    return true;
  if (dynamic_cast<
        const FE_Poly<TensorProductPolynomials<dim, Polynomials::PiecewisePolynomial<double>>,
                      dim,
  ...
  if (level == numbers::invalid_unsigned_int)
    {
      ...
      typename Triangulation<dim>::cell_iterator cell = tria.begin(0), end_cell = tria.end(0);
      // For serial Triangulations always take all cells
      const unsigned int subdomain_id = (dynamic_cast<const parallel::Triangulation<dim> *>(
  ...
  const AdditionalData &                          additional_data)
{
  ...
  dof_handlers.level              = additional_data.level_mg_handler;
  Assert(dof_handlers.level == numbers::invalid_unsigned_int, ExcNotImplemented());
  dof_handlers.n_dof_handlers = dof_handler.size();
  ...
  Assert(n_mpi_procs > 1 || cell_level_index.size() == tria.n_active_cells(), ExcInternalError());
...
template <int dim, typename Number>
void
MatrixFree<dim, Number>::initialize_indices(const std::vector<const ConstraintMatrix *> &constraint,
                                            const std::vector<IndexSet> &locally_owned_set,
                                            const AdditionalData &       additional_data)
{
  ...
  for (unsigned int no = 0; no < n_fe; ++no)
    {
      ...
      if (dof_handlers.active_dof_handler == DoFHandlers::hp)
        {
          ...
          if (fe.size() > 1)
            dof_info[no].cell_active_fe_index.resize(n_active_cells, numbers::invalid_unsigned_int);

          Assert(additional_data.cell_vectorization_category.empty(), ExcNotImplemented());
        }
      else
        {
          ...
          for (unsigned int c = 0; c < dof_info[no].n_base_elements; ++c)
            {
              ...
              lexicographic[no][fe_index].insert(
                ...
                shape_info(dof_info[no].global_base_element_offset + c, 0, fe_index, 0)
                  .lexicographic_numbering.begin(),
                shape_info(dof_info[no].global_base_element_offset + c, 0, fe_index, 0)
                  .lexicographic_numbering.end());
            }

          AssertDimension(lexicographic[no][fe_index].size(), dof_info[no].dofs_per_cell[fe_index]);
      ...
      dof_info[no].dof_indices.reserve((n_active_cells * dof_info[no].dofs_per_cell[0] * 3) / 2);

      // cache the constrained indices for use in matrix-vector products and
      // the like
      const types::global_dof_index start_index =
      ...
      for (types::global_dof_index i = start_index; i < end_index; ++i)
        if (constraint[no]->is_constrained(i) == true)
          dof_info[no].constrained_dofs.push_back(static_cast<unsigned int>(i - start_index));
    }
  ...
  for (unsigned int counter = 0; counter < n_active_cells; ++counter)
    {
      bool cell_at_subdomain_boundary =
        ...
        (additional_data.overlap_communication_computation == false && task_info.n_procs > 1);
      ...
      for (unsigned int no = 0; no < n_fe; ++no)
        {
          ...
          // we are requested to use a multigrid level
                   ...
                   dof_handlers.level != numbers::invalid_unsigned_int)
            {
              ...
              typename DoFHandler<dim>::cell_iterator cell_it(&dofh->get_triangulation(),
                                                              cell_level_index[counter].first,
                                                              cell_level_index[counter].second,
                                                              dofh);
              local_dof_indices.resize(dof_info[no].dofs_per_cell[0]);
          ...
          // hp case where we need to decode the FE index and similar
          else if (dof_handlers.active_dof_handler == DoFHandlers::hp)
            {
              ...
              if (dofh->get_fe_collection().size() > 1)
                dof_info[no].cell_active_fe_index[counter] = cell_it->active_fe_index();
  ...
  task_info.collect_boundary_cells(
    cell_level_index_end_local, n_active_cells, vectorization_length, subdomain_boundary_cells);
  ...
  // Finalize the creation of the ghost indices
  {
    ...
    for (unsigned int no = 0; no < n_fe; ++no)
      {
        if (do_face_integrals && additional_data.level_mg_handler != numbers::invalid_unsigned_int)
          {
            ...
            if (additional_data.level_mg_handler + 1 <
              ...
              for (typename DoFHandler<dim>::cell_iterator cell =
                   ...
                   cell != dof_handler.end(additional_data.level_mg_handler + 1);
                   ++cell)
                if (cell->level_subdomain_id() == task_info.my_pid)
                  for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
                    if ((cell->at_boundary(f) == false || cell->has_periodic_neighbor(f) == true) &&
                        cell->level() > cell->neighbor_or_periodic_neighbor(f)->level() &&
                        cell->neighbor_or_periodic_neighbor(f)->level_subdomain_id() !=
                          task_info.my_pid)
                      {
                        dof_indices.resize(
                          cell->neighbor_or_periodic_neighbor(f)->get_fe().dofs_per_cell);
  ...
  if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none)
  ...
  else
    {
      ...
      Assert(additional_data.cell_vectorization_category.empty(), ExcNotImplemented());
      task_info.initial_setup_blocks_tasks(subdomain_boundary_cells, renumbering, irregular_cells);
          ...
          if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::color)
            {
              start_nonboundary =
                task_info.n_procs > 1 ?
                  std::min(((task_info.cell_partition_data[2] - task_info.cell_partition_data[1] +
          ...
          if (dof_handlers.active_dof_handler == DoFHandlers::hp)
            {
              ...
              for (counter = 0; counter < std::min(start_nonboundary * vectorization_length,
                   ...
                   counter++)
                {
                  ...
                  AssertIndexRange(renumbering[counter], dof_info[0].cell_active_fe_index.size());
              ...
              for (unsigned int j = 0; j < dof_info[0].max_fe_index; j++)
                {
                  ...
                  n_macro_cells_before +=
                    (renumbering_fe_index[j].size() + vectorization_length - 1) /
              ...
              for (counter = start_nonboundary * vectorization_length;
                   ...
                   counter++)
                {
                  renumbering_fe_index[dof_info[0].cell_active_fe_index.empty() ?
                                         0 :
              ...
              AssertIndexRange(n_macro_cells_before,
                               task_info.cell_partition_data.back() + 2 * dof_info[0].max_fe_index +
                                 1);
      ...
      task_info.n_blocks = (n_macro_cells() + task_info.block_size - 1) / task_info.block_size;
      ...
      Assert(irregular_cells.size() >= task_info.cell_partition_data.back(), ExcInternalError());
      ...
      task_info.cell_partition_data.push_back(task_info.cell_partition_data.back() + n_ghost_slots);
    }
...
#ifdef DEBUG
  {
    AssertDimension(renumbering.size(), task_info.n_active_cells + task_info.n_ghost_cells);
...
#endif
  {
    ...
    for (unsigned int i = 0; i < task_info.cell_partition_data.back(); ++i)
      {
        unsigned int n_comp = (irregular_cells[i] > 0) ? irregular_cells[i] : vectorization_length;
        for (unsigned int j = 0; j < n_comp; ++j)
          cell_level_index.push_back(cell_level_index_old[renumbering[position_cell + j]]);
        ...
        for (unsigned int j = n_comp; j < vectorization_length; ++j)
          cell_level_index.push_back(cell_level_index_old[renumbering[position_cell + n_comp - 1]]);
    ...
    AssertDimension(cell_level_index.size(),
                    task_info.cell_partition_data.back() * vectorization_length);
  }
  ...
  for (unsigned int i = 0; i < constraints.size(); ++i)
    {
      ...
      constraint_pool_data.insert(
        constraint_pool_data.end(), constraints[i]->begin(), constraints[i]->end());
      constraint_pool_row_index.push_back(constraint_pool_data.size());
    }

  AssertDimension(constraint_pool_data.size(), length);
  for (unsigned int no = 0; no < n_fe; ++no)
    dof_info[no].reorder_cells(task_info, renumbering, constraint_pool_row_index, irregular_cells);
  ...
  if ((additional_data.mapping_update_flags_inner_faces |
       additional_data.mapping_update_flags_boundary_faces) != update_default)
    {
      ...
      std::vector<bool> hard_vectorization_boundary(task_info.face_partition_data.size(), false);
      if (task_info.scheme == internal::MatrixFreeFunctions::TaskInfo::none &&
      ...
      internal::MatrixFreeFunctions::collect_faces_vectorization(face_setup.inner_faces,
      ...
      internal::MatrixFreeFunctions::collect_faces_vectorization(face_setup.boundary_faces,
                                                                 hard_vectorization_boundary,
                                                                 task_info.boundary_partition_data,
                                                                 face_info.faces);
      hard_vectorization_boundary.clear();
      hard_vectorization_boundary.resize(task_info.ghost_face_partition_data.size(), false);
      ...
      for (unsigned int f = 0; f < task_info.boundary_partition_data.back(); ++f)
        for (unsigned int v = 0;
             ...
             face_info.faces[f].cells_interior[v] != numbers::invalid_unsigned_int;
             ++v)
          {
            TableIndices<3> index(
              face_info.faces[f].cells_interior[v] / VectorizedArray<Number>::n_array_elements,
            ...
            if (face_info.faces[f].cells_exterior[v] != numbers::invalid_unsigned_int)
              {
                TableIndices<3> index(
                  face_info.faces[f].cells_exterior[v] / VectorizedArray<Number>::n_array_elements,
      ...
      for (unsigned int no = 0; no < n_fe; ++no)
        {
          ...
          std::vector<types::global_dof_index> ghost_indices;
          {
            for (unsigned int cell = 0;
                 cell < VectorizedArray<Number>::n_array_elements * n_macro_cells_before;
                 ++cell)
              if (cell > 0 && cell_level_index[cell] != cell_level_index[cell - 1])
                {
                  for (unsigned int i =
                         dof_info[no].row_starts[cell * dof_info[no].start_components.back()].first;
                       i < dof_info[no]
                             .row_starts[(cell + 1) * dof_info[no].start_components.back()]
                             .first;
                       ++i)
                  ...
                  for (unsigned int i = dof_info[no].row_starts_plain_indices[cell];
                    ...
                    if (dof_info[no].plain_dof_indices[i] > part.local_size())
                      ghost_indices.push_back(
                        part.local_to_global(dof_info[no].plain_dof_indices[i]));
                }
            ...
            if (all_ghosts_equal)
              dof_info[no].vector_partitioner_face_variants[0] = dof_info[no].vector_partitioner;
            else
              {
          ...
          // partitioner 1: values on faces
          {
            ...
            for (unsigned int c = 0; c < dof_info[no].n_base_elements; ++c)
              if (!shape_info(dof_info[no].global_base_element_offset + c, 0, 0, 0)
                ...
                all_nodal = false;
            if (all_nodal == false)
              dof_info[no].vector_partitioner_face_variants[1] = dof_info[no].vector_partitioner;
            else
              {
                  ...
                  for (unsigned int v = 0;
                       ...
                       ++v)
                    {
                      ...
                      if (dof_info[no].index_storage_variants
                              ...
                              [f] >= internal::MatrixFreeFunctions::DoFInfo::IndexStorageVariants::
                                       contiguous &&
                          dof_info[no].dof_indices_contiguous
                              [internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
                              [face_info.faces[f].cells_exterior[v]] >= part.local_size())
                        {
                            ...
                            for (unsigned int c = 0; c < dof_info[no].n_components[e]; ++c)
                              {
                                ...
                                for (unsigned int j = 0; j < shape.dofs_per_component_on_face; ++j)
                                  ghost_indices.push_back(part.local_to_global(
                                    dof_info[no].dof_indices_contiguous
                                      [internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][p] +
                                    i +
                          ...
                          AssertDimension(i, dof_info[no].dofs_per_cell[0] * stride);
                                 ...
                                 [internal::MatrixFreeFunctions::DoFInfo::dof_access_face_exterior]
                                 [f] < internal::MatrixFreeFunctions::DoFInfo::
                                         IndexStorageVariants::contiguous)
                        has_noncontiguous_cell = true;
                    }
                has_noncontiguous_cell =
                  Utilities::MPI::min((int)has_noncontiguous_cell, task_info.communicator);
          ...
          // partitioner 2: values and gradients on faces
          {
            ...
            for (unsigned int c = 0; c < dof_info[no].n_base_elements; ++c)
              if (shape_info(dof_info[no].global_base_element_offset + c, 0, 0, 0).element_type !=
                ...
                all_hermite = false;
            if (all_hermite == false || dof_info[no].vector_partitioner_face_variants[1].get() ==
                                          dof_info[no].vector_partitioner.get())
              dof_info[no].vector_partitioner_face_variants[2] = dof_info[no].vector_partitioner;
            else
              {
                for (unsigned int f = 0; f < n_inner_face_batches(); ++f)
                  for (unsigned int v = 0;
                            ...
                            for (unsigned int c = 0; c < dof_info[no].n_components[e]; ++c)
                              {
                                const internal::MatrixFreeFunctions::ShapeInfo<
                                  VectorizedArray<Number>> &shape =
                                  shape_info(dof_info[no].global_base_element_offset + e, 0, 0, 0);
                                for (unsigned int j = 0; j < 2 * shape.dofs_per_component_on_face;
                                     ++j)
                                  ghost_indices.push_back(part.local_to_global(
                                    dof_info[no].dof_indices_contiguous
                                      [internal::MatrixFreeFunctions::DoFInfo::dof_access_cell][p] +
  ...
  namespace
  {
      ...
      for (unsigned int cell = begin; cell < end; ++cell)
        {
          ...
          connectivity_direct.add_entries(
            cell, new_indices.begin(), std::unique(new_indices.begin(), new_indices.end()));
        }
    }
                                        ...
                                        DynamicSparsityPattern &      connectivity)
    {
      ...
      for (unsigned int block = begin; block < end; ++block)
        {
          new_indices.clear();
          for (DynamicSparsityPattern::iterator it = connectivity_direct.begin(block);
          ...
          connectivity.add_entries(
            block, new_indices.begin(), std::unique(new_indices.begin(), new_indices.end()));
        }
    }
  } // namespace
} // namespace internal
...
template <int dim, typename Number>
void
MatrixFree<dim, Number>::make_connectivity_graph_faces(DynamicSparsityPattern &connectivity)
{
  (void)connectivity;
#ifdef DEAL_II_WITH_THREADS
  ...
  DynamicSparsityPattern    connectivity_direct(connectivity.n_rows(), connectivity.n_cols());
  const Triangulation<dim> &tria = dof_handlers.active_dof_handler == DoFHandlers::usual ?
                                     dof_handlers.dof_handler[0]->get_triangulation() :
                                     dof_handlers.hp_dof_handler[0]->get_triangulation();
  parallel::apply_to_subranges(0,
                               task_info.n_active_cells,
...
MatrixFree<dim, Number>::print_memory_consumption(StreamType &out) const
{
  ...
  task_info.print_memory_statistics(out, MemoryConsumption::memory_consumption(cell_level_index));
  if (Utilities::MPI::sum(face_info.faces.size(), task_info.communicator) > 0)
    {
      ...
      task_info.print_memory_statistics(out,
                                        MemoryConsumption::memory_consumption(face_info.faces));
    }
  ...
  task_info.print_memory_statistics(out, MemoryConsumption::memory_consumption(shape_info));
  if (task_info.scheme != internal::MatrixFreeFunctions::TaskInfo::none)
    {
      out << "   Memory task partitioning info:    ";
      task_info.print_memory_statistics(out, MemoryConsumption::memory_consumption(task_info));
    }
}
