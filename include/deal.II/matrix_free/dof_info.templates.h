...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    template <typename Number>
    ConstraintValues<Number>::ConstraintValues() : constraints(FPArrayComparator<double>(1.))
    {}
      ...
      if (entries.size() > 0)
        {
          ...
          std::sort(constraint_entries.begin(), constraint_entries.end(), ConstraintComparator());
      ...
      for (unsigned int comp = 0; comp < n_components; ++comp)
        {
          ...
          for (unsigned int i = component_dof_indices_offset[fe_index][comp];
               ...
               i++)
            {
              types::global_dof_index current_dof = local_indices[lexicographic_inv[i]];
              const std::vector<std::pair<types::global_dof_index, double>> *entries_ptr =
              ...
              if (entries_ptr != nullptr)
                  ...
                  if (n_entries > 0)
                    {
                      ...
                      for (unsigned int j = 0; j < n_entries; ++j)
                        {
                          if (n_mpi_procs > 1 && (constraint_indices[j] < first_owned ||
                            ...
                            dof_indices.push_back(
                              static_cast<unsigned int>(constraint_indices[j] - first_owned));
                        }
                    }
                }
              else
                ...
                no_constraint:
                  ...
                  if (n_mpi_procs > 1 && (current_dof < first_owned || current_dof >= last_owned))
                    {
                      ghost_dofs.push_back(current_dof);
                  ...
                  Assert(constraint_iterator.first < (1 << (8 * sizeof(unsigned short))) - 1,
                         ExcInternalError());
    ...
    DoFInfo ::assign_ghosts(const std::vector<unsigned int> &boundary_cells)
    {
      ...
      const unsigned int n_owned =
        (vector_partitioner->local_range().second - vector_partitioner->local_range().first);
      const std::size_t n_ghosts = ghost_dofs.size();
#ifdef DEBUG
      for (std::vector<unsigned int>::iterator dof = dof_indices.begin(); dof != dof_indices.end();
           ++dof)
      ...
      if (n_ghosts > 0)
        {
          ...
          for (std::size_t i = 1; i < n_ghosts; i++)
            {
              if (ghost_origin[i].first > ghost_origin[i - 1].first + 1)
                {
                  ghost_indices.add_range(last_contiguous_start, ghost_origin[i - 1].first + 1);
          ...
          ++n_unique_ghosts;
          ghost_indices.add_range(last_contiguous_start, ghost_origin.back().first + 1);
          ...
          // dofs. the ghost index set should store the same number
          {
            ...
            for (std::size_t i = 0; i < n_ghosts; ++i)
              Assert(ghost_numbering[i] == ghost_indices.index_within_set(ghost_dofs[i]),
                     ExcInternalError());
          }
          ...
          for (unsigned int i = 0; i < n_boundary_cells; ++i)
            {
              unsigned int *data_ptr =
                dof_indices.data() + row_starts[boundary_cells[i] * n_components].first;
              const unsigned int *row_end =
                dof_indices.data() + row_starts[(boundary_cells[i] + 1) * n_components].first;
              for (; data_ptr != row_end; ++data_ptr)
                *data_ptr =
                  ((*data_ptr < n_owned) ? *data_ptr :
              ...
              if (store_plain_indices == true)
                {
                  if (row_starts[boundary_cells[i] * n_components].second !=
                    ...
                    {
                      unsigned int *data_ptr =
                        plain_dof_indices.data() + row_starts_plain_indices[boundary_cells[i]];
                      const unsigned int *row_end =
                        data_ptr +
                            ...
                            const std::vector<unsigned char> &irregular_cells)
    {
          ...
          for (unsigned int cell = 0; cell < task_info.cell_partition_data.back(); ++cell)
            {
              const unsigned int n_comp =
                (irregular_cells[cell] > 0 ? irregular_cells[cell] : vectorization_length);
              ...
              for (unsigned int j = 1; j < n_comp; ++j)
                fe_index = std::max(fe_index, cell_active_fe_index[renumbering[position_cell + j]]);
      ...
      if (have_hp)
        AssertDimension(cell_active_fe_index.size(), task_info.cell_partition_data.back());

      const unsigned int n_components = start_components.back();

      std::vector<std::pair<unsigned int, unsigned int>> new_row_starts(
        vectorization_length * n_components * task_info.cell_partition_data.back() + 1);
      std::vector<unsigned int>                              new_dof_indices;
      std::vector<std::pair<unsigned short, unsigned short>> new_constraint_indicator;
      ...
      if (store_plain_indices == true)
        {
          new_rowstart_plain.resize(vectorization_length * task_info.cell_partition_data.back() + 1,
      ...
      for (unsigned int i = 0; i < task_info.cell_partition_data.back(); ++i)
        {
          ...
          const unsigned int dofs_per_cell =
            have_hp ? this->dofs_per_cell[cell_active_fe_index[i]] : this->dofs_per_cell[0];
          ...
          for (unsigned int j = 0; j < n_vect; ++j)
            {
              ...
              if (store_plain_indices &&
                  row_starts[cell_no].second != row_starts[cell_no + n_components].second)
                {
                  new_rowstart_plain[i * vectorization_length + j] = new_plain_indices.size();
      ...
      new_row_starts[task_info.cell_partition_data.back() * vectorization_length * n_components]
        .first = new_dof_indices.size();
      new_row_starts[task_info.cell_partition_data.back() * vectorization_length * n_components]
        .second = new_constraint_indicator.size();

      AssertDimension(constraint_indicator.size(), new_constraint_indicator.size());
      ...
      for (unsigned int row = 0; row < task_info.cell_partition_data.back(); ++row)
        {
          ...
          const std::pair<unsigned short, unsigned short>
            *con_it = constraint_indicator.data() +
                      row_starts[row * vectorization_length * n_components].second,
            *end_con = constraint_indicator.data() +
          ...
          for (; con_it != end_con; ++con_it)
            {
              ...
              AssertIndexRange(con_it->second, constraint_pool_row_index.size() - 1);
            }
        }
    ...
    void
    DoFInfo::compute_cell_index_compression(const std::vector<unsigned char> &irregular_cells)
    {
      const bool         have_hp      = dofs_per_cell.size() > 1;
      const unsigned int n_components = start_components.back();

      Assert(vectorization_length == 1 || row_starts.size() % vectorization_length == 1,
             ExcInternalError());
      ...
      for (unsigned int i = 0; i < irregular_cells.size(); ++i)
        {
          ...
          if (has_constraints)
          ...
          else
            {
              ...
              for (unsigned int j = 0; j < n_comp; ++j)
                {
                  ...
                  const unsigned int *dof_indices =
                    this->dof_indices.data() + row_starts[cell_no * n_components].first;
              ...
              if (indices_are_contiguous || indices_are_interleaved_and_contiguous)
                {
                  for (unsigned int j = 0; j < n_comp; ++j)
                    dof_indices_contiguous[dof_access_cell][i * vectorization_length + j] =
                      this->dof_indices[row_starts[(i * vectorization_length + j) * n_components]
                                          .first];
                }
    ...
    template <int length>
    void
    DoFInfo::compute_face_index_compression(const std::vector<FaceToCellTopology<length>> &faces)
    {
      AssertDimension(length, vectorization_length);

      index_storage_variants[dof_access_face_interior].resize(faces.size(),
                                                              IndexStorageVariants::full);
      dof_indices_contiguous[dof_access_face_interior].resize(faces.size() * length,
                                                              numbers::invalid_unsigned_int);
      ...
      for (; n_exterior_faces < faces.size(); ++n_exterior_faces)
        if (faces[n_exterior_faces].cells_exterior[0] == numbers::invalid_unsigned_int)
          break;
      index_storage_variants[dof_access_face_exterior].resize(n_exterior_faces,
                                                              IndexStorageVariants::full);
      dof_indices_contiguous[dof_access_face_exterior].resize(n_exterior_faces * length,
                                                              numbers::invalid_unsigned_int);
      ...
      const std::vector<FaceToCellTopology<length>> &faces)
    {
      ...
      const unsigned int n_dofs =
        vector_partitioner->local_size() + vector_partitioner->n_ghost_indices();
             ...
             ++chunk)
          {
            for (unsigned int cell = task_info.cell_partition_data[chunk];
                 ...
                 ++cell)
              {
                for (unsigned int it = row_starts[cell * vectorization_length * n_components].first;
                     it != row_starts[(cell + 1) * vectorization_length * n_components].first;
                     ++it)
                  {
            ...
            if (faces.size() > 0)
              for (unsigned int face = task_info.face_partition_data[chunk];
                   ...
                   ++face)
                for (unsigned int v = 0;
                     v < length && faces[face].cells_exterior[v] != numbers::invalid_unsigned_int;
                     ++v)
                  {
      ...
      vector_zero_range_list_index.resize(
        1 + task_info.partition_row_index[task_info.partition_row_index.size() - 2],
        numbers::invalid_unsigned_int);
    ...
    namespace
    {
      ...
      public:
        ...
        void
        insert(const unsigned int entry, std::vector<types::global_dof_index>::iterator &dat)
        {
        ...
        for (unsigned int block = begin; block < end; ++block)
          {
            ...
            const unsigned int *it = dof_info.dof_indices.data() +
    ...
    void
    DoFInfo::make_connectivity_graph(const TaskInfo &                 task_info,
                                     const std::vector<unsigned int> &renumbering,
                                     DynamicSparsityPattern &         connectivity) const
    {
      ...
      SparsityPattern connectivity_dof(n_rows, task_info.n_active_cells, row_lengths);
      parallel::apply_to_subranges(0,
                                   task_info.n_active_cells,
    ...
    void
    DoFInfo ::compute_dof_renumbering(std::vector<types::global_dof_index> &renumbering)
    {
              ...
              row_starts[(cell_no + 1) * n_components * vectorization_length].second)
            {
              const unsigned int ndofs =
                dofs_per_cell.size() == 1 ?
                  dofs_per_cell[0] :
                  (dofs_per_cell[cell_active_fe_index.size() > 0 ? cell_active_fe_index[cell_no] :
                                                                   0]);
              const unsigned int *dof_ind =
                dof_indices.data() +
                row_starts[cell_no * n_components * vectorization_length].first;
              for (unsigned int i = 0; i < ndofs; ++i)
                for (unsigned int j = 0; j < n_vectorization_lanes_filled[dof_access_cell][cell_no];
                     ++j)
    ...
    void
    DoFInfo::print_memory_consumption(StreamType &out, const TaskInfo &task_info) const
    {
      out << "       Memory row starts indices:    ";
      task_info.print_memory_statistics(out, (row_starts.capacity() * sizeof(*row_starts.begin())));
      out << "       Memory dof indices:           ";
      task_info.print_memory_statistics(out, MemoryConsumption::memory_consumption(dof_indices));
      ...
      task_info.print_memory_statistics(out,
                                        MemoryConsumption::memory_consumption(*vector_partitioner));
    }
