...
namespace Portable
{
  namespace internal
  {
    ...
    namespace batched
    {
       ...
       *
       * TypeMatrix, TypeIn and TypeOut can be any 1-dimensional array type
       * compatible with execution on device, including Kokkos::View and raw
       * pointer arrays.
       */
      template <int  n_rows,
                int  n_columns,
      ...
      apply_matrix_vector_product(const TypeMatrix matrix,
                                  const TypeIn     in,
                                  TypeOut          out)
      {
        ...
        // Deduce the type of the underlying TypeOut array type. This is
        // needed for caching below
        using Number =
          std::remove_cv_t<std::remove_reference_t<decltype(out[0])>>;
                                  ...
                                  const int        stride_out)
      {
        ...
        // Deduce the type of the underlying ViewTypeOut array type. This is
        // needed for caching below
        using Number =
          std::remove_cv_t<std::remove_reference_t<decltype(out[0])>>;
      ...
      public:
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        value(const ViewTypeIn in, ViewTypeOut out) const;
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        derivative(const ViewTypeIn in, ViewTypeOut out) const;
        ...
        template <int  direction,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        co_derivative(const ViewTypeIn in, ViewTypeOut out) const;
         ...
         * direction-by-direction co_derivative() calls into a single pass
         ...
         * components (the forward, evaluate(UpdateFlags::gradients) case);
         * transpose = true is the adjoint. i.e.,  it reduces `dim` gradient
         * components back into one scalar field (the backward,
         * integrate(UpdateFlags::gradients), case). This is a faster
         * alternative to dim consecutive exuction of the above co_derivative()
         * function.
         */
        template <bool transpose,
                  ...
                  typename ViewTypeOut>
        DEAL_II_HOST_DEVICE void
        co_gradient(const ViewTypeIn in, ViewTypeOut out) const;
        ...
        co_gradient(const ViewTypeIn in, ViewTypeOut out) const
      {
        ...
        Kokkos::parallel_for(
          ...
          [&](const int tid) {
            ...
            // The main two-nested loop below follows the sum-factorization
            // layout of the last (dim-1) direction. We cache in the register
            // what varies only in the inner-most loop over 'n', but stays
            // constant in the outer loop over 'last', namely co_shape_gradients
            // for directions 0,1,..., dim-2, and the in values in the last
            // (dim-1) direction.
            ...
            for (int d = 0; d < dim - 1; ++d)
              {
                // cache stride and index to access the quad points in 1D view
                // layout
                stride_d[d] = Utilities::pow(n_columns, d);
                idx_d[d]    = (reminder / stride_d[d]) % n_columns;

                // cache co_shape_gradients in 0,1,..., dim-2 directions
            ...
            // cache the in vector values in the last (dim-1) direction
            for (int n = 0; n < n_columns; ++n)
              {
                if constexpr (!transpose)
            ...
            // Main  loop following the sum factotization layout of the last
            // (dim-1) direction
                    ...
                    for (int d = 0; d < dim - 1; ++d)
                      {
                        ...
                        // compute the sum factorization in d<dim-1 direction
                        // with co_shape_gradients cached
                        Number res_d = 0;
                        for (int n = 0; n < n_columns; ++n)
