...
namespace dealii::MatrixFreeUtils
{
   ...
   namespace
   {
      ...
      using vertex_index_map   = boost::property_map<Graph, boost::vertex_index_t>::const_type;
   ...
   struct GraphCache
   {
       ...
       * Pattern needs to provide: n_rows(), n_cols(), begin(r), end(r), iterator->row(), iterator->colum().
       */
      template <typename Pattern>
      void make_graph(const Pattern &pattern)
      {
         std::set<Edge> edges;
       ...
       * By default, we do not reorder them, as the benefit does not seem to be huge.
       *
       ...
       */
      template <typename Order = boost::typed_identity_property_map<vertices_size_type>>
      void make_coloring(const Order &&order = boost::typed_identity_property_map<vertices_size_type>{})
      {
         color_vec.resize(boost::num_vertices(graph));
         coloring   = boost::make_iterator_property_map(&color_vec.front(), get(boost::vertex_index, graph));
         ...
         for (vertices_size_type color = 0; color < num_colors; color++)
         {
            ...
            for (vertices_size_type i = 0; i < color_vec.size(); i++)
               if (color_vec[i] == color) entries_per_color[color].push_back(i);
         }
      }
   };

   /**
    * Assemble a given operator into a matrix, using the provided sparsity pattern.
    *
    * @param <tt>output</tt> resulting matrix. Needs to provide a function set(row, column, value) that allows to write to each entry of the sparsity
    pattern.
    ...
    * Note that <tt>pattern</tt> can be a subset of the sparsity pattern from <tt>output</tt>

    * @param <tt>dummy_domain</tt> a vector used to initialize vectors in the domain of <tt>op</tt> by copying the structure of <tt>dummy_domain</tt>.
    * The type <tt>DomainVector</tt> needs to provide reinit(const DomainVector&, bool=false) to create a zero vector.
    ...
    * This allows to reuse the graph in case the sparsity pattern does not change.
    *
    * @return returns <tt>cache</tt>.
    */
   template <typename Matrix, typename Operator, typename Pattern, typename DomainVector, typename RangeVector>
   std::shared_ptr<GraphCache> assemble_operator(Matrix &                          output,
                                                 const Operator &                  op,
   ...
   {
      // Coloring: for each edge (v1,v2) the colors of the vertices is different, i.e. c(v1) != c(v2).
      // To obtain the matrix from the operator, we need to test (multiply) with one vector e per color.
      // For each color c, e[i] = 1 if c(v[i]) == c else 0 (entries = {i : v[i] == c}).
      // This gives us a vector col = sum_i A e_i = sum_i column_i(A).
      // We need to decompose col into its contributions from the different columns.
      // This is possible, as all the columns listed in entries have no non-zero row in common.
      for (vertices_size_type color = 0; color < cache->num_colors; color++)
      {
         ...
         for (vertices_size_type i = 0; i < pattern.n_cols(); i++)
            if (cache->color_vec[i] == color) entries.push_back(i);
         ...
         // Split the result into its contributions from the single unit vectors, i.e. A*e_i.
         // This is possible, since for each row, only one column in entry can store a value.
            ...
            if (image[r] != 0.0)
            {
               ...
               for (auto c : entries)
               {
                  if (pattern.exists(r, c))
                  {
                     ...
                     for (auto j : entries)
                        Assert(c == j || !pattern.exists(r, j), dealii::ExcInternalError());
               ...
               // Make sure that each non-zero entry found its way into the matrix.
               Assert(found, dealii::ExcMessage("The provided pattern does not include all required entries."));
            }
         }
      }
    ...
    * @return returns a cache object storing intermediate results.
    * This allows to reuse the graph in case the sparsity pattern does not change.
    */
   template <typename Matrix, typename Operator, typename Pattern, typename DomainVector, typename RangeVector>
   std::shared_ptr<GraphCache>
   assemble_operator(Matrix &output, const Operator &op, const Pattern &pattern, const DomainVector &dummy_domain, const RangeVector &dummy_range)
   {
      auto cache = std::make_shared<GraphCache>();

      // Early exit for empty pattern.
      if (pattern.n_nonzero_elements() == 0) return cache;
      ...
      AssertDimension(cache->color_vec.size(), pattern.n_cols());

      return assemble_operator(output, op, pattern, dummy_domain, dummy_range, cache);
   }

   /**
    * Variant of assemble_operator using dealii::Vector as domain and range vectors.
    */
   template <typename Matrix, typename Operator, typename Pattern>
   std::shared_ptr<GraphCache> assemble_operator(Matrix &output, const Operator &op, const Pattern &pattern, const std::shared_ptr<GraphCache> cache)
   {
    ...
    */
   template <typename Matrix, typename Operator, typename Pattern>
   std::shared_ptr<GraphCache> assemble_operator(Matrix &output, const Operator &op, const Pattern &pattern)
   {
      dealii::Vector<typename Matrix::value_type> domain(pattern.n_cols());
      dealii::Vector<typename Matrix::value_type> range(pattern.n_rows());
