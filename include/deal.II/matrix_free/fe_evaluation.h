...
public:
  ...
  template <typename VectorType>
  void
  read_dof_values_plain(const VectorType &src, const unsigned int first_index = 0);
  ...
  template <typename VectorType>
  void
  distribute_local_to_global(VectorType &dst, const unsigned int first_index = 0) const;
   ...
   */
  void
  submit_divergence(const VectorizedArray<Number> div_in, const unsigned int q_point);
   ...
   */
  void
  submit_symmetric_gradient(const SymmetricTensor<2, dim, VectorizedArray<Number>> grad_in,
   ...
   */
  void
  submit_curl(const Tensor<1, dim == 2 ? 1 : dim, VectorizedArray<Number>> curl_in,
              const unsigned int                                           q_point);
...
protected:
  ...
  template <typename VectorType, typename VectorOperation>
  void
  read_write_operation_contiguous(const VectorOperation &operation, VectorType *vectors[]) const;
  ...
  template <typename VectorType, typename VectorOperation>
  void
  read_write_operation_global(const VectorOperation &operation, VectorType *vectors[]) const;
   ...
   * reference to it.
   */
  const internal::MatrixFreeFunctions::MappingInfoStorage<(is_face ? dim - 1 : dim), dim, Number>
    *mapping_data;
...
public:
   ...
   */
  void
  submit_value(const Tensor<1, 1, VectorizedArray<Number>> val_in, const unsigned int q_point);
...
public:
   ...
   */
  void
  submit_normal_derivative(const gradient_type grad_in, const unsigned int q_point);
...
private:
   ...
   */
  void
  check_template_arguments(const unsigned int fe_no, const unsigned int first_selected_component);
};
...
protected:
   ...
   */
  void
  adjust_for_face_orientation(const bool integrate, const bool values, const bool gradients);
};
...
inline FEEvaluationBase<dim, n_components_, Number, is_face>::FEEvaluationBase(
  ...
  active_fe_index(
    fe_degree != numbers::invalid_unsigned_int ?
      data_in.get_dof_info(dof_no).fe_index_from_degree(first_selected_component, fe_degree) :
      0),
  active_quad_index(
    fe_degree != numbers::invalid_unsigned_int ?
      (is_face ?
         data_in.get_mapping_info().face_data[quad_no_in].quad_index_from_n_q_points(n_q_points) :
         data_in.get_mapping_info().cell_data[quad_no_in].quad_index_from_n_q_points(n_q_points)) :
      0),
  n_quadrature_points(
    fe_degree != numbers::invalid_unsigned_int ?
      n_q_points :
      (is_face ? data_in.get_shape_info(dof_no, quad_no_in, active_fe_index, active_quad_index)
                   .n_q_points_face :
                 data_in.get_shape_info(dof_no, quad_no_in, active_fe_index, active_quad_index)
                   .n_q_points)),
  matrix_info(&data_in),
  dof_info(&data_in.get_dof_info(dof_no)),
  mapping_data(internal::MatrixFreeFunctions::MappingInfoCellsOrFaces<dim, Number, is_face>::get(
    data_in.get_mapping_info(),
    quad_no)),
  data(&data_in.get_shape_info(dof_no,
                               quad_no_in,
  ...
  dof_access_index(is_face ? (is_interior_face ?
  ...
  first_selected_component(first_selected_component)
{
  ...
  AssertDimension((is_face ? data->n_q_points_face : data->n_q_points), n_quadrature_points);
  AssertDimension(n_quadrature_points, mapping_data->descriptor[active_quad_index].n_q_points);
  Assert(
    dof_info->start_components.back() == 1 ||
      (int)n_components_ <=
        (int)dof_info
            ->start_components[dof_info->component_to_base_index[first_selected_component] + 1] -
    ...
    ExcMessage(
      ...
      Utilities::to_string(
        first_selected_component -
        dof_info->start_components[dof_info->component_to_base_index[first_selected_component]]) +
  ...
  first_selected_component(first_selected_component)
{
  ...
  if (other != nullptr && other->mapped_geometry->get_quadrature() == quadrature)
...
inline FEEvaluationBase<dim, n_components_, Number, is_face>::FEEvaluationBase(
  ...
  scratch_data_array(other.matrix_info == nullptr ? new AlignedVector<VectorizedArray<Number>>() :
  ...
  normal_vectors(nullptr),
  normal_x_jacobian(nullptr),
  quadrature_weights(other.matrix_info == nullptr ?
                       nullptr :
  ...
  first_selected_component(other.first_selected_component)
{
  ...
  if (other.mapped_geometry.get() != nullptr)
    {
      mapped_geometry.reset(new internal::MatrixFreeFunctions::MappingDataOnTheFly<dim, Number>(
...
FEEvaluationBase<dim, n_components_, Number, is_face>::set_data_pointers()
{
  ...
  const unsigned int shift =
    std::max(tensor_dofs_per_component + 1, dofs_per_component) * n_components_ * 3 +
  ...
  for (unsigned int c = 0; c < n_components_; ++c)
    {
      ...
      this->values_quad[c] =
        scratch_data_array->begin() + n_components * dofs_per_component + c * n_quadrature_points;
      for (unsigned int d = 0; d < dim; ++d)
...
template <int dim, int n_components_, typename Number, bool is_face>
inline unsigned int
FEEvaluationBase<dim, n_components_, Number, is_face>::get_cell_data_number() const
{
...
template <int dim, int n_components_, typename Number, bool is_face>
inline unsigned int
FEEvaluationBase<dim, n_components_, Number, is_face>::get_mapping_data_index_offset() const
{
  if (matrix_info == 0)
    return 0;
  else
  ...
  if (is_face)
    {
      ...
      const unsigned int *    cells = is_interior_face ?
...
namespace internal
{
  ...
  inline Number &
  vector_access(LinearAlgebra::distributed::Vector<Number> &vec, const unsigned int entry)
  {
  ...
  template <typename VectorType>
  inline void
  check_vector_compatibility(const VectorType &                            vec,
                             const internal::MatrixFreeFunctions::DoFInfo &dof_info)
  {
    (void)vec;
    (void)dof_info;
  ...
  template <typename Number>
  inline void
  check_vector_compatibility(const LinearAlgebra::distributed::Vector<Number> &vec,
                             const internal::MatrixFreeFunctions::DoFInfo &    dof_info)
  {
    (void)vec;
    (void)dof_info;
  ...
  struct VectorReader
  {
    ...
    {
      dealii::vectorized_load_and_transpose(dofs_per_cell, vec.begin(), dof_indices, dof_values);
    }
    ...
    template <typename VectorType>
    void
    process_dof_global(const types::global_dof_index index, VectorType &vec, Number &res) const
    {
  ...
  struct VectorDistributorLocalToGlobal
  {
    ...
    process_dofs_vectorized_transpose(const unsigned int       dofs_per_cell,
                                      ...
                                      std::integral_constant<bool, true>) const
    {
      vectorized_transpose_and_store(true, dofs_per_cell, dof_values, dof_indices, vec.begin());
    }
...
#  endif
  ...
  struct VectorSetter
  {
    ...
    process_dofs_vectorized_transpose(const unsigned int       dofs_per_cell,
                                      ...
                                      std::integral_constant<bool, true>) const
    {
      vectorized_transpose_and_store(false, dofs_per_cell, dof_values, dof_indices, vec.begin());
    }
    ...
    template <typename VectorType>
    void
    process_constraint(const unsigned int, const Number, VectorType &, Number &) const
    {}
  ...
  struct BlockVectorSelector<std::vector<VectorType>, false>
  {
    ...
    static BaseVectorType *
    get_vector_component(std::vector<VectorType> &vec, const unsigned int component)
    {
  ...
  const bool             apply_constraints) const
{
  ...
  AssertIndexRange(cell, dof_info->index_storage_variants[dof_access_index].size());
  if (dof_info
        ->index_storage_variants[is_face ? dof_access_index :
                                           internal::MatrixFreeFunctions::DoFInfo::dof_access_cell]
                                [cell] >=
  ...
  if (is_face)
    {
      ...
      cells = dof_access_index == internal::MatrixFreeFunctions::DoFInfo::dof_access_cell ?
                &cells_copied[0] :
                (is_interior_face ? &this->matrix_info->get_face_info(cell).cells_interior[0] :
                                    &this->matrix_info->get_face_info(cell).cells_exterior[0]);
      for (unsigned int v = 0; v < n_vectorization_actual; ++v)
        {
          ...
          has_constraints =
            has_constraints &&
            dof_info
                ->row_starts[cells[v] * n_fe_components + first_selected_component + n_components]
                .second !=
      ...
      if (n_components == 1 || n_fe_components == 1)
        {
          for (unsigned int v = 0; v < n_vectorization_actual; ++v)
            for (unsigned int i = 0; i < dofs_per_component; ++i)
              for (unsigned int comp = 0; comp < n_components; ++comp)
                operation.process_dof(dof_indices[v][i], *src[comp], values_dofs[comp][i][v]);
        }
      else
        {
          for (unsigned int comp = 0; comp < n_components; ++comp)
            for (unsigned int v = 0; v < n_vectorization_actual; ++v)
              for (unsigned int i = 0; i < dofs_per_component; ++i)
                operation.process_dof(
                  dof_indices[v][comp * dofs_per_component + i], *src[0], values_dofs[comp][i][v]);
        }
      return;
    }
      ...
      if (is_face)
        {
          ...
          next_index_indicators =
            dof_info->row_starts[cells[v] * n_fe_components + first_selected_component + 1].second;
        }
      else
        {
      ...
      if (n_components == 1 || n_fe_components == 1)
        {
          ...
          for (; index_indicators != next_index_indicators; ++index_indicators)
            {
              ...
              for (unsigned int j = 0; j < indicator.first; ++j)
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  operation.process_dof(
                    dof_indices[v][j], *src[comp], values_dofs[comp][ind_local + j][v]);
              ...
              for (unsigned int comp = 0; comp < n_components; ++comp)
                operation.pre_constraints(values_dofs[comp][ind_local][v], value[comp]);

              const Number *data_val = matrix_info->constraint_pool_begin(indicator.second);
              const Number *end_pool = matrix_info->constraint_pool_end(indicator.second);
              for (; data_val != end_pool; ++data_val, ++dof_indices[v])
                for (unsigned int comp = 0; comp < n_components; ++comp)
                  operation.process_constraint(*dof_indices[v], *data_val, *src[comp], value[comp]);

              for (unsigned int comp = 0; comp < n_components; ++comp)
                operation.post_constraints(value[comp], values_dofs[comp][ind_local][v]);
              ind_local++;
            }
          ...
          for (; ind_local < dofs_per_component; ++dof_indices[v], ++ind_local)
            for (unsigned int comp = 0; comp < n_components; ++comp)
              operation.process_dof(*dof_indices[v], *src[comp], values_dofs[comp][ind_local][v]);
        }
      else
        {
              ...
              for (; index_indicators != next_index_indicators; ++index_indicators)
                {
                  ...
                  for (unsigned int j = 0; j < indicator.first; ++j)
                    operation.process_dof(
                      dof_indices[v][j], *src[0], values_dofs[comp][ind_local + j][v]);
                  ind_local += indicator.first;
                  dof_indices[v] += indicator.first;
                  ...
                  for (; data_val != end_pool; ++data_val, ++dof_indices[v])
                    operation.process_constraint(*dof_indices[v], *data_val, *src[0], value);
              ...
              for (; ind_local < dofs_per_component; ++dof_indices[v], ++ind_local)
                {
                  ...
                  operation.process_dof(*dof_indices[v], *src[0], values_dofs[comp][ind_local][v]);
                }
...
template <typename VectorType, typename VectorOperation>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face>::read_write_operation_global(
  const VectorOperation &operation,
...
template <typename VectorType, typename VectorOperation>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face>::read_write_operation_contiguous(
  const VectorOperation &operation,
  VectorType *           src[]) const
{
  ...
  if (vectorization_populated == VectorizedArray<Number>::n_array_elements)
    {
      if (n_components == 1 || n_fe_components == 1)
        ...
        operation.process_dofs_vectorized_transpose(data->dofs_per_component_on_cell * n_components,
                                                    dof_indices,
                                                    *src[0],
                                                    &values_dofs[0][0],
                                                    vector_selector);
  ...
  const unsigned int first_index)
{
  ...
  for (unsigned int d = 0; d < n_components; ++d)
    src_data[d] = internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::
...
template <typename VectorType>
inline void
FEEvaluationBase<dim, n_components_, Number, is_face>::distribute_local_to_global(
  VectorType &       dst,
  const unsigned int first_index) const
{
  Assert(dof_values_initialized == true, internal::ExcAccessToUninitializedField());
  ...
  for (unsigned int d = 0; d < n_components; ++d)
    dst_data[d] = internal::BlockVectorSelector<VectorType, IsBlockVector<VectorType>::value>::
      get_vector_component(dst, d + first_index);
...
FEEvaluationBase<dim, n_components, Number, is_face>::get_internal_dof_numbering() const
{
...
{
  Assert(gradients_quad_initialized || gradients_quad_submitted, ExcNotInitialized());
...
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, VectorizedArray<Number>>
                             FEEvaluationBase<dim, n_components_, Number, is_face>::get_dof_value(const unsigned int dof) const
{
...
template <int dim, int n_components_, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, VectorizedArray<Number>>
                             FEEvaluationBase<dim, n_components_, Number, is_face>::get_value(const unsigned int q_point) const
{
  Assert(this->values_quad_initialized == true, internal::ExcAccessToUninitializedField());
...
template <int dim, int n_components_, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE Tensor<1, n_components_, Tensor<1, dim, VectorizedArray<Number>>>
                             FEEvaluationBase<dim, n_components_, Number, is_face>::get_gradient(
  const unsigned int q_point) const
{
  Assert(this->gradients_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int comp = 0; comp < n_components; comp++)
        for (unsigned int d = 0; d < dim; ++d)
          grad_out[comp][d] = (this->gradients_quad[comp][d][q_point] * jacobian[0][d][d]);
    }
  // cell with general/affine Jacobian
  else
    {
      const Tensor<2, dim, VectorizedArray<Number>> &jac =
        jacobian[this->cell_type > internal::MatrixFreeFunctions::affine ? q_point : 0];
      for (unsigned int comp = 0; comp < n_components; comp++)
        for (unsigned int d = 0; d < dim; ++d)
          {
            grad_out[comp][d] = jac[d][0] * this->gradients_quad[comp][0][q_point];
            for (unsigned int e = 1; e < dim; ++e)
              grad_out[comp][d] += jac[d][e] * this->gradients_quad[comp][e][q_point];
          }
    }
  return grad_out;
}
  ...
  const unsigned int q_point) const
{
  ...
  if (this->cell_type == internal::MatrixFreeFunctions::cartesian)
    for (unsigned int comp = 0; comp < n_components; comp++)
      grad_out[comp] =
        this->gradients_quad[comp][dim - 1][q_point] * (this->normal_x_jacobian[0][dim - 1]);
  else
    {
      ...
      for (unsigned int comp = 0; comp < n_components; comp++)
        {
          grad_out[comp] =
            this->gradients_quad[comp][0][q_point] * this->normal_x_jacobian[index][0];
          for (unsigned int d = 1; d < dim; ++d)
            grad_out[comp] +=
              this->gradients_quad[comp][d][q_point] * this->normal_x_jacobian[index][d];
        }
    }
  return grad_out;
}
...
namespace internal
{
                         ...
                         VectorizedArray<Number> (&tmp)[2][2])
  {
    for (unsigned int d = 0; d < 2; ++d)
      {
        tmp[0][d] = (jac[d][0] * hessians_quad[0][q_point] + jac[d][1] * hessians_quad[2][q_point]);
        tmp[1][d] = (jac[d][0] * hessians_quad[2][q_point] + jac[d][1] * hessians_quad[1][q_point]);
      }
  }
                         ...
                         VectorizedArray<Number> (&tmp)[3][3])
  {
    for (unsigned int d = 0; d < 3; ++d)
      {
        tmp[0][d] = (jac[d][0] * hessians_quad[0][q_point] + jac[d][1] * hessians_quad[3][q_point] +
                     jac[d][2] * hessians_quad[4][q_point]);
        tmp[1][d] = (jac[d][0] * hessians_quad[3][q_point] + jac[d][1] * hessians_quad[1][q_point] +
                     jac[d][2] * hessians_quad[5][q_point]);
        tmp[2][d] = (jac[d][0] * hessians_quad[4][q_point] + jac[d][1] * hessians_quad[5][q_point] +
...
FEEvaluationBase<dim, n_components_, Number, is_face>::get_hessian(const unsigned int q_point) const
{
  Assert(!is_face, ExcNotImplemented());
  Assert(this->hessians_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
            ...
            switch (dim)
              {
                ...
                case 2:
                  hessian_out[comp][0][1] =
                    (this->hessians_quad[comp][2][q_point] * jac[0][0] * jac[1][1]);
                  break;
                case 3:
                  hessian_out[comp][0][1] =
                    (this->hessians_quad[comp][3][q_point] * jac[0][0] * jac[1][1]);
                  hessian_out[comp][0][2] =
                    (this->hessians_quad[comp][4][q_point] * jac[0][0] * jac[2][2]);
                  hessian_out[comp][1][2] =
                    (this->hessians_quad[comp][5][q_point] * jac[1][1] * jac[2][2]);
                  break;
                default:
      ...
      for (unsigned int comp = 0; comp < n_components; comp++)
        {
          ...
          internal::hessian_unit_times_jac(jac, this->hessians_quad[comp], q_point, tmp);
  ...
  // cell with general Jacobian
  else
    {
      const Tensor<1, dim *(dim + 1) / 2, Tensor<1, dim, VectorizedArray<Number>>> &jac_grad =
        mapping_data
          ->jacobian_gradients[1 - this->is_interior_face]
      ...
      for (unsigned int comp = 0; comp < n_components; comp++)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int e = 0; e < dim; ++e)
              hessian_out[comp][d][d] += (jac_grad[d][e] * this->gradients_quad[comp][e][q_point]);
  ...
  const unsigned int q_point) const
{
  ...
  if (this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int comp = 0; comp < n_components; comp++)
        for (unsigned int d = 0; d < dim; ++d)
          hessian_out[comp][d] = (this->hessians_quad[comp][d][q_point] * jac[d][d] * jac[d][d]);
    }
  // cell with general Jacobian, but constant within the cell
  else if (this->cell_type == internal::MatrixFreeFunctions::affine)
    {
  ...
  // cell with general Jacobian
  else
    {
      const Tensor<1, dim *(dim + 1) / 2, Tensor<1, dim, VectorizedArray<Number>>> &jac_grad =
        mapping_data
          ->jacobian_gradients[0][this->mapping_data->data_index_offsets[this->cell] + q_point];
      for (unsigned int comp = 0; comp < n_components; comp++)
        {
          // compute laplacian before the gradient because it needs to access
          // unscaled gradient data
          ...
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int e = 0; e < dim; ++e)
              hessian_out[comp][d] += (jac_grad[d][e] * this->gradients_quad[comp][e][q_point]);
        }
    }
  return hessian_out;
}
...
FEEvaluationBase<dim, n_components_, Number, is_face>::submit_gradient(
  const Tensor<1, n_components_, Tensor<1, dim, VectorizedArray<Number>>> grad_in,
  const unsigned int                                                      q_point)
{
#  ifdef DEBUG
  Assert(this->cell != numbers::invalid_unsigned_int, ExcNotInitialized());
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      ...
      for (unsigned int comp = 0; comp < n_components; comp++)
        for (unsigned int d = 0; d < dim; ++d)
          this->gradients_quad[comp][d][q_point] = (grad_in[comp][d] * jacobian[0][d][d] * JxW);
    }
  else
    {
      const Tensor<2, dim, VectorizedArray<Number>> &jac =
        this->cell_type > internal::MatrixFreeFunctions::affine ? jacobian[q_point] : jacobian[0];
      const VectorizedArray<Number> JxW = this->cell_type > internal::MatrixFreeFunctions::affine ?
                                            J_value[q_point] :
    ...
    for (unsigned int comp = 0; comp < n_components; comp++)
      {
        ...
        this->gradients_quad[comp][dim - 1][q_point] =
          grad_in[comp] * (this->normal_x_jacobian[0][dim - 1] * this->J_value[0] *
      ...
      for (unsigned int comp = 0; comp < n_components; comp++)
        {
          ...
          for (unsigned int d = 0; d < dim; ++d)
            this->gradients_quad[comp][d][q_point] = factor * this->normal_x_jacobian[index][d];
        }
    }
}
...
template <int dim, int n_components_, typename Number, bool is_face>
inline FEEvaluationAccess<dim, n_components_, Number, is_face>::FEEvaluationAccess(
  const MatrixFree<dim, Number> &data_in,
  const unsigned int             dof_no,
  const unsigned int             first_selected_component,
  const unsigned int             quad_no_in,
  ...
  const unsigned int             n_q_points,
  const bool                     is_interior_face) :
  FEEvaluationBase<dim, n_components_, Number, is_face>(data_in,
                                                        dof_no,
                                                        first_selected_component,
                                                        quad_no_in,
                                                        fe_degree,
...
inline FEEvaluationAccess<dim, n_components_, Number, is_face>::FEEvaluationAccess(
  ...
  const unsigned int                                                first_selected_component,
  const FEEvaluationBase<dim, n_components_other, Number, is_face> *other) :
  FEEvaluationBase<dim, n_components_, Number, is_face>(mapping,
                                                        fe,
                                                        quadrature,
                                                        update_flags,
                                                        first_selected_component,
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<dim, 1, Number, is_face>::get_dof_value(const unsigned int dof) const
{
  AssertIndexRange(dof, this->data->dofs_per_component_on_cell);
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<dim, 1, Number, is_face>::get_value(const unsigned int q_point) const
{
  Assert(this->values_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<dim, 1, Number, is_face>::get_normal_derivative(const unsigned int q_point) const
{
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE Tensor<1, dim, VectorizedArray<Number>>
                             FEEvaluationAccess<dim, 1, Number, is_face>::get_gradient(const unsigned int q_point) const
{
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      for (unsigned int d = 0; d < dim; ++d)
        grad_out[d] = (this->gradients_quad[0][d][q_point] * this->jacobian[0][d][d]);
    }
  // cell with general/affine Jacobian
  else
    {
      const Tensor<2, dim, VectorizedArray<Number>> &jac =
        this->jacobian[this->cell_type > internal::MatrixFreeFunctions::affine ? q_point : 0];
      for (unsigned int d = 0; d < dim; ++d)
        {
...
template <int dim, typename Number, bool is_face>
inline Tensor<2, dim, VectorizedArray<Number>>
FEEvaluationAccess<dim, 1, Number, is_face>::get_hessian(const unsigned int q_point) const
{
...
FEEvaluationAccess<dim, 1, Number, is_face>::get_hessian_diagonal(const unsigned int q_point) const
{
...
template <int dim, typename Number, bool is_face>
inline VectorizedArray<Number>
FEEvaluationAccess<dim, 1, Number, is_face>::get_laplacian(const unsigned int q_point) const
{
...
inline void DEAL_II_ALWAYS_INLINE
            FEEvaluationAccess<dim, 1, Number, is_face>::submit_dof_value(const VectorizedArray<Number> val_in,
                                                              const unsigned int            dof)
{
#  ifdef DEBUG
  this->dof_values_initialized = true;
...
template <int dim, typename Number, bool is_face>
inline void DEAL_II_ALWAYS_INLINE
            FEEvaluationAccess<dim, 1, Number, is_face>::submit_value(const VectorizedArray<Number> val_in,
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      const VectorizedArray<Number> JxW = this->J_value[0] * this->quadrature_weights[q_index];
      for (unsigned int d = 0; d < dim; ++d)
        this->gradients_quad[0][d][q_index] = (grad_in[d] * this->jacobian[0][d][d] * JxW);
    }
  // general/affine cell type
  else
    {
      const Tensor<2, dim, VectorizedArray<Number>> &jac =
        this->cell_type > internal::MatrixFreeFunctions::affine ? this->jacobian[q_index] :
                                                                  this->jacobian[0];
      const VectorizedArray<Number> JxW = this->cell_type > internal::MatrixFreeFunctions::affine ?
...
inline DEAL_II_ALWAYS_INLINE Tensor<2, dim, VectorizedArray<Number>>
                             FEEvaluationAccess<dim, dim, Number, is_face>::get_gradient(const unsigned int q_point) const
{
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<dim, dim, Number, is_face>::get_divergence(const unsigned int q_point) const
{
  Assert(this->gradients_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
  Assert(this->jacobian != nullptr, ExcNotInitialized());
  ...
  if (!is_face && this->cell_type == internal::MatrixFreeFunctions::cartesian)
    {
      divergence = (this->gradients_quad[0][0][q_point] * this->jacobian[0][0][0]);
      for (unsigned int d = 1; d < dim; ++d)
        divergence += (this->gradients_quad[d][d][q_point] * this->jacobian[0][d][d]);
    }
  // cell with general/constant Jacobian
  else
    {
      const Tensor<2, dim, VectorizedArray<Number>> &jac =
        this->cell_type == internal::MatrixFreeFunctions::general ? this->jacobian[q_point] :
                                                                    this->jacobian[0];
...
template <int dim, typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE Tensor<1, (dim == 2 ? 1 : dim), VectorizedArray<Number>>
                             FEEvaluationAccess<dim, dim, Number, is_face>::get_curl(const unsigned int q_point) const
{
  // copy from generic function into dim-specialization function
  const Tensor<2, dim, VectorizedArray<Number>>            grad = get_gradient(q_point);
...
inline DEAL_II_ALWAYS_INLINE Tensor<3, dim, VectorizedArray<Number>>
                             FEEvaluationAccess<dim, dim, Number, is_face>::get_hessian(const unsigned int q_point) const
{
  Assert(this->hessians_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
        ...
        for (unsigned int d = e + 1; d < dim; ++d, ++counter)
          {
            ...
            this->gradients_quad[e][d][q_point] = (value * this->jacobian[0][d][d]);
            this->gradients_quad[d][e][q_point] = (value * this->jacobian[0][e][e]);
          }
    }
  // general/affine cell type
  else
  ...
  const unsigned int                                           q_point)
{
  ...
  switch (dim)
    {
      case 1:
        Assert(false, ExcMessage("Testing by the curl in 1d is not a useful operation"));
        break;
      case 2:
        grad[1][0] = curl[0];
        grad[0][1] = -curl[0];
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_dof_value(const unsigned int dof) const
{
  AssertIndexRange(dof, this->data->dofs_per_component_on_cell);
  return this->values_dofs[0][dof];
}
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_value(const unsigned int q_point) const
{
  Assert(this->values_quad_initialized == true, internal::ExcAccessToUninitializedField());
  AssertIndexRange(q_point, this->n_quadrature_points);
  return this->values_quad[0][q_point];
...
inline DEAL_II_ALWAYS_INLINE Tensor<1, 1, VectorizedArray<Number>>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_gradient(const unsigned int q_point) const
{
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_normal_derivative(const unsigned int q_point) const
{
  return BaseClass::get_normal_derivative(q_point)[0];
}
...
inline DEAL_II_ALWAYS_INLINE Tensor<2, 1, VectorizedArray<Number>>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_hessian(const unsigned int q_point) const
{
  return BaseClass::get_hessian(q_point)[0];
}
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE Tensor<1, 1, VectorizedArray<Number>>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_hessian_diagonal(const unsigned int q_point) const
{
  return BaseClass::get_hessian_diagonal(q_point)[0];
}
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE VectorizedArray<Number>
                             FEEvaluationAccess<1, 1, Number, is_face>::get_laplacian(const unsigned int q_point) const
{
  return BaseClass::get_laplacian(q_point)[0];
}
...
inline DEAL_II_ALWAYS_INLINE void DEAL_II_ALWAYS_INLINE
                                  FEEvaluationAccess<1, 1, Number, is_face>::submit_dof_value(const VectorizedArray<Number> val_in,
                                                            const unsigned int            dof)
{
#  ifdef DEBUG
  this->dof_values_initialized = true;
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<1, 1, Number, is_face>::submit_value(const VectorizedArray<Number> val_in,
                                                        const unsigned int            q_point)
{
#  ifdef DEBUG
  Assert(this->cell != numbers::invalid_unsigned_int, ExcNotInitialized());
...
template <typename Number, bool is_face>
inline DEAL_II_ALWAYS_INLINE void
FEEvaluationAccess<1, 1, Number, is_face>::submit_gradient(const VectorizedArray<Number> grad_in,
                                                           const unsigned int            q_point)
{
#  ifdef DEBUG
  Assert(this->cell != numbers::invalid_unsigned_int, ExcNotInitialized());
...
#  endif
  ...
  const Tensor<2, 1, VectorizedArray<Number>> &jac =
    this->cell_type == internal::MatrixFreeFunctions::general ? this->jacobian[q_point] :
                                                                this->jacobian[0];
  const VectorizedArray<Number> JxW = this->cell_type == internal::MatrixFreeFunctions::general ?
                                        this->J_value[q_point] :
                                        this->J_value[0] * this->quadrature_weights[q_point];
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
inline FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::FEEvaluation(
  const MatrixFree<dim, Number> &data_in,
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::check_template_arguments(
  const unsigned int dof_no,
  const unsigned int first_selected_component)
{
  (void)dof_no;
...
#  ifdef DEBUG
  ...
  if ((static_cast<unsigned int>(fe_degree) != numbers::invalid_unsigned_int &&
      ...
      n_q_points != this->n_quadrature_points)
    {
      ...
      if (dof_no != numbers::invalid_unsigned_int)
        {
          if (static_cast<unsigned int>(fe_degree) == this->data->fe_degree)
            ...
            for (unsigned int no = 0; no < this->matrix_info->n_components(); ++no)
              for (unsigned int nf = 0; nf < this->matrix_info->n_base_elements(no); ++nf)
                if (this->matrix_info->get_shape_info(no, 0, nf, this->active_fe_index, 0)
          ...
          if (n_q_points == this->mapping_data->descriptor[this->active_quad_index].n_q_points)
            proposed_quad_comp = this->quad_no;
          else
            for (unsigned int no = 0; no < this->matrix_info->get_mapping_info().cell_data.size();
                 ++no)
      ...
      const unsigned int proposed_n_q_points_1d =
  ...
  if (dof_no != numbers::invalid_unsigned_int)
    AssertDimension(n_q_points, this->mapping_data->descriptor[this->active_quad_index].n_q_points);
#  endif
}
...
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::quadrature_point(
...
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::evaluate(
  ...
  const bool                     evaluate_hessians)
{
  SelectEvaluator<dim, fe_degree, n_q_points_1d, n_components, VectorizedArray<Number>>::evaluate(
    *this->data,
    const_cast<VectorizedArray<Number> *>(values_array),
    this->values_quad[0],
...
template <typename VectorType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::gather_evaluate(
  ...
  VectorizedArray<Number> *values_array)
{
  ...
  SelectEvaluator<dim, fe_degree, n_q_points_1d, n_components, VectorizedArray<Number>>::integrate(
    *this->data,
    values_array,
    this->values_quad[0],
    this->gradients_quad[0][0],
...
template <typename VectorType>
inline void
FEEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::integrate_scatter(
  const bool  integrate_values,
  const bool  integrate_gradients,
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
inline FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::FEFaceEvaluation(
  ...
  const bool                     evaluate_gradients)
{
  ...
  const unsigned int static_dofs_per_face =
    fe_degree > -1 ? Utilities::pow(fe_degree + 1, dim - 1) : numbers::invalid_unsigned_int;
  const unsigned int dofs_per_face =
    fe_degree > -1 ? static_dofs_per_face : Utilities::pow(this->data->fe_degree + 1, dim - 1);
  ...
  if (fe_degree > -1 && this->subface_index >= GeometryInfo<dim>::max_children_per_cell &&
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components, typename Number>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number>::integrate(
  const bool integrate_values,
  const bool integrate_gradients)
{
  integrate(integrate_values, integrate_gradients, this->values_dofs[0]);
  ...
  VectorizedArray<Number> *values_array)
{
  ...
  const unsigned int n_q_points_1d_actual = fe_degree > -1 ? n_q_points_1d : 0;
  if (fe_degree > -1 && this->subface_index >= GeometryInfo<dim - 1>::max_children_per_cell &&
      this->data->element_type <= internal::MatrixFreeFunctions::tensor_symmetric)
...
template <typename VectorType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::gather_evaluate(
  const VectorType &input_vector,
  const bool        evaluate_values,
  const bool        evaluate_gradients)
{
  ...
  VectorizedArray<Number>
    temp_data[dofs_per_face < stack_array_size_threshold ? n_components_ * 2 * dofs_per_face : 1];
  ...
  if (this->dof_info->index_storage_variants[this->dof_access_index][this->cell] ==
      ...
      if (evaluate_gradients == true &&
          this->data->element_type == internal::MatrixFreeFunctions::tensor_symmetric_hermite)
        {
          ...
          const VectorizedArray<Number> grad_weight0 =
            (side ? -1. : 1.) * this->data->shape_data_on_face[0][fe_degree + 1];
          const VectorizedArray<Number> grad_weight1 =
            (side ? -1. : 1.) * this->data->shape_data_on_face[0][fe_degree + 2];
          AssertDimension(this->data->face_to_cell_index_hermite.size(1), 2 * dofs_per_face);
              ...
              for (unsigned int comp = 0; comp < n_components_; ++comp)
                {
                  ...
                  temp1[i + dofs_per_face + 2 * comp * dofs_per_face] =
                    grad_weight0 * temp1[i + 2 * comp * dofs_per_face] + grad_weight1 * grad;
                }
            }
        }
      else
        {
          AssertDimension(this->data->face_to_cell_index_nodal.size(1), dofs_per_face);
  ...
  else
    {
      ...
      internal::FEFaceNormalEvaluationImpl<dim, fe_degree, n_components_, VectorizedArray<Number>>::
        template interpolate<true, false>(
          *this->data, this->values_dofs[0], temp1, evaluate_gradients, this->face_no);
    }
...
template <typename VectorType>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::integrate_scatter(
  const bool  integrate_values,
  const bool  integrate_gradients,
  VectorType &destination)
{
...
#  endif
  ...
  if (this->dof_info->index_storage_variants[this->dof_access_index][this->cell] ==
  ...
  else
    {
      internal::FEFaceNormalEvaluationImpl<dim, fe_degree, n_components_, VectorizedArray<Number>>::
        template interpolate<false, false>(
          *this->data, temp1, this->values_dofs[0], integrate_gradients, this->face_no);
      this->distribute_local_to_global(destination);
    }
}
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components, typename Number>
inline void
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components, Number>::adjust_for_face_orientation(
  const bool integrate,
  const bool values,
  const bool gradients)
{
  VectorizedArray<Number> *tmp_values   = this->scratch_data;
  const unsigned int *     orientations = &this->mapping_data->descriptor[this->active_fe_index]
...
template <int dim, int fe_degree, int n_q_points_1d, int n_components_, typename Number>
inline Point<dim, VectorizedArray<Number>>
FEFaceEvaluation<dim, fe_degree, n_q_points_1d, n_components_, Number>::quadrature_point(
  const unsigned int q) const
{
  AssertIndexRange(q, n_q_points);
