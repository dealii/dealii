...
namespace internal
{
  ...
  template <MatrixFreeFunctions::ElementType type,
            ...
            typename Number>
  inline void
  FEEvaluationImpl<type, dim, fe_degree, n_q_points_1d, n_components, Number>::evaluate(
    ...
    const bool                                    evaluate_hessians)
  {
    ...
    typedef EvaluatorTensorProduct<variant, dim, fe_degree + 1, n_q_points_1d, Number> Eval;
    Eval                                                                               eval(
      variant == evaluate_evenodd ? shape_info.shape_values_eo : shape_info.shape_values,
      variant == evaluate_evenodd ? shape_info.shape_gradients_eo : shape_info.shape_gradients,
      variant == evaluate_evenodd ? shape_info.shape_hessians_eo : shape_info.shape_hessians,
    ...
    const unsigned int temp_size =
      Eval::n_rows_of_product == numbers::invalid_unsigned_int ?
        0 :
        (Eval::n_rows_of_product > Eval::n_columns_of_product ? Eval::n_rows_of_product :
    ...
    if (type == MatrixFreeFunctions::truncated_tensor)
      {
        Number *values_dofs_tmp =
          scratch_data +
          2 * (std::max(shape_info.dofs_per_component_on_cell, shape_info.n_q_points));
                ...
                for (int k = 0; k < degree + 1 - j - i; ++k, ++count_p, ++count_q)
                  for (unsigned int c = 0; c < n_components; ++c)
                    values_dofs_tmp[c * dofs_per_comp + count_q] =
                      values_dofs_actual[c * shape_info.dofs_per_component_on_cell + count_p];
  ...
  template <MatrixFreeFunctions::ElementType type,
            ...
            typename Number>
  inline void
  FEEvaluationImpl<type, dim, fe_degree, n_q_points_1d, n_components, Number>::integrate(
    const MatrixFreeFunctions::ShapeInfo<Number> &shape_info,
    Number *                                      values_dofs_actual,
    Number *                                      values_quad,
    Number *                                      gradients_quad,
  ...
  struct FEEvaluationImplBasisChange
  {
    ...
    {
      Assert(basis_size_1 != 0 || basis_size_1_variable <= basis_size_2_variable,
      ...
      constexpr int next_dim =
        (dim > 2 || ((basis_size_1 == 0 || basis_size_2 > basis_size_1) && dim > 1)) ? dim - 1 :
                                                                                       dim;
            ...
            Number *                      values_out)
    {
      ...
      EvaluatorTensorProduct<variant, dim, basis_size_1, basis_size_2, Number, Number2> eval_val(
        ...
        for (unsigned int c = 0; c < n_components; ++c)
          {
            for (unsigned int i = ii; i < ii + n_inner_blocks; ++i)
              eval_val.template values_one_line<dim - 1, true, false>(my_scratch + i,
                                                                      my_scratch + i);
            for (unsigned int q = 0; q < basis_size_2; ++q)
              for (unsigned int i = ii; i < ii + n_inner_blocks; ++i)
                my_scratch[i + q * n_blocks] *= coefficients[i + q * n_blocks];
            for (unsigned int i = ii; i < ii + n_inner_blocks; ++i)
              eval_val.template values_one_line<dim - 1, false, false>(my_scratch + i,
                                                                       my_scratch + i);
          }
      for (unsigned int q = 0; q < basis_size_1; ++q)
        FEEvaluationImplBasisChange<variant,
                                    ...
                                    Number2>::do_backward(transformation_matrix,
                                                          false,
                                                          my_scratch + q * Utilities::pow(
                                                                             basis_size_2, dim - 1),
                                                          values_out +
                                                            q * Utilities::pow(basis_size_1,
                                                                               dim - 1));
    }
  };
    ...
    const bool evaluate_hessians)
  {
    ...
    EvaluatorTensorProduct<evaluate_evenodd, dim, fe_degree + 1, fe_degree + 1, Number> eval(
            ...
            if (dim > 2)
              {
                eval.template gradients<2, true, false>(gradients_quad,
                                                        hessians_quad + 4 * n_q_points);
                eval.template gradients<2, true, false>(gradients_quad + n_q_points,
                                                        hessians_quad + 5 * n_q_points);
             ...
             const bool evaluate_hessians)
  {
    ...
    for (unsigned int c = 0; c < n_components; c++)
      {
        FEEvaluationImplBasisChange<evaluate_evenodd,
                                    dim,
        ...
        if (evaluate_gradients == true || evaluate_hessians == true)
          FEEvaluationImplCollocation<dim, n_q_points_1d - 1, 1, Number>::evaluate(
            shape_info,
            values_quad,
            nullptr,
            gradients_quad,
    ...
    for (unsigned int c = 0; c < n_components; c++)
      {
        ...
        if (integrate_gradients == true)
          FEEvaluationImplCollocation<dim, n_q_points_1d - 1, 1, Number>::integrate(
            shape_info,
            values_quad,
            nullptr,
            gradients_quad,
  ...
  struct FEFaceEvaluationImpl
  {
                     ...
                     const unsigned int                            subface_index)
    {
      ...
      typedef internal::EvaluatorTensorProduct<symmetric_evaluate ? internal::evaluate_evenodd :
                                               ...
                                               Number>
           Eval;
      Eval eval1(val1, grad1, AlignedVector<Number>(), data.fe_degree + 1, data.n_q_points_1d);
      Eval eval2(val2, grad2, AlignedVector<Number>(), data.fe_degree + 1, data.n_q_points_1d);
      ...
      const unsigned int n_q_points =
        fe_degree > -1 ? Utilities::pow(n_q_points_1d, dim - 1) : data.n_q_points_face;
                  ...
                  if (symmetric_evaluate && n_q_points_1d > fe_degree)
                    {
                      ...
                      if (integrate_val)
                        ...
                        eval_grad.template gradients<1, false, false>(gradients_quad + n_q_points,
                                                                      values_quad);
                      eval_grad.template gradients<0, false, true>(gradients_quad, values_quad);
  ...
  struct FEFaceNormalEvaluationImpl
  {
    ...
    {
      internal::EvaluatorTensorProduct<internal::evaluate_general, dim, fe_degree + 1, 0, Number>
      ...
      const unsigned int in_stride =
        do_evaluate ? data.dofs_per_component_on_cell : 2 * data.dofs_per_component_on_face;
      const unsigned int out_stride =
        do_evaluate ? 2 * data.dofs_per_component_on_face : data.dofs_per_component_on_cell;
