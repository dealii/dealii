...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    {
      FaceIdentifier() : n_hanging_faces_smaller_subdomain(0), n_hanging_faces_larger_subdomain(0)
      {}
      ...
      if (dynamic_cast<const parallel::Triangulation<dim> *>(&triangulation))
        {
                  ...
                  else
                    {
                      ...
                      if (neighbor->level() < dcell->level() && use_active_cells)
                      ...
                      else if (neighbor->level() == dcell->level() && my_domain != neigh_domain)
                        {
                          ...
                          if (my_domain < neigh_domain)
                            inner_faces_at_proc_boundary[neigh_domain].shared_faces.emplace_back(
                              id_mine, id_neigh);
                          else
                            inner_faces_at_proc_boundary[neigh_domain].shared_faces.emplace_back(
                              id_neigh, id_mine);
          ...
          for (std::map<types::subdomain_id, FaceIdentifier>::iterator it =
               ...
               ++it)
            {
              Assert(it->first != my_domain, ExcInternalError("Should not send info to myself"));
              std::sort(it->second.shared_faces.begin(), it->second.shared_faces.end());
              it->second.shared_faces.erase(
                std::unique(it->second.shared_faces.begin(), it->second.shared_faces.end()),
...
#  endif
              ...
              for (unsigned int i = 0; i < other_range.size(); ++i)
                other_range[i] = std::make_tuple(
                  it->second.shared_faces[i].second, it->second.shared_faces[i].first, i);
                        ...
                        if (count > 0)
                          {
                            for (unsigned int k = 0; k <= count; ++k)
                              {
                                Assert(
                                  it->second.shared_faces[std::get<2>(other_range[i - 1])].second ==
                                    it->second.shared_faces[std::get<2>(other_range[i - 1 - k])]
                                      .second,
                                  ExcInternalError());
              ...
              const unsigned int n_total_faces_at_proc_boundary =
                (it->second.shared_faces.size() + it->second.n_hanging_faces_smaller_subdomain +
              ...
              if (split_index < n_faces_lower_proc)
                ...
                split_index =
                  n_total_faces_at_proc_boundary - n_faces_higher_proc - n_faces_lower_proc;
...
#  endif
              ...
              AssertIndexRange(split_index,
                               it->second.shared_faces.size() + 1 - owned_faces_lower.size() -
...
#  ifdef DEBUG
              ...
              check_faces.insert(
                check_faces.end(), owned_faces_lower.begin(), owned_faces_lower.end());
              check_faces.insert(
                check_faces.end(), owned_faces_higher.begin(), owned_faces_higher.end());
...
#  endif
          ...
          for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
            {
              ...
              // inside the domain in case of multigrid separately
              ...
              else
                {
                  ...
                  if ((id1 == id2 &&
                       (use_active_cells == false || neighbor->has_children() == false)) ||
                  ...
                  if (add_to_ghost)
                    {
                      ghost_cells.insert(std::pair<unsigned int, unsigned int>(neighbor->level(),
                                                                               neighbor->index()));
      ...
      // cells
      for (std::set<std::pair<unsigned int, unsigned int>>::iterator it = ghost_cells.begin();
           it != ghost_cells.end();
           ++it)
      ...
      cells_close_to_boundary.erase(
        std::unique(cells_close_to_boundary.begin(), cells_close_to_boundary.end()),
      ...
      TaskInfo &                                                task_info)
    {
      ...
      for (unsigned int partition = 0; partition < task_info.cell_partition_data.size() - 2;
           ++partition)
        {
                            ...
                            if (neigh_domain != my_domain ||
                            ...
                            else
                              {
                                ...
                                if (dcell->has_periodic_neighbor(f))
                                  face_visited[neighbor->face(dcell->periodic_neighbor_face_no(f))
                                                 ->index()] = 1;
                              }
                            if (face_is_owned[dcell->face(f)->index()] ==
                                FaceCategory::multigrid_refinement_edge)
                              {
                                refinement_edge_faces.push_back(create_face(
                                  f, dcell, cell, neighbor, refinement_edge_faces.size()));
                              }
                          }
                      }
                  }
                                ...
                                const unsigned int number_cell_exterior)
    {
      ...
      info.face_orientation                    = 0;
      const unsigned int left_face_orientation = !cell->face_orientation(face_no) +
                                                 2 * cell->face_flip(face_no) +
      ...
      for (unsigned int partition = 0; partition < face_partition_data.size() - 1; ++partition)
        {
          ...
          std::set<FaceToCellTopology<vectorization_width>, FaceComparator<vectorization_width>>
            new_faces;
          for (unsigned int type = 0; type < faces_type.size(); ++type)
            {
                        ...
                        for (unsigned int v = 0; v < vectorization_width; ++v)
                          {
                            macro_face.cells_interior[v] =
                              faces_in[faces_type[type][f + v]].cells_interior[0];
                            macro_face.cells_exterior[v] =
                              faces_in[faces_type[type][f + v]].cells_exterior[0];
                            touched[f + v] = 1;
                          }
...
#  ifdef DEBUG
      ...
      for (unsigned int i = 0; i < faces_in.size(); ++i)
        in_faces.emplace_back(faces_in[i].cells_interior[0], faces_in[i].cells_exterior[0]);
      for (unsigned int i = face_partition_data[0]; i < face_partition_data.back(); ++i)
