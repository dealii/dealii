 ...
 *
 * In 2D, each cell stores the indices of the lines which bound it: i.e., each
 * cell stores either 3 (for triangles) or 4 (for quadrilaterals) integers which
 ...
 *
 * Since each line has a unique index it also has a canonical vertex ordering.
 * For example, consider two adjacent triangles with vertices `{0, 1, 2}` and
 * `{1, 0, 3}`. The order of the vertices on each cell is defined by the
 ...
 * form a cell whose mapping to the reference cell has a positive Jacobian. In
 * this example, the first face of the first cell is `{0, 1}` whereas the first
 * face of the second cell is `{1, 0}`. In deal.II, the canonical order of a
 * line's vertices is set by the first cell with that line: i.e., line `0` will
 * have vertices `{0, 1}` since it first appears in the first cell.
 * Canonicalization creates an inconsistency because this ordering does not
 * match the same line on the second cell. To resolve this inconsistency, each
 * 2D structure (either cells in 2D or faces in 3D) also stores the relative
 ...
 * (i.e., a 3D face) has 8 possible orientations and a triangle has 6. In
 * deal.II, we encode the orientation of an object with three booleans:
 ...
 * types::geometric_orientation by the internal::combined_face_orientation() and
 ...
 * consequence of this choice is that QProjector uses the inverse orientation (via ReferenceCell::get_inverse_combined_orientation()) to
 * compute the locations of quadrature points, since either
 *
 * 1. If we are on the face which defined the canonical ordering of the face
