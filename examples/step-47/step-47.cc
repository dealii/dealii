...
namespace Step47
{
  ...
  exact_solution(const Point<dim>& p)
  {
    ...
    return (r < 0.5 ? 1. / 20 * (-1. / 4 * r * r + 61. / 16) :
                      1. / 4 * (1 - r * r));
  }
    ...
    if(type == 2)
      {
        ...
        Assert(xfem_quadrature.size() == plain_quadrature.size() * 5,
               ExcInternalError());
    ...
    if(type == 3)
      {
        ...
        if(sign_ls[0] == sign_ls[1] && sign_ls[2] == sign_ls[3])
          ...
          {
            std::cout
              << "Error: the level set function has not the right values"
              << std::endl;
            assert(0);
          }
        ...
        Assert(xfem_quadrature.size() == plain_quadrature.size() * 2,
               ExcInternalError());
