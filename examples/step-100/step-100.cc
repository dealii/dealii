...
namespace Step100
{
  ...
  // tutorial, we will avoid the use of deal.II's complex arithmetic
  // capabilities and only use the complex functions that are defined in the C++
  // standard library. Consequently, in what follows, we will separate the real
  // and imaginary components of our spaces. Therefore, we will also define two
  // implementations of each function, one for the real component and one for
  // the imaginary one.

  // We start by creating the analytical solution class for the kinematic
  // pressure
  ...
  // the solution into real and imaginary parts, we can directly take $\Re(p^*)
  ...
  // goes for the real and imaginary parts of the velocity field. The only
  // difference is that the velocity field is a vector field, so it will be
  // derived from the TensorFunction class and return a Tensor<1,dim> in the
  // value function.
  template <int dim>
  ...
  // from the definition $\hat{u}_{n} = \mathbf{u} \cdot n$ and there are the
  // real and imaginary parts of both fields. The returned value will be based
  // on the following component convention:
  ...
  // FESystem objects. The DoFHandler objects that we rely on are the following:
  ...
  // The same applies for the three FESystem objects:
  ...
  // closed interval $[0, \pi/2]$ for the boundary conditions to make
  ...
  // that needs to be solved which helps for memory consumption and for the
  // iterative solver convergence, but this requires assembling the system
  // twice. The boolean flag introduced is interesting since it allows reusing
  ...
  // This function sets up the multiple DOFHandler objects and records the
  // number of DoFs associated with each space in the ConvergenceTable for later
  ...
  // pressure shape functions. This enumeration uses bits as boolean flags
  // to facilitate efficient checks during assembly of the DPG matrices and
  ...
  // and $M_5$ defined in the last section of the introduction). Further
    ...
    for (const auto &cell : dof_handler_trial_interior.active_cell_iterators())
      {
        ...
        //  space associated to the test functions $q$, we build $\langle
        //  \frac{k_n}{\omega}q,
            ...
            for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
              {
                ...
                //  -  If <code>i</code> is in the skeleton trace
                //  associated to
                //  $\hat{p}^*$, we assemble the term $\langle
                //  \frac{k_n}{\omega} \hat{p}^*, g_R \rangle_{\Gamma_1 \cup
                //  \Gamma_3}$.
                //
  ...
  // is handled separately using the DataOutFaces class as presented in
  ...
  void DPGHelmholtz<dim>::calculate_L2_error()
  {
    ...
    // similar idea to the one used to define the flux orientation during the
