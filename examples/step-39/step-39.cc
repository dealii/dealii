...
namespace Step39
{
  ...
  template <int dim>
  void RHSIntegrator<dim>::boundary(MeshWorker::DoFInfo<dim> &                 dinfo,
                                    typename MeshWorker::IntegrationInfo<dim> &info) const
  {
  ...
  template <int dim>
  void ErrorIntegrator<dim>::boundary(MeshWorker::DoFInfo<dim> &                 dinfo,
                                      typename MeshWorker::IntegrationInfo<dim> &info) const
  {
    const FEValuesBase<dim> &fe = info.fe_values();
  ...
  void InteriorPenaltyProblem<dim>::solve()
  {
    ...
    // preconditioner.
    Multigrid<Vector<double>> mg(mgmatrix, mg_coarse, mg_transfer, mg_smoother, mg_smoother);
  ...
  {
    const std::string filename = "sol-" + Utilities::int_to_string(cycle, 2) + ".gnuplot";
