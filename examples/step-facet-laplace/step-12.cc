...
namespace Step12
{
  ...
  void AdvectionProblem<dim>::assemble_system()
  {
        ...
        for (unsigned int i = 0; i < fe_v.dofs_per_cell; ++i)
          {
            for (unsigned int j = 0; j < fe_v.dofs_per_cell; ++j)
              copy_data.cell_matrix(i, j) +=
                // nu \nabla u \nabla v
                nu[point] * fe_v.shape_grad(i, point) *
    ...
    auto boundary_worker = [&](const Iterator &    cell,
      ...
      const double degree =
        std::max(1.0, static_cast<double>(fe_face.get_fe().degree));
          ...
          for (unsigned int i = 0; i < fe_face.dofs_per_cell; ++i)
            for (unsigned int j = 0; j < fe_face.dofs_per_cell; ++j)
              copy_data.cell_matrix(i, j) +=
                (
                  // - nu (\nabla u . n) v
                  -nu[point] * (fe_face.shape_grad(j, point) * normals[point]) *
                    fe_face.shape_value(i, point)

                  // - nu u (\nabla v . n)  // NIPG: use +
                  - nu[point] * fe_face.shape_value(j, point) *
                      (fe_face.shape_grad(i, point) * normals[point])

                  // + nu * penalty u v
                  + nu[point] * penalty * fe_face.shape_value(j, point) *
                      fe_face.shape_value(i, point)) *
                JxW[point];
    ...
    auto face_worker = [&](const Iterator &    cell,
      ...
      const double degree =
        std::max(1.0,
                 static_cast<double>(fe_facet.get_fe_values().get_fe().degree));
      ...
      const double penalty =
        4.0 * degree * (degree + 1.0) * 0.5 * (1.0 / extent1 + 1.0 / extent2);
