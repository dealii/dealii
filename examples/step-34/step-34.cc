...
namespace Step34
{
  ...
  {
  public:
    BEMProblem(const unsigned int fe_degree = 1, const unsigned int mapping_degree = 1);
  ...
  private:
    ...
    get_singular_quadrature(const typename DoFHandler<dim - 1, dim>::active_cell_iterator &cell,
  ...
  template <int dim>
  BEMProblem<dim>::BEMProblem(const unsigned int fe_degree, const unsigned int mapping_degree) :
    fe(fe_degree),
    dh(tria),
  ...
  void BEMProblem<dim>::read_parameters(const std::string &filename)
  {
    ...
    prm.enter_subsection("Quadrature rules");
    {
      quadrature = std::shared_ptr<Quadrature<dim - 1>>(new QuadratureSelector<dim - 1>(
    ...
    prm.enter_subsection(std::string("Exact solution ") + Utilities::int_to_string(dim) +
                         std::string("d"));
    {
    ...
    run_in_this_dimension = prm.get_bool("Run " + Utilities::int_to_string(dim) + "d simulation");
  }
  ...
  void BEMProblem<dim>::assemble_system()
  {
    ...
    typename DoFHandler<dim - 1, dim>::active_cell_iterator cell = dh.begin_active(),
                                                            endc = dh.end();
                ...
                for (unsigned int q = 0; q < n_q_points; ++q)
                  {
                    ...
                    system_rhs(i) += (LaplaceKernel::single_layer(R) * normal_wind * fe_v.JxW(q));

                    for (unsigned int j = 0; j < fe.dofs_per_cell; ++j)

                      local_matrix_row_i(j) -= ((LaplaceKernel::double_layer(R) * normals[q]) *
                ...
                for (unsigned int q = 0; q < singular_quadrature.size(); ++q)
                  {
                    ...
                    for (unsigned int d = 0; d < dim; ++d)
                      normal_wind += (singular_cell_wind[q](d) * singular_normals[q][d]);

                    system_rhs(i) +=
                      (LaplaceKernel::single_layer(R) * normal_wind * fe_v_singular.JxW(q));

                    for (unsigned int j = 0; j < fe.dofs_per_cell; ++j)
                      {
                        local_matrix_row_i(j) -=
                          ((LaplaceKernel::double_layer(R) * singular_normals[q]) *
  ...
  void BEMProblem<dim>::compute_errors(const unsigned int cycle)
  {
    ...
    const double L2_error =
      VectorTools::compute_global_error(tria, difference_per_cell, VectorTools::L2_norm);
  ...
  template <>
  const Quadrature<2> &
  BEMProblem<3>::get_singular_quadrature(const DoFHandler<2, 3>::active_cell_iterator &,
                                         const unsigned int index) const
  {
    ...
    if (quadratures.size() == 0)
      for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
        quadratures.emplace_back(singular_quadrature_order, fe.get_unit_support_points()[i], true);
  ...
  template <>
  const Quadrature<1> &
  BEMProblem<2>::get_singular_quadrature(const DoFHandler<1, 2>::active_cell_iterator &cell,
                                         const unsigned int                            index) const
  {
    Assert(index < fe.dofs_per_cell, ExcIndexRange(0, fe.dofs_per_cell, index));
    ...
    q_pointer = new QGaussLogR<1>(
      singular_quadrature_order, fe.get_unit_support_points()[index], 1. / cell->measure(), true);
    return (*q_pointer);
  }
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              ...
              external_phi(i) += ((LaplaceKernel::single_layer(R) * normal_wind[q] +
  ...
  void BEMProblem<dim>::output_results(const unsigned int cycle)
  {
    ...
    dataout.add_data_vector(phi, "phi", DataOut<dim - 1, DoFHandler<dim - 1, dim>>::type_dof_data);
    dataout.add_data_vector(
      alpha, "alpha", DataOut<dim - 1, DoFHandler<dim - 1, dim>>::type_dof_data);
    dataout.build_patches(mapping,
                          mapping.get_degree(),
                          DataOut<dim - 1, DoFHandler<dim - 1, dim>>::curved_inner_cells);

    std::string   filename = (Utilities::int_to_string(dim) + "d_boundary_solution_" +
    ...
    if (cycle == n_cycles - 1)
      {
        ...
        convergence_table.evaluate_convergence_rates("L2(phi)",
                                                     ConvergenceTable::reduction_rate_log2);
        convergence_table.evaluate_convergence_rates("Linfty(alpha)",
                                                     ConvergenceTable::reduction_rate_log2);
        deallog << std::endl;
  ...
  void BEMProblem<dim>::run()
  {
    ...
    if (run_in_this_dimension == false)
      {
        deallog << "Run in dimension " << dim << " explicitly disabled in parameter file. "
                << std::endl;
        return;
      }
