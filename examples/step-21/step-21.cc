...
namespace Step21
{
  ...
  template <int dim>
  double SaturationBoundaryValues<dim>::value(const Point<dim> &p,
                                              const unsigned int /*component*/) const
  {
    if (p[0] == 0)
      return 1;
      ...
      for (unsigned int p = 0; p < points.size(); ++p)
        {
          ...
          const double permeability =
            std::max(std::exp(-(distance_to_flowline * distance_to_flowline) / (0.1 * 0.1)), 0.01);
  ...
  TwoPhaseFlowProblem<dim>::TwoPhaseFlowProblem(const unsigned int degree) :
    degree(degree),
    fe(FE_RaviartThomas<dim>(degree), 1, FE_DGQ<dim>(degree), 1, FE_DGQ<dim>(degree), 1),
    ...
    for (; cell != endc; ++cell)
      {
        ...
        pressure_right_hand_side.value_list(fe_values.get_quadrature_points(), pressure_rhs_values);
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  local_matrix(i, j) +=
                    (phi_i_u * k_inverse_values[q] * mobility_inverse(old_s, viscosity) * phi_j_u -
                     div_phi_i_u * phi_j_p - phi_i_p * div_phi_j_u + phi_i_s * phi_j_s) *
                    fe_values.JxW(q);
                }
  ...
  void TwoPhaseFlowProblem<dim>::assemble_rhs_S()
  {
    ...
    FEFaceValues<dim> fe_face_values(fe,
                                     ...
                                     update_values | update_normal_vectors |
                                       update_quadrature_points | update_JxW_values);
    FEFaceValues<dim> fe_face_values_neighbor(fe, face_quadrature_formula, update_values);
    ...
    std::vector<Vector<double>> old_solution_values_face_neighbor(n_face_q_points,
                                                                  Vector<double>(dim + 2));
    std::vector<Vector<double>> present_solution_values(n_q_points, Vector<double>(dim + 2));
    std::vector<Vector<double>> present_solution_values_face(n_face_q_points,
                                                             Vector<double>(dim + 2));
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              local_rhs(i) +=
                (time_step * fractional_flow(old_s, viscosity) * present_u * grad_phi_i_s +
                 old_s * phi_i_s) *
                fe_values.JxW(q);
            }
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  local_rhs(i) -= time_step * normal_flux *
                                  fractional_flow((is_outflow_q_point == true ?
                                                  ...
                                                  viscosity) *
                                  fe_face_values[saturation].value(i, q) * fe_face_values.JxW(q);
              }
          }
  ...
  void TwoPhaseFlowProblem<dim>::solve()
  {
    ...
    assemble_rhs_S();
    {
      ...
      SolverCG<>    cg(solver_control);
      cg.solve(
        system_matrix.block(2, 2), solution.block(2), system_rhs.block(2), PreconditionIdentity());

      project_back_saturation();

      std::cout << "   " << solver_control.last_step() << " CG iterations for saturation."
                << std::endl;
    }
  ...
  void TwoPhaseFlowProblem<dim>::run()
  {
...

    {
      ...
      VectorTools::project(
        dof_handler, constraints, QGauss<dim>(degree + 2), InitialValues<dim>(), old_solution);
    }
    ...
    do
      {
        ...
        time += time_step;
        ++timestep_number;
        std::cout << "   Now at t=" << time << ", dt=" << time_step << '.' << std::endl
                  << std::endl;
      }
    while (time <= 1.);
  }
