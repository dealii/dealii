...
namespace Step45
{
  ...
  public:
    SchurComplement(
      ...
      const InverseMatrix<TrilinosWrappers::SparseMatrix, PreconditionerType> &A_inverse,
  ...
  private:
    ...
    const SmartPointer<const InverseMatrix<TrilinosWrappers::SparseMatrix, PreconditionerType>>
                                          A_inverse;
  ...
  template <class PreconditionerType>
  void SchurComplement<PreconditionerType>::vmult(TrilinosWrappers::MPI::Vector &      dst,
                                                  const TrilinosWrappers::MPI::Vector &src) const
  {
  ...
  void StokesProblem<dim>::create_mesh()
  {
    ...
    GridTools::collect_periodic_faces(
      triangulation, 2, 3, 1, periodicity_vector, Tensor<1, dim>(), rotation_matrix);
  ...
  void StokesProblem<dim>::setup_dofs()
  {
...

    {
      ...
      VectorTools::interpolate_boundary_values(
        mapping, dof_handler, 0, BoundaryValues<dim>(), constraints, fe.component_mask(velocities));
      VectorTools::interpolate_boundary_values(
        mapping, dof_handler, 1, BoundaryValues<dim>(), constraints, fe.component_mask(velocities));
      ...
      GridTools::collect_periodic_faces(
        dof_handler, 2, 3, direction, periodicity_vector, offset, rotation_matrix);
  ...
  void StokesProblem<dim>::solve()
  {
...

    {
      ...
      InverseMatrix<TrilinosWrappers::SparseMatrix, TrilinosWrappers::PreconditionAMG> m_inverse(
  ...
  template <int dim>
  void StokesProblem<dim>::output_results(const unsigned int refinement_cycle) const
  {
    ...
    std::ofstream output("solution-" + Utilities::int_to_string(refinement_cycle, 2) + "." +
  ...
  void StokesProblem<dim>::run()
  {
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle < 9; ++refinement_cycle)
      {
