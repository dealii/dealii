...
namespace Step92
{
  ...
  constexpr double epsilon_vacuum =
    8.8541878176204199e-12; // vacuum permittivity  [ A s / (V m) ]
  ...
  public:
    ...
    const boundary_potential        no_boundary_value = {ID_NO_BOUNDARY_VALUE,
                                                         "none",
                                                         0};
  ...
  // In the constructor ProblemParameters::ProblemParameters(), all parameters
  // are defined, which shall be read when parsing the input parameter file:
  // <ul>
  //   <li>
  //     "mesh filename" is a string specifying a gmsh file (without suffix
  ...
  // After parsing all parameters, some processing of the domains data and the
  // boundary values is done:
  void ProblemParameters::finalize()
  {
    ...
    // Only the electric permittivity is converted into an absolute value
    // [As/Vm] by multiplying the relative permittivity number from parameter
    // file with vacuum permittivity If a domain ID 0 is assigned, its
    // properties will be defined as default values
  ...
  // will be used to retrieve the domain properties by providing a domain ID
  // number.
  ...
  // will be used to retrieve the boundary value by providing a boundary ID
  // number.
  ...
  // using a direct solver on a Laplace problem for electristatic field
  // computation consisting of conductive and dielectric domains.
        ...
        if (cell->is_locally_owned())
          {
                    ...
                    for (const auto j : fe_values.dof_indices())
                      {
                        ...
                        cell_matrix(i, j) += a * grad_phi_e_j * grad_phi_e_i *
            ...
            // Assemble only the boundary faces without Dirichlet boundary
            // condition
                ...
                if (face->at_boundary())
                  {
                    ...
                    auto boundary_value =
                      problem_parameters.get_boundary_value(boundary_id);
                    if (boundary_value.boundary_id !=
                        problem_parameters.ID_NO_BOUNDARY_VALUE)
                      continue;
  ...
  void Laplace<dim>::solve()
  {
    ...
    dealii::LinearAlgebraPETSc::MPI::Vector completely_distributed_solution(
  ...
  // @sect4{Data Output}
  // After solving the problem, some output data shall be written into a
  // vtk-file. The output-filename has been specified in the parameter file and
  // shall contain not only the solution of the Laplace PDE, but also electric
  // field strengths, current densities and domain properties for each
  // evaluation point. For this purpose a Cells Postprocessor is used.
  ...
  // is the function that takes the solution values and its gradients from
  // <code>inputs</code>, calculates electrostatic potential, electric field
  // strengths, electric current density, and domain properties (such as ID
  // number, relative electric permittivity and electric conductivity) and
  // arranges it into <code>computed_quantities</code> for each evaluation point
  template <int dim>
    ...
    std::vector<Vector<double>>                &computed_quantities) const
  {
    ...
    const auto domain =
      problem->problem_parameters.get_domain_properties(domain_id);

    for (unsigned int p = 0; p < n_evaluation_points; p++)
      {
        Laplace::fieldvector_type solution_E_field, solution_current_density_J;

        // The <code>solution_values</code> can be directly taken as the
        // electrostatic potential $\varphi_e$
        // (<code>solution_phi_e</code> resp.)
        // as the electric field strength $\mathbf{E}$ is calculated using the
        // <code>solution_gradients</code>. The current density $\mathbf{J}$ is
        // derived from electric field strength under terms of Ohm's law
        ...
        solution_current_density_J =
          solution_E_field * domain.kappa; // Ohm's law


        // For arranging the output data in the sequence as defined in function
        // <code>Laplace<dim>::CellsPostprocessor::get_names()</code>
        // the following offset-values are used to indicate the correct
        // positions
        constexpr unsigned int
          offset_phi_e = 0, // position index for electrostatic potential
          offset_E_field =
            1, // position index for first electric field component (Ex)
          offset_current_density_J =
            offset_E_field +
            dim, // position index for first current density component (Jx)
          offset_material_properties =
            offset_current_density_J +
            dim; // position index for domain ID number
        ...
        for (unsigned int d = 0; d < dim; d++)
          {
            ...
            computed_quantities[p](offset_current_density_J + d) =
              solution_current_density_J[d];
          }

        computed_quantities[p](offset_material_properties + 0) = domain_id;
        computed_quantities[p](offset_material_properties + 1) =
          domain.epsilon / epsilon_vacuum;
...
// An optional parameter can be passed via the command line call: a parameter
// filename If this optional parameter filename is not provided, parameter file
// "example-1.prm" will be taken per default. In the main function
// - MPI is initialized
