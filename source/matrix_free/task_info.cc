...
namespace internal
{
  namespace MatrixFreeFunctions
  {
    ...
    namespace partition
    {
      ...
      public:
        ...
        execute() override
        {
          ...
          const unsigned int n_blocked_workers = task_info.partition_n_blocked_workers[partition];
          ...
          for (unsigned int j = 0; j < evens; j++)
            {
              worker[j] = new (root->allocate_child()) CellWork(
                function, task_info.partition_row_index[partition] + 2 * j, task_info, false);
              if (j > 0)
                {
                  worker[j]->set_ref_count(2);
                  blocked_worker[j - 1]->dummy = new (worker[j]->allocate_child()) tbb::empty_task;
              ...
              if (j < evens - 1)
              ...
              else
                {
                  if (odds == evens)
                    ...
                    {
                      tbb::empty_task *child = new (worker[j]->allocate_child()) tbb::empty_task();
      ...
      public:
        ...
        operator()(const tbb::blocked_range<unsigned int> &r) const
        {
          const unsigned int start_index =
            task_info.cell_partition_data[partition] + task_info.block_size * r.begin();
              ...
              if (odds > 0)
                {
                  ...
                  const unsigned int n_workers = cell_partition_data.size() - 1 - n_blocked_workers;
                      ...
                      if (part < partition_row_index.size() - 1)
                      ...
                      else
                        {
                          tbb::empty_task *final =
                            new (worker[worker_index - 1]->allocate_child()) tbb::empty_task;
          ...
          for (unsigned int part = 0; part < partition_row_index.size() - 2; ++part)
            {
              ...
              for (unsigned int i = partition_row_index[part]; i < partition_row_index[part + 1];
                   ++i)
                {
                  ...
                  if (cell_partition_data[i + 1] > cell_partition_data[i])
                    {
                      ...
                      funct.cell(
                        std::make_pair(cell_partition_data[i], cell_partition_data[i + 1]));
                    }
                  ...
                  if (face_partition_data.empty() == false)
                    {
                      if (face_partition_data[i + 1] > face_partition_data[i])
                        funct.face(
                          std::make_pair(face_partition_data[i], face_partition_data[i + 1]));
    ...
    void
    TaskInfo::print_memory_statistics(StreamType &out, const std::size_t data_length) const
    {
    ...
    {
      return (sizeof(*this) + MemoryConsumption::memory_consumption(partition_row_index) +
      ...
      if (fillup_needed > 0 && boundary_cells.size() < n_active_cells)
        {
              ...
              if (next_free_slot < boundary_cells[bound_index])
                {
                  ...
                  // next boundary cell
                    ...
                    {
                      for (unsigned int j = next_free_slot; j < boundary_cells[bound_index]; ++j)
                        new_boundary_cells.push_back(j);
          ...
          while (fillup_needed > 0 &&
                 (new_boundary_cells.size() == 0 || new_boundary_cells.back() < n_active_cells - 1))
    ...
    void
    TaskInfo ::create_blocks_serial(const std::vector<unsigned int> &boundary_cells,
                                    ...
                                    std::vector<unsigned char> &incompletely_filled_vectorization)
    {
      ...
      renumbering.resize(n_active_cells + n_ghost_cells, numbers::invalid_unsigned_int);
      ...
      if (cell_vectorization_categories.empty() == false)
        {
          AssertDimension(cell_vectorization_categories.size(), n_active_cells + n_ghost_cells);
              ...
              if (block < 4)
                for (unsigned int k = 0; k < n_my_macro_cells; k += block_size)
                  cell_partition_data.push_back(n_cells +
                                                std::min(k + block_size, n_my_macro_cells));
              else
      ...
      if (cell_vectorization_categories_strict == true)
        Assert(n_cells >= n_macro_cells + n_ghost_slots,
               ExcInternalError()) else AssertDimension(n_cells, n_macro_cells + n_ghost_slots);
      ...
      std::vector<unsigned char> &     incompletely_filled_vectorization)
    {
      ...
      std::vector<unsigned int> reverse_numbering(n_active_cells, numbers::invalid_unsigned_int);
      ...
      if (block_size == 0)
        {
          ...
          block_size = n_active_cells / (MultithreadInfo::n_threads() * 16 * vectorization_length);
    ...
    void
    TaskInfo::make_thread_graph_partition_color(DynamicSparsityPattern &    connectivity_large,
                                                std::vector<unsigned int> & renumbering,
                                                ...
                                                const bool)
    {
      ...
      // a partition.
      std::vector<unsigned int> cell_partition(n_blocks, numbers::invalid_unsigned_int);
      ...
      make_partitioning(
        connectivity, cluster_size, cell_partition, partition_list, partition_size, partition);
      ...
      for (unsigned int block = 0; block < n_blocks; block++)
        {
          ...
          for (unsigned int mcell = mcell_start;
            ...
            {
              unsigned int n_comp =
                (irregular_cells[mcell] > 0) ? irregular_cells[mcell] : vectorization_length;
      ...
      for (unsigned int block = 0; block < n_blocks; block++)
        {
          ...
          for (unsigned int j = 0; j < this_block_size; j++)
            irregular[counter_macro++] = irregular_cells[present_block * block_size + j];
        }
    ...
    void
    TaskInfo::make_thread_graph(const std::vector<unsigned int> &cell_active_fe_index,
      ...
      const bool                       hp_bool)
    {
      ...
      std::vector<unsigned int> cell_partition(n_active_cells, numbers::invalid_unsigned_int);
    ...
    void
    TaskInfo::make_connectivity_cells_to_blocks(const std::vector<unsigned char> &irregular_cells,
                                                ...
                                                DynamicSparsityPattern &connectivity_blocks) const
    {
      ...
      for (unsigned int i = 0, mcell = 0; i < n_blocks; ++i)
        {
          for (unsigned int c = 0; c < block_size && mcell < *(cell_partition_data.end() - 2);
               ++c, ++mcell)
            {
              unsigned int ncomp =
                (irregular_cells[mcell] > 0) ? irregular_cells[mcell] : vectorization_length;
    ...
    {
      const unsigned int n_macro_cells = *(cell_partition_data.end() - 2);
      const unsigned int n_ghost_slots = *(cell_partition_data.end() - 1) - n_macro_cells;
      ...
      Assert(!hp_bool || cell_active_fe_index.size() == n_active_cells, ExcInternalError());

      {
        ...
        std::vector<unsigned int> cell_partition_l2(n_active_cells, numbers::invalid_unsigned_int);
        partition_row_index.clear();
                ...
                if (neighbor_list.size() == 0)
                ...
                else
                  {
                    ...
                    for (unsigned int j = 0; j < neighbor_list.size(); ++j)
                      {
                        Assert(cell_partition[neighbor_list[j]] == part, ExcInternalError());
                ...
                if (partition_counter > 0)
                  {
                    int index_before = neighbor_neighbor_list.size(), index = index_before;
                    {
                      ...
                      missing_macros = 0;
                      std::vector<unsigned int> remaining_per_macro_cell(max_fe_index + 1);
                      ...
                      if (hp_bool == true)
                      ...
                      else
                        {
                          ...
                          remaining_per_macro_cell[0] = partition_counter % vectorization_length;
                                    ...
                                    numbers::invalid_unsigned_int)
                                {
                                  ...
                                  if (missing_macros > 0 ||
                                      remaining_per_macro_cell[this_index] > 0)
                                    {
                                      ...
                                      counter++;
                                      partition_counter++;
                                      if (remaining_per_macro_cell[this_index] == 0 &&
                                          missing_macros > 0)
                                        missing_macros--;
                          ...
                          for (unsigned int j = 0; j < max_fe_index + 1; j++)
                            {
                              for (unsigned int jj = 0; jj < renumbering_fe_index[j].size(); jj++)
                                renumbering[cell++] = renumbering_fe_index[j][jj];
      ...
      for (unsigned int part = 0; part < partition; part++)
        {
          ...
          for (unsigned int k = partition_size[part]; k < partition_size[part + 1]; k++)
            {
...

    {
      ...
      unsigned int start_nonboundary =
        cell_partition_data.size() == 5 ?
          vectorization_length * (cell_partition_data[2] - cell_partition_data[1]) :
          0;
      ...
      while (work)
        {
          ...
          int index_before = neighbor_list.size(), index = index_before, index_stop = 0;
          while (remainder > 0)
            {
              if (index == index_stop)
                {
              ...
              DynamicSparsityPattern::iterator neighbor   = connectivity.begin(additional),
                                               end        = connectivity.end(additional);
              for (; neighbor != end; ++neighbor)
                {
          ...
          while (neighbor_list.size() > 0)
            {
              ...
              for (unsigned int j = 0; j < neighbor_list.size(); ++j)
                {
                  Assert(cell_partition[neighbor_list[j]] == partition - 1, ExcInternalError());
                  DynamicSparsityPattern::iterator neighbor = connectivity.begin(neighbor_list[j]),
                                                   end      = connectivity.end(neighbor_list[j]);
                  for (; neighbor != end; ++neighbor)
                    {
                      if (cell_partition[neighbor->column()] == numbers::invalid_unsigned_int)
      ...
      for (unsigned int part = 0; part < partition; part++)
        {
          ...
          partition_odds[part] = (partition_row_index[part + 1] - partition_row_index[part]) / 2;
          partition_n_blocked_workers[part] =
            partition_odds[part] - (partition_odds[part] + partition_evens[part] + 1) % 2;
          partition_n_workers[part] =
            partition_evens[part] + partition_odds[part] - partition_n_blocked_workers[part];
        }
    }
  } // namespace MatrixFreeFunctions
} // namespace internal
...
template void
internal::MatrixFreeFunctions::TaskInfo::print_memory_statistics<ConditionalOStream>(
  ConditionalOStream &,
  const std::size_t) const;
