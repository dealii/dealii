...
template <typename VectorType>
MGTransferPrebuilt<VectorType>::MGTransferPrebuilt(const ConstraintMatrix & /*c*/,
...
template <typename VectorType>
void
MGTransferPrebuilt<VectorType>::initialize_constraints(const MGConstrainedDoFs &mg_c)
{
  this->mg_constrained_dofs = &mg_c;
}
...
template <typename VectorType>
void
MGTransferPrebuilt<VectorType>::initialize_constraints(const ConstraintMatrix & /*c*/,
                                                       const MGConstrainedDoFs &mg_c)
{
...
void
MGTransferPrebuilt<VectorType>::build_matrices(const DoFHandler<dim, spacedim> &mg_dof)
{
  ...
  for (unsigned int i = 0; i < n_levels - 1; ++i)
    {
      prolongation_sparsities.emplace_back(new
                                           typename internal::MatrixSelector<VectorType>::Sparsity);
      prolongation_matrices.emplace_back(new typename internal::MatrixSelector<VectorType>::Matrix);
    }
  ...
  for (unsigned int level = 0; level < n_levels - 1; ++level)
    {
      ...
      DoFTools::extract_locally_relevant_level_dofs(mg_dof, level + 1, level_p1_relevant_dofs);
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  {
                    ...
                    dsp.add_entries(dof_indices_child[i], entries.begin(), entries.end());
                  }
              }
          }
      ...
      if (internal::MatrixSelector<VectorType>::requires_distributed_sparsity_pattern)
        {
          ...
          const std::vector<::dealii::IndexSet> &locally_owned_mg_dofs_per_processor =
            mg_dof.locally_owned_mg_dofs_per_processor(level + 1);
          std::vector<::dealii::types::global_dof_index> n_locally_owned_mg_dofs_per_processor(
          ...
          ::dealii::SparsityTools::distribute_sparsity_pattern(
            dsp, n_locally_owned_mg_dofs_per_processor, communicator, dsp.row_index_set());
        }
#endif

      internal::MatrixSelector<VectorType>::reinit(
        *prolongation_matrices[level], *prolongation_sparsities[level], level, dsp, mg_dof);
      dsp.reinit(0, 0);
