  ...
  const CurvedCellRegion curved_cell_region)
{
  ...
  for (unsigned int vertex = 0; vertex < GeometryInfo<DoFHandlerType::dimension>::vertices_per_cell;
       ++vertex)
    if (scratch_data.mapping_collection[0].preserves_vertex_locations())
      ...
      patch.vertices[vertex] = scratch_data.mapping_collection[0].transform_unit_to_real_cell(
        cell_and_index->first, GeometryInfo<DoFHandlerType::dimension>::unit_cell_vertex(vertex));
              ...
              if (n_components == 1)
                {
                  ...
                  postprocessor->evaluate_scalar_field(scratch_data.patch_values_scalar,
              ...
              else
                {
                  ...
                  postprocessor->evaluate_vector_field(scratch_data.patch_values_system,
                                                       scratch_data.postprocessed_values[dataset]);
                }
  ...
  const unsigned int patch_idx =
    (*scratch_data
        .cell_to_patch_index_map)[cell_and_index->first->level()][cell_and_index->first->index()];
  ...
  const CurvedCellRegion                                                     curved_region)
{
  ...
  for (unsigned int l = 0; l < this->triangulation->n_levels(); ++l)
    {
      ...
      for (cell_iterator cell = first_locally_owned_cell(); cell != this->triangulation->end();
        ...
        if (static_cast<unsigned int>(cell->level()) == l)
          max_index = std::max(max_index, static_cast<unsigned int>(cell->index()));
  ...
  std::vector<std::pair<cell_iterator, unsigned int>> all_cells;
  {
    ...
    for (; cell != this->triangulation->end(); cell = next_locally_owned_cell(cell))
      {
        ...
        Assert(static_cast<unsigned int>(cell->level()) < cell_to_patch_index_map.size(),
               ExcInternalError());
        Assert(static_cast<unsigned int>(cell->index()) <
                 cell_to_patch_index_map[cell->level()].size(),
               ExcInternalError());
        Assert(active_index < this->triangulation->n_active_cells(), ExcInternalError());
  ...
  for (unsigned int i = 0; i < this->dof_data.size(); ++i)
    n_datasets +=
      (this->dof_data[i]->n_output_variables * (this->dof_data[i]->is_complex_valued() ? 2 : 1));
  ...
  const typename DataOut<dim, DoFHandlerType>::cell_iterator &cell)
{
  ...
  typename Triangulation<DoFHandlerType::dimension,
                         DoFHandlerType::space_dimension>::active_cell_iterator active_cell = cell;
  ++active_cell;
  return active_cell;
}
...
{
  typename DataOut<dim, DoFHandlerType>::cell_iterator cell = next_cell(old_cell);
  while ((cell != this->triangulation->end()) && (cell->has_children() == false) &&
         !cell->is_locally_owned())
    cell = next_cell(cell);
  return cell;
