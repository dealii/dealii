...
namespace
{
  ...
  bool
  cell_is_patch_level_1(const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
  {
  ...
  template <int dim, int spacedim>
  bool
  cell_will_be_coarsened(const TriaIterator<dealii::CellAccessor<dim, spacedim>> &cell)
  {
    ...
    if (nb_ref_flag != RefinementCase<dim>::no_refinement)
      {
        ...
        if (face_ref_case != RefinementCase<dim - 1>::no_refinement)
          {
            const typename Triangulation<dim, spacedim>::face_iterator neighbor_face =
                  ...
                  if (neighbor_face->child_index(c) == this_face_index)
                    {
                      ...
                      if ((neighbor_face->refinement_case() | face_ref_case) ==
                      ...
                      else
                        {
                          ...
                          // that the face has
                          // now...
                          Assert(face_ref_case == RefinementCase<dim - 1>::isotropic_refinement,
                                 ExcInternalError());
  ...
  face_will_be_refined_by_neighbor(
    ...
    RefinementCase<dim - 1> &                                      expected_face_ref_case)
  {
    return face_will_be_refined_by_neighbor_internal(cell, face_no, expected_face_ref_case);
  }
  ...
  satisfies_level1_at_vertex_rule(const Triangulation<dim, spacedim> &triangulation)
  {
    ...
    std::vector<unsigned int> max_adjacent_cell_level(triangulation.n_vertices(), 0);
  ...
  void reorder_compatibility(std::vector<CellData<2>> &cells, const SubCellData &)
  {
  ...
  void reorder_compatibility(std::vector<CellData<3>> &cells, SubCellData &subcelldata)
  {
    ...
    std::vector<CellData<2>>::iterator boundary_quad = subcelldata.boundary_quads.begin();
    std::vector<CellData<2>>::iterator end_quad      = subcelldata.boundary_quads.end();
  ...
  collect_distorted_coarse_cells(const Triangulation<dim, dim> &triangulation)
  {
    ...
    for (typename Triangulation<dim, dim>::cell_iterator cell = triangulation.begin(0);
    ...
    for (unsigned int c = 0; c < cell->n_children(); ++c)
      {
        ...
        for (unsigned int i = 0; i < GeometryInfo<dim>::vertices_per_cell; ++i)
          if (determinants[i] <= 1e-9 * std::pow(cell->child(c)->diameter(), 1. * dim))
            return true;
      }
  ...
  update_neighbors(Triangulation<dim, spacedim> &triangulation)
  {
    ...
    std::vector<typename Triangulation<dim, spacedim>::cell_iterator> adjacent_cells(
      2 * triangulation.n_raw_faces(), dummy);
      ...
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
        {
          ...
          const unsigned int offset =
            (cell->direction_flag() ? left_right_offset[dim - 2][f][cell->face_orientation(f)] :
          ...
          if (dim == 2)
          ...
          else // -> dim == 3
            {
              ...
              if (face->has_children() &&
                  (cell->active() ||
                   GeometryInfo<dim>::face_refinement_case(cell->refinement_case(), f) ==
      ...
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
        {
          const unsigned int offset =
            (cell->direction_flag() ? left_right_offset[dim - 2][f][cell->face_orientation(f)] :
                                      1 - left_right_offset[dim - 2][f][cell->face_orientation(f)]);
          cell->set_neighbor(f, adjacent_cells[2 * cell->face(f)->index() + 1 - offset]);
        }
  }
  ...
  update_periodic_face_map_recursively(
    ...
    typename std::map<
      ...
      std::pair<std::pair<typename Triangulation<dim, spacedim>::cell_iterator, unsigned int>,
                std::bitset<3>>> &periodic_face_map)
  {
    ...
    const std::pair<CellFace, std::pair<CellFace, std::bitset<3>>> periodic_faces(
      cell_face_1, cell_face_orientation_2);
            ...
            for (unsigned int i = 0; i < GeometryInfo<dim>::max_children_per_face; ++i)
              {
                ...
                unsigned int child_cell_1 =
                  GeometryInfo<dim>::child_cell_on_face(cell_1->refinement_case(),
                                                        n_face_1,
                                                        i,
                ...
                unsigned int child_cell_2 =
                  GeometryInfo<dim>::child_cell_on_face(cell_2->refinement_case(),
                                                        n_face_2,
                                                        j,
                ...
                Assert(cell_1->child(child_cell_1)->face(n_face_1) == face_1->child(i),
                       ExcInternalError());
                Assert(cell_2->child(child_cell_2)->face(n_face_2) == face_2->child(j),
                       ExcInternalError());

                // precondition: subcell has the same orientation as cell (so
                // that the face numbers coincide) recursive call
                update_periodic_face_map_recursively<dim, spacedim>(cell_1->child(child_cell_1),
...
namespace internal
{
  namespace TriangulationImplementation
  {
    ...
    DeclException3(ExcInvalidVertexIndex,
                   ...
                   int,
                   int,
                   << "Error while creating cell " << arg1 << ": the vertex index " << arg2
    ...
    DeclException3(ExcInteriorLineCantBeBoundary,
                   ...
                   << "to check the SubCellData object you pass to this function." << std::endl
                   << std::endl
    ...
    DeclException5(ExcInteriorQuadCantBeBoundary,
                   ...
                   << arg3 << ", and " << arg4 << " that is described to have boundary indicator "
                   << (int)arg5 << ". However, this is an internal quad not located on the "
                   << "boundary. You cannot assign a boundary indicator to it." << std::endl
                   << std::endl
                   << "If this happened at a place where you call "
                   << "Triangulation::create_triangulation() yourself, you need "
    ...
    struct Implementation
    {
      ...
      template <int dim, int spacedim>
      static void
      compute_number_cache(const Triangulation<dim, spacedim> &                   triangulation,
                           const unsigned int                                     level_objects,
                           internal::TriangulationImplementation::NumberCache<1> &number_cache)
      {
        typedef typename Triangulation<dim, spacedim>::line_iterator line_iterator;
        ...
        if (dim == 1)
        ...
        else
          {
            ...
            line_iterator line = triangulation.begin_line(), endc = triangulation.end_line();
            for (; line != endc; ++line)
              {
                           ...
                           internal::TriangulationImplementation::NumberCache<2> &number_cache)
      {
        ...
        Threads::Task<void> update_lines = Threads::new_task(
          ...
          triangulation,
          level_objects,
          static_cast<internal::TriangulationImplementation::NumberCache<1> &>(number_cache));

        typedef typename Triangulation<dim, spacedim>::quad_iterator quad_iterator;
        ...
        if (dim == 2)
            ...
            for (unsigned int level = 0; level < n_levels; ++level)
              {
                ...
                quad_iterator quad = triangulation.begin_quad(level),
                              endc =
                                (level == n_levels - 1 ? quad_iterator(triangulation.end_quad()) :
        ...
        else
          {
            ...
            quad_iterator quad = triangulation.begin_quad(), endc = triangulation.end_quad();
            for (; quad != endc; ++quad)
              {
                           ...
                           internal::TriangulationImplementation::NumberCache<3> &number_cache)
      {
        ...
        Threads::Task<void> update_quads_and_lines = Threads::new_task(
          ...
          triangulation,
          level_objects,
          static_cast<internal::TriangulationImplementation::NumberCache<2> &>(number_cache));

        typedef typename Triangulation<dim, spacedim>::hex_iterator hex_iterator;
        ...
        if (dim == 3)
            ...
            for (unsigned int level = 0; level < n_levels; ++level)
              {
                ...
                hex_iterator hex  = triangulation.begin_hex(level),
                             endc = (level == n_levels - 1 ? hex_iterator(triangulation.end_hex()) :
        ...
        else
          {
            ...
            hex_iterator hex = triangulation.begin_hex(), endc = triangulation.end_hex();
            for (; hex != endc; ++hex)
              {
                           ...
                           Triangulation<1, spacedim> &triangulation)
      {
                ...
                if (!triangulation.check_for_distorted_cells)
                  {
                    const double cell_measure =
                      GridTools::cell_measure<1>(triangulation.vertices, cells[cell_no].vertices);
        ...
        triangulation.levels.push_back(
          std_cxx14::make_unique<internal::TriangulationImplementation::TriaLevel<dim>>());
        ...
        for (unsigned int cell = 0; cell < cells.size(); ++cell)
          {
            ...
            next_free_line->set(internal::TriangulationImplementation ::TriaObject<1>(
        ...
        // some security tests
        {
          ...
          // points at all)
          AssertThrow(((spacedim == 1) && (boundary_nodes == 2)) || (spacedim > 1),
        ...
        for (typename Triangulation<dim, spacedim>::active_cell_iterator cell =
          ...
          for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
            {
              (*triangulation.vertex_to_manifold_id_map_1d)[cell->face(f)->vertex_index()] =
                numbers::invalid_manifold_id;

              if (cell->at_boundary(f))
                (*triangulation.vertex_to_boundary_id_map_1d)[cell->face(f)->vertex_index()] = f;
            }
      }
                           ...
                           Triangulation<2, spacedim> &        triangulation)
      {
            ...
            for (unsigned int cell_no = 0; cell_no < cells.size(); ++cell_no)
              {
                ...
                if (!triangulation.check_for_distorted_cells)
                  {
                    const double cell_measure =
                      GridTools::cell_measure<2>(triangulation.vertices, cells[cell_no].vertices);
                    AssertThrow(cell_measure > 0, ExcGridHasInvalidCell(cell_no));
                  }
              }
          }
        ...
        // afterwards
        std::map<std::pair<int, int>, typename Triangulation<dim, spacedim>::line_iterator>
          needed_lines;
        for (unsigned int cell = 0; cell < cells.size(); ++cell)
          {
            for (unsigned int vertex = 0; vertex < 4; ++vertex)
              AssertThrow(cells[cell].vertices[vertex] < triangulation.vertices.size(),
                          ExcInvalidVertexIndex(
                            cell, cells[cell].vertices[vertex], triangulation.vertices.size()));
            ...
            for (unsigned int line = 0; line < GeometryInfo<dim>::faces_per_cell; ++line)
              {
                ...
                AssertThrow(needed_lines.find(std::make_pair(
                              line_vertices.second, line_vertices.first)) == needed_lines.end(),
        ...
        // least two adjacent lines
        {
          ...
          AssertThrow(*(std::min_element(vertex_touch_count.begin(), vertex_touch_count.end())) >=
                        2,
        ...
        triangulation.levels.push_back(
          std_cxx14::make_unique<internal::TriangulationImplementation::TriaLevel<dim>>());
        triangulation.faces =
          std_cxx14::make_unique<internal::TriangulationImplementation::TriaFaces<dim>>();
        triangulation.levels[0]->reserve_space(cells.size(), dim, spacedim);
          ...
          for (unsigned int c = 0; c < cells.size(); ++c, ++cell)
            {
              ...
              cell->set(internal::TriangulationImplementation::TriaObject<2>(
                lines[0]->index(), lines[1]->index(), lines[2]->index(), lines[3]->index()));
        ...
        for (typename Triangulation<dim, spacedim>::line_iterator line = triangulation.begin_line();
             ...
             ++line)
          {
            ...
            line->set_boundary_id_internal((n_adj_cells == 1) ? 0 :
                                                                numbers::internal_face_boundary_id);
            line->set_manifold_id(numbers::flat_manifold_id);
          }

        // set boundary indicators where given
        std::vector<CellData<1>>::const_iterator boundary_line = subcelldata.boundary_lines.begin();
        ...
        for (; boundary_line != end_boundary_line; ++boundary_line)
          {
            ...
            std::pair<int, int>                                  line_vertices(
              std::make_pair(boundary_line->vertices[0], boundary_line->vertices[1]));
            if (needed_lines.find(line_vertices) != needed_lines.end())
            ...
            else
              {
                ...
                if (needed_lines.find(line_vertices) != needed_lines.end())
                  ...
                  // line does not exist
                  AssertThrow(false, ExcLineInexistant(line_vertices.first, line_vertices.second));
              }
        ...
        for (typename Triangulation<dim, spacedim>::cell_iterator cell = triangulation.begin();
             cell != triangulation.end();
             ++cell)
            ...
            if (adjacent_cells[cell->line(side)->index()][0] == cell)
              ...
              // this one
              {
                if (adjacent_cells[cell->line(side)->index()].size() == 2)
                  // there is another
                  // adjacent cell
                  cell->set_neighbor(side, adjacent_cells[cell->line(side)->index()][1]);
              }
            // first adjacent cell is not this
            // one, -> it must be the neighbor
            // we are looking for
            else
              cell->set_neighbor(side, adjacent_cells[cell->line(side)->index()][0]);
      }
      ...
      {
        inline bool
        operator()(const internal::TriangulationImplementation::TriaObject<2> &q1,
                           ...
                           Triangulation<3, spacedim> &        triangulation)
      {
        ...
        for (unsigned int cell_no = 0; cell_no < cells.size(); ++cell_no)
          {
            ...
            if (!triangulation.check_for_distorted_cells)
              {
                const double cell_measure =
                  GridTools::cell_measure<3>(triangulation.vertices, cells[cell_no].vertices);
                AssertThrow(cell_measure > 0, ExcGridHasInvalidCell(cell_no));
              }
          }
#endif
        ...
        // quads
        typename std::map<std::pair<int, int>, typename Triangulation<dim, spacedim>::line_iterator>
          needed_lines;
        for (unsigned int cell = 0; cell < cells.size(); ++cell)
          {
            ...
            for (unsigned int line = 0; line < GeometryInfo<dim>::lines_per_cell; ++line)
              {
                ...
                // insert the line
                if ((needed_lines.find(std::make_pair(line_vertices.second, line_vertices.first)) ==
                     needed_lines.end()))
                  {
                    // insert line, with
                    // invalid iterator. if line
            ...
            for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
              {
                ...
                for (unsigned int l = 0; l < GeometryInfo<dim>::lines_per_face; ++l)
                  if (needed_lines.find(
                        inverse_line_list[GeometryInfo<dim>::face_to_cell_lines(face, l)]) ==
                      needed_lines.end())
                    {
                      face_line_list[l] =
                        needed_lines[line_list[GeometryInfo<dim>::face_to_cell_lines(face, l)]]
                          ->index();
                      orientation[l] = true;
                    }
                  else
                    {
                      face_line_list[l] =
                        needed_lines[inverse_line_list[GeometryInfo<dim>::face_to_cell_lines(face,
                                                                                             l)]]
                          ->index();
                      orientation[l] = false;
                    }
                ...
                internal::TriangulationImplementation::TriaObject<2> quad(
                  face_line_list[0], face_line_list[1], face_line_list[2], face_line_list[3]);
                ...
                const internal::TriangulationImplementation::TriaObject<2> test_quad_1(
                  quad.face(2),
                  quad.face(3),
                  quad.face(0),
          ...
          for (unsigned int c = 0; c < cells.size(); ++c, ++cell)
            {
              ...
              // these iterators
              cell->set(
                internal::TriangulationImplementation ::TriaObject<3>(face_iterator[0]->index(),
              ...
              std::multimap<unsigned int, std::pair<unsigned int, unsigned int>> cell_to_face_lines;
              for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
              ...
              for (; map_iter != cell_to_face_lines.end(); ++map_iter)
                {
                  ...
                  // orientation;
                  Assert(
                    face_iterator[face1]->line(GeometryInfo<dim>::standard_to_real_face_line(
                      line1, face_orientation[face1], face_flip[face1], face_rotation[face1])) ==
                      face_iterator[face2]->line(GeometryInfo<dim>::standard_to_real_face_line(
                        line2, face_orientation[face2], face_flip[face2], face_rotation[face2])),
        ...
        for (typename Triangulation<dim, spacedim>::quad_iterator quad = triangulation.begin_quad();
             ...
             ++quad)
          {
            ...
            AssertThrow((n_adj_cells >= 1) && (n_adj_cells <= 2), ExcInternalError());

            // if only one cell: quad is at boundary -> give it the boundary
            // indicator zero by default
            quad->set_boundary_id_internal((n_adj_cells == 1) ? 0 :
                                                                numbers::internal_face_boundary_id);
        ...
        std::vector<CellData<2>>::const_iterator boundary_quad = subcelldata.boundary_quads.begin();
        ...
        for (; boundary_quad != end_boundary_quad; ++boundary_quad)
          {
                ...
                if (needed_lines.find(line_vertices) != needed_lines.end())
                  ...
                  // in reverse direction
                  {
                    ...
                    if (needed_lines.find(line_vertices) != needed_lines.end())
                      ...
                      // not exist
                      AssertThrow(false,
                                  ExcLineInexistant(line_vertices.first, line_vertices.second));
                  }
              }
            ...
            internal::TriangulationImplementation::TriaObject<2> quad_compare_1(
              line[0]->index(), line[1]->index(), line[2]->index(), line[3]->index());
            internal::TriangulationImplementation::TriaObject<2> quad_compare_2(
              line[2]->index(), line[3]->index(), line[0]->index(), line[1]->index());
            ...
            typename Triangulation<dim, spacedim>::line_iterator line_counterclock[4];
            for (unsigned int i = 0; i < 4; ++i)
            ...
            while ((not_found_quad_1 = (needed_quads.find(quad_compare_1) == needed_quads.end())) &&
                   (needed_quads.find(quad_compare_2) == needed_quads.end()) && (n_rotations < 4))
              {
            ...
            if (adjacent_cells[cell->quad(face)->index()][0] == cell)
              ...
              // this one
              {
                if (adjacent_cells[cell->quad(face)->index()].size() == 2)
                  // there is another
                  // adjacent cell
                  cell->set_neighbor(face, adjacent_cells[cell->quad(face)->index()][1]);
              }
            // first adjacent cell is not this
            // one, -> it must be the neighbor
            // we are looking for
            else
              cell->set_neighbor(face, adjacent_cells[cell->quad(face)->index()][0]);
      }
       ...
       */
      template <int spacedim>
      static void delete_children(Triangulation<1, spacedim> &                        triangulation,
                                  ...
                                  std::vector<unsigned int> &)
      {
        ...
        if (cell->neighbor(0).state() == IteratorState::valid)
          if (cell->neighbor(0)->has_children())
            {
              typename Triangulation<dim, spacedim>::cell_iterator neighbor = cell->neighbor(0);
              ...
              while (true)
                {
                  Assert(neighbor->neighbor(1) == cell->child(0), ExcInternalError());
        ...
        if (cell->neighbor(1).state() == IteratorState::valid)
          if (cell->neighbor(1)->has_children())
            {
              typename Triangulation<dim, spacedim>::cell_iterator neighbor = cell->neighbor(1);
              Assert(neighbor->level() == cell->level(), ExcInternalError());
              ...
              while (true)
                {
                  Assert(neighbor->neighbor(0) == cell->child(1), ExcInternalError());
      ...
      template <int spacedim>
      static void delete_children(Triangulation<2, spacedim> &                        triangulation,
                                  ...
                                  std::vector<unsigned int> &)
      {
        ...
        // may be deleted
        std::vector<typename Triangulation<dim, spacedim>::line_iterator> lines_to_delete(0);
        ...
        for (unsigned int c = 0; c < cell->n_children(); ++c)
          {
            typename Triangulation<dim, spacedim>::cell_iterator child = cell->child(c);
            for (unsigned int l = 0; l < GeometryInfo<dim>::lines_per_cell; ++l)
        ...
        for (unsigned int line_no = 0; line_no < GeometryInfo<dim>::lines_per_cell; ++line_no)
          {
            typename Triangulation<dim, spacedim>::line_iterator line = cell->line(line_no);
                ...
                if (line_cell_count[line->child_index(0)] == 0)
                  {
                    for (unsigned int c = 0; c < 2; ++c)
                      Assert(!line->child(c)->has_children(), ExcInternalError());
      ...
      template <int spacedim>
      static void delete_children(Triangulation<3, spacedim> &                        triangulation,
                                  ...
                                  std::vector<unsigned int> &quad_cell_count)
      {
        ...
        // may be deleted
        std::vector<typename Triangulation<dim, spacedim>::line_iterator> lines_to_delete(0);
        std::vector<typename Triangulation<dim, spacedim>::quad_iterator> quads_to_delete(0);
        ...
        for (unsigned int quad_no = 0; quad_no < GeometryInfo<dim>::faces_per_cell; ++quad_no)
          {
            typename Triangulation<dim, spacedim>::quad_iterator quad = cell->face(quad_no);

            Assert((GeometryInfo<dim>::face_refinement_case(ref_case, quad_no) &&
                    quad->has_children()) ||
                ...
                case RefinementCase<dim - 1>::cut_y:
                  {
                          ...
                          if (quad_cell_count[quad->child(c)->child_index(0)] == 0)
                            {
                              ...
                              Assert(quad->refinement_case() + quad->child(c)->refinement_case() ==
                    ...
                    if (number_of_child_refinements > 0 &&
                        deleted_grandchildren == number_of_child_refinements)
                      {
                        ...
                        triangulation
                          .vertices_used[middle_vertex_index<dim, spacedim>(middle_line)] = false;
                          ...
                          if (quad->refinement_case() == RefinementCase<2>::cut_x)
                            {
                              ...
                              for (unsigned int l = 0; l < triangulation.levels.size(); ++l)
                                for (unsigned int h = 0;
                                     h < triangulation.levels[l]->cells.cells.size();
                                     ++h)
                                  for (unsigned int q = 0; q < GeometryInfo<dim>::faces_per_cell;
                                       ++q)
                                    {
                                      const int index =
                                        triangulation.levels[l]->cells.cells[h].face(q);
                                      if (index == switch_1_index)
                                        triangulation.levels[l]->cells.cells[h].set_face(
                                          q, switch_2_index);
                              ...
                              switch_1->set(internal::TriangulationImplementation::TriaObject<2>(
                              ...
                              switch_2->set(internal::TriangulationImplementation::TriaObject<2>(
                                switch_1_lines[0],
                                switch_1_lines[1],
                                switch_1_lines[2],
                                switch_1_lines[3]));
        ...
        for (unsigned int line_no = 0; line_no < GeometryInfo<dim>::lines_per_cell; ++line_no)
          {
            ...
            Assert((GeometryInfo<dim>::line_refinement_case(ref_case, line_no) &&
                    line->has_children()) ||
                ...
                if (cell->user_flag_set())
                  {
                    ...
                    cell->clear_user_flag();
                    triangulation.vertices[next_unused_vertex] = cell->center(true, true);
                  }
              }
            else
              {
        ...
        if (ref_case == RefinementCase<dim>::cut_xy)
          {
            ...
            new_lines[8]->set(internal::TriangulationImplementation::TriaObject<1>(
              new_vertices[6], new_vertices[8]));
            new_lines[9]->set(internal::TriangulationImplementation::TriaObject<1>(
              new_vertices[8], new_vertices[7]));
            new_lines[10]->set(internal::TriangulationImplementation::TriaObject<1>(
              new_vertices[4], new_vertices[8]));
            new_lines[11]->set(internal::TriangulationImplementation::TriaObject<1>(
        ...
        else if (ref_case == RefinementCase<dim>::cut_x)
          {
            ...
            new_lines[6]->set(internal::TriangulationImplementation::TriaObject<1>(
        ...
        if (ref_case == RefinementCase<dim>::cut_xy)
          {
            ...
            //   .-4-.-5-.
            subcells[0]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[0]->index(),
                                                                   new_lines[8]->index(),
                                                                   new_lines[4]->index(),
                                                                   new_lines[10]->index()));
            subcells[1]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[8]->index(),
                                                                   new_lines[2]->index(),
                                                                   new_lines[5]->index(),
                                                                   new_lines[11]->index()));
            subcells[2]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[1]->index(),
                                                                   new_lines[9]->index(),
                                                                   new_lines[10]->index(),
                                                                   new_lines[6]->index()));
            subcells[3]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[9]->index(),
                                                                   new_lines[3]->index(),
                                                                   new_lines[11]->index(),
                                                                   new_lines[7]->index()));
          }
        else if (ref_case == RefinementCase<dim>::cut_x)
          {
            ...
            subcells[0]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[0]->index(),
                                                                   ...
                                                                   new_lines[4]->index()));
            subcells[1]->set(
              internal::TriangulationImplementation::TriaObject<2>(new_lines[6]->index(),
                                                                   new_lines[1]->index(),
                                                                   new_lines[3]->index(),
                                                                   new_lines[5]->index()));
          }
                           ...
                           const bool /*check_for_distorted_cells*/)
      {
        ...
        for (int level = triangulation.levels.size() - 2; level >= 0; --level)
          {
            ...
            triangulation.levels[level + 1]->reserve_space(
              used_cells + GeometryInfo<1>::max_children_per_cell * flagged_cells, 1, spacedim);
        ...
        needed_vertices +=
          std::count_if(triangulation.vertices_used.begin(),
        ...
        for (int level = triangulation.levels.size() - 2; level >= 0; --level)
          {
            ...
            typename Triangulation<dim, spacedim>::raw_cell_iterator next_unused_cell =
              ...
              if (cell->refine_flag_set())
                {
                  ...
                  typename Triangulation<dim, spacedim>::raw_cell_iterator first_child,
                    second_child;
                  while (next_unused_cell->used() == true)
                    ++next_unused_cell;
                  ...
                  first_child->set(internal::TriangulationImplementation ::TriaObject<1>(
                  ...
                  if (cell->neighbor(0).state() != IteratorState::valid)
                  ...
                  else if (cell->neighbor(0)->active())
                    {
                      ...
                      Assert(cell->neighbor(0)->level() <= cell->level(), ExcInternalError());
                      first_child->set_neighbor(0, cell->neighbor(0));
                    }
                  else
                    // left neighbor is refined
                    {
                      ...
                      typename Triangulation<dim, spacedim>::cell_iterator left_neighbor =
                        cell->neighbor(0);
                  ...
                  second_child->set(internal::TriangulationImplementation ::TriaObject<1>(
                  ...
                  if (cell->neighbor(1).state() != IteratorState::valid)
                    ...
                    {
                      Assert(cell->neighbor(1)->level() <= cell->level(), ExcInternalError());
                      second_child->set_neighbor(1, cell->neighbor(1));
                    }
                  else
                    // right neighbor is refined same as above
                    {
                      ...
                      typename Triangulation<dim, spacedim>::cell_iterator right_neighbor =
                        cell->neighbor(1);
                           ...
                           const bool                  check_for_distorted_cells)
      {
        ...
        for (int level = triangulation.levels.size() - 2; level >= 0; --level)
          {
            ...
            triangulation.levels[level + 1]->reserve_space(used_cells + needed_cells, 2, spacedim);

            // reserve space for needed_cells new quads on the next
            // higher level
            triangulation.levels[level + 1]->cells.reserve_space(needed_cells, 0);
          }
        ...
        if (true)
          {
            ...
            typename Triangulation<dim, spacedim>::raw_line_iterator next_unused_line =
              triangulation.begin_raw_line();
              ...
              if (line->user_flag_set())
                {
                  ...
                  for (; next_unused_line != endl; ++next_unused_line)
                    if (!next_unused_line->used() && !(++next_unused_line)->used())
                      {
                  ...
                  const typename Triangulation<dim, spacedim>::raw_line_iterator children[2] = {
                  ...
                  children[0]->set(internal::TriangulationImplementation ::TriaObject<1>(
                    line->vertex_index(0), next_unused_vertex));
                  children[1]->set(internal::TriangulationImplementation ::TriaObject<1>(
        ...
        typename Triangulation<2, spacedim>::DistortedCellList cells_with_distorted_children;
              ...
              if (cell->refine_flag_set())
                {
                  ...
                  // information
                  create_children(
                    triangulation, next_unused_vertex, next_unused_line, next_unused_cell, cell);
                           ...
                           const bool                  check_for_distorted_cells)
      {
        ...
        for (int level = triangulation.levels.size() - 2; level >= 0; --level)
          {
            ...
            triangulation.levels[level + 1]->reserve_space(used_cells + new_cells, 3, spacedim);
            ...
            if (quad->user_index())
              {
                ...
                if (quad->has_children())
                  {
                    Assert(quad->refinement_case() == RefinementCase<dim - 1>::isotropic_refinement,
                           ExcInternalError());
        ...
        triangulation.faces->lines.reserve_space(needed_lines_pair, needed_lines_single);
        // reserve space for needed_quads new quads stored in pairs
        triangulation.faces->quads.reserve_space(needed_quads_pair, needed_quads_single);
          ...
          if (!cell->refine_flag_set())
            for (unsigned int line = 0; line < GeometryInfo<dim>::lines_per_cell; ++line)
              if (cell->line(line)->has_children())
                for (unsigned int c = 0; c < 2; ++c)
                  Assert(cell->line(line)->child(c)->user_flag_set() == false, ExcInternalError());
#endif
              ...
              if (line->user_flag_set())
                {
                  ...
                  // vector)
                  next_unused_line =
                    triangulation.faces->lines.next_free_pair_object(triangulation);
                  Assert(next_unused_line.state() == IteratorState::valid, ExcInternalError());
        ...
        for (unsigned int loop = 0; loop < 2; ++loop)
          {
            ...
            typename Triangulation<dim, spacedim>::quad_iterator quad = triangulation.begin_quad(),
                                                                 endq = triangulation.end_quad();
            typename Triangulation<dim, spacedim>::raw_line_iterator next_unused_line =
              triangulation.begin_raw_line();
            typename Triangulation<dim, spacedim>::raw_quad_iterator next_unused_quad =
              triangulation.begin_raw_quad();
                ...
                if (quad->user_index())
                  {
                    ...
                    new_line->set(internal::TriangulationImplementation::TriaObject<1>(
                    ...
                    next_unused_quad =
                      triangulation.faces->quads.next_free_pair_object(triangulation);
                    ...
                    if (aniso_quad_ref_case == RefinementCase<dim - 1>::cut_x)
                      {
                        new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                          quad->line_index(0),
                          new_line->index(),
                          quad->line(2)->child(index[0][quad->line_orientation(2)])->index(),
                          quad->line(3)->child(index[0][quad->line_orientation(3)])->index()));
                        new_quads[1]->set(internal::TriangulationImplementation ::TriaObject<2>(
                          new_line->index(),
                          quad->line_index(1),
                    ...
                    if (quad->refinement_case() == RefinementCase<dim - 1>::cut_xy)
                      {
                        ...
                        if (aniso_quad_ref_case == RefinementCase<dim - 1>::cut_x)
                          ...
                          {
                            Assert(aniso_quad_ref_case == RefinementCase<dim - 1>::cut_y,
                                   ExcInternalError());
                        ...
                        if (old_child[0]->index() + 1 != old_child[1]->index())
                          {
                            ...
                            // lines
                            for (unsigned int q = 0; q < triangulation.faces->quads.cells.size();
                                 ++q)
                              for (unsigned int l = 0; l < GeometryInfo<dim>::lines_per_face; ++l)
                                {
                            ...
                            for (unsigned int i = 0; i < 2; ++i)
                              {
                                ...
                                new_child[i]->set(
                                  internal::TriangulationImplementation::TriaObject<1>(
                                    old_child[i]->vertex_index(0), old_child[i]->vertex_index(1)));
                        ...
                        if (aniso_quad_ref_case == RefinementCase<dim - 1>::cut_x)
                          {
                            ...
                            // now we have to copy all information of
                            // the two quads
                            const unsigned int switch_1_lines[4] = {switch_1->line_index(0),
                            ...
                            const unsigned int       switch_1_user_index  = switch_1->user_index();
                            const bool               switch_1_user_flag = switch_1->user_flag_set();
                            const RefinementCase<dim - 1> switch_1_refinement_case =
                              switch_1->refinement_case();
                            const int switch_1_first_child_pair =
                              (switch_1_refinement_case ? switch_1->child_index(0) : -1);
                            const int switch_1_second_child_pair =
                              (switch_1_refinement_case == RefinementCase<dim - 1>::cut_xy ?
                ...
                if (quad->user_flag_set())
                  {
                    ...
                    if (quad_ref_case == RefinementCase<dim - 1>::cut_x ||
                        quad_ref_case == RefinementCase<dim - 1>::cut_y)
                      {
                        ...
                        quad->child(0)->set_user_index(RefinementCase<dim - 1>::cut_xy -
                                                       quad_ref_case);
                        quad->child(1)->set_user_index(RefinementCase<dim - 1>::cut_xy -
                                                       quad_ref_case);
                        ...
                        if (!middle_line->has_children())
                          {
                            ...
                            triangulation.vertices[next_unused_vertex] = middle_line->center(true);
                            triangulation.vertices_used[next_unused_vertex] = true;
                            ...
                            children[0]->set(internal::TriangulationImplementation::TriaObject<1>(
                              middle_line->vertex_index(0), next_unused_vertex));
                            children[1]->set(internal::TriangulationImplementation::TriaObject<1>(
                              next_unused_vertex, middle_line->vertex_index(1)));
                    ...
                    // isotropic refinement
                    Assert(quad_ref_case == RefinementCase<dim - 1>::no_refinement,
                           ExcInternalError());
                    ...
                    // implementation
                    triangulation.vertices[next_unused_vertex]      = quad->center(true, true);
                    triangulation.vertices_used[next_unused_vertex] = true;
                    ...
                    new_lines[0]->set(internal::TriangulationImplementation::TriaObject<1>(
                      vertex_indices[2], vertex_indices[4]));
                    new_lines[1]->set(internal::TriangulationImplementation::TriaObject<1>(
                      vertex_indices[4], vertex_indices[3]));
                    new_lines[2]->set(internal::TriangulationImplementation::TriaObject<1>(
                      vertex_indices[0], vertex_indices[4]));
                    new_lines[3]->set(internal::TriangulationImplementation::TriaObject<1>(
                    ...
                    new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                      line_indices[0], line_indices[8], line_indices[4], line_indices[10]));
                    ...
                    new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                      line_indices[0], line_indices[8], line_indices[4], line_indices[10]));
                    new_quads[1]->set(internal::TriangulationImplementation ::TriaObject<2>(
                      line_indices[8], line_indices[2], line_indices[5], line_indices[11]));
                    new_quads[2]->set(internal::TriangulationImplementation ::TriaObject<2>(
                      line_indices[1], line_indices[9], line_indices[10], line_indices[6]));
                    new_quads[3]->set(internal::TriangulationImplementation ::TriaObject<2>(
                      line_indices[9], line_indices[3], line_indices[11], line_indices[7]));
                    for (unsigned int i = 0; i < 4; ++i)
                      {
                        new_quads[i]->set_used_flag();
                        new_quads[i]->clear_user_flag();
        ...
        typename Triangulation<3, spacedim>::DistortedCellList cells_with_distorted_children;
        ...
        for (unsigned int level = 0; level != triangulation.levels.size() - 1; ++level)
          {
            ...
            typename Triangulation<dim, spacedim>::raw_hex_iterator next_unused_hex =
              ...
              if (hex->refine_flag_set())
                {
                  ...
                  std::vector<typename Triangulation<dim, spacedim>::raw_line_iterator> new_lines(
                    n_new_lines);
                  for (unsigned int i = 0; i < n_new_lines; ++i)
                    {
                      new_lines[i] =
                        triangulation.faces->lines.next_free_single_object(triangulation);
                  ...
                  std::vector<typename Triangulation<dim, spacedim>::raw_quad_iterator> new_quads(
                    n_new_quads);
                  for (unsigned int i = 0; i < n_new_quads; ++i)
                    {
                      new_quads[i] =
                        triangulation.faces->quads.next_free_single_object(triangulation);
                  ...
                  // and initialize them.
                  std::vector<typename Triangulation<dim, spacedim>::raw_hex_iterator> new_hexes(
                    n_new_hexes);
                  for (unsigned int i = 0; i < n_new_hexes; ++i)
                    {
                      if (i % 2 == 0)
                        next_unused_hex = triangulation.levels[level + 1]->cells.next_free_hex(
                      ...
                      case RefinementCase<dim>::cut_x:
                        {
                          ...
                          const typename Triangulation<dim, spacedim>::raw_line_iterator lines[4] =
                            {
                              hex->face(2)->child(0)->line(
                                (hex->face(2)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3), // 0
                              hex->face(3)->child(0)->line(
                                (hex->face(3)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3), // 1
                              hex->face(4)->child(0)->line(
                                (hex->face(4)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3), // 2
                              hex->face(5)->child(0)->line(
                                (hex->face(5)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3) // 3
                            };
                          ...
                          // indicated above
                          new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[0], line_indices[1], line_indices[2], line_indices[3]));
                          ...
                          const int quad_indices[11] = {
                            ...
                            hex->face(2)->child_index(
                              child_at_origin[hex->face(2)->refinement_case() - 1][f_fl[2]]
                                             [f_ro[2]]), // 3
                            hex->face(2)->child_index(
                              1 - child_at_origin[hex->face(2)->refinement_case() - 1][f_fl[2]]
                                                 [f_ro[2]]),

                            hex->face(3)->child_index(
                              child_at_origin[hex->face(3)->refinement_case() - 1][f_fl[3]]
                                             [f_ro[3]]), // 5
                            hex->face(3)->child_index(
                              1 - child_at_origin[hex->face(3)->refinement_case() - 1][f_fl[3]]
                                                 [f_ro[3]]),

                            hex->face(4)->child_index(
                              child_at_origin[hex->face(4)->refinement_case() - 1][f_fl[4]]
                                             [f_ro[4]]), // 7
                            hex->face(4)->child_index(
                              1 - child_at_origin[hex->face(4)->refinement_case() - 1][f_fl[4]]
                                                 [f_ro[4]]),

                            hex->face(5)->child_index(
                              child_at_origin[hex->face(5)->refinement_case() - 1][f_fl[5]]
                                             [f_ro[5]]), // 9
                            hex->face(5)->child_index(
                              1 - child_at_origin[hex->face(5)->refinement_case() - 1][f_fl[5]]
                                                 [f_ro[5]])

                          };

                          new_hexes[0]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[1],
                                                                                  quad_indices[0],
                                                                                  quad_indices[3],
                                                                                  quad_indices[5],
                                                                                  quad_indices[7],
                                                                                  quad_indices[9]));
                          new_hexes[1]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[0],
                            quad_indices[2],
                            quad_indices[4],
                            quad_indices[6],
                      ...
                      case RefinementCase<dim>::cut_y:
                        {
                          ...
                          const typename Triangulation<dim, spacedim>::raw_line_iterator lines[4] =
                            {
                              hex->face(0)->child(0)->line(
                                (hex->face(0)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3), // 0
                              hex->face(1)->child(0)->line(
                                (hex->face(1)->refinement_case() == RefinementCase<2>::cut_x) ?
                                  1 :
                                  3), // 1
                              hex->face(4)->child(0)->line(
                                (hex->face(4)->refinement_case() == RefinementCase<2>::cut_x) ?
                          ...
                          // indicated above
                          new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[2], line_indices[3], line_indices[0], line_indices[1]));
                          ...
                          const int quad_indices[11] = {
                            ...
                            hex->face(0)->child_index(
                              child_at_origin[hex->face(0)->refinement_case() - 1][f_fl[0]]
                                             [f_ro[0]]), // 1
                            hex->face(0)->child_index(
                              1 - child_at_origin[hex->face(0)->refinement_case() - 1][f_fl[0]]
                                                 [f_ro[0]]),

                            hex->face(1)->child_index(
                              child_at_origin[hex->face(1)->refinement_case() - 1][f_fl[1]]
                                             [f_ro[1]]), // 3
                            hex->face(1)->child_index(
                              1 - child_at_origin[hex->face(1)->refinement_case() - 1][f_fl[1]]
                                                 [f_ro[1]]),
                      ...
                      case RefinementCase<dim>::cut_xy:
                        {
                          ...
                          const typename Triangulation<dim,
                            ...
                            hex->face(4)
                              ->isotropic_child(GeometryInfo<dim>::standard_to_real_face_vertex(
                          ...
                          new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[2], line_indices[12], line_indices[4], line_indices[8]));
                          new_quads[1]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[12], line_indices[3], line_indices[5], line_indices[9]));
                          new_quads[2]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[6], line_indices[10], line_indices[0], line_indices[12]));
                          new_quads[3]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[7], line_indices[11], line_indices[12], line_indices[1]));
                          ...
                          new_hexes[0]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[4],
                            quad_indices[0],
                            quad_indices[8],
                            quad_indices[2],
                          ...
                          new_hexes[1]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            ...
                            quad_indices[13],
                            quad_indices[17]));
                          new_hexes[2]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[5],
                            quad_indices[1],
                            quad_indices[2],
                            quad_indices[10],
                            quad_indices[14],
                            quad_indices[18]));
                          new_hexes[3]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[1],
                            quad_indices[7],
                            quad_indices[3],
                            quad_indices[11],
                      ...
                      case RefinementCase<dim>::cut_xz:
                        {
                          ...
                          new_quads[0]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[0], line_indices[12], line_indices[6], line_indices[10]));
                          new_quads[1]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[12], line_indices[1], line_indices[7], line_indices[11]));
                          new_quads[2]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[4], line_indices[8], line_indices[2], line_indices[12]));
                          new_quads[3]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[5], line_indices[9], line_indices[12], line_indices[3]));
                          ...
                          // *---*---*
                          new_hexes[0]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[4],
                                                                                  quad_indices[2],
                                                                                  quad_indices[8],
                                                                                  quad_indices[12],
                                                                                  quad_indices[16],
                          ...
                          new_hexes[1]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            ...
                            quad_indices[18]));
                          new_hexes[2]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[2],
                                                                                  quad_indices[6],
                                                                                  quad_indices[10],
                                                                                  quad_indices[14],
                                                                                  quad_indices[17],
                      ...
                      case RefinementCase<dim>::cut_yz:
                        {
                          ...
                          new_hexes[0]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[4],
                                                                                  ...
                                                                                  quad_indices[0]));
                          new_hexes[1]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[5],
                                                                                  quad_indices[9],
                                                                                  quad_indices[2],
                                                                                  quad_indices[14],
                                                                                  quad_indices[17],
                      ...
                      case RefinementCase<dim>::cut_xyz:
                        {
                          ...
                          // object.
                          triangulation.vertices[next_unused_vertex] = hex->center(true, true);
                          ...
                          new_lines[4]->set(internal::TriangulationImplementation::TriaObject<1>(
                            vertex_indices[4], vertex_indices[6]));
                          new_lines[5]->set(internal::TriangulationImplementation::TriaObject<1>(
                          ...
                          // this, construct lists of line_indices and
                          // line orientations later on
                          const typename Triangulation<dim, spacedim>::raw_line_iterator lines[30] =
                            {
                              hex->face(0)
                                ->isotropic_child(GeometryInfo<dim>::standard_to_real_face_vertex(
                                  0, f_or[0], f_fl[0], f_ro[0]))
                          ...
                          new_quads[3]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            ...
                            line_indices[25],
                            line_indices[21]));
                          new_quads[4]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[18], line_indices[26], line_indices[0], line_indices[28]));
                          new_quads[5]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[26], line_indices[22], line_indices[1], line_indices[29]));
                          new_quads[6]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[19], line_indices[27], line_indices[28], line_indices[4]));
                          new_quads[7]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[27], line_indices[23], line_indices[29], line_indices[5]));
                          new_quads[8]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[2], line_indices[24], line_indices[8], line_indices[26]));
                          new_quads[9]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[24], line_indices[6], line_indices[9], line_indices[27]));
                          new_quads[10]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[3], line_indices[25], line_indices[26], line_indices[12]));
                          new_quads[11]->set(internal::TriangulationImplementation ::TriaObject<2>(
                            line_indices[25], line_indices[7], line_indices[27], line_indices[13]));
                          ...
                          // bottom children
                          new_hexes[0]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[12],
                                                                                  quad_indices[0],
                                                                                  quad_indices[20],
                                                                                  quad_indices[4],
                                                                                  quad_indices[28],
                                                                                  quad_indices[8]));
                          new_hexes[1]->set(
                            internal::TriangulationImplementation ::TriaObject<3>(quad_indices[0],
                                                                                  quad_indices[16],
                                                                                  quad_indices[22],
                                                                                  quad_indices[6],
                                                                                  quad_indices[29],
                          ...
                          // top children
                          new_hexes[4]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[14],
                            quad_indices[2],
                            quad_indices[21],
                            quad_indices[5],
                            quad_indices[8],
                            quad_indices[32]));
                          new_hexes[5]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[2],
                            quad_indices[18],
                            quad_indices[23],
                            quad_indices[7],
                            quad_indices[9],
                            quad_indices[33]));
                          new_hexes[6]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[15],
                            quad_indices[3],
                            quad_indices[5],
                            quad_indices[25],
                            quad_indices[10],
                            quad_indices[34]));
                          new_hexes[7]->set(internal::TriangulationImplementation ::TriaObject<3>(
                            quad_indices[3],
                            quad_indices[19],
                            quad_indices[7],
                            quad_indices[27],
                  ...
                  for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
                    for (unsigned int s = 0;
                         s < std::max(GeometryInfo<dim - 1>::n_children(
                                        GeometryInfo<dim>::face_refinement_case(ref_case, f)),
                                      1U);
                         ++s)
                      {
                        const unsigned int current_child = GeometryInfo<dim>::child_cell_on_face(
                          ref_case,
                          f,
                          s,
                          f_or[f],
        ...
        do
          {
                               ...
                               RefinementCase<1>::no_refinement))
                            {
                              ...
                              for (unsigned int l = 0; l < GeometryInfo<dim>::lines_per_cell; ++l)
                                if (GeometryInfo<dim>::line_refinement_case(
                                      cell->refine_flag_set(), line) == RefinementCase<1>::cut_x)
            ...
            for (typename Triangulation<dim, spacedim>::cell_iterator cell = triangulation.last();
                 cell != triangulation.end();
                 --cell)
              {
                if (cell->user_flag_set())
        ...
        for (unsigned int n = 0; n < GeometryInfo<dim>::faces_per_cell; ++n)
          {
            ...
            const unsigned int n_subfaces = GeometryInfo<dim - 1>::n_children(face_ref_case);
            ...
            for (unsigned int c = 0; c < n_subfaces; ++c)
              {
                const typename Triangulation<dim, spacedim>::cell_iterator child =
                  cell->child(GeometryInfo<dim>::child_cell_on_face(ref_case, n, c));

                const typename Triangulation<dim, spacedim>::cell_iterator child_neighbor =
                  child->neighbor(n);
                if (!child->neighbor_is_coarser(n))
                  ...
                  // not a coarser one.
                  if ((child_neighbor->has_children() && !child_neighbor->user_flag_set()) ||
...
template <int dim, int spacedim>
Triangulation<dim, spacedim>::Triangulation(Triangulation<dim, spacedim> &&tria) noexcept :
...
Triangulation<dim, spacedim>::~Triangulation()
{
  ...
  AssertNothrow((dim == 1) || (vertex_to_boundary_id_map_1d == nullptr), ExcInternalError());

  // the vertex_to_manifold_id_map_1d field should be also unused
  // except in 1d. check this as well
  AssertNothrow((dim == 1) || (vertex_to_manifold_id_map_1d == nullptr), ExcInternalError());
}
...
Triangulation<dim, spacedim>::set_all_manifold_ids(const types::manifold_id m_number)
{
  ...
  typename Triangulation<dim, spacedim>::active_cell_iterator cell = this->begin_active(),
                                                              endc = this->end();
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::copy_triangulation(const Triangulation<dim, spacedim> &other_tria)
{
  ...
  if (dim > 1)
    faces = std_cxx14::make_unique<internal::TriangulationImplementation::TriaFaces<dim>>(
      *other_tria.faces);
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::create_triangulation(const std::vector<Point<spacedim>> &v,
                                                   const std::vector<CellData<dim>> &  cells,
                                                   const SubCellData &                 subcelldata)
{
  Assert((vertices.size() == 0) && (levels.size() == 0) && (faces == nullptr),
  ...
  if (dim < spacedim)
    {
      Table<2, bool> correct(GeometryInfo<dim>::faces_per_cell, GeometryInfo<dim>::faces_per_cell);
      switch (dim)
        {
          case 1:
            {
                      ...
                      if (neighbor->user_flag_set())
                        {
                          ...
                          Assert(!(correct(i, j) ^
                                   (neighbor->direction_flag() == (*cell)->direction_flag())),
...
Triangulation<dim, spacedim>::save_refine_flags(std::ostream &out) const
{
  std::vector<bool> v;
  save_refine_flags(v);
  write_bool_vector(mn_tria_refine_flags_begin, v, mn_tria_refine_flags_end, out);
}
  ...
  for (; cell != endc; ++cell)
    {
      ...
      for (unsigned int j = 0; j < dim; ++j, ++i)
        if (*i == true)
          ref_case += 1 << j;
      Assert(ref_case < RefinementCase<dim>::isotropic_refinement + 1, ExcGridReadError());
      if (ref_case > 0)
...
Triangulation<dim, spacedim>::save_coarsen_flags(std::ostream &out) const
{
  ...
  write_bool_vector(mn_tria_coarsen_flags_begin, v, mn_tria_coarsen_flags_end, out);
}
...
{
  std::vector<bool> v;
  read_bool_vector(mn_tria_coarsen_flags_begin, v, mn_tria_coarsen_flags_end, in);
...
namespace
{
  void clear_user_flags_line(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<1>>> &levels,
  ...
  clear_user_flags_line(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<dim>>> &,
...
namespace
{
  void clear_user_flags_quad(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<1>>> &,
    internal::TriangulationImplementation::TriaFaces<1> *)
  {
    // nothing to do in 1d
  }
  ...
  void clear_user_flags_quad(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<2>>> &levels,
...
namespace
{
  ...
  void clear_user_flags_hex(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<2>>> &,
    internal::TriangulationImplementation::TriaFaces<2> *)
  {
    // nothing to do in 2d
  }
  ...
  void clear_user_flags_hex(
    std::vector<std::unique_ptr<internal::TriangulationImplementation::TriaLevel<3>>> &levels,
...
Triangulation<dim, spacedim>::load_user_flags(const std::vector<bool> &v)
{
  ...
  if (dim >= 2)
    {
      tmp.clear();
      tmp.insert(tmp.end(), v.begin() + n_lines(), v.begin() + n_lines() + n_quads());
  ...
  if (dim >= 3)
    {
      tmp.clear();
      tmp.insert(
        tmp.end(), v.begin() + n_lines() + n_quads(), v.begin() + n_lines() + n_quads() + n_hexs());
...
Triangulation<dim, spacedim>::save_user_flags_line(std::ostream &out) const
{
  ...
  write_bool_vector(mn_tria_line_user_flags_begin, v, mn_tria_line_user_flags_end, out);
}
...
{
  std::vector<bool> v;
  read_bool_vector(mn_tria_line_user_flags_begin, v, mn_tria_line_user_flags_end, in);
...
namespace
{
  ...
  template <int structdim, int dim, int spacedim>
  void
  clear_user_flag(const TriaIterator<InvalidAccessor<structdim, dim, spacedim>> &)
  {
    Assert(false, ExcInternalError());
  }
} // namespace
...
Triangulation<dim, spacedim>::save_user_flags_quad(std::ostream &out) const
{
  ...
  write_bool_vector(mn_tria_quad_user_flags_begin, v, mn_tria_quad_user_flags_end, out);
}
...
{
  std::vector<bool> v;
  read_bool_vector(mn_tria_quad_user_flags_begin, v, mn_tria_quad_user_flags_end, in);
...
Triangulation<dim, spacedim>::save_user_flags_hex(std::ostream &out) const
{
  ...
  write_bool_vector(mn_tria_hex_user_flags_begin, v, mn_tria_hex_user_flags_end, out);
}
...
{
  std::vector<bool> v;
  read_bool_vector(mn_tria_hex_user_flags_begin, v, mn_tria_hex_user_flags_end, in);
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_indices(std::vector<unsigned int> &v) const
{
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_indices(const std::vector<unsigned int> &v)
{
  Assert(v.size() == n_lines() + n_quads() + n_hexs(), ExcInternalError());
  std::vector<unsigned int> tmp;
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_indices_line(std::vector<unsigned int> &v) const
{
  v.resize(n_lines(), 0);
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_indices_line(const std::vector<unsigned int> &v)
{
  Assert(v.size() == n_lines(), ExcGridReadError());
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_indices_quad(std::vector<unsigned int> &v) const
{
  v.resize(n_quads(), 0);
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_indices_quad(const std::vector<unsigned int> &v)
{
  Assert(v.size() == n_quads(), ExcGridReadError());
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_indices_hex(std::vector<unsigned int> &v) const
{
  v.resize(n_hexs(), 0);
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_indices_hex(const std::vector<unsigned int> &v)
{
  Assert(v.size() == n_hexs(), ExcGridReadError());
...
namespace
{
  ...
  template <int structdim, int dim, int spacedim>
  void *
  get_user_pointer(const TriaIterator<InvalidAccessor<structdim, dim, spacedim>> &)
  {
    Assert(false, ExcInternalError());
    return nullptr;
  }
  ...
  template <int structdim, int dim, int spacedim>
  void
  set_user_pointer(const TriaIterator<InvalidAccessor<structdim, dim, spacedim>> &, void *)
  {
    Assert(false, ExcInternalError());
  }
} // namespace
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_pointers_line(std::vector<void *> &v) const
{
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_pointers_line(const std::vector<void *> &v)
{
  Assert(v.size() == n_lines(), ExcGridReadError());
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_pointers_quad(std::vector<void *> &v) const
{
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_pointers_quad(const std::vector<void *> &v)
{
  Assert(v.size() == n_quads(), ExcGridReadError());
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::save_user_pointers_hex(std::vector<void *> &v) const
{
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::load_user_pointers_hex(const std::vector<void *> &v)
{
  Assert(v.size() == n_hexs(), ExcGridReadError());
...
Triangulation<dim, spacedim>::last() const
{
  ...
  // this level
  raw_cell_iterator ri(
    const_cast<Triangulation<dim, spacedim> *>(this), level, levels[level]->cells.cells.size() - 1);
...
{
  Assert(level < n_global_levels() || level < levels.size(), ExcInvalidLevel(level));
  return (level >= levels.size() - 1 ? active_cell_iterator(end()) : begin_active(level + 1));
}
...
{
  return IteratorRange<typename Triangulation<dim, spacedim>::cell_iterator>(begin(), end());
}
...
{
  return IteratorRange<typename Triangulation<dim, spacedim>::active_cell_iterator>(begin_active(),
                                                                                    end());
}
...
{
  return IteratorRange<typename Triangulation<dim, spacedim>::cell_iterator>(begin(level),
                                                                             end(level));
}
...
{
  return IteratorRange<typename Triangulation<dim, spacedim>::active_cell_iterator>(
...
Triangulation<dim, spacedim>::end_vertex() const
{
  if (dim == 1)
    ...
    return raw_vertex_iterator(
      const_cast<Triangulation<dim, spacedim> *>(this), -1, numbers::invalid_unsigned_int);
}
  ...
  switch (dim)
    {
      case 1:
        ...
        return raw_line_iterator(const_cast<Triangulation<dim, spacedim> *>(this), level, 0);

      default:
        Assert(level == 0, ExcFacesHaveNoLevel());
        return raw_line_iterator(const_cast<Triangulation<dim, spacedim> *>(this), 0, 0);
    }
}
...
{
  return raw_line_iterator(const_cast<Triangulation<dim, spacedim> *>(this), -1, -1);
}
      ...
      case 2:
        {
          ...
          return raw_quad_iterator(const_cast<Triangulation<dim, spacedim> *>(this), level, 0);
        }
      ...
      case 3:
        {
          ...
          return raw_quad_iterator(const_cast<Triangulation<dim, spacedim> *>(this), 0, 0);
        }
...
{
  return raw_quad_iterator(const_cast<Triangulation<dim, spacedim> *>(this), -1, -1);
}
      ...
      case 3:
        {
          ...
          return raw_hex_iterator(const_cast<Triangulation<dim, spacedim> *>(this), level, 0);
        }
...
{
  return raw_hex_iterator(const_cast<Triangulation<dim, spacedim> *>(this), -1, -1);
}
...
{
  cell_iterator cell = begin(0), endc = (n_levels() > 1 ? begin(1) : cell_iterator(end()));
  ...
  return std::max(
    ...
    static_cast<unsigned int>(*std::max_element(usage_count.begin(), usage_count.end())));
}
...
Triangulation<dim, spacedim>::add_periodicity(
  const std::vector<GridTools::PeriodicFacePair<cell_iterator>> &periodicity_vector)
{
  periodic_face_pairs_level_0.insert(
    periodic_face_pairs_level_0.end(), periodicity_vector.begin(), periodicity_vector.end());
      ...
      if (cell->child(0)->coarsen_flag_set())
        {
          ...
          for (unsigned int child = 0; child < cell->n_children(); ++child)
            {
              Assert(cell->child(child)->coarsen_flag_set(), ExcInternalError());
      ...
      if (cell->level() <= static_cast<int>(levels.size() - 2) && cell->user_flag_set())
        {
          ...
          internal::TriangulationImplementation::Implementation ::delete_children(
            *this, cell, line_cell_count, quad_cell_count);
        }
              ...
              if (cell->refine_flag_set())
                for (unsigned int vertex = 0; vertex < GeometryInfo<dim>::vertices_per_cell;
                  ...
                  vertex_level[cell->vertex_index(vertex)] =
                    std::max(vertex_level[cell->vertex_index(vertex)], cell->level() + 1);
              else if (!cell->coarsen_flag_set())
                for (unsigned int vertex = 0; vertex < GeometryInfo<dim>::vertices_per_cell;
                     ++vertex)
                  vertex_level[cell->vertex_index(vertex)] =
                    std::max(vertex_level[cell->vertex_index(vertex)], cell->level());
              else
                {
            ...
            if (cell->refine_flag_set() == false)
              {
                for (unsigned int vertex = 0; vertex < GeometryInfo<dim>::vertices_per_cell;
                     ++vertex)
                  if (vertex_level[cell->vertex_index(vertex)] >= cell->level() + 1)
                    {
                      // remove coarsen flag...
                      cell->clear_coarsen_flag();
                      ...
                      if (vertex_level[cell->vertex_index(vertex)] > cell->level() + 1)
                        {
                          ...
                          for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_cell; ++v)
                            vertex_level[cell->vertex_index(v)] =
                              std::max(vertex_level[cell->vertex_index(v)], cell->level() + 1);
                        }
      ...
      for (cell = last(); cell != endc; --cell)
        if (cell->user_flag_set())
          ...
          if (internal::TriangulationImplementation::Implementation::
            ...
            for (unsigned int c = 0; c < cell->n_children(); ++c)
              {
                Assert(cell->child(c)->refine_flag_set() == false, ExcInternalError());
...
namespace
{
    ...
    const typename Triangulation<dim, spacedim>::cell_iterator &cell)
  {
    ...
    if (count == n_neighbors ||
        (count >= n_neighbors - 1 && n_neighbors == GeometryInfo<dim>::faces_per_cell))
      {
        for (unsigned int c = 0; c < cell->n_children(); ++c)
          cell->child(c)->clear_coarsen_flag();
  ...
  template <int dim, int spacedim>
  void
  possibly_refine_unrefined_island(const typename Triangulation<dim, spacedim>::cell_iterator &cell,
                                   const bool allow_anisotropic_smoothing)
  {
    ...
    if (allow_anisotropic_smoothing == false)
            ...
            if (unrefined_neighbors > 0)
              for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
                if (!cell->at_boundary(face) &&
                  ...
                  possibly_refine_unrefined_island<dim, spacedim>(cell->neighbor(face),
    ...
    else
      {
        ...
        // direction
        for (unsigned int face_pair = 0; face_pair < GeometryInfo<dim>::faces_per_cell / 2;
             ++face_pair)
          {
...
Triangulation<dim, spacedim>::prepare_coarsening_and_refinement()
{
  ...
  //    coarsen flags on level 1 to avoid level 0 cells being created
  //    by coarsening.
  if (((smooth_grid & coarsest_level_1) || (smooth_grid & patch_level_1)) && n_levels() >= 2)
    {
  ...
  do
    {
      ...
      if (smooth_grid & do_not_produce_unrefined_islands && !(smooth_grid & patch_level_1))
        {
          cell_iterator       cell;
      ...
      //    locally owned
      if (smooth_grid & (eliminate_refined_inner_islands | eliminate_refined_boundary_islands) &&
          !(smooth_grid & patch_level_1))
        {
          ...
          for (cell = begin(); cell != endc; ++cell)
            if (!cell->active() ||
                (cell->active() && cell->refine_flag_set() && cell->is_locally_owned()))
              {
            ...
            if (!cell->active())
              {
                ...
                for (unsigned int i = 0; i < cell->n_children(); ++i)
                  combined_ref_case = combined_ref_case | cell->child(i)->refine_flag_set();
                ...
                for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; ++i)
                  {
                    ...
                    if (cell->neighbor(i).state() == IteratorState::valid &&
                        (GeometryInfo<dim>::face_refinement_case(cell->refine_flag_set(), i) !=
                         RefinementCase<dim - 1>::no_refinement))
                      {
                            ...
                            if (cell->neighbor_is_coarser(i))
                                ...
                                if (dim == 2)
                                  {
                                    if (smooth_grid & allow_anisotropic_smoothing)
                                      changed = cell->neighbor(i)->flag_for_face_refinement(
                                        cell->neighbor_of_coarser_neighbor(i).first,
                                        RefinementCase<dim - 1>::cut_x);
                                    else
                                      {
                                ...
                                else // i.e. if (dim==3)
                                  {
                                    ...
                                    unsigned int refined_along_x = 0, refined_along_y = 0,
                                    ...
                                    if ((this_face_index == cell->neighbor(i)
                                         ...
                                         cell->neighbor(i)->face(nb_indices.first)->child_index(1)))
                                      {
                                        ...
                                        // direction.
                                        RefinementCase<dim - 1> frc = cell->neighbor(i)
                                                                        ->face(nb_indices.first)
                                                                        ->refinement_case();
                                    ...
                                    if ((smooth_grid & allow_anisotropic_smoothing) ||
                                        cell->neighbor(i)->refine_flag_set())
                                      {
                                        if (refined_along_x + to_be_refined_along_x > 1)
                                          changed |= cell->neighbor(i)->flag_for_face_refinement(
                                            nb_indices.first, RefinementCase<dim - 1>::cut_axis(0));
                                        if (refined_along_y + to_be_refined_along_y > 1)
                                          changed |= cell->neighbor(i)->flag_for_face_refinement(
                                            nb_indices.first, RefinementCase<dim - 1>::cut_axis(1));
                                      }
                                    else
                                      {
                                        if (cell->neighbor(i)->refine_flag_set() !=
                            ...
                            else // -> now the neighbor is not coarser
                              {
                                ...
                                if ((face_ref_case == RefinementCase<dim>::cut_x &&
                                     needed_face_ref_case == RefinementCase<dim>::cut_y) ||
                                    (face_ref_case == RefinementCase<dim>::cut_y &&
                                     needed_face_ref_case == RefinementCase<dim>::cut_x))
                                  {
      ...
      mesh_changed_in_this_loop = ((flags_before_loop[0] != flags_after_loop[0]) ||
  ...
  return ((flags_before[0] != flags_before_loop[0]) || (flags_before[1] != flags_before_loop[1]));
}
...
template <int dim, int spacedim>
void
Triangulation<dim, spacedim>::write_bool_vector(const unsigned int       magic_number1,
