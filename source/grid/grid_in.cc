  ...
  if (keyword == "CELLS")
    {
      ...
      if (vtk_version ==
          "5.1") // VTK 5.1 introduced OFFSETS and CONNECTIVITY arrays
        {
          ...
          AssertThrow(
            keyword == "OFFSETS",
            ExcMessage(
              "While reading VTK file, failed to find OFFSETS section"));
          ...
          AssertThrow(
            n_points_per_cell.size() == n_cells,
            ExcMessage(
          ...
          // Now that we now how many points correspond to each cell, we can
          // read the CONNECTIVITY array
          in >> keyword;
          AssertThrow(
            keyword == "CONNECTIVITY",
            ExcMessage(
              "While reading VTK file, failed to find CONNECTIVITY section"));

          in >>
            vtktype; // vtktypeint64, vtktypeint32, etc...we do not need this
          if (dim == 3)
            {
              for (unsigned int count = 0; count < n_points_per_cell.size();
                   ++count)
                {
            ...
            AssertThrow(
              false, ExcMessage("The number of dimensions is not 1, 2, or 3"));
        }
