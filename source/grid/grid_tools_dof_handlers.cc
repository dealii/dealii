...
namespace GridTools
{
    ...
    const std::function<bool(const typename MeshType::active_cell_iterator &)> &predicate)
  {
    ...
    // the halo layer
    for (typename MeshType::active_cell_iterator cell = mesh.begin_active(); cell != mesh.end();
         ++cell)
    ...
    const unsigned int                                                   level)
  {
    ...
    // These are the cells around which we wish to construct
    // the halo layer
    for (typename MeshType::cell_iterator cell = mesh.begin(level); cell != mesh.end(level); ++cell)
      if (predicate(cell)) // True predicate --> Part of subdomain
        for (unsigned int v = 0; v < GeometryInfo<MeshType::dimension>::vertices_per_cell; ++v)
  ...
  namespace
  {
    ...
    {
      for (typename std::vector<typename MeshType::active_cell_iterator>::const_iterator it =
             cells.begin();
           it != cells.end();
           ++it)
        {
  ...
  compute_ghost_cell_halo_layer(const MeshType &mesh)
  {
    ...
    const std::vector<typename MeshType::active_cell_iterator> active_halo_layer =
  ...
  {
    std::vector<typename MeshType::active_cell_iterator> subdomain_boundary_cells,
      active_cell_layer_within_distance;
    std::vector<bool> vertices_outside_subdomain(mesh.get_triangulation().n_vertices(), false);
    ...
    std::vector<Point<spacedim>> subdomain_boundary_cells_centers; // cache all the subdomain
                                                                   // boundary cells centers here
    std::vector<double> subdomain_boundary_cells_radii; // cache all the subdomain boundary cells
                                                        // radii
    ...
    for (typename std::vector<typename MeshType::active_cell_iterator>::const_iterator
      ...
      {
        const std::pair<Point<spacedim>, double> &subdomain_boundary_cell_enclosing_ball =
    ...
    for (typename MeshType::active_cell_iterator cell = mesh.begin_active(); cell != mesh.end();
         ++cell)
      {
        ...
        const std::pair<Point<spacedim>, double> &cell_enclosing_ball = cell->enclosing_ball();
        ...
        for (unsigned int d = 0; d < spacedim; ++d)
          cell_inside &=
            (cell_enclosing_ball_center[d] + cell_enclosing_ball_radius > bounding_box.first[d]) &&
          ...
          for (unsigned int i = 0; i < subdomain_boundary_cells_radii.size(); ++i)
            if (cell_enclosing_ball_center.distance_square(subdomain_boundary_cells_centers[i]) <
                Utilities::fixed_power<2>(cell_enclosing_ball_radius +
                                          subdomain_boundary_cells_radii[i] + layer_thickness +
                                          DOUBLE_EPSILON))
              {
  ...
  compute_ghost_cell_layer_within_distance(const MeshType &mesh, const double layer_thickness)
  {
    ...
    const std::vector<typename MeshType::active_cell_iterator> ghost_cell_layer_within_distance =
      compute_active_cell_layer_within_distance(mesh, predicate, layer_thickness);
  ...
  get_finest_common_cells(const MeshType &mesh_1, const MeshType &mesh_2)
  {
    ...
    // consideration
    typedef std::list<std::pair<typename MeshType::cell_iterator, typename MeshType::cell_iterator>>
      CellList;

    CellList cell_list;

    // first push the coarse level cells
    typename MeshType::cell_iterator cell_1 = mesh_1.begin(0), cell_2 = mesh_2.begin(0);
    ...
    while (cell_pair != cell_list.end())
      {
        ...
        // children instead
        if (cell_pair->first->has_children() && cell_pair->second->has_children())
          {
            Assert(cell_pair->first->refinement_case() == cell_pair->second->refinement_case(),
                   ExcNotImplemented());
            for (unsigned int c = 0; c < cell_pair->first->n_children(); ++c)
              cell_list.emplace_back(cell_pair->first->child(c), cell_pair->second->child(c));
    ...
    for (cell_pair = cell_list.begin(); cell_pair != cell_list.end(); ++cell_pair)
      Assert(cell_pair->first->active() || cell_pair->second->active() ||
               (cell_pair->first->refinement_case() != cell_pair->second->refinement_case()),
             ExcInternalError());
                        ...
                        const Triangulation<dim, spacedim> &mesh_2)
  {
    ...
    // of the coarse mesh
    typename Triangulation<dim, spacedim>::cell_iterator cell_1 = mesh_1.begin(0),
  ...
  {
    return have_same_coarse_mesh(mesh_1.get_triangulation(), mesh_2.get_triangulation());
  }
  ...
  find_active_cell_around_point(const hp::MappingCollection<dim, spacedim> &mapping,
                                ...
                                const Point<spacedim> &                     p)
  {
    Assert((mapping.size() == 1) || (mapping.size() == mesh.get_fe_collection().size()),
    ...
    if (mapping.size() == 1)
    ...
    else
      {
        ...
        std::vector<cell_iterator> adjacent_cells_tmp = find_cells_adjacent_to_vertex(mesh, vertex);
      ...
      if (cell->face(face_number)->at_boundary() == false)
        {
          if (cell->neighbor(face_number)->has_children() == false)
            ...
            {
              for (unsigned int subface = 0; subface < cell->face(face_number)->n_children();
                   ++subface)
          ...
          else
            {
              ...
              typename MeshType::cell_iterator neighbor = cell->neighbor(face_number);
...

  {
    ...
    std::map<
      typename Triangulation<Container::dimension, Container::space_dimension>::cell_iterator,
                  ...
                  if (patch_to_global_tria_map_tmp[active_tria_cell] == patch[i])
                    {
                      ...
                      Assert(active_tria_cell->center().distance(
                               patch_to_global_tria_map_tmp[active_tria_cell]->center()) <=
                        ...
                        for (unsigned int c = 0; c < cell->n_children(); ++c)
                          {
                            if (patch_to_global_tria_map_tmp.find(cell->child(c)) ==
                              ...
                              {
                                patch_to_global_tria_map_tmp.insert(std::make_pair(
                                  cell->child(c), patch_to_global_tria_map_tmp[cell]->child(c)));
        ...
        if (cell->user_flag_set())
          {
            ...
            Assert(cell->center().distance(patch_to_global_tria_map_tmp[cell]->center()) <=
                     1e-15 * cell->diameter(),
                   ExcInternalError());
          }
      }
    ...
    typename std::map<
      ...
      typename Container::cell_iterator>::iterator map_tmp_it =
                                                     patch_to_global_tria_map_tmp.begin(),
  ...
  get_dof_to_support_patch_map(DoFHandlerType &dof_handler)
  {
    ...
    // functions.
    std::map<types::global_dof_index, std::set<typename DoFHandlerType::active_cell_iterator>>
    ...
    std::map<typename DoFHandlerType::active_line_iterator, typename DoFHandlerType::line_iterator>
        ...
        if (cell->is_artificial() == false)
          {
            ...
            for (unsigned int f = 0; f < GeometryInfo<DoFHandlerType::dimension>::faces_per_cell;
                 ++f)
              {
                if (cell->face(f)->has_children())
                ...
                else if ((cell->face(f)->at_boundary() == false) && (cell->neighbor_is_coarser(f)))
                  {
                    ...
                    std::pair<unsigned int, unsigned int> neighbor_face_no_subface_no =
                    ...
                    // parent face
                    for (unsigned int c = 0; c < cell->neighbor(f)->face(face_no)->n_children();
                         ++c)
                      {
                        if (c != subface) // don't repeat work on dofs of
                                          // original cell
                          {
                            ...
                            Assert(cell->neighbor(f)->face(face_no)->child(c)->has_children() ==
                                     false,
                                   ExcInternalError());
                            cell->neighbor(f)->face(face_no)->child(c)->get_dof_indices(
                    ...
                    // children
                    else if (cell->line(l)->user_flag_set() == true)
                      {
                        ...
                        for (unsigned int c = 0; c < parent_line->n_children(); ++c)
                          {
                            Assert(parent_line->child(c)->has_children() == false,
                                   ExcInternalError());
    ...
    typename std::map<types::global_dof_index,
                      std::set<typename DoFHandlerType::active_cell_iterator>>::iterator
    ...
    const FullMatrix<double> &                                                matrix)
  {
    ...
    Assert(pairs1.size() == pairs2.size(), ExcMessage("Unmatched faces on periodic boundaries"));

    unsigned int n_matches = 0;

    // Match with a complexity of O(n^2). This could be improved...
    std::bitset<3>                                                                   orientation;
    typedef typename std::set<std::pair<CellIterator, unsigned int>>::const_iterator PairIterator;
    ...
    const FullMatrix<double> &                                       matrix)
  {
    ...
    for (typename MeshType::cell_iterator cell = mesh.begin(0); cell != mesh.end(0); ++cell)
      {
    ...
    match_periodic_face_pairs(pairs1, pairs2, direction, matched_pairs, offset, matrix);
  ...
  orthogonal_equality(const Point<spacedim> &    point1,
                      ...
                      const FullMatrix<double> & matrix)
  {
    Assert(0 <= direction && direction < spacedim, ExcIndexRange(direction, 0, spacedim));
  ...
  inline bool
    orthogonal_equality(std::bitset<3> &                                              orientation,
                        ...
                        const FullMatrix<double> &                                    matrix)
  {
    Assert(matrix.m() == matrix.n(), ExcMessage("The supplied matrix must be a square matrix"));
    ...
    for (unsigned int i = 0; i < GeometryInfo<dim>::vertices_per_face; ++i)
      {
        for (std::set<unsigned int>::iterator it = face2_vertices.begin();
             ...
             ++it)
          {
            if (orthogonal_equality(
                  face1->vertex(i), face2->vertex(*it), direction, offset, matrix))
              {
                matching[i] = *it;
