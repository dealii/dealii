    ...
    case 2:
    {
      if (spacedim == 1)
      ...
      else
        {
          ...
          // line is linear
          const Point<spacedim> p1 = surrounding_points[0],
                                p2 = surrounding_points[1];
          ...
          do
            {
              Tensor<1,dim> F_k;
              for (unsigned int i=0; i<GeometryInfo<dim>::vertices_per_cell; ++i)
                F_k += (x_k-trial_point)*surrounding_points[i] *
                ...
                for (unsigned int j=0; j<GeometryInfo<dim>::vertices_per_cell; ++j)
                  {
                    ...
                    H_k += (surrounding_points[i] * surrounding_points[j]) * tmp;
                  }
