...
#  endif
  ...
  TriangulationBase<dim, spacedim>::reset_global_cell_indices()
  {
    ...
    types::global_cell_index cell_index =
      Utilities::MPI::prefix_sum(n_locally_owned_cells, this->mpi_communicator)
        .first;
    ...
    if (this->is_multilevel_hierarchy_constructed() == true)
      {
        ...
        std::vector<types::global_cell_index> n_local_cells_level(
          this->n_global_levels(), 0);

        for (auto cell : this->cell_iterators())
          if (cell->level_subdomain_id() == this->locally_owned_subdomain())
            n_local_cells_level[cell->level()]++;

        // 2/3) determine the offset of each process and global
        // number of "active" cells on each level
        auto cell_index_and_n_cells_level =
          Utilities::MPI::prefix_sum(n_local_cells_level,
                                     this->mpi_communicator);
        ...
        for (unsigned int l = 0; l < this->n_global_levels(); ++l)
          my_ranges[l].first = cell_index_and_n_cells_level[l].first;

        for (auto cell : this->cell_iterators())
          if (cell->level_subdomain_id() == this->locally_owned_subdomain())
            cell->set_global_level_cell_index(
              cell_index_and_n_cells_level[cell->level()].first++);
          else
            cell->set_global_level_cell_index(numbers::invalid_dof_index);

        for (unsigned int l = 0; l < this->n_global_levels(); ++l)
          my_ranges[l].second = cell_index_and_n_cells_level[l].first;
        ...
        for (unsigned int l = 0; l < this->n_global_levels(); ++l)
          {
            IndexSet is_local(cell_index_and_n_cells_level[l].second);
            is_local.add_range(my_ranges[l].first, my_ranges[l].second);

            IndexSet is_ghost(cell_index_and_n_cells_level[l].second);
