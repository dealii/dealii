...
namespace parallel
{
  namespace distributed
  {
      ...
      handle(numbers::invalid_unsigned_int)
    {
      Assert(
        (dynamic_cast<
           const parallel::distributed::Triangulation<dim, DoFHandlerType::space_dimension> *>(
    ...
    SolutionTransfer<dim, VectorType, DoFHandlerType>::register_data_attach(const std::size_t size)
    {
      ...
      handle = tria->register_data_attach(
        size,
        std::bind(&SolutionTransfer<dim, VectorType, DoFHandlerType>::pack_callback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2,
                  std::placeholders::_3));
      ...
      std::vector<VectorType *> &all_out)
    {
      ...
      tria->notify_ready_to_unpack(
        handle,
        std::bind(&SolutionTransfer<dim, VectorType, DoFHandlerType>::unpack_callback,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2,
                  std::placeholders::_3,
      ...
      for (typename std::vector<VectorType *>::iterator it = all_out.begin(); it != all_out.end();
           ++it)
      ...
      void *data)
    {
      ...
      for (typename std::vector<const VectorType *>::iterator it = input_vectors.begin();
           ...
           ++it)
        {
          ...
          std::memcpy(
            data_store, &dofvalues(0), sizeof(typename VectorType::value_type) * dofs_per_cell);
      ...
      std::vector<VectorType *> &all_out)
    {
      ...
      for (typename std::vector<VectorType *>::iterator it = all_out.begin(); it != all_out.end();
        ...
        {
          std::memcpy(
            &dofvalues(0), data_store, sizeof(typename VectorType::value_type) * dofs_per_cell);
