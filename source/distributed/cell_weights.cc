...
namespace parallel
{
    ...
    const bool                       enable_fe_cache)
  {
    ...
    if (enable_fe_cache)
      connection = dof_handler.get_triangulation().signals.weight.connect(
        make_cached_weighting_callback(dof_handler, weighting_function));
    else
      connection = dof_handler.get_triangulation().signals.weight.connect(
  ...
  std::function<unsigned int(
    const typename dealii::Triangulation<dim, spacedim>::cell_iterator &cell,
    const CellStatus                                                    status)>
  CellWeights<dim, spacedim>::make_cached_weighting_callback(
    const DoFHandler<dim, spacedim> &dof_handler,
    const WeightingFunction         &weighting_function)
  {
    ...
    return [&dof_handler, tria, weight_cache](
             const typename dealii::Triangulation<dim, spacedim>::cell_iterator
                             &cell,
             const CellStatus status) -> unsigned int {
      return CellWeights<dim, spacedim>::cached_weighting_callback(
        cell, status, std::cref(dof_handler), std::cref(*tria), weight_cache);
    };
  }
  ...
  template <int dim, int spacedim>
  unsigned int
  CellWeights<dim, spacedim>::cached_weighting_callback(
    const typename dealii::Triangulation<dim, spacedim>::cell_iterator &cell_,
    const CellStatus                                                    status,
    const DoFHandler<dim, spacedim>                  &dof_handler,
    const parallel::TriangulationBase<dim, spacedim> &triangulation,
