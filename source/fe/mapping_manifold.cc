...
template <int dim, int spacedim>
void
MappingManifold<dim, spacedim>::InternalData::initialize(const UpdateFlags      update_flags,
  ...
  const Point<dim> &                                          p) const
{
  ...
  return cell->get_manifold().get_new_point(make_array_view(vertices.begin(), vertices.end()),
        ...
        if (update_flags & update_contravariant_transformation)
          {
            ...
            AssertDimension(GeometryInfo<dim>::vertices_per_cell, data.vertices.size());
                ...
                for (unsigned int i = 0; i < dim; ++i)
                  {
                    ...
                    const Point<spacedim> NP = data.manifold->get_new_point(
                      make_array_view(data.vertices), make_array_view(data.vertex_weights));
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
        {
          if (dim == spacedim)
            {
              ...
              Assert(det > 1e-12 *
                             Utilities::fixed_power<dim>(cell->diameter() / std::sqrt(double(dim))),
          ...
          // there are also no face normals to compute
          else // codim>0 case
            {
              ...
              output_data.JxW_values[point] = std::sqrt(determinant(G)) * weights[point];
  ...
  namespace MappingManifoldImplementation
  {
    namespace
    {
                       ...
                       const ArrayView<Tensor<rank, spacedim>> &                output)
      {
        ...
        Assert((dynamic_cast<const typename dealii::MappingManifold<dim, spacedim>::InternalData *>(
                  &mapping_data) != nullptr),
               ExcInternalError());
        const typename dealii::MappingManifold<dim, spacedim>::InternalData &data =
          static_cast<const typename dealii::MappingManifold<dim, spacedim>::InternalData &>(
            mapping_data);
