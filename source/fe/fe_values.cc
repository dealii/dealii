...
namespace internal
{
  ...
  IndexSet::value_type inline get_vector_element(const IndexSet &              is,
                                                 const types::global_dof_index cell_number)
  {
  ...
  namespace
  {
    ...
    struct CheckForZero<
      Number,
      typename std::enable_if<Differentiation::AD::is_ad_number<Number>::value>::type>
    {
      static bool
      value(const Number & /*value*/)
      {
...
namespace FEValuesViews
{
    ...
    shape_function_data(this->fe_values->fe->dofs_per_cell)
  {
    ...
    Assert(component < fe.n_components(), ExcIndexRange(component, 0, fe.n_components()));
  ...
  template <int dim, int spacedim>
  Scalar<dim, spacedim>::Scalar() : fe_values(nullptr), component(numbers::invalid_unsigned_int)
  {}
        ...
        for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
          {
            ...
            if (shape_function_data[i].is_nonzero_shape_function_component[d] == true)
              shape_function_data[i].row_index[d] =
                shape_function_to_row_table[i * fe.n_components() + component];
            else
              shape_function_data[i].row_index[d] = numbers::invalid_unsigned_int;
          }
      }
  ...
  template <int dim, int spacedim>
  SymmetricTensor<2, dim, spacedim>::SymmetricTensor(const FEValuesBase<dim, spacedim> &fe_values,
  ...
  {
    const FiniteElement<dim, spacedim> &fe = *this->fe_values->fe;
    Assert(first_tensor_component + (dim * dim + dim) / 2 - 1 < fe.n_components(),
    ...
    for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
      {
        ...
        if (n_nonzero_components == 0)
          ...
          {
            for (unsigned int d = 0; d < dealii::SymmetricTensor<2, dim>::n_independent_components;
                 ++d)
              if (shape_function_data[i].is_nonzero_shape_function_component[d] == true)
                {
                  shape_function_data[i].single_nonzero_component =
    ...
    shape_function_data(this->fe_values->fe->dofs_per_cell)
  {
    ...
    Assert(first_tensor_component + dim * dim - 1 < fe.n_components(),
           ExcIndexRange(first_tensor_component + dim * dim - 1, 0, fe.n_components()));
    // TODO: we'd like to use the fields with the same name as these
    // variables from FEValuesBase, but they aren't initialized yet
    // at the time we get here, so re-create it all
    const std::vector<unsigned int> shape_function_to_row_table =
  ...
  namespace internal
  {
      ...
      std::vector<typename ProductType<Number, double>::type> &             values)
    {
      ...
      std::fill(values.begin(), values.end(), dealii::internal::NumberType<Number>::value(0.0));

      for (unsigned int shape_function = 0; shape_function < dofs_per_cell; ++shape_function)
    ...
    do_function_laplacians(
      ...
      const std::vector<typename Scalar<dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename Scalar<dim, spacedim>::template OutputType<Number>::laplacian_type>
        &laplacians)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
    ...
    do_function_derivatives(
      ...
      const Table<2, dealii::Tensor<order, spacedim>> &                     shape_derivatives,
      const std::vector<typename Vector<dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename ProductType<Number, dealii::Tensor<order + 1, spacedim>>::type>
        &derivatives)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
          ...
          if (snc != -1)
            ...
            for (unsigned int d = 0; d < spacedim; ++d)
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const dealii::Tensor<order, spacedim> *shape_derivative_ptr =
                    &shape_derivatives[shape_function_data[shape_function].row_index[d]][0];
                  for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
    ...
    do_function_symmetric_gradients(
      ...
      const std::vector<typename Vector<dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename ProductType<Number, dealii::SymmetricTensor<2, spacedim>>::type>
        &symmetric_gradients)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
        dofs_per_cell > 0 ? shape_gradients[0].size() : symmetric_gradients.size();
      AssertDimension(symmetric_gradients.size(), n_quadrature_points);

      std::fill(symmetric_gradients.begin(),
    ...
    do_function_divergences(
      ...
      const Table<2, dealii::Tensor<1, spacedim>> &                         shape_gradients,
      const std::vector<typename Vector<dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename Vector<dim, spacedim>::template OutputType<Number>::divergence_type>
        &divergences)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
              ...
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const dealii::Tensor<1, spacedim> *shape_gradient_ptr =
                    &shape_gradients[shape_function_data[shape_function].row_index[d]][0];
                  for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
    ...
    do_function_laplacians(
      ...
      const Table<2, dealii::Tensor<2, spacedim>> &                         shape_hessians,
      const std::vector<typename Vector<dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename Vector<dim, spacedim>::template OutputType<Number>::laplacian_type>
        &laplacians)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
          ...
          if (snc != -1)
            {
              const unsigned int comp =
                shape_function_data[shape_function].single_nonzero_component_index;
              const dealii::Tensor<2, spacedim> *shape_hessian_ptr = &shape_hessians[snc][0];
              for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
            ...
            for (unsigned int d = 0; d < spacedim; ++d)
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const dealii::Tensor<2, spacedim> *shape_hessian_ptr =
                    &shape_hessians[shape_function_data[shape_function].row_index[d]][0];
                  for (unsigned int q_point = 0; q_point < n_quadrature_points; ++q_point)
    ...
    do_function_values(
      ...
      const std::vector<typename SymmetricTensor<2, dim, spacedim>::ShapeFunctionData>
        &shape_function_data,
      ...
      for (unsigned int shape_function = 0; shape_function < dofs_per_cell; ++shape_function)
        {
          ...
          if (snc != -1)
            ...
            for (unsigned int d = 0;
                 d < dealii::SymmetricTensor<2, spacedim>::n_independent_components;
                 ++d)
              if (shape_function_data[shape_function].is_nonzero_shape_function_component[d])
                {
                  const TableIndices<2> comp =
    ...
    do_function_divergences(
      ...
      const std::vector<typename SymmetricTensor<2, dim, spacedim>::ShapeFunctionData>
        &shape_function_data,
      std::vector<
        typename SymmetricTensor<2, dim, spacedim>::template OutputType<Number>::divergence_type>
        &divergences)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
          ...
          if (snc != -1)
            {
              ...
              const unsigned int ii =
                dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(comp)[0];
              const unsigned int jj =
                dealii::SymmetricTensor<2, spacedim>::unrolled_to_component_indices(comp)[1];
                        ...
                        for (unsigned int j = 0; j < spacedim; ++j)
                          {
                            const unsigned int vector_component =
                              dealii::SymmetricTensor<2, spacedim>::component_to_unrolled_index(
    ...
    do_function_divergences(
      ...
      const Table<2, dealii::Tensor<1, spacedim>> &                            shape_gradients,
      const std::vector<typename Tensor<2, dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename Tensor<2, dim, spacedim>::template OutputType<Number>::divergence_type>
        &divergences)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
      ...
      std::fill(divergences.begin(),
                divergences.end(),
                typename Tensor<2, dim, spacedim>::template OutputType<Number>::divergence_type());
    ...
    do_function_gradients(
      ...
      const Table<2, dealii::Tensor<1, spacedim>> &                            shape_gradients,
      const std::vector<typename Tensor<2, dim, spacedim>::ShapeFunctionData> &shape_function_data,
      std::vector<typename Tensor<2, dim, spacedim>::template OutputType<Number>::gradient_type>
        &gradients)
    {
      const unsigned int dofs_per_cell = dof_values.size();
      const unsigned int n_quadrature_points =
    ...
    const
  {
    ...
    AssertDimension(fe_function.size(), fe_values->present_cell->n_dofs_for_dof_handler());
  ...
  Scalar<dim, spacedim>::get_function_gradients(
    const InputVector &fe_function,
    std::vector<typename ProductType<gradient_type, typename InputVector::value_type>::type>
      &gradients) const
  {
    Assert(
      fe_values->update_flags & update_gradients,
  ...
  Scalar<dim, spacedim>::get_function_hessians(
    const InputVector &fe_function,
    std::vector<typename ProductType<hessian_type, typename InputVector::value_type>::type>
      &hessians) const
  {
    Assert(
      fe_values->update_flags & update_hessians,
  ...
  Scalar<dim, spacedim>::get_function_laplacians(
    const InputVector &fe_function,
    std::vector<typename ProductType<value_type, typename InputVector::value_type>::type>
      &laplacians) const
  {
    Assert(
      fe_values->update_flags & update_hessians,
  ...
  Scalar<dim, spacedim>::get_function_third_derivatives(
    const InputVector &fe_function,
    std::vector<typename ProductType<third_derivative_type, typename InputVector::value_type>::type>
  ...
  Scalar<dim, spacedim>::get_function_third_derivatives_from_local_dof_values(
    const InputVector &dof_values,
    std::vector<typename OutputType<typename InputVector::value_type>::third_derivative_type>
      &third_derivatives) const
  {
    Assert(fe_values->update_flags & update_3rd_derivatives,
           (typename FEValuesBase<dim, spacedim>::ExcAccessToUninitializedField(
  ...
  Vector<dim, spacedim>::get_function_symmetric_gradients(
    const InputVector &fe_function,
    std::vector<
      typename ProductType<symmetric_gradient_type, typename InputVector::value_type>::type>
  ...
  Vector<dim, spacedim>::get_function_symmetric_gradients_from_local_dof_values(
    const InputVector &dof_values,
    std::vector<typename OutputType<typename InputVector::value_type>::symmetric_gradient_type>
      &symmetric_gradients) const
  {
    Assert(
      fe_values->update_flags & update_gradients,
  ...
  Vector<dim, spacedim>::get_function_divergences(
    const InputVector &fe_function,
    std::vector<typename ProductType<divergence_type, typename InputVector::value_type>::type>
      &divergences) const
  {
    Assert(
      fe_values->update_flags & update_gradients,
  ...
  Vector<dim, spacedim>::get_function_divergences_from_local_dof_values(
    const InputVector &dof_values,
    std::vector<typename OutputType<typename InputVector::value_type>::divergence_type>
      &divergences) const
  {
    Assert(
      fe_values->update_flags & update_gradients,
  ...
  Vector<dim, spacedim>::get_function_curls(
    const InputVector &fe_function,
    std::vector<typename ProductType<curl_type, typename InputVector::value_type>::type> &curls)
    const
  {
    Assert(
      fe_values->update_flags & update_gradients,
  ...
  template <class InputVector>
  void
  SymmetricTensor<2, dim, spacedim>::get_function_divergences_from_local_dof_values(
    const InputVector &dof_values,
    std::vector<typename OutputType<typename InputVector::value_type>::divergence_type>
      &divergences) const
  {
      ...
      for (unsigned int component = 0; component < n_symmetric_second_order_tensors; ++component)
        {
          ...
          new (&symmetric_second_order_tensors[component])
            dealii::FEValuesViews::SymmetricTensor<2, dim, spacedim>(fe_values, component);
        }
      ...
      for (unsigned int component = 0; component < n_second_order_tensors; ++component)
        {
          ...
          new (&second_order_tensors[component])
            dealii::FEValuesViews::Tensor<2, dim, spacedim>(fe_values, component);
        }
    }
  } // namespace FEValuesViews
} // namespace internal
...
#include "fe_values.decl.1.inst"
   ...
   */
  virtual void
  get_interpolated_dof_values(const IndexSet &in, Vector<IndexSet::value_type> &out) const = 0;
};
...
#include "fe_values.decl.2.inst"
   ...
   */
  virtual void
  get_interpolated_dof_values(const IndexSet &in, Vector<IndexSet::value_type> &out) const override;
...
template <int dim, int spacedim>
template <typename CI>
FEValuesBase<dim, spacedim>::CellIterator<CI>::CellIterator(const CI &cell) : cell(cell)
{}
...
namespace internal
{
  namespace FEValuesImplementation
  {
                                                  ...
                                                  const UpdateFlags  flags)
    {
      if (flags & update_quadrature_points)
        this->quadrature_points.resize(
          n_quadrature_points, Point<spacedim>(numbers::signaling_nan<Tensor<1, spacedim>>()));

      if (flags & update_JxW_values)
        this->JxW_values.resize(n_quadrature_points, numbers::signaling_nan<double>());
      ...
      if (flags & update_jacobian_2nd_derivatives)
        this->jacobian_2nd_derivatives.resize(
          n_quadrature_points, numbers::signaling_nan<DerivativeForm<3, dim, spacedim>>());
                                                        ...
                                                        const UpdateFlags                   flags)
    {
      ...
      // nonzero component
      this->shape_function_to_row_table = dealii::internal::make_shape_function_to_row_table(fe);
      ...
      if (flags & update_gradients)
        {
          this->shape_gradients.reinit(n_nonzero_shape_components, n_quadrature_points);
          this->shape_gradients.fill(numbers::signaling_nan<Tensor<1, spacedim>>());
        }

      if (flags & update_hessians)
        {
          this->shape_hessians.reinit(n_nonzero_shape_components, n_quadrature_points);
          this->shape_hessians.fill(numbers::signaling_nan<Tensor<2, spacedim>>());
        }

      if (flags & update_3rd_derivatives)
        {
          this->shape_3rd_derivatives.reinit(n_nonzero_shape_components, n_quadrature_points);
...
namespace internal
{
                     ...
                     std::vector<Number> &           values)
  {
    ...
    std::fill_n(
      values.begin(), n_quadrature_points, dealii::internal::NumberType<Number>::value(0.0));
                     ...
                     const unsigned int                     component_multiple        = 1)
  {
    ...
    for (unsigned int i = 0; i < values.size(); ++i)
      std::fill_n(values[i].begin(), values[i].size(), typename VectorType::value_type());
      ...
      for (unsigned int shape_func = 0; shape_func < dofs_per_cell; ++shape_func)
        {
          ...
          if (fe.is_primitive(shape_func))
            {
              const unsigned int comp =
                fe.system_to_component_index(shape_func).first + mc * n_components;
            ...
            for (unsigned int c = 0; c < n_components; ++c)
              {
                ...
                const unsigned int row = shape_function_to_row_table[shape_func * n_components + c];
    ...
    for (unsigned int shape_func = 0; shape_func < dofs_per_cell; ++shape_func)
      {
        ...
        const Tensor<order, spacedim> *shape_derivative_ptr = &shape_derivatives[shape_func][0];
        for (unsigned int point = 0; point < n_quadrature_points; ++point)
                          ...
                          const unsigned int component_multiple        = 1)
  {
    ...
    for (unsigned int i = 0; i < derivatives.size(); ++i)
      std::fill_n(derivatives[i].begin(), derivatives[i].size(), Tensor<order, spacedim, Number>());
          ...
          if (fe.is_primitive(shape_func))
            {
              ...
              const Tensor<order, spacedim> *shape_derivative_ptr = &shape_derivatives[row][0];
                         ...
                         std::vector<Number> &                        laplacians)
  {
    ...
    // initialize with zero
    std::fill_n(
      laplacians.begin(), n_quadrature_points, dealii::internal::NumberType<Number>::value(0.0));
    ...
    for (unsigned int shape_func = 0; shape_func < dofs_per_cell; ++shape_func)
      {
        ...
        const Tensor<2, spacedim> *shape_hessian_ptr = &shape_hessians[shape_func][0];
        for (unsigned int point = 0; point < n_quadrature_points; ++point)
                         ...
                         const unsigned int component_multiple        = 1)
  {
    ...
    for (unsigned int i = 0; i < laplacians.size(); ++i)
      std::fill_n(laplacians[i].begin(), laplacians[i].size(), typename VectorType::value_type());
          ...
          if (fe.is_primitive(shape_func))
            {
              ...
              const Tensor<2, spacedim> *shape_hessian_ptr = &shape_hessians[row][0];
              if (quadrature_points_fastest)
                {
  ...
  std::vector<typename InputVector::value_type> &values) const
{
  ...
  Assert(present_cell.get() != nullptr, ExcMessage("FEValues object is not reinit'ed to any cell"));
  ...
  std::vector<Vector<typename InputVector::value_type>> &        values) const
{
  ...
  Assert(indices.size() % dofs_per_cell == 0, ExcNotMultiple(indices.size(), dofs_per_cell));
  Assert(this->update_flags & update_values, ExcAccessToUninitializedField("update_values"));
...
FEValuesBase<dim, spacedim>::get_function_gradients(
  const InputVector &                                            fe_function,
  const VectorSlice<const std::vector<types::global_dof_index>> &indices,
  VectorSlice<std::vector<std::vector<Tensor<1, spacedim, typename InputVector::value_type>>>>
       gradients,
  bool quadrature_points_fastest) const
{
  typedef typename InputVector::value_type Number;
...
FEValuesBase<dim, spacedim>::get_function_hessians(
  const InputVector &                                            fe_function,
  const VectorSlice<const std::vector<types::global_dof_index>> &indices,
  VectorSlice<std::vector<std::vector<Tensor<2, spacedim, typename InputVector::value_type>>>>
       hessians,
  bool quadrature_points_fastest) const
{
  typedef typename InputVector::value_type Number;
  ...
  std::vector<Tensor<3, spacedim, typename InputVector::value_type>> &third_derivatives) const
{
  ...
  present_cell->get_interpolated_dof_values(fe_function, dof_values);
  internal::do_function_derivatives(
    dof_values.begin(), this->finite_element_output.shape_3rd_derivatives, third_derivatives);
}
  ...
  std::vector<Tensor<3, spacedim, typename InputVector::value_type>> &third_derivatives) const
{
  ...
  for (unsigned int i = 0; i < dofs_per_cell; ++i)
    dof_values[i] = internal::get_vector_element(fe_function, indices[i]);
  internal::do_function_derivatives(
    dof_values.data(), this->finite_element_output.shape_3rd_derivatives, third_derivatives);
}
...
FEValuesBase<dim, spacedim>::get_function_third_derivatives(
  const InputVector &fe_function,
  std::vector<std::vector<Tensor<3, spacedim, typename InputVector::value_type>>>
    &  third_derivatives,
  bool quadrature_points_fastest) const
{
  typedef typename InputVector::value_type Number;
...
FEValuesBase<dim, spacedim>::get_function_third_derivatives(
  const InputVector &                                            fe_function,
  const VectorSlice<const std::vector<types::global_dof_index>> &indices,
  VectorSlice<std::vector<std::vector<Tensor<3, spacedim, typename InputVector::value_type>>>>
       third_derivatives,
  bool quadrature_points_fastest) const
{
  typedef typename InputVector::value_type Number;
...
{
  Assert(
    this->update_flags & update_normal_vectors,
...
{
  return (sizeof(this->update_flags) + MemoryConsumption::memory_consumption(n_quadrature_points) +
          sizeof(cell_similarity) + MemoryConsumption::memory_consumption(dofs_per_cell) +
  ...
  if (present_cell.get() != nullptr)
    {
      if (&cell->get_triangulation() !=
             ...
             ->get_triangulation())
        {
          ...
          tria_listener_refinement = cell->get_triangulation().signals.any_change.connect(
            std::bind(&FEValuesBase<dim, spacedim>::invalidate_present_cell,
                      std::ref(static_cast<FEValuesBase<dim, spacedim> &>(*this))));
          tria_listener_mesh_transform = cell->get_triangulation().signals.mesh_movement.connect(
            std::bind(&FEValuesBase<dim, spacedim>::invalidate_present_cell,
                      std::ref(static_cast<FEValuesBase<dim, spacedim> &>(*this))));
        }
    }
  else
    {
      ...
      // changes
      tria_listener_refinement = cell->get_triangulation().signals.post_refinement.connect(
        std::bind(&FEValuesBase<dim, spacedim>::invalidate_present_cell,
                  std::ref(static_cast<FEValuesBase<dim, spacedim> &>(*this))));
      tria_listener_mesh_transform = cell->get_triangulation().signals.mesh_movement.connect(
        std::bind(&FEValuesBase<dim, spacedim>::invalidate_present_cell,
...
FEValues<dim, spacedim>::FEValues(const Mapping<dim, spacedim> &      mapping,
  ...
  FEValuesBase<dim, spacedim>(q.size(), fe.dofs_per_cell, update_default, mapping, fe),
  quadrature(q)
{
...
FEValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  if (flags & update_mapping)
    mapping_get_data =
      Threads::new_task(&Mapping<dim, spacedim>::get_data, *this->mapping, flags, quadrature);
  ...
  if (flags & update_mapping)
    ...
    this->mapping_data =
      std_cxx14::make_unique<typename Mapping<dim, spacedim>::InternalDataBase>();
}
  ...
  const TriaIterator<DoFCellAccessor<DoFHandlerType<dim, spacedim>, lda>> &cell)
{
  ...
  reset_pointer_in_place_if_possible<typename FEValuesBase<dim, spacedim>::template CellIterator<
    TriaIterator<DoFCellAccessor<DoFHandlerType<dim, spacedim>, lda>>>>(this->present_cell, cell);
...
FEValues<dim, spacedim>::do_reinit()
{
  ...
  if (this->update_flags & update_mapping)
    {
      this->cell_similarity = this->get_mapping().fill_fe_values(*this->present_cell,
...
FEFaceValuesBase<dim, spacedim>::FEFaceValuesBase(const unsigned int n_q_points,
  ...
  FEValuesBase<dim, spacedim>(n_q_points, dofs_per_cell, update_default, mapping, fe),
...
{
  Assert(
    this->update_flags & update_boundary_forms,
...
FEFaceValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  if (flags & update_mapping)
    mapping_get_data = Threads::new_task(
      &Mapping<dim, spacedim>::get_face_data, *this->mapping, flags, this->quadrature);
...
FEFaceValues<dim, spacedim>::do_reinit(const unsigned int face_no)
{
  ...
  if (this->update_flags & update_mapping)
    {
      this->get_mapping().fill_fe_face_values(
        *this->present_cell, face_no, this->quadrature, *this->mapping_data, this->mapping_output);
    }
...
FESubfaceValues<dim, spacedim>::initialize(const UpdateFlags update_flags)
{
  ...
  if (flags & update_mapping)
    mapping_get_data = Threads::new_task(
      &Mapping<dim, spacedim>::get_subface_data, *this->mapping, flags, this->quadrature);
...
template <int dim, int spacedim>
void
FESubfaceValues<dim, spacedim>::do_reinit(const unsigned int face_no, const unsigned int subface_no)
{
  // first of all, set the present_face_index (if available)
  const typename Triangulation<dim, spacedim>::cell_iterator cell = *this->present_cell;
  ...
  if (!cell->face(face_no)->has_children())
  ...
  else
    {
      ...
      Assert(subface_index != numbers::invalid_unsigned_int, ExcInternalError());
