...
FE_ABF<dim>::FE_ABF(const unsigned int deg) :
  FE_PolyTensor<PolynomialsABF<dim>, dim>(
    deg,
    FiniteElementData<dim>(get_dpo_vector(deg), dim, deg + 2, FiniteElementData<dim>::Hdiv),
  ...
  rt_order(deg)
{
  ...
  std::vector<FullMatrix<double>> face_embeddings(
    1 << (dim - 1), FullMatrix<double>(this->dofs_per_face, this->dofs_per_face));
  ...
  // 0);
  this->interface_constraints.reinit((1 << (dim - 1)) * this->dofs_per_face, this->dofs_per_face);
...
FE_ABF<dim>::initialize_support_points(const unsigned int deg)
{
          ...
          for (unsigned int i = 0; i < legendre.n(); ++i)
            {
              boundary_weights(k, i) =
                face_points.weight(k) * legendre.compute_value(i, face_points.point(k));
            }
        }

      Quadrature<dim> faces = QProjector<dim>::project_to_all_faces(face_points);
      for (; current < GeometryInfo<dim>::faces_per_cell * n_face_points; ++current)
        {
          ...
          for (unsigned int i = 0; i < polynomials_abf[0]->n() * dim; ++i)
            {
              boundary_weights_abf(k, i) =
                polynomials_abf[i % dim]->compute_value(i / dim, faces.point(k)) * faces.weight(k);
            }
        }
    }
  ...
  interior_weights_abf.reinit(
    TableIndices<3>(cell_quadrature.size(), polynomials_abf[0]->n() * dim, dim));
  ...
  Assert(current == this->generalized_support_points.size(), ExcInternalError());
}
      ...
      for (unsigned int sub = 0; sub < GeometryInfo<dim>::max_children_per_face; ++sub)
        {
          ...
          // only.
          Quadrature<dim>    q_sub = QProjector<dim>::project_to_subface(q_base, face, sub);
              ...
              for (unsigned int i_face = 0; i_face < this->dofs_per_face; ++i_face)
                {
                  ...
                  // transformed, so we
                  // have to do it here.
                  this->restriction[iso][child](face * this->dofs_per_face + i_face, i_child) +=
        ...
        for (unsigned int i_child = 0; i_child < this->dofs_per_cell; ++i_child)
          for (unsigned int d = 0; d < dim; ++d)
            for (unsigned int i_weight = 0; i_weight < polynomials[d]->n(); ++i_weight)
              {
                this->restriction[iso][child](start_cell_dofs + i_weight * dim + d, i_child) +=
...
{
  Assert(
    support_point_values.size() == this->generalized_support_points.size(),
      ...
      for (unsigned int fp = 0; fp < n_face_points; ++fp)
        {
          ...
          // have to be in 3D
          unsigned int k =
            QProjector<dim>::DataSetDescriptor::face(face, false, false, false, n_face_points);
          for (unsigned int i = 0; i < boundary_weights_abf.size(1); ++i)
            nodal_values[start_abf_dofs + i] +=
              n_orient * boundary_weights_abf(k + fp, i) *
              support_point_values[face * n_face_points + fp]
                                  [GeometryInfo<dim>::unit_normal_direction[face]];
        }
    }
