                               ...
                               std::vector<bool>(dim,true)))
{
  ...
  Assert (deg >= 1, ExcMessage("Lowest order RT_Bubbles element is degree 1, but you requested for degree 0"));
...
FE_RT_Bubbles<dim>::initialize_support_points (const unsigned int deg)
{
  ...
  // as required to determine the normal component uniquely. This is the deg of
  // the RT_Bubble element plus one.
  if (dim>1)
    {
...
FE_RT_Bubbles<dim>::get_dpo_vector (const unsigned int deg)
{
  ...
  const unsigned int
  interior_dofs = dim*(deg-1)*pow(deg+1,dim-1);
...
{
  const unsigned int dofs_per_cell = PolynomialsRT_Bubbles<dim>::compute_n_pols(deg);
...
template <int dim>
bool
FE_RT_Bubbles<dim>::has_support_on_face (
  ...
  const unsigned int face_index) const
{
  ...
  // The first degrees of freedom are on the faces and each face has
  // (degree+1) DoFs.
  const unsigned int support_face = shape_index / this->degree;

  // The only thing we know for sure is that shape functions with
  // support on one face are zero on the opposite face.
