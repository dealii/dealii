...
FE_DGQ<dim, spacedim>::FE_DGQ(const std::vector<Polynomials::Polynomial<double>> &polynomials) :
  FE_Poly<TensorProductPolynomials<dim>, dim, spacedim>(
    ...
    std::vector<bool>(
      FiniteElementData<dim>(get_dpo_vector(polynomials.size() - 1), 1, polynomials.size() - 1)
        .dofs_per_cell,
      true),
    std::vector<ComponentMask>(
      FiniteElementData<dim>(get_dpo_vector(polynomials.size() - 1), 1, polynomials.size() - 1)
                                                ...
                                                FullMatrix<double> &interpolation_matrix) const
{
  ...
  FullMatrix<double> cell_interpolation(this->dofs_per_cell, this->dofs_per_cell);
  FullMatrix<double> source_interpolation(this->dofs_per_cell, source_fe.dofs_per_cell);
...
FE_DGQArbitraryNodes<dim, spacedim>::get_name() const
{
  ...
  if (gauss == true)
    {
      namebuf << "FE_DGQArbitraryNodes<" << Utilities::dim_string(dim, spacedim) << ">(QGauss("
              << this->degree + 1 << "))";
      return namebuf.str();
    }
  ...
  if (gauss_log == true)
    {
      namebuf << "FE_DGQArbitraryNodes<" << Utilities::dim_string(dim, spacedim) << ">(QGaussLog("
              << this->degree + 1 << "))";
      return namebuf.str();
    }

  // All guesses exhausted
  namebuf << "FE_DGQArbitraryNodes<" << Utilities::dim_string(dim, spacedim) << ">(QUnknownNodes("
          << this->degree + 1 << "))";
  return namebuf.str();
}
