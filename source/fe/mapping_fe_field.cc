  ...
  const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
  ...
  const typename DoFHandler<dim, spacedim>::cell_iterator dof_cell(*cell, euler_dof_handler);
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
void
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::compute_shapes_virtual(
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
UpdateFlags
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::requires_update_flags(
  ...
  InternalData &         data) const
{
  ...
  if (data.update_each & (update_covariant_transformation | update_contravariant_transformation |
              ...
              for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; ++i)
                {
                  ...
                  std::fill(data.unit_tangentials[i].begin(), data.unit_tangentials[i].end(), tang);
                }
            }
          else if (dim == 3)
            {
              for (unsigned int i = 0; i < GeometryInfo<dim>::faces_per_cell; ++i)
                {
                  ...
                  // points on this face
                  std::fill(
                    data.unit_tangentials[i].begin(), data.unit_tangentials[i].end(), tang1);
  ...
  namespace MappingFEFieldImplementation
  {
    namespace
    {
      ...
      maybe_compute_q_points(
        ...
        const typename dealii::MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::
          InternalData &                    data,
        const FiniteElement<dim, spacedim> &fe,
        ...
        internal::FEValuesImplementation::MappingRelatedData<dim, spacedim> &output_data)
      {
        ...
        maybe_compute_face_data<dim, spacedim, VectorType, DoFHandlerType>(
          mapping, cell, face_no, subface_no, quadrature.get_weights(), data, output_data);
      }
    } // namespace
  }   // namespace MappingFEFieldImplementation
} // namespace internal
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
void
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::fill_fe_subface_values(
  const typename Triangulation<dim, spacedim>::cell_iterator &         cell,
  const unsigned int                                                   face_no,
  const unsigned int                                                   subface_no,
  const Quadrature<dim - 1> &                                          quadrature,
            ...
            for (unsigned int i = 0; i < spacedim; ++i)
              for (unsigned int j = 0; j < spacedim; ++j)
                for (unsigned int k = 0; k < spacedim; ++k)
                  {
                    output[q][i][j][k] =
                      data.covariant[q][j][0] * data.covariant[q][k][0] * input[q][i][0][0];
                    for (unsigned int J = 0; J < dim; ++J)
                      {
                        const unsigned int K0 = (0 == J) ? 1 : 0;
                        for (unsigned int K = K0; K < dim; ++K)
                          output[q][i][j][k] +=
                            data.covariant[q][j][J] * data.covariant[q][k][K] * input[q][i][J][K];
                      }
                  }
          return;
        }
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
Point<spacedim>
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::transform_unit_to_real_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const Point<dim> &                                          p) const
{
  ...
  Assert(dynamic_cast<InternalData *>(mdata.get()) != nullptr, ExcInternalError());
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
Point<spacedim>
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::do_transform_unit_to_real_cell(
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
Point<dim>
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::transform_real_to_unit_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  const Point<spacedim> &                                     p) const
{
  ...
  try
    {
      initial_p_unit = StaticMappingQ1<dim, spacedim>::mapping.transform_real_to_unit_cell(cell, p);
    }
...
template <int dim, int spacedim, typename VectorType, typename DoFHandlerType>
Point<dim>
MappingFEField<dim, spacedim, VectorType, DoFHandlerType>::do_transform_real_to_unit_cell(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell,
  ...
  while (p_minus_F.norm_square() > eps * eps)
    {
      ...
      // Solve  [f'(x)]d=f(x)
      const Tensor<1, dim> delta = invert(df) * static_cast<const Tensor<1, dim> &>(f);
      // do a line search
  ...
  const
{
  ...
  for (unsigned int i = 0; i < data.local_dof_values.size(); ++i)
    data.local_dof_values[i] =
      internal::ElementAccess<VectorType>::get(*euler_vector, data.local_dof_indices[i]);
}
