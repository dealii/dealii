...
FE_RaviartThomas<dim>::FE_RaviartThomas(const unsigned int deg) :
  FE_PolyTensor<PolynomialsRaviartThomas<dim>, dim>(
    deg,
    FiniteElementData<dim>(get_dpo_vector(deg), dim, deg + 1, FiniteElementData<dim>::Hdiv),
      ...
      for (unsigned int i = 0; i < polynomials[0]->n(); ++i)
        for (unsigned int d = 0; d < dim; ++d)
          interior_weights(k, i, d) =
            cell_quadrature.weight(k) * polynomials[d]->compute_value(i, cell_quadrature.point(k));
    }
...
FE_RaviartThomas<dim>::get_constant_modes() const
{
  ...
  return std::pair<Table<2, bool>, std::vector<unsigned int>>(constant_modes, components);
}
      ...
      for (unsigned int i = 0; i < boundary_weights.size(1); ++i)
        {
          nodal_values[i + face * this->dofs_per_face] +=
            boundary_weights(k, i) * support_point_values[face * n_face_points + k](
                                       GeometryInfo<dim>::unit_normal_direction[face]);
        }
