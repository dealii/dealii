...
MappingQEulerian<dim, VectorType, spacedim>::MappingQEulerianGeneric::MappingQEulerianGeneric(
  ...
  level(level)
{
  ...
  // current vector
  this->q1_mapping =
    std::make_shared<MappingQ1Eulerian<dim, VectorType, spacedim>>(euler_dof_handler, euler_vector);
...
template <int dim, class VectorType, int spacedim>
MappingQEulerian<dim, VectorType, spacedim>::MappingQEulerianGeneric::SupportQuadrature::
  ...
  const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
  ...
  std::copy(a.begin(), a.begin() + GeometryInfo<dim>::vertices_per_cell, vertex_locations.begin());
...
template <int dim, class VectorType, int spacedim>
std::array<Point<spacedim>, GeometryInfo<dim>::vertices_per_cell>
MappingQEulerian<dim, VectorType, spacedim>::MappingQEulerianGeneric::get_vertices(
  const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
...
template <int dim, class VectorType, int spacedim>
bool
MappingQEulerian<dim, VectorType, spacedim>::MappingQEulerianGeneric::preserves_vertex_locations()
  const
{
  return false;
}
    ...
    const typename Triangulation<dim, spacedim>::cell_iterator &cell) const
{
  ...
  const types::global_dof_index n_dofs =
    mg_vector ? mapping_q_eulerian.euler_dof_handler->n_dofs(mapping_q_eulerian.level) :
  ...
  if (mg_vector)
    {
      ...
      fe_values.get_function_values(*mapping_q_eulerian.euler_vector, dof_indices, shift_vector);
    }
  else
