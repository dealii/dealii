...
namespace internal
{
  namespace FESystemImplementation
  {
    namespace
    {
      ...
      {
        return std::count_if(vec.begin(), vec.end(), [](const unsigned int i) { return i > 0; });
      }
    } // namespace
  }   // namespace FESystemImplementation
} // namespace internal
...
FESystem<dim, spacedim>::InternalData::get_fe_data(const unsigned int base_no) const
{
  Assert(base_no < base_fe_datas.size(), ExcIndexRange(base_no, 0, base_fe_datas.size()));
...
FESystem<dim, spacedim>::InternalData::get_fe_output_object(const unsigned int base_no) const
{
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  FiniteElement<dim, spacedim>(
    ...
    FETools::Compositing::compute_restriction_is_additive_flags(&fe1, n1, &fe2, n2),
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  FiniteElement<dim, spacedim>(
    ...
    FETools::Compositing::compute_restriction_is_additive_flags(&fe1, n1, &fe2, n2, &fe3, n3),
    FETools::Compositing::compute_nonzero_components(&fe1, n1, &fe2, n2, &fe3, n3)),
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  FiniteElement<dim, spacedim>(
    FETools::Compositing::multiply_dof_numbers(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4),
    FETools::Compositing::compute_restriction_is_additive_flags(&fe1,
                                                                ...
                                                                &fe4,
                                                                n4),
    FETools::Compositing::compute_nonzero_components(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4)),
...
FESystem<dim, spacedim>::FESystem(const FiniteElement<dim, spacedim> &fe1,
  ...
  FiniteElement<dim, spacedim>(
    FETools::Compositing::multiply_dof_numbers(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4, &fe5, n5),
    FETools::Compositing::
      compute_restriction_is_additive_flags(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4, &fe5, n5),
    FETools::Compositing::
      compute_nonzero_components(&fe1, n1, &fe2, n2, &fe3, n3, &fe4, n4, &fe5, n5)),
...
FESystem<dim, spacedim>::get_sub_fe(const unsigned int first_component,
...
template <int dim, int spacedim>
double
FESystem<dim, spacedim>::shape_value(const unsigned int i, const Point<dim> &p) const
{
...
template <int dim, int spacedim>
Tensor<1, dim>
FESystem<dim, spacedim>::shape_grad(const unsigned int i, const Point<dim> &p) const
{
  Assert(i < this->dofs_per_cell, ExcIndexRange(i, 0, this->dofs_per_cell));
  Assert(this->is_primitive(i),
         (typename FiniteElement<dim, spacedim>::ExcShapeFunctionNotPrimitive(i)));
...
template <int dim, int spacedim>
Tensor<2, dim>
FESystem<dim, spacedim>::shape_grad_grad(const unsigned int i, const Point<dim> &p) const
{
  Assert(i < this->dofs_per_cell, ExcIndexRange(i, 0, this->dofs_per_cell));
  Assert(this->is_primitive(i),
         (typename FiniteElement<dim, spacedim>::ExcShapeFunctionNotPrimitive(i)));
...
template <int dim, int spacedim>
Tensor<3, dim>
FESystem<dim, spacedim>::shape_3rd_derivative(const unsigned int i, const Point<dim> &p) const
{
  Assert(i < this->dofs_per_cell, ExcIndexRange(i, 0, this->dofs_per_cell));
  Assert(this->is_primitive(i),
         (typename FiniteElement<dim, spacedim>::ExcShapeFunctionNotPrimitive(i)));
...
template <int dim, int spacedim>
Tensor<4, dim>
FESystem<dim, spacedim>::shape_4th_derivative(const unsigned int i, const Point<dim> &p) const
{
  Assert(i < this->dofs_per_cell, ExcIndexRange(i, 0, this->dofs_per_cell));
  Assert(this->is_primitive(i),
         (typename FiniteElement<dim, spacedim>::ExcShapeFunctionNotPrimitive(i)));
...
template <int dim, int spacedim>
void
FESystem<dim, spacedim>::get_interpolation_matrix(const FiniteElement<dim, spacedim> &x_source_fe,
                                                  FullMatrix<double> &interpolation_matrix) const
{
  ...
  Assert((interpolation_matrix.m() == this->dofs_per_cell) || (x_source_fe.dofs_per_cell == 0),
         ExcDimensionMismatch(interpolation_matrix.m(), this->dofs_per_cell));
  Assert((interpolation_matrix.n() == x_source_fe.dofs_per_cell) || (this->dofs_per_cell == 0),
  ...
  for (unsigned int i = 0; i < this->n_base_elements(); ++i)
    AssertThrow(this->element_multiplicity(i) == source_fe.element_multiplicity(i),
                (typename FiniteElement<dim, spacedim>::ExcInterpolationNotImplemented()));
  ...
  for (unsigned int i = 0; i < this->n_base_elements(); ++i)
    {
      ...
      base_element(i).get_interpolation_matrix(source_fe.base_element(i), base_matrices[i]);
    }
  ...
  for (unsigned int i = 0; i < this->dofs_per_cell; ++i)
    for (unsigned int j = 0; j < source_fe.dofs_per_cell; ++j)
      if (this->system_to_base_table[i].first == source_fe.system_to_base_table[j].first)
        interpolation_matrix(i, j) = (base_matrices[this->system_to_base_table[i].first.first](
          this->system_to_base_table[i].second, source_fe.system_to_base_table[j].second));
}
...
FESystem<dim, spacedim>::get_restriction_matrix(const unsigned int         child,
  ...
  if (this->restriction[refinement_case - 1][child].n() == 0)
    {
      ...
      std::vector<const FullMatrix<double> *> base_matrices(this->n_base_elements());

      for (unsigned int i = 0; i < this->n_base_elements(); ++i)
        {
          base_matrices[i] = &base_element(i).get_restriction_matrix(child, refinement_case);
          if (base_matrices[i]->n() != base_element(i).dofs_per_cell)
            do_restriction = false;
        }
      Assert(do_restriction, (typename FiniteElement<dim, spacedim>::ExcProjectionVoid()));
            ...
            for (unsigned int j = 0; j < this->dofs_per_cell; ++j)
              {
                ...
                if (this->system_to_base_table[i].first != this->system_to_base_table[j].first)
                  continue;

                // so get the common base element and the indices therein:
                const unsigned int base = this->system_to_base_table[i].first.first;

                const unsigned int base_index_i = this->system_to_base_table[i].second,
...
template <int dim, int spacedim>
const FullMatrix<double> &
FESystem<dim, spacedim>::get_prolongation_matrix(const unsigned int         child,
                                                 const RefinementCase<dim> &refinement_case) const
{
  Assert(refinement_case < RefinementCase<dim>::isotropic_refinement + 1,
         ExcIndexRange(refinement_case, 0, RefinementCase<dim>::isotropic_refinement + 1));
  ...
  if (this->prolongation[refinement_case - 1][child].n() == 0)
    {
      ...
      for (unsigned int i = 0; i < this->n_base_elements(); ++i)
        {
          base_matrices[i] = &base_element(i).get_prolongation_matrix(child, refinement_case);
          if (base_matrices[i]->n() != base_element(i).dofs_per_cell)
            do_prolongation = false;
        }
      Assert(do_prolongation, (typename FiniteElement<dim, spacedim>::ExcEmbeddingVoid()));
                                            ...
                                            const bool         face_rotation) const
{
  ...
  const std::pair<std::pair<unsigned int, unsigned int>, unsigned int> face_base_index =
    this->face_system_to_base_index(face_dof_index);

  const unsigned int base_face_to_cell_index =
    this->base_element(face_base_index.first.first)
      .face_to_cell_index(face_base_index.second, face, face_orientation, face_flip, face_rotation);
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      ...
      base_fe_output_object.initialize(quadrature.size(),
      ...
      auto base_fe_data =
        base_element(base_no).get_data(flags, mapping, quadrature, base_fe_output_object);
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      ...
      // copied and/or will want to update on every cell
      auto base_fe_data =
        base_element(base_no).get_face_data(flags, mapping, quadrature, base_fe_output_object);
  ...
  for (unsigned int base_no = 0; base_no < this->n_base_elements(); ++base_no)
    {
      ...
      // copied and/or will want to update on every cell
      auto base_fe_data =
        base_element(base_no).get_subface_data(flags, mapping, quadrature, base_fe_output_object);
        ...
        if (face_no == invalid_face_number)
          {
            ...
            cell_quadrature = static_cast<const Quadrature<dim> *>(quadrature_base_pointer);
          }
        else
          {
            ...
            face_quadrature = static_cast<const Quadrature<dim - 1> *>(quadrature_base_pointer);
          }
...
FESystem<dim, spacedim>::build_interface_constraints()
{
  ...
  this->interface_constraints.TableBase<2, double>::reinit(this->interface_constraints_size());
    ...
    for (unsigned int m = 0; m < this->interface_constraints.m(); ++m)
      {
            ...
            case 2:
              {
                ...
                if (m < this->dofs_per_vertex)
                  ...
                  // then come the two sets of line indices
                  {
                    ...
                    const unsigned int sub_line = (m - this->dofs_per_vertex) / this->dofs_per_line;
                ...
                if (m < 5 * this->dofs_per_vertex)
                  ...
                  // on one of the four sub-quads
                  {
                    ...
                    const unsigned int index_in_quad =
                      ...
                      this->dofs_per_quad;
                    Assert(index_in_quad < this->dofs_per_quad, ExcInternalError());
                    ...
                    const unsigned int tmp1 =
                      4 * this->dofs_per_vertex + 4 * this->dofs_per_line + index_in_quad;
                    Assert(tmp1 < this->face_system_to_base_table.size(), ExcInternalError());
                    m_index.first = this->face_system_to_base_table[tmp1].first;
        ...
        if (n_index.first == m_index.first)
          this->interface_constraints(m, n) =
            (base_element(n_index.first.first).constraints()(m_index.second, n_index.second));
      };
}
                                    ...
                                    const std::vector<unsigned int> &multiplicities)
{
...

  {
    ...
    FETools::Compositing::build_face_tables(
      this->face_system_to_base_table, this->face_system_to_component_table, *this);
  }
  ...
  init_tasks += Threads::new_task([&]() { this->build_interface_constraints(); });
    ...
    for (unsigned int i = 0; i < this->dofs_per_cell; ++i)
      {
        ...
        Assert(base_index < base_element(base).unit_support_points.size(), ExcInternalError());
    ...
    for (unsigned int base = 0; base < this->n_base_elements(); ++base)
      {
        ...
        for (const auto &point : base_element(base).get_generalized_support_points())
          {
            // Is point already an element of generalized_support_points?
            const auto p = std::find(std::begin(this->generalized_support_points),
    ...
    for (unsigned int i = 0; i < base_elements.size(); ++i)
      {
        ...
        const auto &points = base_elements[i].first->get_generalized_support_points();
        for (unsigned int j = 0; j < points.size(); ++j)
          {
            const auto n = generalized_support_points_index_table[i][j];
            Assert(this->generalized_support_points[n] == points[j], ExcInternalError());
          }
      }
#endif /* DEBUG */
  });
  ...
  if (dim >= 3)
    init_tasks += Threads::new_task([&]() {
          ...
          for (unsigned int c = 0; c < this->element_multiplicity(b); ++c)
            {
              for (unsigned int i = 0; i < temp.size(0); ++i)
                for (unsigned int j = 0; j < 8; ++j)
                  this->adjust_quad_dof_index_for_face_orientation_table(index + i, j) = temp(i, j);
              index += temp.size(0);
            }
        }
      ...
      Assert(this->adjust_line_dof_index_for_line_orientation_table.size() == this->dofs_per_line,
             ExcInternalError());
      index = 0;
      for (unsigned int b = 0; b < this->n_base_elements(); ++b)
        {
          ...
          for (unsigned int c = 0; c < this->element_multiplicity(b); ++c)
            {
              std::copy(temp2.begin(),
                        temp2.end(),
                        this->adjust_line_dof_index_for_line_orientation_table.begin() + index);
              index += temp2.size();
            }
        }
      ...
      while (true)
        {
          const FiniteElement<dim, spacedim> &base = base_element(base_index),
                                             &base_other =
                                               fe_other_system->base_element(base_index_other);

          Assert(base.n_components() == base_other.n_components(), ExcNotImplemented());

          // get the interpolation from the bases
          base_to_base_interpolation.reinit(base_other.dofs_per_face, base.dofs_per_face);
          ...
          ++multiplicity_other;
          if (multiplicity_other == fe_other_system->element_multiplicity(base_index_other))
            {
          ...
          if (base_index == this->n_base_elements())
            {
              Assert(base_index_other == fe_other_system->n_base_elements(), ExcInternalError());
              break;
            }
  ...
  FullMatrix<double> &                interpolation_matrix) const
{
  ...
  if (fe_other_system != nullptr)
      ...
      while (true)
        {
          ...
          // get the interpolation from the bases
          base_to_base_interpolation.reinit(base_other.dofs_per_face, base.dofs_per_face);
          base.get_subface_interpolation_matrix(base_other, subface, base_to_base_interpolation);
  ...
  else
    {
      ...
      Assert(fe_other_system != nullptr,
             (typename FiniteElement<dim, spacedim>::ExcInterpolationNotImplemented()));
    }
}
  ...
  const FiniteElement<dim, spacedim> &fe_other) const
{
  ...
  if (const FESystem<dim, spacedim> *fe_sys_other =
        dynamic_cast<const FESystem<dim, spacedim> *>(&fe_other))
    {
      Assert(this->n_components() == fe_sys_other->n_components(), ExcNotImplemented());
      Assert(this->n_base_elements() == fe_sys_other->n_base_elements(), ExcNotImplemented());
      ...
      for (unsigned int b = 0; b < this->n_base_elements(); ++b)
        {
          Assert(this->base_element(b).n_components() ==
                 ...
                 ExcNotImplemented());
          Assert(this->element_multiplicity(b) == fe_sys_other->element_multiplicity(b),
                 ExcNotImplemented());
          ...
          const FiniteElementDomination::Domination base_domination =
            (this->base_element(b).compare_for_face_domination(fe_sys_other->base_element(b)));
...
{
  Assert(index < base_elements.size(), ExcIndexRange(index, 0, base_elements.size()));
                                             ...
                                             const unsigned int face_index) const
{
  return (base_element(this->system_to_base_index(shape_index).first.first)
            .has_support_on_face(this->system_to_base_index(shape_index).second, face_index));
}
...
{
  Assert(index < this->dofs_per_face, ExcIndexRange(index, 0, this->dofs_per_face));
  ...
  for (unsigned int i = 0; i < base_elements.size(); ++i)
    {
      ...
      if (constant_modes.n_rows() < comp + base_table.first.n_rows() * element_multiplicity)
        {
          Table<2, bool> new_constant_modes(comp + base_table.first.n_rows() * element_multiplicity,
  ...
  std::vector<double> &              dof_values) const
{
  ...
  AssertDimension(point_values.size(), this->get_generalized_support_points().size());
  ...
  for (unsigned int base = 0; base < base_elements.size(); ++base)
    {
      ...
      if (base_element.has_generalized_support_points())
        {
          const unsigned int n_base_points = base_element.get_generalized_support_points().size();
               ...
               ++m, current_vector_component += n_base_components)
            {
              ...
              for (unsigned int j = 0; j < base_point_values.size(); ++j)
                {
                  ...
                  // vector of values:
                  const auto begin = std::begin(point_values[n]) + current_vector_component;
                  const auto end   = begin + n_base_components;
                  std::copy(begin, end, std::begin(base_point_values[j]));
                }

              base_element.convert_generalized_support_point_values_to_dof_values(base_point_values,
                                                                                  base_dof_values);
              ...
              for (unsigned int i = 0; i < this->dofs_per_cell; ++i)
                if (this->system_to_base_index(i).first == std::make_pair(base, m))
                  dof_values[i] = base_dof_values[this->system_to_base_index(i).second];
            } /*for*/
        }
      else
        {
...
FESystem<dim, spacedim>::memory_consumption() const
{
  ...
  std::size_t mem = (FiniteElement<dim, spacedim>::memory_consumption() + sizeof(base_elements));
  for (unsigned int i = 0; i < base_elements.size(); ++i)
