...
template <int dim>
Tensor<1, dim>
AutoDerivativeFunction<dim>::gradient(const Point<dim> &p, const unsigned int comp) const
{
  Tensor<1, dim> grad;
  switch (formula)
    {
...
template <int dim>
void
AutoDerivativeFunction<dim>::vector_gradient(const Point<dim> &           p,
  ...
  switch (formula)
    {
      ...
      case FourthOrder:
        {
          Point<dim>     q1, q2, q3, q4;
          Vector<double> v1(this->n_components), v2(this->n_components), v3(this->n_components),
...
template <int dim>
void
AutoDerivativeFunction<dim>::gradient_list(const std::vector<Point<dim>> &points,
            ...
            for (unsigned int i = 0; i < dim; ++i)
              {
                ...
                q3              = points[p] - ht[i];
                q4              = q3 - ht[i];
                gradients[p][i] = (-this->value(q1, comp) + 8 * this->value(q2, comp) -
                                   8 * this->value(q3, comp) + this->value(q4, comp)) /
                                  (12 * h);
              }
          break;
            ...
            for (unsigned int i = 0; i < dim; ++i)
              {
                ...
                q4 = q3 - ht[i];
                for (unsigned int comp = 0; comp < this->n_components; ++comp)
                  gradients[p][comp][i] = (-this->value(q1, comp) + 8 * this->value(q2, comp) -
                                           8 * this->value(q3, comp) + this->value(q4, comp)) /
                                          (12 * h);
              }
          break;
