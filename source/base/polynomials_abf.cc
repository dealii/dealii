  ...
  for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      polynomial_space.compute(
        p, p_values, p_grads, p_grad_grads, p_third_derivatives, p_fourth_derivatives);
      ...
      for (unsigned int i = 0; i < p_grad_grads.size(); ++i)
        for (unsigned int d1 = 0; d1 < dim; ++d1)
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            grad_grads[i + d * n_sub][d][(d1 + d) % dim][(d2 + d) % dim] = p_grad_grads[i][d1][d2];
          ...
          for (unsigned int d2 = 0; d2 < dim; ++d2)
            for (unsigned int d3 = 0; d3 < dim; ++d3)
              for (unsigned int d4 = 0; d4 < dim; ++d4)
                fourth_derivatives[i + d * n_sub][d][(d1 + d) % dim][(d2 + d) % dim][(d3 + d) % dim]
                                  [(d4 + d) % dim] = p_fourth_derivatives[i][d1][d2][d3][d4];
    }
}
