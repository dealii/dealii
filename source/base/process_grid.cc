...
namespace
{
                               ...
                               const unsigned int block_size_n)
  {
    ...
    const int n_processes_heuristic =
      int(std::ceil((1. * m) / block_size_m)) * int(std::ceil((1. * n) / block_size_n));
...
namespace Utilities
{
  namespace MPI
  {
      ...
      n_process_columns(grid_dimensions.second)
    {
      ...
      Cblacs_gridinfo(
        blacs_context, &procrows_, &proccols_, &this_process_row, &this_process_column);
      ...
      ierr =
        MPI_Group_incl(all_group, n, inactive_with_root_ranks.data(), &inactive_with_root_group);
      AssertThrowMPI(ierr);
      ...
      ierr = Utilities::MPI::create_group(
        mpi_communicator, inactive_with_root_group, 55, &mpi_communicator_inactive_with_root);
      AssertThrowMPI(ierr);
