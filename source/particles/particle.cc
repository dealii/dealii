...
namespace Particles
{
  ...
  Particle<dim, spacedim>::Particle(const Particle<dim, spacedim> &particle) :
    ...
    properties((particle.has_properties()) ? property_pool->allocate_properties_array() :
  ...
  template <int dim, int spacedim>
  Particle<dim, spacedim>::Particle(const void *&data, PropertyPool *const new_property_pool)
  {
  ...
  {
    std::size_t size =
      sizeof(types::particle_index) + sizeof(location) + sizeof(reference_location);
  ...
  Particle<dim, spacedim>::set_properties(const ArrayView<const double> &new_properties)
  {
    ...
    Assert(
      ...
      ExcMessage(std::string("You are trying to assign properties with an incompatible length. ") +
                 "The particle has space to store " + Utilities::to_string(old_properties.size()) +
                 " properties, " + "and this function tries to assign" +
  ...
  {
    return (property_pool != nullptr) && (properties != PropertyPool::invalid_handle);
  }
} // namespace Particles
