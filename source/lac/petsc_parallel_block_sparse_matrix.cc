...
namespace PETScWrappers
{
  namespace MPI
  {
    ...
    BlockSparseMatrix::reinit(const std::vector<IndexSet> &      rows,
                              ...
                              const MPI_Comm &                   com)
    {
      DEAL_II_Assert(rows.size() == bdsp.n_block_rows(),
                     ExcMessage("invalid size"));
      DEAL_II_Assert(cols.size() == bdsp.n_block_cols(),
                     ExcMessage("invalid size"));
      ...
      for (unsigned int r = 0; r < this->n_block_rows(); ++r)
        for (unsigned int c = 0; c < this->n_block_cols(); ++c)
          {
            DEAL_II_Assert(rows[r].size() == bdsp.block(r, c).n_rows(),
                           ExcMessage("invalid size"));
            DEAL_II_Assert(cols[c].size() == bdsp.block(r, c).n_cols(),
                           ExcMessage("invalid size"));
