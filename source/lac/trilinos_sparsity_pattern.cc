...
namespace TrilinosWrappers
{
  namespace SparsityPatternIterators
  {
    ...
    Accessor::visit_present_row()
    {
      ...
      colnum_cache =
        std::make_shared<std::vector<size_type>>(sparsity_pattern->row_length(this->a_row));
  ...
  SparsityPattern::SparsityPattern()
  {
    ...
    graph =
      std_cxx14::make_unique<Epetra_FECrsGraph>(View, *column_space_map, *column_space_map, 0);
    graph->FillComplete();
  }


  SparsityPattern::SparsityPattern(const Epetra_Map &input_map, const size_type n_entries_per_row)
  {
  ...
  SparsityPattern::SparsityPattern(const IndexSet &parallel_partitioning,
                                   ...
                                   const size_type n_entries_per_row)
  {
    reinit(parallel_partitioning, parallel_partitioning, communicator, n_entries_per_row);
  }
  ...
  SparsityPattern::SparsityPattern(const IndexSet &row_parallel_partitioning,
                                   ...
                                   const size_type n_entries_per_row)
  {
    reinit(row_parallel_partitioning, col_parallel_partitioning, communicator, n_entries_per_row);
  }
  ...
  void
  SparsityPattern::reinit(const size_type m, const size_type n, const size_type n_entries_per_row)
  {
    reinit(complete_index_set(m), complete_index_set(n), MPI_COMM_SELF, n_entries_per_row);
  }
  ...
  namespace
  {
              ...
              std::unique_ptr<Epetra_CrsGraph> &  nonlocal_graph)
    {
      ...
      if (row_map.Comm().NumProc() > 1)
        graph =
          std_cxx14::make_unique<Epetra_FECrsGraph>(Copy, row_map, n_entries_per_row.data(), false);
      else
        graph = std_cxx14::make_unique<Epetra_FECrsGraph>(
          Copy, row_map, col_map, n_entries_per_row.data(), false);

      AssertDimension(sp.n_rows(), static_cast<size_type>(n_global_rows(*graph)));
      ...
      int ierr = graph->GlobalAssemble(
        *column_space_map, static_cast<const Epetra_Map &>(graph->RangeMap()), true);
      AssertThrow(ierr == 0, ExcTrilinosError(ierr));
  ...
  void
  SparsityPattern::reinit(const Epetra_Map &input_map, const size_type n_entries_per_row)
  {
    reinit_sp(input_map, input_map, n_entries_per_row, column_space_map, graph, nonlocal_graph);
  }
  ...
  {
    reinit_sp(
      input_row_map, input_col_map, n_entries_per_row, column_space_map, graph, nonlocal_graph);
  }
                          ...
                          const size_type n_entries_per_row)
  {
    ...
    reinit_sp(map, map, n_entries_per_row, column_space_map, graph, nonlocal_graph);
  }
                          ...
                          const size_type n_entries_per_row)
  {
    ...
    reinit_sp(row_map, col_map, n_entries_per_row, column_space_map, graph, nonlocal_graph);
  }
...
#  endif
    ...
    if (Utilities::MPI::n_mpi_processes(communicator) > 1)
      {
        Epetra_Map nonlocal_map = nonlocal_partitioner.make_trilinos_map(communicator, true);
  ...
  SparsityPattern::reinit(const Epetra_Map &         input_map,
                          ...
                          const bool                 exchange_data)
  {
    reinit_sp(input_map, input_map, sp, exchange_data, column_space_map, graph, nonlocal_graph);
  }
  ...
  {
    reinit_sp(
      input_row_map, input_col_map, sp, exchange_data, column_space_map, graph, nonlocal_graph);
  ...
  SparsityPattern::copy_from(const SparsityPatternType &sp)
  {
    const Epetra_Map rows(
      TrilinosWrappers::types::int_type(sp.n_rows()), 0, Utilities::Trilinos::comm_self());
    const Epetra_Map columns(
      TrilinosWrappers::types::int_type(sp.n_cols()), 0, Utilities::Trilinos::comm_self());

    reinit_sp(rows, columns, sp, false, column_space_map, graph, nonlocal_graph);
  }
        ...
        if (nonlocal_graph->IndicesAreGlobal() == false &&
            nonlocal_graph->RowMap().NumMyElements() > 0)
          {
            ...
            TrilinosWrappers::types::int_type row =
              nonlocal_graph->RowMap().MyGID(static_cast<TrilinosWrappers::types::int_type>(0));
    ...
    else
      {
        ...
        if (graph->Filled() == false)
        ...
        else
          {
            ...
            int nnz_present =
              graph->NumGlobalIndices(static_cast<TrilinosWrappers::types::int_type>(i));
            int  nnz_extracted;
            int *col_indices;
            ...
            // sure that we have not generated
            // an error.
            int ierr = graph->ExtractMyRowView(trilinos_i, nnz_extracted, col_indices);
            (void)ierr;
            Assert(ierr == 0, ExcTrilinosError(ierr));
            ...
            // Search the index
            int *el_find =
              std::find(col_indices, col_indices + nnz_present, static_cast<int>(trilinos_j));
    ...
    for (int i = 0; i < (int)local_size(); ++i)
      {
        ...
        for (unsigned int j = 0; j < (unsigned int)num_entries; ++j)
          {
            if (static_cast<size_type>(std::abs(
                  static_cast<TrilinosWrappers::types::int_type>(i - indices[j]))) > local_b)
...
#  endif
  ...
  // multiple processors.
  void
  SparsityPattern::print(std::ostream &out, const bool write_extended_trilinos_info) const
  {
    ...
    for (dealii::types::global_dof_index row = 0; row < local_size(); ++row)
      {
        ...
        for (dealii::types::global_dof_index j = 0; j < num_entries_; ++j)
          ...
          out << static_cast<int>(TrilinosWrappers::global_index(graph->ColMap(), indices[j]))
              << " " << -static_cast<int>(TrilinosWrappers::global_index(graph->RowMap(), row))
              << std::endl;
      }
  ...
  template void
  SparsityPattern::reinit(const Epetra_Map &, const dealii::SparsityPattern &, bool);
  template void
  SparsityPattern::reinit(const Epetra_Map &, const dealii::DynamicSparsityPattern &, bool);
