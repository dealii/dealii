...
namespace PETScWrappers
{
  namespace internal
  {
    ...
    {
      DEAL_II_Assert(index < vector.size(),
      ...
      if (vector.ghosted)
        {
          ...
          if (index >= static_cast<size_type>(begin) &&
          ...
          else
            {
              ...
              DEAL_II_Assert(ghostidx + end - begin < (size_type)lsize,
                             ExcInternalError());
      ...
      DEAL_II_AssertThrow((index >= static_cast<size_type>(begin)) &&
                            (index < static_cast<size_type>(end)),
  ...
  {
    DEAL_II_Assert(MultithreadInfo::is_running_single_threaded(),
                   ExcMessage(
...
#  endif
                                   ...
                                   const bool         add_values)
  {
    ...
    DEAL_II_Assert((last_action == action) ||
                     (last_action == ::dealii::VectorOperation::unknown),
