...
#  endif
            ...
            for (size_type row = 0; row < my_size; ++row)
              {
                ...
                distributed_constant_modes[d][row] = additional_data.constant_modes[d][mode_index];
              }
          }
  ...
  {
    preconditioner.reset();
    preconditioner = std::make_shared<ML_Epetra::MultiLevelPreconditioner>(matrix, ml_parameters);
  }
