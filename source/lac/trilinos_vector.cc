...
namespace TrilinosWrappers
{
  ...
  namespace MPI
  {
    ...
    Vector::Vector(const IndexSet &parallel_partitioning, const MPI_Comm &communicator) : Vector()
    {
...
#  endif
    ...
    Vector::reinit(const MPI::BlockVector &v, const bool import_data)
    {
      ...
      Epetra_Map new_map(
        v.size(), n_elements, global_ids.data(), 0, v.block(0).vector_partitioner().Comm());
      ...
      if (same_communicators && v.vector->Map().SameAs(vector->Map()))
        {
          *vector = *v.vector;
          if (v.nonlocal_vector.get() != nullptr)
            nonlocal_vector =
              std_cxx14::make_unique<Epetra_MultiVector>(v.nonlocal_vector->Map(), 1);
          last_action = Zero;
        }
      ...
      if (trilinos_i == -1)
        {
          Assert(false,
                 ExcAccessToNonLocalElement(
                   index, local_size(), vector->Map().MinMyGID(), vector->Map().MaxMyGID()));
        }
      else
...
#  endif
    ...
    Vector::memory_consumption() const
    {
      ...
      // entry.
      return sizeof(*this) +
             this->local_size() * (sizeof(double) + sizeof(TrilinosWrappers::types::int_type));
    }
