...
SuperLU_MT::initialize(const matrix_type& A, const vector_type& b)
{
 ...
 //   Assert(typeid(number_type) == typeid(matrix_type::value_type),ExcMessage("Types mismatch"));
   ...
   for(size_type row_ctr = 1; row_ctr < this->n_rows; ++row_ctr)
       this->rowptr[row_ctr] = this->rowptr[row_ctr -1] + A.get_row_length(row_ctr-1);
    Assert(static_cast<size_type>(this->rowptr.back()) == this->colind.size(), ExcInternalError());
...
void SuperLU_MT::create_supermatrices()
{
    ...
    if(typeid(number_type) == typeid(double))
    {
        ...
        dCreate_CompCol_Matrix(&A, this->n_rows, this->n_cols, this->nzval.size(), this->nzval.data(),
                               this->colind.data(), this->rowptr.data(), Stype_matrix, Dtype, Mtype);
        dCreate_Dense_Matrix(&b, this->n_rows, 1, rhs_vals.data(), this->n_rows, Stype_rhs, Dtype, Mtype);
    }
    else if(typeid(number_type) == typeid(float))
    {
     //   const auto& Dtype = Dtype_t::SLU_S;
     //   sCreate_CompCol_Matrix(&A, this->n_rows, this->n_cols, this->nzval.size(), this->nzval.data(),
     //                          this->colind.data(), this->rowptr.data(), Stype_matrix, Dtype, Mtype);
     //   sCreate_Dense_Matrix(&b, this->n_rows, 1, rhs_vals.data(), this->n_rows, Stype_rhs, Dtype, Mtype);
...
void SuperLU_MT::solve(vector_type &x)
{
    ...
    {
        pdgssv(this->additional_data.options_.nprocs, &this->A, perm_c.data(), perm_r.data(), &L, &U, &this->b, &info);
    }
    else if(this->A.Dtype == Dtype_t::SLU_S)
    {
      //  psgssv(this->additional_data.options_.nprocs, &this->A, perm_c.data(), perm_r.data(), &L, &U, &this->b,
      //  &info);
      Assert(false, ExcInternalError());
    }
   ...
   // std::copy(bStore->nzval, bStore->nzval + this->nzval.size() * sizeof(number_type), x.begin());
   // \todo remove error in this copy operation
