...
{
  Assert(my_rows.size() > 0, ExcMessage("selected rows should be non-empty."));
...
{
  const unsigned int n_subcells = VectorizedArray<NumberType>::n_array_elements;
  ...
  clear();
  dof_indices.reserve(tria->n_locally_owned_active_cells() * fe.dofs_per_cell);
  ...
  for (unsigned int cell = 0; cell < n_cells; ++cell)
    {
      ...
      for (unsigned int subcell = 0; subcell < n_filled; ++subcell)
        {
          const auto cell_it = matrix_free->get_cell_iterator(cell, subcell);
      ...
      // beginning of the next SIMD cell chunk and fill in the rest on this
      // chunk with something meaningless
      if (n_filled < n_subcells)
        {
          // copy the last valid entry
          const auto last_valid = row_starts.back();
          for (unsigned int subcell = n_filled + 1; subcell < n_subcells;
               ++subcell)
    ...
    &cell_dof_indices_local)
{
  ...
  row_starts.push_back({row_starts.back().first + cell_dof_indices_local.size(),
...
SparsityPatternStandard::SparsityPatternStandard()
  : SparsityPatternBase()
{
  reinit(0, 0, 0);
}
...
void
SparsityPatternStandard::reinit(
  const size_type                      m,
  const size_type                      n,
  const ArrayView<const unsigned int> &row_lengths)
{
  ...
  for (size_type i = 1; i <= rows; ++i)
    rowstart[i] =
      rowstart[i - 1] + std::min(static_cast<size_type>(row_lengths[i - 1]), n);
...
SparsityPatternStandard::size_type
SparsityPatternStandard::operator()(const size_type i, const size_type j) const
{
  ...
  const size_type *const p =
    dealii::Utilities::lower_bound<const size_type *>(&colnums[rowstart[i]],
...
void
SparsityPatternStandard::copy_from(const DynamicSparsityPattern &dsp)
{
...
std::size_t
BlockCSRMatrix<NumberType>::memory_consumption() const
{
  std::size_t memory =
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::print(std::ostream &     s,
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::setup_ghosts(
  const DynamicSparsityPattern &locally_owned_sparsity)
{
  ...
  if (block_partitioner)
    {
      ...
      const std::vector<unsigned int> local_sizes =
        dealii::Utilities::MPI::all_gather(partitioner->get_mpi_communicator(),
                                           n_owned_rows);
  ...
  const auto objects_received =
    dealii::Utilities::MPI::some_to_some(partitioner->get_mpi_communicator(),
                                         objects_to_send);

  AssertDimension(objects_received.size(), partitioner->ghost_targets().size());
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::reinit(
  ...
  const bool symmetric_)
{
  ...
  n_row_blocks =
    dealii::Utilities::MPI::sum(n_owned_row_blocks,
                                partitioner->get_mpi_communicator());
  n_rows = dealii::Utilities::MPI::sum(row_blocks->total_size(),
                                       partitioner->get_mpi_communicator());
                                       ...
                                       const size_type j) const
{
  ...
  Assert(!block_partitioner || partitioner->n_mpi_processes() == 1,
         ExcNotImplemented());
  const size_type i_ = block_partitioner ? i : partitioner->global_to_local(i);

  const std::pair<unsigned int, unsigned int> row_pair =
    row_blocks->global_to_local(i_);
  const std::pair<unsigned int, unsigned int> col_pair =
    col_blocks->global_to_local(j);
...
template <typename NumberType>
NumberType
BlockCSRMatrix<NumberType>::local_el(const unsigned int i_,
                                     const size_type    j) const
{
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::mmult(BlockCSRMatrix<NumberType> &      C,
          ...
          while (B_kj != last_B_kj)
            {
              ...
              // the product exactly. For example, A and C have the same
              // sparsity. Therefore we search for C_ij only until its column is
              // less than j:
              ...
              const types::blas_int mm_p =
                internal::padded_size<NumberType>(mm); // stride of left
              const types::blas_int nn = row_blocks->block_size(i);
              const types::blas_int kk = col_blocks->block_size(k);
              const types::blas_int kk_p =
                internal::padded_size<NumberType>(kk); // stride of right
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::Tmmult(BlockCSRMatrix<NumberType> &      C,
                                   const BlockCSRMatrix<NumberType> &B,
                                   const bool rebuild_sparsity_C) const
{
  Assert(!sp.empty(), ExcNotInitialized());
  ...
  if (rebuild_sparsity_C == true)
    {
      ...
      // reinit matrix C from that information
      // C.reinit(dsp, this->get_col_blocks(), B.get_col_blocks(),
      // C.is_symmetric());
    }
              ...
              if (!C_symmetric || (k >= l))
                {
                  ...
                  const NumberType beta  = 1.; // adding
                  // C_kl += A_ik^T * B_il
                  // when doing row-blocks, we essentially do (B^T A)^T =
                  // (A^T*B)
                  const types::blas_int mm = B.get_col_blocks()->block_size(l);
                  const types::blas_int mm_p =
                    internal::padded_size<NumberType>(mm);
                  const types::blas_int nn = this->col_blocks->block_size(k);
                  const types::blas_int nn_p =
                    internal::padded_size<NumberType>(nn);
            ...
            if (i > it->column())
              {
                ...
                const types::blas_int m_p =
                  internal::padded_size<NumberType>(m);
                (void)m_p;
                ...
                for (types::blas_int ii = 0; ii < m; ++ii)
                  for (types::blas_int jj = 0; jj < n; ++jj)
                    *(dst +
                      BlockCSRMatrix<NumberType>::local_index(ii, jj, m, n)) =
                      *(src +
                        BlockCSRMatrix<NumberType>::local_index(jj, ii, n, m));
#endif
              }
        }
    }
...
BlockCSRMatrix<NumberType>::Tr_Tmmult(const BlockCSRMatrix<NumberType> &B) const
{
  ...
  diagonal_data_start[0] = 0;
  {
    ...
    for (unsigned int i = 0; i < col_blocks->size(); ++i, ++it)
      {
        C_size += col_blocks->block_size(i) *
      ...
      while (both_valid)
        {
          ...
          Assert(B_il->column() >= A_ik->column(), ExcInternalError());
          ...
          Assert(A_ik->column() >= B_il->column(), ExcInternalError());

          // at this point both should be valid iterators
          Assert(A_ik != last_A_ik && B_il != last_B_il, ExcInternalError());
          ...
          if (A_ik->column() == B_il->column())
            {
              ...
              const types::blas_int mm =
                B.get_col_blocks()->block_size(B_il->column());
              const types::blas_int mm_p =
                internal::padded_size<NumberType>(mm); // stride of B_il
              const types::blas_int nn =
                this->col_blocks->block_size(A_ik->column());
              const types::blas_int nn_p =
                internal::padded_size<NumberType>(nn); // stride of A_ik
              const types::blas_int kk    = row_blocks->block_size(i);
              const NumberType      alpha = 1.;
              const NumberType      beta  = 1.; // adding
              gemm("N",
    ...
    for (unsigned int i = 0; i < col_blocks->size(); ++i)
      {
        ...
        for (unsigned int ii = 0; ii < M; ++ii)
          res +=
            *(data + BlockCSRMatrix<NumberType>::local_index(ii, ii, M, M));

        data += M * internal::padded_size<NumberType>(M);
      }
  }

  return dealii::Utilities::MPI::sum(res,
                                     this->partitioner->get_mpi_communicator());
}
...
namespace BlockCSRMatrixIterators
{
  ...
  RowsAccessorBase<NumberType, Constness>::RowsAccessorBase(MatrixType *matrix)
    : matrix(matrix)
  ...
  template <typename NumberType, bool Constness>
  void
  RowsAccessorBase<NumberType, Constness>::reinit(
    const std::vector<unsigned int> &local_rows)
  {
    ...
    my_row_blocks.erase(std::unique(my_row_blocks.begin(), my_row_blocks.end()),
                        my_row_blocks.end());
  ...
  template <typename NumberType, bool Constness>
  void
  RowsAccessorBase<NumberType, Constness>::reinit_blocks(
  ...
  RowsBlockAccessor<NumberType, Constness>::advance()
  {
    ...
    // 1. increment all non-end iterators with column() < col_block + 1
    {
      ...
      for (auto &el : row_blocks)
        if (el.it != el.end && el.it->column() < block_col_plus_one)
          ++el.it;
    }
  ...
  template <typename NumberType, bool Constness>
  types::global_dof_index
  RowsBlockAccessor<NumberType, Constness>::reinit(const unsigned int cell,
                                                   const unsigned int subcell)
  {
    ...
    const auto &this_row_starts =
      dof_info.row_starts[cell * VectorizedArray<NumberType>::n_array_elements +
                          subcell];
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::compress(
...
#  endif
  ...
  if (compress_requests.size() == 0)
    {
      ...
      for (unsigned int i = 0; i < n_import_targets; i++)
        {
          AssertThrow(
            static_cast<size_type>(import_targets[i]) * sizeof(NumberType) <
      ...
      for (unsigned int i = 0; i < n_ghost_targets; i++)
        {
          AssertThrow(
            static_cast<size_type>(ghost_targets[i]) * sizeof(NumberType) <
              static_cast<size_type>(std::numeric_limits<int>::max()),
            ExcMessage("Index overflow: Maximum message size in MPI is 2GB. "
                       "The number of ghost entries times the size of 'Number' "
                       "exceeds this value. This is not supported."));
          const int ierr =
            MPI_Send_init(&this->values[current_index_start],    // send buffer
  ...
  AssertDimension(n_import_targets + n_ghost_targets, compress_requests.size());
  if (compress_requests.size() > 0)
    {
      const int ierr =
...
template <typename NumberType>
void
BlockCSRMatrix<NumberType>::compress_finish(
  ::dealii::VectorOperation::values operation)
{
#ifdef DEAL_II_WITH_MPI
...
#  endif
  ...
  if (compress_requests.size() > 0 && n_import_targets > 0)
    {
      const int ierr = MPI_Waitall(n_import_targets,
                                   &compress_requests[0],
                                   MPI_STATUSES_IGNORE);
      AssertThrowMPI(ierr);
...
namespace internal
{
  ...
  modifier(NumberType *import, NumberType *data, const unsigned int n = 1)
  {
    ...
    dealii::internal::VectorOperations::Vectorization_add_v<NumberType>
      vector_add(data, import);
    vector_add(0, n);
  }
  ...
  template <bool Addition, typename NumberType>
  inline void
  modifier(NumberType *import, const NumberType *data, const unsigned int n = 1)
  {
...
void
BlockCSRMatrix<NumberType>::read_write_import(
  ...
  if (mtr.block_partitioner)
    {
            ...
            for (; it != end; ++it)
              {
                ...
                internal::modifier<Addition>(read_write_position,
                                             it->data(),
                                             M * N_p);
      ...
      // Assert(read_write_position == mtr.import_data.end(),
      // ExcInternalError()); and Partitioner pointer should point at the end
      ...
      for (unsigned int j = i_ind.first; j < i_ind.second; ++j)
        {
          ...
          if (current_block != numbers::invalid_unsigned_int &&
              j_block != current_block)
            write_into();
      ...
      for (unsigned int i = 0; i < n_ghost_targets; i++)
        {
          ...
          // const function
          const int ierr =
            MPI_Recv_init(const_cast<NumberType *>(
                            &values[current_index_start]), // where to write
                          ghost_targets[i] * sizeof(NumberType), // count
                          MPI_BYTE,                              // type
                          part.ghost_targets()[i].first,         // source
                          part.ghost_targets()[i].first +
                            communication_channel *
                              part.n_mpi_processes(),        // tag
                          part.get_mpi_communicator(),       // communicator
      ...
      for (unsigned int i = 0; i < n_import_targets; i++)
        {
          const int ierr = MPI_Send_init(
            &import_data[current_index_start],      // buffer
            import_targets[i] * sizeof(NumberType), // count
            MPI_BYTE,                               // type
...
template <typename NumberType>
NumberType
BlockCSRMatrix<NumberType>::frobenius_norm() const
{
...
BlockCSRMatrix<NumberType>::operator-=(const BlockCSRMatrix<NumberType> &B)
{
  ...
  Assert(col_blocks == B.col_blocks, ExcMessage("Column blocks should match"));
  ...
  dealii::internal::VectorOperations::functions<NumberType, NumberType,
  MemorySpace::Host>:: subtract_vector(thread_loop_partitioner,
                    data_start[sp.rowstart[n_owned_row_blocks]],
                    &B.values[0],
                    &values[0]);
  */
