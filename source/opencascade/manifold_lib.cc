...
namespace OpenCASCADE
{
  namespace
  {
    ...
    {
      Assert((shape.ShapeType() == TopAbs_WIRE) || (shape.ShapeType() == TopAbs_EDGE),
             ExcUnsupportedShape());
...
#  endif
            ...
            for (unsigned int i = 0; i < surrounding_points.size(); ++i)
              {
                std::tuple<Point<3>, Tensor<1, 3>, double, double> p_and_diff_forms =
                  closest_point_and_differential_forms(sh, surrounding_points[i], tolerance);
   ...
   * ==============================*/
  template <int dim, int spacedim>
  ArclengthProjectionLineManifold<dim, spacedim>::ArclengthProjectionLineManifold(
    const TopoDS_Shape &sh,
    const double        tolerance) :
    ...
    const Point<spacedim> &space_point) const
  {
    ...
    return Point<1>(
      GCPnts_AbscissaPoint::Length(curve->GetCurve(), curve->GetCurve().FirstParameter(), t));
  }
