...
namespace OpenCASCADE
{
  ...
  join_edges(const TopoDS_Shape &in_shape, const double tolerance)
  {
    ...
    Assert(one_failed == false, ExcMessage("Joining some of the Edges failed."));
  ...
  create_curves_from_triangulation_boundary(const Triangulation<2, spacedim> &triangulation,
        ...
        if (cell->face(f)->at_boundary())
          {
            ...
            std::array<Point<spacedim>, GeometryInfo<2>::vertices_per_cell> verts =
              mapping.get_vertices(cell);
            vert_to_point[v0] =
              verts[GeometryInfo<2>::face_to_cell_vertices(f, 0, true, false, false)];
            vert_to_point[v1] =
              verts[GeometryInfo<2>::face_to_cell_vertices(f, 1, true, false, false)];
  ...
  template <int dim>
  Point<dim>
  closest_point(const TopoDS_Shape &in_shape, const Point<dim> &origin, const double tolerance)
  {
