            ...
            if (cell->face(f)->at_boundary())
              {
                ...
                if (cell->reference_cell() == ReferenceCells::Triangle)
                  {
                    // add mapped vertices of the face
                    vertex_to_point[v0] = v_mapped[f];
                    vertex_to_point[v1] = v_mapped[(f + 1) % 3];
                         ...
                         ReferenceCells::Quadrilateral)
                  {
                    // add mapped vertices of the face
                    vertex_to_point[v0] =
                      v_mapped[GeometryInfo<2>::face_to_cell_vertices(f, 0)];
                    vertex_to_point[v1] =
                      v_mapped[GeometryInfo<2>::face_to_cell_vertices(f, 1)];
