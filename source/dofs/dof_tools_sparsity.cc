...
namespace DoFTools
{
                        ...
                        const types::subdomain_id subdomain_id)
  {
    ...
    Assert((dof.get_triangulation().locally_owned_subdomain() == numbers::invalid_subdomain_id) ||
    ...
    for (; cell != endc; ++cell)
      if (((subdomain_id == numbers::invalid_subdomain_id) ||
          ...
          cell->is_locally_owned())
        {
          ...
          // constraints existed
          constraints.add_entries_local_to_global(
            dofs_on_this_cell, sparsity, keep_constrained_dofs, bool_dof_mask[fe_index]);
        }
  }
                        ...
                        SparsityPatternType & sparsity)
  {
    ...
    Assert(sparsity.n_cols() == n_dofs_col, ExcDimensionMismatch(sparsity.n_cols(), n_dofs_col));

    // TODO: Looks like wasteful memory management here

    const std::list<
      std::pair<typename DoFHandlerType::cell_iterator, typename DoFHandlerType::cell_iterator>>
    ...
    for (; cell_iter != cell_list.end(); ++cell_iter)
      {
        ...
        if (!cell_row->has_children() && !cell_col->has_children())
          {
            const unsigned int dofs_per_cell_row = cell_row->get_fe().dofs_per_cell;
            const unsigned int dofs_per_cell_col = cell_col->get_fe().dofs_per_cell;
            std::vector<types::global_dof_index> local_dof_indices_row(dofs_per_cell_row);
            std::vector<types::global_dof_index> local_dof_indices_col(dofs_per_cell_col);
          ...
          {
            const std::vector<typename DoFHandlerType::active_cell_iterator> child_cells =
              GridTools::get_active_child_cells<DoFHandlerType>(cell_row);
            for (unsigned int i = 0; i < child_cells.size(); i++)
              {
                const typename DoFHandlerType::cell_iterator cell_row_child = child_cells[i];
                const unsigned int dofs_per_cell_row = cell_row_child->get_fe().dofs_per_cell;
                const unsigned int dofs_per_cell_col = cell_col->get_fe().dofs_per_cell;
                std::vector<types::global_dof_index> local_dof_indices_row(dofs_per_cell_row);
                std::vector<types::global_dof_index> local_dof_indices_col(dofs_per_cell_col);
          ...
          {
            std::vector<typename DoFHandlerType::active_cell_iterator> child_cells =
              GridTools::get_active_child_cells<DoFHandlerType>(cell_col);
            for (unsigned int i = 0; i < child_cells.size(); i++)
              {
                const typename DoFHandlerType::active_cell_iterator cell_col_child = child_cells[i];
                const unsigned int dofs_per_cell_row = cell_row->get_fe().dofs_per_cell;
                const unsigned int dofs_per_cell_col = cell_col_child->get_fe().dofs_per_cell;
                std::vector<types::global_dof_index> local_dof_indices_row(dofs_per_cell_row);
                std::vector<types::global_dof_index> local_dof_indices_col(dofs_per_cell_col);
                cell_row->get_dof_indices(local_dof_indices_row);
    ...
    if (DoFHandlerType::dimension == 1)
      {
        ...
        std::map<types::boundary_id, const Function<DoFHandlerType::space_dimension, double> *>
          boundary_ids;
        ...
        if (cell->at_boundary(f))
          {
            ...
            cell->face(f)->get_dof_indices(dofs_on_this_face, cell->active_fe_index());
  ...
  make_boundary_sparsity_pattern(
    const DoFHandlerType &dof,
    const std::map<types::boundary_id, const Function<DoFHandlerType::space_dimension, number> *>
      &                                         boundary_ids,
    const std::vector<types::global_dof_index> &dof_to_boundary_mapping,
    SparsityPatternType &                       sparsity)
  {
              ...
              if (!cell->at_boundary(face) || periodic_neighbor)
                {
                  ...
                  if (neighbor->has_children())
                    {
                      for (unsigned int sub_nr = 0; sub_nr != cell_face->number_of_children();
                           ++sub_nr)
                        {
                          ...
                          constraints.add_entries_local_to_global(
                            dofs_on_this_cell, dofs_on_other_cell, sparsity, keep_constrained_dofs);
                          constraints.add_entries_local_to_global(
                            dofs_on_other_cell, dofs_on_this_cell, sparsity, keep_constrained_dofs);
  ...
  template <typename DoFHandlerType, typename SparsityPatternType>
  void
  make_flux_sparsity_pattern(const DoFHandlerType &dof, SparsityPatternType &sparsity)
  {
    ...
    for (unsigned int i = 0; i < n_dofs; ++i)
      {
        const unsigned int ii =
          (fe.is_primitive(i) ? fe.system_to_component_index(i).first :
        ...
        for (unsigned int j = 0; j < n_dofs; ++j)
          {
            const unsigned int jj =
              (fe.is_primitive(j) ? fe.system_to_component_index(j).first :
                                         ...
                                         const Table<2, Coupling> &             component_couplings)
  {
    ...
    for (unsigned int i = 0; i < fe.size(); ++i)
      return_value[i] = dof_couplings_from_component_couplings(fe[i], component_couplings);
  ...
  namespace internal
  {
    namespace
    {
        ...
        for (; cell != endc; ++cell)
          if (((subdomain_id == numbers::invalid_subdomain_id) ||
              ...
              cell->is_locally_owned())
            {
              ...
              // make sparsity pattern for this cell
              constraints.add_entries_local_to_global(
                dofs_on_this_cell, sparsity, keep_constrained_dofs, bool_int_dof_mask);
              // Loop over all interior neighbors
              for (unsigned int face_n = 0;
                   face_n < GeometryInfo<DoFHandlerType::dimension>::faces_per_cell;
                   ++face_n)
                {
                  const typename DoFHandlerType::face_iterator cell_face = cell->face(face_n);
                  ...
                  if (cell->at_boundary(face_n) && (!periodic_neighbor))
                          ...
                          for (unsigned int j = 0; j < fe.dofs_per_cell; ++j)
                            {
                              ...
                              if (flux_dof_mask(i, j) == always ||
                                  (flux_dof_mask(i, j) == nonzero && i_non_zero_i && j_non_zero_i))
                  ...
                  else
                    {
                      ...
                      const unsigned int neighbor_face_n =
                        periodic_neighbor ? cell->periodic_neighbor_face_no(face_n) :
                                 ...
                                 const types::subdomain_id                    subdomain_id)
      {
        ...
        typename dealii::hp::DoFHandler<dim, spacedim>::active_cell_iterator cell =
                                                                               dof.begin_active(),
                                                                             endc = dof.end();
        for (; cell != endc; ++cell)
          if (((subdomain_id == numbers::invalid_subdomain_id) ||
              ...
              cell->is_locally_owned())
            {
              ...
              for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
                {
                  const typename dealii::hp::DoFHandler<dim, spacedim>::face_iterator cell_face =
                    cell->face(face);
                  ...
                  if ((!cell->at_boundary(face)) || periodic_neighbor)
                    {
                      typename dealii::hp::DoFHandler<dim, spacedim>::level_cell_iterator neighbor =
                        cell->neighbor_or_periodic_neighbor(face);
                              ...
                              for (unsigned int i = 0; i < cell->get_fe().dofs_per_cell; ++i)
                                {
                                  ...
                                  for (unsigned int j = 0; j < sub_neighbor->get_fe().dofs_per_cell;
                                       ++j)
                                    {
                                      ...
                                      Assert(jj < sub_neighbor->get_fe().n_components(),
                                             ExcInternalError());
                             ...
                             const types::subdomain_id subdomain_id)
  {
    ...
    make_flux_sparsity_pattern(
      dof, sparsity, constraints, keep_constrained_dofs, int_mask, flux_mask, subdomain_id);
  }
                             ...
                             const types::subdomain_id subdomain_id)
  {
    ...
    Assert(sparsity.n_rows() == n_dofs, ExcDimensionMismatch(sparsity.n_rows(), n_dofs));
    Assert(sparsity.n_cols() == n_dofs, ExcDimensionMismatch(sparsity.n_cols(), n_dofs));
    Assert(int_mask.n_rows() == n_comp, ExcDimensionMismatch(int_mask.n_rows(), n_comp));
    Assert(int_mask.n_cols() == n_comp, ExcDimensionMismatch(int_mask.n_cols(), n_comp));
    Assert(flux_mask.n_rows() == n_comp, ExcDimensionMismatch(flux_mask.n_rows(), n_comp));
    Assert(flux_mask.n_cols() == n_comp, ExcDimensionMismatch(flux_mask.n_cols(), n_comp));
    ...
    internal::make_flux_sparsity_pattern(
      dof, sparsity, constraints, keep_constrained_dofs, int_mask, flux_mask, subdomain_id);
  }
